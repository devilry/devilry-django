...
if __name__ == "__main__":
    ...
    period = Period.objects.get(
        parentnode__short_name=argument_dict["subject_shortname"], short_name=argument_dict["period_shortname"]
    )
    ...
    for user_shortname in argument_dict["username_list"]:
        ...
        #
        assignment_group_cached_data = (
            AssignmentGroupCachedData.objects.select_related(
                "group", "group__parentnode", "group__parentnode__parentnode"
            )
            .filter(
                group__parentnode__publishing_time__lt=now,
                group_id__in=Candidate.objects.select_related(
                    "assignment_group", "relatedstudent", "relatedstudent__period"
                )
                .filter(
        ...
        for cached_group_data in assignment_group_cached_data:
            ...
            last_student_comment_with_files = (
                ...
                .annotate(has_files=models.Exists(CommentFile.objects.filter(comment_id=models.OuterRef("id"))))
            ...
            feedback_num = FeedbackSet.objects.filter(
