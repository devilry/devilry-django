...
class TestChangeLanguage(TestCase, LoginTestCaseMixin):
    ...
    def test_post_valid(self):
        ...
        with self.settings(LANGUAGES=[("en", "English")]):
            response = self.post_as(testuser.user, self.url, {"languagecode": "en", "redirect_url": "/successtest"})
            self.assertEqual(response.status_code, 302)
    ...
    def test_post_invalid(self):
        ...
        testuser.update(languagecode="nb")
        with self.settings(LANGUAGES=[("en", "English")]):
            response = self.post_as(testuser.user, self.url, {"languagecode": "tu", "redirect_url": "/successtest"})
