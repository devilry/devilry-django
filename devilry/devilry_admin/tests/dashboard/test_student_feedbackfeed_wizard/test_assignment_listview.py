...
class TestStudentAssignmentGroupListView(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_title(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(viewkwargs={"user_id": testuser.id})
    ...
    def test_user_can_only_see_assignments_where_user_is_admin_on_subject(self):
        ...
        testgroup1 = baker.make(
            "core.AssignmentGroup",
            parentnode__long_name="Accessible Assignment",
            parentnode__parentnode__parentnode=testsubject1,
        )
        testgroup2 = baker.make(
            "core.AssignmentGroup",
            parentnode__long_name="Inaccessible Assignment",
        ...
        selector = self.mock_http200_getrequest_htmls(
            requestuser=adminuser, viewkwargs={"user_id": testuser.id}
        ).selector
    ...
    def test_user_can_only_see_assignments_where_user_is_admin_on_period(self):
        ...
        testgroup1 = baker.make(
            "core.AssignmentGroup", parentnode__long_name="Accessible Assignment", parentnode__parentnode=testperiod1
        )
        testgroup2 = baker.make(
            "core.AssignmentGroup", parentnode__long_name="Inaccessible Assignment", parentnode__parentnode=testperiod2
        )
        baker.make("core.Candidate", assignment_group=testgroup1, relatedstudent__user=testuser)
        baker.make("core.Candidate", assignment_group=testgroup2, relatedstudent__user=testuser)
        selector = self.mock_http200_getrequest_htmls(
    ...
    def test_default_ordering_by_deadline_datetime_ascending(self):
        ...
        selector = self.mock_http200_getrequest_htmls(
            requestuser=baker.make(settings.AUTH_USER_MODEL, is_superuser=True), viewkwargs={"user_id": testuser.id}
        ).selector
        ...
        self.assertListEqual(["Assignment C", "Assignment B", "Assignment A"], assignment_longname_list)
    ...
    def test_assignment_info_deadline_datetime(self):
        ...
        self.assertEqual(
            ...
            "Deadline: {}".format(
                defaultfilters.date(
                    timezone.localtime(testgroup.cached_data.last_feedbackset.deadline_datetime), "DATETIME_FORMAT"
                )
            ),
        )
    ...
    def test_search_assignment_long_name(self):
        ...
        selector = self.mock_http200_getrequest_htmls(
            ...
            viewkwargs={
                ...
                "filters_string": "search-{}".format(testgroup.parentnode.long_name),
            },
        ).selector
        self.assertEqual(selector.count(".cradmin-legacy-listbuilder-itemvalue"), 1)
    ...
    def test_search_assignment_short_name(self):
        ...
        selector = self.mock_http200_getrequest_htmls(
            ...
            viewkwargs={
                "user_id": teststudent.id,
                "filters_string": "search-{}".format(testgroup.parentnode.short_name),
            },
        ).selector
        self.assertEqual(selector.count(".cradmin-legacy-listbuilder-itemvalue"), 1)
    ...
    def test_order_deadline_descending(self):
        ...
        self.assertListEqual(["Assignment A", "Assignment B", "Assignment C"], assignment_longname_list)
    ...
    def test_filter_semesters_blank_shows_all_periods(self):
        ...
        testuser = baker.make(settings.AUTH_USER_MODEL, shortname="testuser@example.com", fullname="Test User")
        testgroup_old_period = baker.make(
            "core.AssignmentGroup", parentnode__long_name="Assignment Old", parentnode__parentnode=testperiod_old
        )
        testgroup_active_period = baker.make(
            "core.AssignmentGroup", parentnode__long_name="Assignment Active", parentnode__parentnode=testperiod_active
        )
