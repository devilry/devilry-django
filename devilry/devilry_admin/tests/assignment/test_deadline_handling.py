...
class TestAssignmentDeadlineHandlingUpdateView(TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_title(self):
        ...
        self.assertEqual(mockresponse.selector.one("h1").alltext_normalized, "Edit deadline handling")
    ...
    def test_user_is_periodadmin_raises_404(self):
        ...
        baker.make(
            ...
            permissiongroup=baker.make(
                "devilry_account.PeriodPermissionGroup", period=testassignment.parentnode
            ).permissiongroup,
        )
    ...
    def test_post_deadline_handling_soft_to_hard(self):
        ...
        self.mock_http302_postrequest(
            ...
            requestkwargs={"data": {"deadline_handling": 1}},
        ...
        messagesmock.add.assert_called_once_with(
            messages.SUCCESS, 'Changed deadline handling from "SOFT" to "HARD".', ""
        )

    def test_post_deadline_handling_hard_to_soft(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        testassignment = baker.make_recipe(
            "devilry.apps.core.assignment_activeperiod_start", deadline_handling=Assignment.DEADLINEHANDLING_HARD
        )
        baker.make(
            "devilry_account.PermissionGroupUser",
            user=testuser,
        ...
        self.mock_http302_postrequest(
            ...
            requestuser=testuser,
            viewkwargs={"pk": testassignment.id},
            requestkwargs={"data": {"deadline_handling": 0}},
            messagesmock=messagesmock,
        )
        testassignment.refresh_from_db()
        self.assertEqual(testassignment.deadline_handling, Assignment.DEADLINEHANDLING_SOFT)
        messagesmock.add.assert_called_once_with(
            messages.SUCCESS, 'Changed deadline handling from "HARD" to "SOFT".', ""
        )
