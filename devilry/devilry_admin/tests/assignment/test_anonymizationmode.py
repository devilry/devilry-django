...
class TestAssignmentAnonymizationmodeUpdateView(TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_get_h1(self):
        ...
        self.assertEqual(mockresponse.selector.one("h1").alltext_normalized, "Edit anonymization settings")

    def test_get_anonymizationmode_choices_sanity(self):
        assignment = baker.make("core.Assignment")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=assignment, viewkwargs={"pk": assignment.id})
        self.assertEqual(mockresponse.selector.count('input[name="anonymizationmode"]'), 2)
    ...
    def test_post_sanity(self):
        ...
        self.assertEqual(assignment.anonymizationmode, Assignment.ANONYMIZATIONMODE_SEMI_ANONYMOUS)
    ...
    def test_post_success_message(self):
        ...
        messagesmock.add.assert_called_once_with(
            messages.SUCCESS, 'Changed anonymization mode from "off" to "semi anonymous".', ""
        )
