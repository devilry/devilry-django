...
class TestOverviewApp(TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_title(self):
        ...
        self.assertEqual(mockresponse.selector.one("title").alltext_normalized, "testsubject.testperiod.testassignment")

    def test_devilry_admin_assignment_edit_long_name(self):
        assignment = baker.make("core.Assignment")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=assignment)
        self.assertEqual(
            mockresponse.selector.one("#devilry_admin_assignment_edit_long_name").alltext_normalized, "Edit name"
        )
    ...
    def test_published_row_buttons(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=assignment)
        self.assertEqual(
            mockresponse.selector.one('#devilry_admin_assignment_published_publishnow_form input[type="submit"]')[
                "value"
            ],
            "Publish now",
        )
        self.assertEqual(
            mockresponse.selector.one("#devilry_admin_assignment_published_buttonrow a").alltext_normalized,
    ...
    def test_published_row_buttons_when_already_published(self):
        assignment = baker.make("core.Assignment", publishing_time=default_timezone_datetime(2000, 1, 1))
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=assignment)
        self.assertFalse(mockresponse.selector.exists("#devilry_admin_assignment_published_publishnow_form"))
    ...
    def test_settings_row_first_deadline(self):
        ...
        self.assertEqual(
            mockresponse.selector.one(
                "#devilry_admin_assignment_overview_settings_first_deadline a"
            ).alltext_normalized,
            "Edit first deadline",
        )
    ...
    def test_settings_row_anonymization(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=assignment)
        self.assertEqual(
            mockresponse.selector.one("#devilry_admin_assignment_overview_settings_anonymization a").alltext_normalized,
    ...
    def test_settings_row_anonymization_description_when_anonymizationmode_off(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=assignment)
        self.assertEqual(
            mockresponse.selector.one("#devilry_admin_assignment_overview_settings_anonymization p").alltext_normalized,
    ...
    def test_gradingconfiguration_row_heading(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=assignment)
        self.assertEqual(
            mockresponse.selector.one("#devilry_admin_assignment_gradingconfiguration h2").alltext_normalized,
    ...
    def test_gradingconfiguration_max_points_100(self):
        assignment = baker.make(
            "core.Assignment", max_points=100, grading_system_plugin_id=Assignment.GRADING_SYSTEM_PLUGIN_ID_POINTS
        )
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=assignment)
        self.assertEqual(
            mockresponse.selector.one(
    ...
    def test_utilities_row_passed_previous(self):
        ...
        self.assertEqual(
            mockresponse.selector.one(
                "#devilry_admin_assignment_overview_utilities_passed_previous h3"
            ).alltext_normalized,
    ...
    def test_utilities_row_passed_previous_description(self):
        ...
        self.assertEqual(
            mockresponse.selector.one(
                "#devilry_admin_assignment_overview_utilities_passed_previous p"
            ).alltext_normalized,
    ...
    def test_settings_project_groups_sanity(self):
        ...
        self.assertEqual(
            mockresponse.selector.one("#devilry_admin_assignment_overview_settings_projectgroups p").alltext_normalized,
        ...
        self.assertEqual(
            mockresponse.selector.one("#devilry_admin_assignment_projectgroups_buttons a").alltext_normalized,
    ...
    def test_settings_anonymization_sanity(self):
        ...
        self.assertEqual(
            mockresponse.selector.one("#devilry_admin_assignment_anonymization_buttons a").alltext_normalized,
            "Edit anonymization mode",
        )
    ...
    def test_settings_deadline_handling_no_access_sanity(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=assignment, requestuser=testuser)
        self.assertEqual(
            mockresponse.selector.one(
        ...
        self.assertEqual(
            mockresponse.selector.list("#devilry_admin_assignment_overview_settings_deadline_handling p")[
                0
            ].alltext_normalized,
    ...
    def test_settings_deadline_handling_periodadmin_no_access_sanity(self):
        ...
        baker.make(
            ...
            permissiongroup=baker.make(
                "devilry_account.PeriodPermissionGroup", period=assignment.parentnode
            ).permissiongroup,
        )
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=assignment, requestuser=testuser)
        self.assertEqual(
    ...
    def test_settings_deadline_handling_soft_subjectadmin_has_access_sanity(self):
        ...
        self.assertEqual(
            mockresponse.selector.one("#devilry_admin_assignment_deadline_handling_buttons a").alltext_normalized,
    ...
    def test_settings_examiner_selfassign_sanity(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=assignment)
        self.assertEqual(
            mockresponse.selector.one("#devilry_admin_assignment_overview_settings_selfassign h3").alltext_normalized,
            "Examiner self-assign",
        )
        self.assertEqual(
            mockresponse.selector.one("#devilry_admin_assignment_overview_settings_selfassign p").alltext_normalized,
...
class TestOverviewExaminerSection(TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_sanity_no_relatedexaminer_no_other_warnings_shown(self):
        ...
        # Warnings rendered
        self.assertTrue(mockresponse.selector.exists("#id_devilry_admin_assignment_examiner_empty_semester_warning"))
    ...
    def test_sanity_students_without_examiners_and_no_examiners(self):
        ...
        # Warnings not rendered
        self.assertFalse(mockresponse.selector.exists("#id_devilry_admin_assignment_examiner_empty_semester_warning"))
        self.assertFalse(
            mockresponse.selector.exists("#id_devilry_admin_assignment_examiner_on_semester_not_on_assignment")
        )
    ...
    def test_no_examiners_on_semester_warning(self):
        ...
        self.assertEqual(
            url,
            mockresponse.selector.one("#id_devilry_admin_assignment_examiner_empty_semester_warning > strong > a").get(
                "href"
            ),
        )
    ...
    def test_no_examiners_configured_for_assignment_groups_warning(self):
        ...
        self.assertEqual(
            url,
            mockresponse.selector.one(
                "#id_devilry_admin_assignment_examiner_no_examiners_on_assignment > strong > a"
            ).get("href"),
        )
    ...
    def test_fewer_examiners_than_relatedexaminers_on_semester_note(self):
        ...
        self.assertEqual(
            url,
            mockresponse.selector.one("#id_devilry_admin_assignment_examiner_on_semester_not_on_assignment > a").get(
                "href"
            ),
        )
...
class TestOverviewStudentSection(TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_sanity_no_relatedstudents_no_other_warnings_shown(self):
        ...
        # Warnings not rendered
        self.assertFalse(mockresponse.selector.exists("#id_devilry_admin_assignment_student_no_students_on_assignment"))
        self.assertFalse(
            mockresponse.selector.exists("#id_devilry_admin_assignment_student_on_semester_not_on_assignment")
        )
    ...
    def test_no_candidates_warning(self):
        ...
        # Check warning link
        url = reverse_cradmin_url(
            instanceid="devilry_admin_assignmentadmin", appname="create_groups", viewname="INDEX", roleid=assignment.id
        )
        self.assertEqual(
            url,
            mockresponse.selector.one(
    ...
    def test_students_on_the_semester_that_are_not_on_the_assignment_warning(self):
        ...
        self.assertEqual(
            url,
            mockresponse.selector.one("#id_devilry_admin_assignment_student_on_semester_not_on_assignment > a").get(
                "href"
            ),
        )
