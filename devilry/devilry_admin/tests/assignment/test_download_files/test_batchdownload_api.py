...
from devilry.devilry_admin.views.assignment.download_files.batch_download_api import BatchCompressionAPIAssignmentView
    ...
    def _register_and_run_actiongroup(self, actiongroup_name, task, context_object, user=None):
        ...
        batchregistry.Registry.get_instance().add_actiongroup(
            batchregistry.ActionGroup(
                name=actiongroup_name, mode=batchregistry.ActionGroup.MODE_ASYNCHRONOUS, actions=[task]
            )
        )
        batchregistry.Registry.get_instance().run(
            actiongroup_name=actiongroup_name, context_object=context_object, test="test", started_by=user
        )


@unittest.skipIf(
    skip_rq_tests.should_skip_tests_that_require_rq_async(),
...
class TestAssignmentBatchDownloadApi(test.TestCase, TestHelper, TestCaseMixin):
    ...
    def test_get_status_no_files(self):
        ...
        mockresponse = self.mock_getrequest(viewkwargs={"content_object_id": testgroup.parentnode.id})
        self.assertEqual('{"status": "no-files"}', mockresponse.response.content.decode())
    ...
    def test_get_status_not_started_unprocessed(self):
        ...
        self._mock_batchoperation(
            context_object=testassignment, status=BatchOperation.STATUS_UNPROCESSED, user=testuser
        )
        ...
        self.assertEqual('{"status": "not-started"}', mockresponse.response.content.decode())
    ...
    def test_get_status_not_created_when_archive_meta_has_deleted_datetime(self):
        ...
        baker.make(
            ...
            created_by=testuser,
            created_by_role=CompressedArchiveMeta.CREATED_BY_ROLE_ADMIN,
        )
        ...
        self.assertEqual('{"status": "not-created"}', mockresponse.response.content.decode())
    ...
    def test_get_status_not_created_when_new_file_is_added(self):
        ...
        self.assertEqual(mockresponse.response.content.decode(), '{"status": "not-created"}')
    ...
    def test_get_status_not_created_user_has_compressed_archive_as_examiner_role_but_not_as_admin(self):
        ...
        testexaminer = baker.make(
            "core.Examiner", relatedexaminer__period=testassignment.parentnode, assignmentgroup=testgroup
        )
        testfeedbackset = devilry_group_baker_factories.feedbackset_first_attempt_unpublished(group=testgroup)
        testcomment = baker.make(
            "devilry_group.GroupComment",
    ...
    def test_get_status_finished_user_has_both_archive_as_examiner_and_admin(self):
        testassignment = baker.make_recipe("devilry.apps.core.assignment_activeperiod_start")
        testuser = baker.make(settings.AUTH_USER_MODEL)
        testgroup = baker.make("core.AssignmentGroup", parentnode=testassignment)
        periodpermissiongroup = baker.make(
    ...
    def test_get_status_not_created_when_examiner_history_with_datetime_greater_than_last_compressed_archive(self):
        ...
        commentfile.file.save("testfile.txt", ContentFile("testcontent"))
        baker.make(
            "core.ExaminerAssignmentGroupHistory",
    ...
    def test_get_status_not_created_when_candidate_history_with_datetime_greater_than_last_compressed_archive(self):
        ...
        commentfile.file.save("testfile.txt", ContentFile("testcontent"))
        baker.make(
            "core.CandidateAssignmentGroupHistory",
            assignment_group=testgroup,
            user=baker.make(settings.AUTH_USER_MODEL),
            created_datetime=timezone.now(),
        )
    ...
    def test_get_status_running(self):
        ...
        commentfile = baker.make("devilry_comment.CommentFile", comment=testcomment, filename="testfile.txt")
        commentfile.file.save("testfile.txt", ContentFile("testcontent"))
        self._mock_batchoperation(context_object=testassignment, status=BatchOperation.STATUS_RUNNING, user=testuser)
        mockresponse = self.mock_getrequest(
            cradmin_app=self.__mock_cradmin_app(),
            requestuser=testuser,
            viewkwargs={"content_object_id": testassignment.id},
        )
        self.assertEqual(mockresponse.response.content.decode(), '{"status": "running"}')
    ...
    def test_get_status_finished_with_link_to_downloadurl(self):
        ...
        mockresponse = self.mock_getrequest(
            cradmin_app=mock_cradmin_app, requestuser=testuser, viewkwargs={"content_object_id": testassignment.id}
        )
        self.assertContains(mockresponse.response, '"status": "finished"')
    ...
    def test_post_marks_archive_as_deleted_if_new_files_are_added(self):
        ...
        compressed_archive_meta = baker.make(
            "devilry_compressionutil.CompressedArchiveMeta",
            content_object=testassignment,
            created_by=testuser,
            created_by_role=CompressedArchiveMeta.CREATED_BY_ROLE_ADMIN,
    ...
    def test_post_compressed_archive_is_saved_as_examiner(self):
        ...
        self.assertTrue(archive_meta.archive_path.startswith("admin/"))
