...
class TestAssignmentBatchTask(TestCompressed):
    ...
    def __make_simple_setup(self, assignment):
        ...
        self.__make_comment_file(feedback_set=feedbackset, file_name="testfile.txt", file_content="testcontent")
    ...
    def test_period_admin_one_group_before_deadline(self):
        ...
        # Period admin
        periodpermissiongroup = baker.make("devilry_account.PeriodPermissionGroup", period=testassignment.parentnode)
        testuser = baker.make(settings.AUTH_USER_MODEL)
        baker.make(
            "devilry_account.PermissionGroupUser", user=testuser, permissiongroup=periodpermissiongroup.permissiongroup
        )
    ...
    def test_period_admin_duplicates_before_and_after_deadline(self):
        ...
        self.assertEqual("first upload", tarfileobject.extractfile(path_to_old_duplicate_file).read().decode())
        self.assertEqual(
            "last upload after deadline", tarfileobject.extractfile(path_to_last_file_after_deadline).read().decode()
        )
        self.assertEqual(
            "first upload after deadline",
            tarfileobject.extractfile(path_to_old_duplicate_file_after_deadline).read().decode(),
        )
