...
class TestChooseMethod(TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_redirect_if_assignment_is_published(self):
        testassignment = baker.make("core.Assignment", publishing_time=timezone.now() - datetime.timedelta(days=2))
        mock_cradmin_instance = mock.MagicMock()
        messagesmock = mock.MagicMock()
        self.mock_http302_getrequest(
            cradmin_role=testassignment, cradmin_instance=mock_cradmin_instance, messagesmock=messagesmock
        )
        mock_cradmin_instance.rolefrontpage_url.assert_called_once_with()
        messagesmock.add.assert_called_once_with(messages.ERROR, self.viewclass.published_assignment_error_message, "")

    def test_title(self):
        testassignment = baker.make(
            "core.Assignment", publishing_time=timezone.now() + datetime.timedelta(days=2), long_name="Assignment One"
        )
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testassignment)
        self.assertIn("Replace students on Assignment One", mockresponse.selector.one("title").alltext_normalized)

    def test_h1(self):
        testassignment = baker.make("core.Assignment", publishing_time=timezone.now() + datetime.timedelta(days=2))
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testassignment)
        self.assertEqual("Replace students", mockresponse.selector.one("h1").alltext_normalized)

    def test_choices_sanity(self):
        testassignment = baker.make("core.Assignment", publishing_time=timezone.now() + datetime.timedelta(days=2))
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testassignment)
        self.assertEqual(3, mockresponse.selector.count(".cradmin-legacy-listbuilder-itemvalue a"))
    ...
    def test_choice_relatedstudents_url(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testassignment, cradmin_app=mockapp)
        self.assertEqual(
            "/confirm/args=(),kwargs={'selected_students': 'relatedstudents'}",
            mockresponse.selector.one("#devilry_admin_create_groups_choosemethod_relatedstudents_link")["href"],
        )
    ...
    def test_choice_relatedstudents_label(self):
        ...
        self.assertEqual(
            ...
            mockresponse.selector.one(
                "#devilry_admin_create_groups_choosemethod_relatedstudents_link"
            ).alltext_normalized,
        )
    ...
    def test_choice_manual_select_value(self):
        ...
        self.assertEqual(
            ...
            mockresponse.selector.one("#devilry_admin_create_groups_choosemethod_manualselect_link")["href"],
        )
    ...
    def test_choice_manual_select_label(self):
        ...
        self.assertEqual(
            "Select manually",
            mockresponse.selector.one("#devilry_admin_create_groups_choosemethod_manualselect_link").alltext_normalized,
        )
    ...
    def test_choices_does_not_include_current_assignment(self):
        ...
        testassignment = baker.make(
            "core.Assignment", parentnode=testperiod, publishing_time=timezone.now() + datetime.timedelta(days=2)
        )
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testassignment)
        self.assertEqual(5, mockresponse.selector.count(".cradmin-legacy-listbuilder-itemvalue a"))
        self.assertFalse(
            mockresponse.selector.exists(
                "#devilry_admin_create_groups_choosemethod_assignment_{}".format(testassignment.pk)
            )
        )
        self.assertTrue(
            mockresponse.selector.exists(
                "#devilry_admin_create_groups_choosemethod_assignment_{}".format(otherassignment.pk)
            )
        )

    def test_other_assignment_rending(self):
        testperiod = baker.make("core.Period")
        otherassignment = baker.make("core.Assignment", parentnode=testperiod, short_name="otherassignment")
        testassignment = baker.make(
            "core.Assignment", parentnode=testperiod, publishing_time=timezone.now() + datetime.timedelta(days=2)
        )
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testassignment)
        self.assertEqual(
            "All students",
            mockresponse.selector.one(
                "#devilry_admin_create_groups_choosemethod_assignment_{}_all_link".format(otherassignment.pk)
            ).alltext_normalized,
        )
        self.assertEqual(
            "Students with passing grade",
            mockresponse.selector.one(
                "#devilry_admin_create_groups_choosemethod_assignment_{}_passing_link".format(otherassignment.pk)
            ).alltext_normalized,
        )
    ...
    def test_other_assignments_ordering(self):
        ...
        assignment1 = baker.make(
            "core.Assignment", parentnode=testperiod, publishing_time=ACTIVE_PERIOD_START + datetime.timedelta(days=1)
        )
        assignment2 = baker.make(
            "core.Assignment", parentnode=testperiod, publishing_time=ACTIVE_PERIOD_START + datetime.timedelta(days=2)
        )
        assignment3 = baker.make(
            "core.Assignment", parentnode=testperiod, publishing_time=ACTIVE_PERIOD_START + datetime.timedelta(days=3)
        )
        assignment4 = baker.make(
            "core.Assignment", parentnode=testperiod, publishing_time=timezone.now() + datetime.timedelta(days=2)
        )
...
class TestConfirmView(TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_title(self):
        testassignment = baker.make(
            "core.Assignment", long_name="Assignment One", publishing_time=timezone.now() + datetime.timedelta(days=2)
        )
    ...
    def test_get_subheader_selected_students_relateadstudents(self):
        ...
        self.assertEqual(
            ...
            mockresponse.selector.one("#devilry_admin_create_groups_confirm_selected_student_label").alltext_normalized,
        )

    def test_get_subheader_selected_students_all_on_assignment(self):
        testperiod = baker.make("core.Period")
        otherassignment = baker.make("core.Assignment", long_name="Assignment One", parentnode=testperiod)
        testassignment = baker.make(
            "core.Assignment", parentnode=testperiod, publishing_time=timezone.now() + datetime.timedelta(days=2)
        )
        mockresponse = self.mock_http200_getrequest_htmls(
    ...
    def test_get_render_submitbutton(self):
        ...
        self.assertEqual(
            "Add students",
            mockresponse.selector.one(
                '#devilry_admin_create_groups_confirm_form button[name="add_students"]'
            ).alltext_normalized,
        )
    ...
    def test_get_render_form_selected_items_selected_students_relatedstudents(self):
        ...
        self.assertEqual({str(relatedstudent1.id), str(relatedstudent2.id)}, set(selected_relatedstudent_ids))
    ...
    def test_get_render_form_selected_items_selected_students_all_on_assignment(self):
        ...
        baker.make("core.RelatedStudent", period=testperiod)
        otherassignment = baker.make("core.Assignment", parentnode=testperiod)
        baker.make("core.Candidate", relatedstudent=relatedstudent1, assignment_group__parentnode=otherassignment)
        baker.make("core.Candidate", relatedstudent=relatedstudent2, assignment_group__parentnode=otherassignment)
    ...
    def test_get_render_form_selected_items_selected_students_passing_grade_on_assignment(self):
        ...
        relatedstudent2 = baker.make("core.RelatedStudent", period=testperiod)
        baker.make("core.RelatedStudent", period=testperiod)
        otherassignment = baker.make("core.Assignment", parentnode=testperiod, passing_grade_min_points=1)
        candidate1 = baker.make(
            "core.Candidate", relatedstudent=relatedstudent1, assignment_group__parentnode=otherassignment
        )
        devilry_group_baker_factories.feedbackset_first_attempt_published(
            group=candidate1.assignment_group, grading_points=1
        )
        baker.make("core.Candidate", relatedstudent=relatedstudent2, assignment_group__parentnode=otherassignment)
        ...
        self.assertEqual({str(relatedstudent1.id)}, set(selected_relatedstudent_ids))
    ...
    def test_get_no_relatedstudents_matching_query(self):
        ...
        self.assertEqual(
            ...
            mockresponse.selector.one(".devilry-admin-create-groups-confirm-no-students").alltext_normalized,
        )

    def test_get_selected_students_relateadstudents(self):
        testperiod = baker.make("core.Period")
        baker.make("core.RelatedStudent", user__fullname="Match User", period=testperiod)
        baker.make("core.RelatedStudent", user__fullname="Other User", period=testperiod)
        testassignment = baker.make(
            "core.Assignment", parentnode=testperiod, publishing_time=timezone.now() + datetime.timedelta(days=2)
        )
        mockresponse = self.mock_http200_getrequest_htmls(
        ...
        self.assertEqual(2, mockresponse.selector.count(".devilry-admin-listbuilder-relatedstudent-readonlyitemvalue"))
    ...
    def test_get_selected_students_all_on_assignment(self):
        ...
        relatedstudent1 = baker.make("core.RelatedStudent", period=testperiod)
        relatedstudent2 = baker.make("core.RelatedStudent", period=testperiod)
        relatedstudent3 = baker.make(
            "core.RelatedStudent", user__fullname="User that is not on the other assignment", period=testperiod
        )
        otherassignment = baker.make("core.Assignment", parentnode=testperiod)
        baker.make("core.Candidate", relatedstudent=relatedstudent1, assignment_group__parentnode=otherassignment)
        baker.make("core.Candidate", relatedstudent=relatedstudent2, assignment_group__parentnode=otherassignment)
    ...
    def test_get_selected_students_passing_grade_on_assignment(self):
        ...
        relatedstudent2 = baker.make(
            "core.RelatedStudent", user__fullname="User that is not candidate", period=testperiod
        )

        relatedstudent3 = baker.make("core.RelatedStudent", user__fullname="User that did not pass", period=testperiod)
        candidate3 = baker.make(
            "core.Candidate", relatedstudent=relatedstudent3, assignment_group__parentnode=otherassignment
        )
        devilry_group_baker_factories.feedbackset_first_attempt_published(
            group=candidate3.assignment_group, grading_points=0
        )

        relatedstudent4 = baker.make(
            "core.RelatedStudent", user__fullname="User that is not on the other assignment", period=testperiod
        )
        ...
        self.assertEqual(1, mockresponse.selector.count(".devilry-admin-listbuilder-relatedstudent-readonlyitemvalue"))
    ...
    def test_post_relatedstudent_already_on_assignment(self):
        ...
        testcandidate = baker.make("core.Candidate", relatedstudent=relatedstudent, assignment_group=testgroup)
        ...
        self.mock_http302_postrequest(
            ...
            cradmin_instance=mock_cradmin_instance,
            requestkwargs={"data": {"selected_items": [relatedstudent.id]}},
        )
        mock_cradmin_instance.appindex_url.assert_called_once_with("studentoverview")
        self.assertEqual(1, AssignmentGroup.objects.count())
    ...
    def test_post_relatedstudent_not_relatedstudent_on_period(self):
        testperiod = baker.make("core.Period")
        relatedstudent = baker.make("core.RelatedStudent", user__shortname="userb@example.com")
        testassignment = baker.make(
            "core.Assignment", parentnode=testperiod, publishing_time=timezone.now() + datetime.timedelta(days=2)
        )
        self.assertEqual(0, AssignmentGroup.objects.count())
        ...
        self.assertEqual(0, AssignmentGroup.objects.count())
        messagesmock.add.assert_called_once_with(messages.ERROR, replace_groups.ConfirmView.form_invalid_message, "")


class TestRelatedStudentMultiselectTarget(TestCase):
    def test_with_items_title(self):
        selector = htmls.S(
            replace_groups.RelatedStudentMultiselectTarget(form=forms.Form()).render(request=mock.MagicMock())
        )
        self.assertEqual("Replace students", selector.one('button[type="submit"]').alltext_normalized)
...
class TestManualSelectStudentsView(TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_h1(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testassignment)
        self.assertEqual(
            "Select the students you want to add to Assignment One", mockresponse.selector.one("h1").alltext_normalized
        )
    ...
    def test_no_relatedstudents(self):
        ...
        self.assertEqual(
            "No students found.",
            mockresponse.selector.one(
                ".devilry-admin-create-groups-manual-select-no-relatedstudents-message"
            ).alltext_normalized,
        )
    ...
    def test_render_relatedstudent_sanity(self):
        ...
        testperiod = baker.make("core.Period")
        baker.make("core.RelatedStudent", user__shortname="testuser", user__fullname="Test User", period=testperiod)
        testassignment = baker.make(
            "core.Assignment", parentnode=testperiod, publishing_time=timezone.now() + datetime.timedelta(days=2)
        )
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testassignment)
    ...
    def test_render_search(self):
        testperiod = baker.make("core.Period")
        baker.make("core.RelatedStudent", user__shortname="testuser", user__fullname="Match User", period=testperiod)
        baker.make("core.RelatedStudent", user__fullname="Other User", period=testperiod)
        testassignment = baker.make(
            "core.Assignment", parentnode=testperiod, publishing_time=timezone.now() + datetime.timedelta(days=2)
        )
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=testassignment, viewkwargs={"filters_string": "search-match"}
        )
        self.assertEqual(1, mockresponse.selector.count(".cradmin-legacy-listbuilder-itemvalue"))
        self.assertEqual(
            "Match User(testuser)",
    ...
    def test_render_orderby_default(self):
        testperiod = baker.make("core.Period")
        baker.make("core.RelatedStudent", user__shortname="userb@example.com", period=testperiod)
        baker.make("core.RelatedStudent", user__shortname="x", user__fullname="UserA", period=testperiod)
        baker.make("core.RelatedStudent", user__shortname="userc", period=testperiod)
        testassignment = baker.make(
            "core.Assignment", parentnode=testperiod, publishing_time=timezone.now() + datetime.timedelta(days=2)
        )
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testassignment)
        titles = [
            ...
            for element in mockresponse.selector.list(".cradmin-legacy-listbuilder-itemvalue-titledescription-title")
        ]
        self.assertEqual(["UserA(x)", "userb@example.com", "userc"], titles)
    ...
    def test_render_orderby_name_descending(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=testassignment, viewkwargs={"filters_string": "orderby-name_descending"}
        )
        titles = [
            element.alltext_normalized
            for element in mockresponse.selector.list(".cradmin-legacy-listbuilder-itemvalue-titledescription-title")
        ]
        self.assertEqual(["userc", "userb@example.com", "UserA(x)"], titles)
    ...
    def test_render_orderby_lastname_ascending(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=testassignment, viewkwargs={"filters_string": "orderby-lastname_ascending"}
        )
        titles = [
            element.alltext_normalized
            for element in mockresponse.selector.list(".cradmin-legacy-listbuilder-itemvalue-titledescription-title")
        ]
        self.assertEqual(["userb@example.com", "User Aaa(x)", "User ccc(y)"], titles)
    ...
    def test_render_orderby_lastname_descending(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=testassignment, viewkwargs={"filters_string": "orderby-lastname_descending"}
        )
        titles = [
            element.alltext_normalized
            for element in mockresponse.selector.list(".cradmin-legacy-listbuilder-itemvalue-titledescription-title")
        ]
        self.assertEqual(["User ccc(y)", "User Aaa(x)", "userb@example.com"], titles)

    def test_render_orderby_shortname_ascending(self):
        testperiod = baker.make("core.Period")
        baker.make("core.RelatedStudent", user__shortname="userb@example.com", period=testperiod)
        baker.make("core.RelatedStudent", user__shortname="usera@example.com", period=testperiod)
        baker.make("core.RelatedStudent", user__shortname="userc@example.com", period=testperiod)
        testassignment = baker.make(
            "core.Assignment", parentnode=testperiod, publishing_time=timezone.now() + datetime.timedelta(days=2)
        )
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=testassignment, viewkwargs={"filters_string": "orderby-shortname_ascending"}
        )
        titles = [
            element.alltext_normalized
            for element in mockresponse.selector.list(".cradmin-legacy-listbuilder-itemvalue-titledescription-title")
        ]
        self.assertEqual(["usera@example.com", "userb@example.com", "userc@example.com"], titles)
    ...
    def test_render_orderby_shortname_descending(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=testassignment, viewkwargs={"filters_string": "orderby-shortname_descending"}
        )
        titles = [
            element.alltext_normalized
            for element in mockresponse.selector.list(".cradmin-legacy-listbuilder-itemvalue-titledescription-title")
        ]
        self.assertEqual(["userc@example.com", "userb@example.com", "usera@example.com"], titles)
    ...
    def test_post_relatedstudent_not_relatedstudent_on_period(self):
        ...
        messagesmock.add.assert_called_once_with(
            messages.ERROR, replace_groups.ManualSelectStudentsView.form_invalid_message, ""
        )
