...
class TestGroupDetailsRenderable(test.TestCase):
    ...
    def test_name(self):
        ...
        selector = htmls.S(
            groupdetails.GroupDetailsRenderable(value=testgroup, assignment=testgroup.assignment).render()
        )
        self.assertEqual(
    ...
    def test_status_is_corrected(self):
        devilry_group_baker_factories.feedbackset_first_attempt_published(grading_points=1)
    ...
    def test_status_is_waiting_for_deliveries(self):
        devilry_group_baker_factories.feedbackset_first_attempt_unpublished(
            group__parentnode=baker.make_recipe(
...
class TestGroupDetailsView(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_title(self):
        testgroup = baker.make("core.AssignmentGroup")
        devilry_core_baker_factories.candidate(group=testgroup, fullname="Test User")
        ...
        self.assertIn("Test User", mockresponse.selector.one("title").alltext_normalized)
    ...
    def test_h1(self):
        ...
        self.assertEqual("Test User", mockresponse.selector.one("h1").alltext_normalized)
    ...
    def test_title_multiple_candidates(self):
        testgroup = baker.make("core.AssignmentGroup")
        devilry_core_baker_factories.candidate(group=testgroup, fullname="UserB")
        devilry_core_baker_factories.candidate(group=testgroup, shortname="usera")
        devilry_core_baker_factories.candidate(group=testgroup, fullname="UserC")
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=testgroup.assignment,
            cradmin_instance=self.__mockinstance_with_devilryrole("subjectadmin"),
            viewkwargs={"pk": testgroup.id},
        )
        self.assertIn("usera, UserB, UserC", mockresponse.selector.one("title").alltext_normalized)
    ...
    def test_h1_multiple_candidates(self):
        ...
        self.assertEqual("usera, UserB, UserC", mockresponse.selector.one("h1").alltext_normalized)
