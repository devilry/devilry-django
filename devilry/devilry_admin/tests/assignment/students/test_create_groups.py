...
class TestChooseMethod(TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_h1(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testassignment)
        self.assertEqual("Add students", mockresponse.selector.one("h1").alltext_normalized)
    ...
    def test_choice_relatedstudents_label(self):
        testassignment = baker.make("core.Assignment", parentnode__parentnode__short_name="testsubject")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testassignment)
        self.assertEqual(
            "All students",
...
class TestConfirmView(TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_title(self):
        ...
        self.assertIn(
            "Confirm that you want to add the following students to testsubject.testperiod.testassignment",
            mockresponse.selector.one("title").alltext_normalized,
        )
    ...
    def test_get_subheader_selected_students_relateadstudents(self):
        testassignment = baker.make(
            "core.Assignment", parentnode__parentnode__short_name="testsubject", parentnode__short_name="testperiod"
        )
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=testassignment,
            viewkwargs={"selected_students": create_groups.ConfirmView.SELECTED_STUDENTS_RELATEDSTUDENTS},
    ...
    def test_get_selected_students_passing_grade_on_assignment(self):
        ...
        self.assertNotIn(relatedstudent2.user.fullname, mockresponse.response.content.decode())
        self.assertNotIn(relatedstudent3.user.fullname, mockresponse.response.content.decode())
        self.assertNotIn(relatedstudent4.user.fullname, mockresponse.response.content.decode())
    ...
    def test_post_relatedstudent_already_on_assignment(self):
        testperiod = baker.make("core.Period")
        relatedstudent = baker.make("core.RelatedStudent", user__shortname="userb@example.com", period=testperiod)
        testassignment = baker.make("core.Assignment", parentnode=testperiod)
        ...
        messagesmock.add.assert_called_once_with(
            messages.ERROR, create_groups.ManualSelectStudentsView.form_invalid_message, ""
        )
    ...
    def test_post_relatedstudent_not_relatedstudent_on_period(self):
        ...
        self.assertEqual("/appindex", mockresponse.response["Location"])
        self.assertEqual(0, AssignmentGroup.objects.count())
        messagesmock.add.assert_called_once_with(messages.ERROR, create_groups.ConfirmView.form_invalid_message, "")
