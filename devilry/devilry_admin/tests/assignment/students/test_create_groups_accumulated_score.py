...
class TestAccumulatedScoreSelectAssignmentsView(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_no_assignments(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=test_assignment)
    ...
    def test_no_assignments_on_same_period(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=test_assignment1)
        self.assertFalse(mockresponse.selector.exists(".cradmin-legacy-listbuilder-itemvalue"))

    def test_assignment_info(self):
        current_assignment = baker.make("core.Assignment")
        baker.make(
            "core.Assignment", long_name="Test Assignment", max_points=123, parentnode=current_assignment.parentnode
        )
    ...
    def test_assignments_multiple(self):
        current_assignment = baker.make("core.Assignment")
        test_assignment1 = baker.make(
            "core.Assignment", long_name="Test Assignment 1", max_points=123, parentnode=current_assignment.parentnode
        )
        test_assignment2 = baker.make(
            "core.Assignment", long_name="Test Assignment 2", max_points=123, parentnode=current_assignment.parentnode
        )
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=current_assignment)
        selector = mockresponse.selector
        assignment_names = [
    ...
    def test_session_data_cleared(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=current_assignment, sessionmock=self.client.session
        )
    ...
    def test_post_session_data_set(self):
        ...
        mockresponse = self.mock_http302_postrequest(
            ...
            requestkwargs={
                "data": {"selected_items": [test_assignment1.id, test_assignment2.id], "points_threshold": 123}
            },
        )
    ...
    def test_post_without_point_threshold(self):
        ...
        mockresponse = self.mock_http200_postrequest_htmls(
            cradmin_role=current_assignment,
            sessionmock=self.client.session,
            requestkwargs={"data": {"selected_items": [test_assignment1.id, test_assignment2.id]}},
        )
        self.assertEqual(len(list(self.client.session.keys())), 0)
        self.assertEqual(
            mockresponse.selector.one("#error_1_id_points_threshold").alltext_normalized, "This field is required."
        )
    ...
    def test_post_without_selected_items(self):
        ...
        mockresponse = self.mock_http200_postrequest_htmls(
            cradmin_role=current_assignment,
            sessionmock=self.client.session,
            requestkwargs={"data": {"selected_items": [], "points_threshold": 123}},
        )
        self.assertEqual(len(list(self.client.session.keys())), 0)
...
class TestPreviewRelatedstudentsListView(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_from_select_assignment_view_not_in_session(self):
        ...
        with self.assertRaises(Http404):
            self.mock_http200_getrequest_htmls(
                cradmin_role=test_assignment, sessionmock={"selected_assignment_ids": [], "points_threshold": 10}
            )
    ...
    def test_points_threshold_not_in_session(self):
        ...
        with self.assertRaises(Http404):
            self.mock_http200_getrequest_htmls(
                ...
                sessionmock={"selected_assignment_ids": [], "from_select_assignment_view": ""},
            )
    ...
    def test_selected_assignment_ids_not_in_session(self):
        ...
        with self.assertRaises(Http404):
            self.mock_http200_getrequest_htmls(
                cradmin_role=test_assignment, sessionmock={"points_threshold": 123, "from_select_assignment_view": ""}
            )

    def test_ok(self):
        current_assignment = baker.make("core.Assignment")
        test_assignment = baker.make(
            "core.Assignment", parentnode=current_assignment.parentnode, long_name="Test Assignment 1"
        )
        self.mock_http200_getrequest_htmls(
            cradmin_role=test_assignment,
            sessionmock={
    ...
    def test_selected_assignments_info_box(self):
        ...
        test_assignment2 = baker.make(
            "core.Assignment", parentnode=current_assignment.parentnode, long_name="Test Assignment 2"
        )
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=current_assignment,
            sessionmock={
    ...
    def test_total_score_for_selected_assignments_info_box(self):
        current_assignment = baker.make("core.Assignment")
        test_assignment1 = baker.make(
            "core.Assignment", parentnode=current_assignment.parentnode, long_name="Test Assignment 1", max_points=100
        )
        test_assignment2 = baker.make(
            "core.Assignment", parentnode=current_assignment.parentnode, long_name="Test Assignment 2", max_points=150
        )
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=current_assignment,
            sessionmock={
        ...
        self.assertEqual(
            mockresponse.selector.one(
                ".devilry-accumulated-score-selected-assignments-total-max-score"
            ).alltext_normalized,
    ...
    def test_threshold_percentage_of_max_score_info_box(self):
        ...
        self.assertEqual(
            mockresponse.selector.one(
                ".devilry-accumulated-score-selected-assignments-threshold-percentage-of-max-score"
            ).alltext_normalized,
    ...
    def test_single_assignment_single_student_not_passed_added_students_count_info_box(self):
        ...
        self.assertEqual(
            mockresponse.selector.one(
                ".devilry-accumulated-score-selected-assignments-student-count"
            ).alltext_normalized,
    ...
    def test_student_already_on_assignment_is_excluded(self):
        ...
        baker.make("core.Candidate", relatedstudent=relatedstudent, assignment_group=group)
    ...
    def test_student_already_on_assignment_is_excluded_with_qualifying_student(self):
        ...
        relatedstudent_on_current_assignment = baker.make(
            "core.RelatedStudent",
            period=current_assignment.parentnode,
            user__fullname="Test User 2",
            user__shortname="testuser2@example.com",
        ...
        group = baker.make("core.AssignmentGroup", parentnode=current_assignment)
        baker.make("core.Candidate", relatedstudent=relatedstudent_on_current_assignment, assignment_group=group)
        baker.make("devilry_group.FeedbackSet", group=group)
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=current_assignment,
            sessionmock={
    ...
    def test_post_success_message(self):
        ...
        self.mock_http302_postrequest(
            ...
            requestkwargs={"data": {"confirm": ""}},
        )
        messagesmock.add.assert_called_once_with(messages.SUCCESS, "1 student(s) added to Current Assignment", "")
    ...
    def test_post_one_student_group_created(self):
        ...
        self.assertEqual(
            Candidate.objects.filter(assignment_group=assignment_group, relatedstudent=relatedstudent).count(), 1
        )
    ...
    def test_post_multiple_students_multiple_groups_created(self):
        ...
        self.assertEqual(
            Candidate.objects.filter(
                relatedstudent=relatedstudent1, assignment_group__parentnode=current_assignment
            ).count(),
            1,
        )
        self.assertEqual(
            Candidate.objects.filter(
                relatedstudent=relatedstudent2, assignment_group__parentnode=current_assignment
            ).count(),
            1,
        )
    ...
    def test_post_student_already_on_assignment_is_excluded(self):
        ...
        self.assertEqual(
            Candidate.objects.filter(
                relatedstudent=relatedstudent_on_current_assignment, assignment_group__parentnode=current_assignment
            ).count(),
            1,
        )
