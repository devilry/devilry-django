...
class TestAssignmentGradingConfigurationUpdateView(TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_get_title(self):
        ...
        self.assertEqual(mockresponse.selector.one("title").alltext_normalized, "Edit grading configuration")

    def test_get_h1(self):
        assignment = baker.make_recipe("devilry.apps.core.assignment_activeperiod_start")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=assignment, viewkwargs={"pk": assignment.id})
        self.assertEqual(mockresponse.selector.one("h1").alltext_normalized, "Edit grading configuration")

    def __get_radio_labels(self, selector, wrapper_element_id):
        return [element.alltext_normalized for element in selector.list("#{} .radio label".format(wrapper_element_id))]

    def __get_radio_values(self, selector, wrapper_element_id):
        return [element.get("value") for element in selector.list('#{} input[type="radio"]'.format(wrapper_element_id))]

    def test_get_grading_system_plugin_id_choices_sanity(self):
        assignment = baker.make("core.Assignment")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=assignment, viewkwargs={"pk": assignment.id})
        self.assertEqual(mockresponse.selector.count('input[name="grading_system_plugin_id"]'), 2)
    ...
    def test_get_grading_system_plugin_id_labels(self):
        ...
        self.assertEqual(
            labels[1], "POINTS. The examiner types in the number of points to award the student(s) for this assignment."
        )
    ...
    def test_get_points_to_grade_mapper_choices_sanity(self):
        assignment = baker.make("core.Assignment")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=assignment, viewkwargs={"pk": assignment.id})
        self.assertEqual(mockresponse.selector.count('input[name="points_to_grade_mapper"]'), 3)
    ...
    def test_get_point_to_grade_map_json_none(self):
        assignment = baker.make_recipe("devilry.apps.core.assignment_activeperiod_start")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=assignment, viewkwargs={"pk": assignment.id})
        self.assertEqual(mockresponse.selector.one('input[name="point_to_grade_map_json"]').alltext_normalized, "")

    def test_get_point_to_grade_map_json_has_value(self):
        assignment = baker.make_recipe("devilry.apps.core.assignment_activeperiod_start", max_points=100)
    ...
    def test_post_max_points_smaller_than_passing_grade_min_points(self):
        ...
        self.assertIn(
            str(
                gradingconfiguration.GradingConfigurationForm.error_messages[
    ...
    def test_post_point_to_grade_map_json_empty(self):
        ...
        self.assertFalse(PointToGradeMap.objects.filter(assignment=assignment).exists())
        self.assertIn(
            str(gradingconfiguration.GradingConfigurationForm.error_messages["point_to_grade_map_json_invalid_format"]),
    ...
    def test_post_point_to_grade_map_json_sanity(self):
        ...
        assignment = Assignment.objects.filter(id=assignment.id).prefetch_point_to_grade_map().get()
        self.assertEqual(assignment.points_to_grade_mapper, Assignment.POINTS_TO_GRADE_MAPPER_CUSTOM_TABLE)
