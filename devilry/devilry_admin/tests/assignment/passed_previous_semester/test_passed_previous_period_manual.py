...
class TestPassAssignmentGroupsView(TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_get_selectable_groups(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=testassignment, cradmin_instance=self.__mock_crinstance()
        )
    ...
    def test_post_feedback_sanity(self):
        ...
        self.mock_http302_postrequest(
            ...
            requestkwargs={"data": {"feedback_comment_text": "feedback comment", "selected_items": [testgroup.id]}},
        )
        ...
        self.assertEqual(
            fbset_passed_previous_period.passed_previous_period_type,
    ...
    def test_post_feedback_for_one_selected_group(self):
        ...
        self.mock_http302_postrequest(
            ...
            cradmin_instance=self.__mock_crinstance(),
            requestuser=testuser,
            requestkwargs={"data": {"feedback_comment_text": "feedback comment", "selected_items": [testgroup_1.id]}},
        )
        self.assertEqual(FeedbacksetPassedPreviousPeriod.objects.count(), 1)
    ...
    def test_post_feedback_updates_score_last_feedbackset_is_first_feedbackset(self):
        ...
        group_baker.feedbackset_first_attempt_published(
            group=testgroup, grading_points=0, grading_published_datetime=grading_time
        )
        testuser = baker.make(settings.AUTH_USER_MODEL)
    ...
    def test_post_feedback_updates_score_last_feedbackset_is_not_first_feedbackset(self):
        ...
        group_baker.feedbackset_first_attempt_published(
            group=testgroup, grading_points=1, grading_published_datetime=first_feedbackset_publishing_datetime
        )
        group_baker.feedbackset_new_attempt_published(
            group=testgroup, grading_points=0, grading_published_datetime=last_feedbackset_publishing_datetime
        )
        testuser = baker.make(settings.AUTH_USER_MODEL)
