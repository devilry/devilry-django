...
class TestOverview(TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_title(self):
        ...
        self.assertEqual(mockresponse.selector.one("title").alltext_normalized, "Passed previous semester")
    ...
    def test_assignment_grading_plugin_not_passed_failed(self):
        testassignment = baker.make(
            "core.Assignment", grading_system_plugin_id=Assignment.GRADING_SYSTEM_PLUGIN_ID_POINTS
        )
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testassignment)
        self.assertEqual(
            mockresponse.selector.one(
                ".devilry-passed-previous-semester-unsupported-plugin-message > h3"
            ).alltext_normalized,
