...
class TestOverview(TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_title(self):
        ...
        self.assertEqual("testsubject", mockresponse.selector.one("title").alltext_normalized)

    def test_h1(self):
        testsubject = baker.make("core.Subject", long_name="Test Subject")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testsubject)
        self.assertEqual("Test Subject", mockresponse.selector.one("h1").alltext_normalized)
    ...
    def test_periodlist_itemrendering_name(self):
        ...
        testperiod = baker.make_recipe(
            "devilry.apps.core.period_active", parentnode=testsubject, long_name="Test Period"
        )
        testuser = baker.make(settings.AUTH_USER_MODEL)
        periodpermissiongroup = baker.make("devilry_account.PeriodPermissionGroup", period=testperiod)
        baker.make(
    ...
    def test_periodlist_itemrendering_url(self):
        ...
        self.assertEqual(
            crinstance.reverse_cradmin_url(
                instanceid="devilry_admin_periodadmin", appname="overview", roleid=testperiod.id
            ),
            mockresponse.selector.one(".devilry-admin-period-overview-perioditemframe")["href"],
        )
    ...
    def test_periodlist_ordering(self):
        testsubject = baker.make("core.Subject")
        testperiod1 = baker.make_recipe("devilry.apps.core.period_active", parentnode=testsubject, long_name="Period 2")
        testperiod2 = baker.make_recipe("devilry.apps.core.period_old", parentnode=testsubject, long_name="Period 1")
        testperiod3 = baker.make_recipe("devilry.apps.core.period_future", parentnode=testsubject, long_name="Period 3")
        testuser = baker.make(settings.AUTH_USER_MODEL)
        ...
        baker.make(
            "devilry_account.PermissionGroupUser", user=testuser, permissiongroup=periodpermissiongroup3.permissiongroup
        )
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testsubject, requestuser=testuser)
        periodnames = [
    ...
    def test_periodlist_only_periods_in_subject(self):
        ...
        testperiod1 = baker.make_recipe(
            "devilry.apps.core.period_active", parentnode=testsubject, long_name="Testsubject Period 1"
        )
        testperiod2 = baker.make_recipe(
            "devilry.apps.core.period_active", parentnode=othersubject, long_name="Othersubject Period 1"
        )
        testuser = baker.make(settings.AUTH_USER_MODEL)
        periodpermissiongroup = baker.make("devilry_account.PeriodPermissionGroup", period=testperiod1)
        baker.make(
            "devilry_account.PermissionGroupUser", user=testuser, permissiongroup=periodpermissiongroup.permissiongroup
        )
        periodpermissiongroup2 = baker.make("devilry_account.PeriodPermissionGroup", period=testperiod2)
