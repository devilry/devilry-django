...
class TestAllResultsCollector(test.TestCase):
    ...
    def test_student_for_period(self):
        ...
        # Run collector
        collector = PeriodAllResultsCollector(period=testassignment.parentnode, related_student_ids=[relatedstudent.id])
    ...
    def test_get_result_for_assignment_feedbackset_is_published(self):
        ...
        # Run collector
        collector = PeriodAllResultsCollector(period=testperiod, related_student_ids=[relatedstudent.id])
        self.assertEqual(10, collector.results[relatedstudent.id].get_result_for_assignment(testassignment.id))
    ...
    def test_get_result_for_assignment_where_student_is_not_registered(self):
        ...
        group_factory.feedbackset_first_attempt_unpublished(group=testgroup, related_student_ids=[relatedstudent.id])
        # Run collector
        collector = PeriodAllResultsCollector(period=testperiod, related_student_ids=[relatedstudent.id])
    ...
    def test_students_are_registered_on_period_but_no_assignments(self):
        ...
        collector = PeriodAllResultsCollector(
            period=testassignment.parentnode, related_student_ids=[relatedstudent_donald.id, relatedstudent_april.id]
        )
    ...
    def test_students_in_separate_groups_points_for_assignment(self):
        ...
        self.assertEqual(
            10, collector.results[relatedstudent_donald.id].get_result_for_assignment(assignment_id=testassignment.id)
        )
        self.assertEqual(
            15, collector.results[relatedstudent_april.id].get_result_for_assignment(assignment_id=testassignment.id)
        )
    ...
    def test_students_in_same_group_points_for_assignment(self):
        ...
        self.assertEqual(10, collector.results[relatedstudent_donald.id].get_result_for_assignment(testassignment.id))
        self.assertEqual(10, collector.results[relatedstudent_april.id].get_result_for_assignment(testassignment.id))

    def test_student_get_total_result_for_period(self):
        testperiod = baker.make("core.Period")
        testassignment1 = baker.make(
            "core.Assignment", parentnode=testperiod, max_points=20, passing_grade_min_points=12
        )
        testassignment2 = baker.make(
            "core.Assignment", parentnode=testperiod, max_points=30, passing_grade_min_points=15
        )
        testassignment3 = baker.make(
            "core.Assignment", parentnode=testperiod, max_points=15, passing_grade_min_points=5
        )
        ...
        group_factory.feedbackset_first_attempt_published(group=testgroup1, grading_points=10)
        group_factory.feedbackset_first_attempt_published(group=testgroup2, grading_points=11)
        group_factory.feedbackset_first_attempt_published(group=testgroup3, grading_points=12)

        collector = PeriodAllResultsCollector(period=testperiod, related_student_ids=[relatedstudent_donald.id])
        self.assertEqual(33, collector.results[relatedstudent_donald.id].get_total_result())
    ...
    def test_num_queries_sanity_test(self):
        ...
        group_factory.feedbackset_first_attempt_published(group=testgroup4, grading_points=20)
        group_factory.feedbackset_first_attempt_published(group=testgroup5, grading_points=30)
        group_factory.feedbackset_first_attempt_published(group=testgroup6, grading_points=15)
        with self.assertNumQueries(2):
            collector = PeriodAllResultsCollector(
                period=testperiod, related_student_ids=[relatedstudent_donald.id, relatedstudent_april.id]
            )
