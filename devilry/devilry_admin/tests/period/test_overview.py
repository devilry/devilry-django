...
class TestOverview(TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def __make_subject_admin_user(self, subject, as_department_admin=False):
        ...
        subjectpermissiongroup = baker.make(
            "devilry_account.SubjectPermissionGroup", permissiongroup__grouptype=permissiongroup_type, subject=subject
        )
        baker.make(
            "devilry_account.PermissionGroupUser",
            user=admin_user,
    ...
    def test_title(self):
        ...
        self.assertEqual("testsubject.testperiod", mockresponse.selector.one("title").alltext_normalized)

    def test_h1(self):
        testperiod = baker.make("core.Period", parentnode__long_name="Test Subject", long_name="Test Period")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testperiod)
        self.assertEqual("Test Period \u2014 Test Subject", mockresponse.selector.one("h1").alltext_normalized)

    def test_no_students_and_examiners_on_semester_meta(self):
        testperiod = baker.make("core.Period", parentnode__long_name="Test Subject", long_name="Test Period")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testperiod)
        self.assertEqual(
            mockresponse.selector.one("#id_devilry-num-relatedusers-on-period").alltext_normalized,
    ...
    def test_link_urls(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testperiod, cradmin_instance=mock_crinstance)
    ...
    def test_assignmentlist_itemrendering_url(self):
        ...
        self.assertEqual(
            crinstance.reverse_cradmin_url(
                instanceid="devilry_admin_assignmentadmin", appname="overview", roleid=testassignment.id
            ),
            mockresponse.selector.one(".devilry-admin-period-overview-assignmentitemframe")["href"],
        )
    ...
    def test_assignmentlist_ordering(self):
        ...
        periodadmin_user = self.__make_period_admin_user(period=testperiod)
        baker.make_recipe(
            "devilry.apps.core.assignment_activeperiod_start", parentnode=testperiod, long_name="Assignment 1"
        )
        baker.make_recipe(
            "devilry.apps.core.assignment_activeperiod_middle", parentnode=testperiod, long_name="Assignment 2"
        )
        baker.make_recipe(
            "devilry.apps.core.assignment_activeperiod_end", parentnode=testperiod, long_name="Assignment 3"
        )
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testperiod, requestuser=periodadmin_user)
        assignmentnames = [
    ...
    def test_periodlist_only_assignments_in_period(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=testperiod, requestuser=periodadmin_user_testperiod
        )
    ...
    def test_subject_admin_can_see_semi_anonymous_assignments(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testperiod, requestuser=subjectadmin_user)
        self.assertFalse(mockresponse.selector.exists("#devilry_admin_period_overview_assignmentlist"))
    ...
    def test_department_admin_can_see_semi_anonymous_assignments(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testperiod, requestuser=departmentadmin_user)
        self.assertFalse(mockresponse.selector.exists("#devilry_admin_period_overview_assignmentlist"))
    ...
    def test_num_queries(self):
        testperiod = baker.make_recipe("devilry.apps.core.period_active")
        periodadmin_user = self.__make_period_admin_user(period=testperiod)
        baker.make_recipe("devilry.apps.core.assignment_activeperiod_start", parentnode=testperiod, _quantity=10)
        baker.make_recipe("devilry.apps.core.assignment_activeperiod_middle", parentnode=testperiod, _quantity=10)
        baker.make_recipe("devilry.apps.core.assignment_activeperiod_end", parentnode=testperiod, _quantity=10)
