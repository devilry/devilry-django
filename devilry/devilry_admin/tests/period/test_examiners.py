...
class TestOverview(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_title(self):
        ...
        self.assertIn("Examiners on testsubject.testperiod", mockresponse.selector.one("title").alltext_normalized)

    def test_h1(self):
        testperiod = baker.make("core.Period", parentnode__short_name="testsubject", short_name="testperiod")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testperiod)
        self.assertEqual("Examiners on testsubject.testperiod", mockresponse.selector.one("h1").alltext_normalized)
    ...
    def test_buttonbar_addbutton_link(self):
        ...
        self.assertEqual(
            "/add", mockresponse.selector.one("#devilry_admin_period_examiners_overview_button_add")["href"]
        )
    ...
    def test_buttonbar_addbutton_label(self):
        ...
        self.assertEqual(
            "Add examiners",
            mockresponse.selector.one("#devilry_admin_period_examiners_overview_button_add").alltext_normalized,
        )
    ...
    def test_buttonbar_importbutton_rendered_for_departmentadmin_sanity(self):
        ...
        self.assertTrue(mockresponse.selector.exists("#devilry_admin_period_examiners_overview_button_importexaminers"))
    ...
    def test_buttonbar_importbutton_label(self):
        ...
        self.assertEqual(
            ...
            mockresponse.selector.one(
                "#devilry_admin_period_examiners_overview_button_importexaminers"
            ).alltext_normalized,
        )
    ...
    def test_default_ordering(self):
        testperiod = baker.make("core.Period")
        baker.make("core.RelatedExaminer", period=testperiod, user__fullname="UserB")
        baker.make("core.RelatedExaminer", period=testperiod, user__shortname="usera")
        baker.make("core.RelatedExaminer", period=testperiod, user__shortname="userc")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testperiod)
        self.assertEqual(["usera", "UserB", "userc"], self.__get_titles(mockresponse.selector))

    def test_only_users_from_current_period(self):
        testperiod = baker.make("core.Period")
        baker.make("core.RelatedExaminer", period=testperiod, user__shortname="usera")
        baker.make("core.RelatedExaminer", user__shortname="fromotherperiod")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testperiod)
    ...
    def test_inactive_relatedexaminer_sanity(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testperiod)
        self.assertFalse(mockresponse.selector.exists(".devilry-admin-relatedexaminer-itemvalue-active"))
        self.assertTrue(mockresponse.selector.exists(".devilry-admin-relatedexaminer-itemvalue-inactive"))
        self.assertFalse(mockresponse.selector.exists(".devilry-admin-period-active-relatedexaminer-block"))
        self.assertTrue(mockresponse.selector.exists(".devilry-admin-period-inactive-relatedexaminer-block"))
    ...
    def test_inactive_relatedexaminer_message(self):
        ...
        self.assertEqual(
            ...
            mockresponse.selector.one(".devilry-admin-period-inactive-relatedexaminer-message").alltext_normalized,
        )
    ...
    def test_inactive_relatedexaminer_link_label(self):
        ...
        self.assertEqual(
            "Re-activate",
            mockresponse.selector.one(".devilry-admin-period-inactive-relatedexaminer-link").alltext_normalized,
        )

    def test_inactive_relatedexaminer_link_arialabel(self):
        testperiod = baker.make("core.Period")
        baker.make("core.RelatedExaminer", period=testperiod, active=False, user__shortname="testuser")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testperiod)
        self.assertEqual(
            "Re-activate testuser",
            mockresponse.selector.one(".devilry-admin-period-inactive-relatedexaminer-link")["aria-label"],
        )
    ...
    def test_active_relatedexaminer_sanity(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testperiod)
        self.assertTrue(mockresponse.selector.exists(".devilry-admin-relatedexaminer-itemvalue-active"))
        self.assertFalse(mockresponse.selector.exists(".devilry-admin-relatedexaminer-itemvalue-inactive"))
        self.assertTrue(mockresponse.selector.exists(".devilry-admin-period-active-relatedexaminer-block"))
        self.assertFalse(mockresponse.selector.exists(".devilry-admin-period-inactive-relatedexaminer-block"))
    ...
    def test_active_relatedexaminer_link_label(self):
        ...
        self.assertEqual(
            "Mark as inactive",
            mockresponse.selector.one(".devilry-admin-period-active-relatedexaminer-link").alltext_normalized,
        )

    def test_active_relatedexaminer_link_arialabel(self):
        testperiod = baker.make("core.Period")
        baker.make("core.RelatedExaminer", period=testperiod, active=True, user__shortname="testuser")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testperiod)
        self.assertEqual(
            "Mark testuser as inactive",
            mockresponse.selector.one(".devilry-admin-period-active-relatedexaminer-link")["aria-label"],
        )
...
class TestDeactivateExaminerView(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_get_title(self):
        ...
        testperiod = baker.make("core.Period", parentnode__short_name="testsubject", short_name="testperiod")
        relatedexaminer = baker.make("core.RelatedExaminer", period=testperiod, user__fullname="John Doe")
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=testperiod, requestuser=requestuser, viewkwargs={"pk": relatedexaminer.pk}
        )
        self.assertEqual(mockresponse.selector.one("title").alltext_normalized, "Deactivate examiner: John Doe?")
    ...
    def test_get_h1(self):
        ...
        self.assertEqual(mockresponse.selector.one("h1").alltext_normalized, "Deactivate examiner: John Doe?")
    ...
    def test_404_if_not_relatedexaminer_on_period(self):
        ...
        relatedexaminer = baker.make("core.RelatedExaminer", period=otherperiod)
    ...
    def test_post_success_message(self):
        ...
        messagesmock.add.assert_called_once_with(messages.SUCCESS, "John Doe was deactivated.", "")
...
class TestActivateExaminerView(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_get_title(self):
        ...
        self.assertEqual(mockresponse.selector.one("title").alltext_normalized, "Re-activate examiner: John Doe?")
    ...
    def test_get_h1(self):
        ...
        self.assertEqual(mockresponse.selector.one("h1").alltext_normalized, "Re-activate examiner: John Doe?")
    ...
    def test_post_success_message(self):
        ...
        messagesmock.add.assert_called_once_with(messages.SUCCESS, "John Doe was re-activated.", "")
...
class TestAddView(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_do_not_include_users_already_relatedexaminer(self):
        ...
        baker.make("core.RelatedExaminer", period=testperiod, user__fullname="Already in period")
        baker.make("core.RelatedExaminer", user__fullname="In other period")
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=mock.MagicMock(), cradmin_role=testperiod)
    ...
    def test_post_creates_relatedexaminers(self):
        ...
        self.mock_http302_postrequest(
            cradmin_role=testperiod, requestkwargs={"data": {"selected_items": [str(examineruser.id)]}}
        )
    ...
    def test_post_multiple_users(self):
        ...
        self.mock_http302_postrequest(
            cradmin_role=testperiod,
            requestkwargs={
                "data": {"selected_items": [str(examineruser1.id), str(examineruser2.id), str(examineruser3.id)]}
            },
        )
    ...
    def test_post_success_message(self):
        testperiod = baker.make("core.Period", parentnode__short_name="testsubject", short_name="testperiod")
        examineruser1 = baker.make(settings.AUTH_USER_MODEL, shortname="testuser")
        examineruser2 = baker.make(settings.AUTH_USER_MODEL, fullname="Test User")
        self.assertEqual(0, RelatedExaminer.objects.count())
        messagesmock = mock.MagicMock()
        self.mock_http302_postrequest(
            cradmin_role=testperiod,
            messagesmock=messagesmock,
            requestkwargs={"data": {"selected_items": [str(examineruser1.id), str(examineruser2.id)]}},
        )
...
class TestImportExaminersView(test.TestCase, AbstractTypeInUsersViewTestMixin):
    ...
    def test_post_valid_with_email_backend_added_message(self):
        ...
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=True):
            ...
            messagesmock.add.assert_any_call(
                messages.SUCCESS, "Added 2 new examiners to {}.".format(testperiod.get_path()), ""
            )
    ...
    def test_post_valid_with_email_backend_none_added_message(self):
        ...
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=True):
            ...
            messagesmock.add.assert_any_call(messages.WARNING, "No new examiners was added.", "")
    ...
    def test_post_valid_with_email_backend_existing_message(self):
        ...
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=True):
            ...
            messagesmock.add.assert_called_with(
                messages.INFO, "1 users was already examiner on {}.".format(testperiod.get_path()), ""
            )
