...
from devilry.apps.core.baker_recipes import (
    ACTIVE_PERIOD_END,
    ACTIVE_PERIOD_START,
...
from devilry.apps.core.models import Assignment, AssignmentGroup, Candidate, Examiner
...
class TestCreateView(TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_get_suggested_name_previous_assignment_not_suffixed_with_number(self):
        period = baker.make_recipe(
            "devilry.apps.core.assignment_activeperiod_start", long_name="Test", short_name="test"
        ).period
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=period)
        self.assertEqual(mockresponse.selector.one("input[name=long_name]").get("value", ""), "")
        self.assertEqual(mockresponse.selector.one("input[name=short_name]").get("value", ""), "")
    ...
    def test_get_suggested_name_previous_assignment_suffixed_with_number(self):
        period = baker.make_recipe(
            "devilry.apps.core.assignment_activeperiod_start", long_name="Test1", short_name="test1"
        ).period
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=period)
    ...
    def test_get_suggested_name_previous_assignment_suffixed_with_number_namecollision_no_first_deadline(self):
        period = baker.make_recipe("devilry.apps.core.period_active")
        baker.make_recipe(
            "devilry.apps.core.assignment_activeperiod_start", parentnode=period, long_name="Test1", short_name="test1"
        )
        baker.make_recipe(
    ...
    def test_get_suggested_deadlines_first_assignment(self):
        period = baker.make_recipe("devilry.apps.core.period_active")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=period)
        self.assertFalse(mockresponse.selector.exists("#devilry_admin_createassignment_suggested_deadlines"))
    ...
    def test_get_suggested_deadlines_not_first_assignment(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=period)
        self.assertTrue(mockresponse.selector.exists("#devilry_admin_createassignment_suggested_deadlines"))

    @unittest.skip("Must be revised. Depends on Assignment.first_deadline being None.")
    def test_get_suggested_deadlines_not_first_assignment_no_previous_with_deadline(self):
        period = baker.make_recipe("devilry.apps.core.assignment_activeperiod_start", first_deadline=None).period
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=period)
        self.assertFalse(mockresponse.selector.exists("#devilry_admin_createassignment_suggested_deadlines"))
    ...
    def test_get_suggested_deadlines_render_values_previous_deadline_in_the_past(self):
        ...
        baker.make_recipe("devilry.apps.core.assignment_activeperiod_start", parentnode=period)

        # This should be the one that is used for suggestions
        baker.make_recipe(
            "devilry.apps.core.assignment_activeperiod_start",
            parentnode=period,
            first_deadline=default_timezone_datetime(2015, 9, 2, 13, 30),
        )  # Wed
        ...
        suggested_deadline_values = [element["cradmin-legacy-setfieldvalue"] for element in suggested_deadline_elements]
        self.assertEqual(
            suggested_deadline_values,
            [
                "2015-09-16 13:30",
                "2015-09-23 13:30",
                "2015-09-30 13:30",
    ...
    def test_post_missing_short_name(self):
        ...
        mockresponse = self.mock_http200_postrequest_htmls(
            ...
            requestkwargs={
                "data": {
                    ...
                    "short_name": "",
                    "first_deadline": datetimeutils.isoformat_noseconds(
                        default_timezone_datetime(3000, 12, 31, 23, 59)
                    ),
                }
            },
        )
        ...
        self.assertEqual(
            "This field is required.", mockresponse.selector.one("#error_1_id_short_name").alltext_normalized
        )
    ...
    def test_post_missing_first_deadline(self):
        ...
        self.assertEqual(Assignment.objects.count(), 0)
        self.assertEqual(
            "This field is required.", mockresponse.selector.one("#error_1_id_first_deadline").alltext_normalized
        )
    ...
    def test_post_missing_student_import_option(self):
        ...
        self.assertEqual(Assignment.objects.count(), 0)
        self.assertEqual(
            "This field is required.", mockresponse.selector.one("#error_1_id_student_import_option").alltext_normalized
        )
    ...
    def test_post_import_all_students_on_semester(self):
        ...
        self.assertEqual(
            Candidate.objects.filter(
                assignment_group__parentnode=created_assignment, relatedstudent=relatedstudent1
            ).count(),
            1,
        )
        self.assertEqual(
            Candidate.objects.filter(
                assignment_group__parentnode=created_assignment, relatedstudent=relatedstudent2
            ).count(),
            1,
        )
        self.assertEqual(
            Candidate.objects.filter(
                assignment_group__parentnode=created_assignment, relatedstudent=relatedstudent3
            ).count(),
            1,
        )
    ...
    def test_post_copy_all_students_from_another_assignment(self):
        ...
        baker.make(
            ...
            relatedstudent__user=relatedstudent_user1,
        )
        baker.make(
            "core.Candidate",
            assignment_group__parentnode=other_assignment,
            relatedstudent__period=period,
            relatedstudent__user=relatedstudent_user2,
        )
        self.mock_http302_postrequest(
            cradmin_role=period,
            requestkwargs={
        ...
        self.assertEqual(
            Candidate.objects.filter(
                assignment_group__parentnode=created_assignment, relatedstudent__user=relatedstudent_user1
            ).count(),
            1,
        )
        self.assertEqual(
            Candidate.objects.filter(
                assignment_group__parentnode=created_assignment, relatedstudent__user=relatedstudent_user2
            ).count(),
            1,
        )
    ...
    def test_post_copy_all_students_from_another_assignment_same_group_structure(self):
        ...
        baker.make(
            ...
            relatedstudent__period=period,
            relatedstudent__user=relatedstudent_user1_group1,
        )
        baker.make(
            "core.Candidate",
            assignment_group=group1,
            relatedstudent__period=period,
            relatedstudent__user=relatedstudent_user2_group1,
        )
        baker.make(
            "core.Candidate",
            assignment_group=group2,
            relatedstudent__period=period,
            relatedstudent__user=relatedstudent_user_group2,
        )
        self.mock_http302_postrequest(
            cradmin_role=period,
            requestkwargs={
        ...
        self.assertEqual(
            Candidate.objects.filter(
                assignment_group__name="group1", assignment_group__parentnode=created_assignment
            ).count(),
            2,
        )
        self.assertTrue(
        ...
        self.assertEqual(
            Candidate.objects.filter(
                assignment_group__name="group2", assignment_group__parentnode=created_assignment
            ).count(),
            1,
        )
        self.assertTrue(
    ...
    def test_post_copy_with_passing_grade_from_another_assignment(self):
        ...
        baker.make(
            "core.Candidate",
            assignment_group=group_passed,
            relatedstudent__period=period,
            relatedstudent__user=relatedstudent_user1,
        )
        baker.make(
            "core.Candidate",
            assignment_group=group_failed,
            relatedstudent__period=period,
            relatedstudent__user=relatedstudent_user2,
        )
        devilry_group_baker_factories.feedbackset_first_attempt_published(grading_points=1, group=group_passed)
        devilry_group_baker_factories.feedbackset_first_attempt_published(grading_points=0, group=group_failed)
        self.mock_http302_postrequest(
            cradmin_role=period,
            requestkwargs={
                "data": {
    ...
    def test_post_copy_with_passing_grade_from_another_assignment_same_group_structure(self):
        ...
        self.assertTrue(
            Candidate.objects.filter(assignment_group=created_group, relatedstudent__user=relatedstudent_user1).exists()
        )
        self.assertTrue(
            Candidate.objects.filter(assignment_group=created_group, relatedstudent__user=relatedstudent_user2).exists()
        )
    ...
    def test_post_first_deadline_before_publishing_time_hours(self):
        ...
        with self.settings(DEVILRY_ASSIGNMENT_PUBLISHING_TIME_DELAY_MINUTES=60 * 3):
            ...
            self.assertTrue(mockresponse.selector.exists("#error_1_id_first_deadline"))
            self.assertEqual(
                "First deadline must be at least 3 hours from now.",
    ...
    def __valid_post_request(
        self, period=None, first_deadline=None, publishing_time_delay_minutes=60, student_import_option="all"
    ):
    ...
    def test_post_sanity(self):
        ...
        self.assertEqual(first_deadline.replace(second=59), created_assignment.first_deadline)
    ...
    def test_post_future_publishing_time(self):
        period = baker.make_recipe("devilry.apps.core.period_future")
        created_assignment, mockresponse = self.__valid_post_request(
            period=period, first_deadline=ASSIGNMENT_FUTUREPERIOD_START_FIRST_DEADLINE, publishing_time_delay_minutes=60
        )
    ...
    def test_post_add_students_from_assignment(self):
        ...
        baker.make(
            "core.Candidate", assignment_group__parentnode=other_assignment, relatedstudent__period=period, _quantity=2
        )
        created_assignment, mockresponse = self.__valid_post_request(
