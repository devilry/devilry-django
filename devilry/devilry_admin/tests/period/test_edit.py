...
class TestUpdateView(TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_get_render_title(self):
        ...
        self.assertIn("Edit testsubject.testperiod", mockresponse.selector.one("title").alltext_normalized)

    def test_get_render_h1(self):
        testperiod = baker.make("core.Period", parentnode__short_name="testsubject", short_name="testperiod")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testperiod)
        self.assertEqual("Edit testsubject.testperiod", mockresponse.selector.one("h1").alltext_normalized)
    ...
    def test_get_render_formfields(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testperiod)
        self.assertEqual("Test period", mockresponse.selector.one("input[name=long_name]")["value"])
        self.assertEqual("testperiod", mockresponse.selector.one("input[name=short_name]")["value"])
        self.assertEqual(
    ...
    def __valid_post_request(self, testperiod, start_time=ACTIVE_PERIOD_START, end_time=ACTIVE_PERIOD_END, **kwargs):
    ...
    def test_post_sanity(self):
        ...
        updated_period, mockresponse = self.__valid_post_request(
            testperiod=testperiod, start_time=ACTIVE_PERIOD_START, end_time=ACTIVE_PERIOD_END
        )
    ...
    def test_post_success_redirect(self):
        ...
        self.__valid_post_request(testperiod=testperiod, cradmin_instance=mock_cradmin_instance)
