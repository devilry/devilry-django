...
class AbstractTypeInUsersViewTestMixin(cradmin_testhelpers.TestCaseMixin):
    ...
    def test_get_render_form(self):
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_instance=self.mock_crinstance_with_devilry_role())
    ...
    def test_get_render_form_help_text_email_backend(self):
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=True):
            mockresponse = self.mock_http200_getrequest_htmls(cradmin_instance=self.mock_crinstance_with_devilry_role())
            self.assertEqual(
                mockresponse.selector.one("#hint_id_users_blob").alltext_normalized,
    ...
    def test_get_render_form_help_text_username_backend(self):
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=False):
            ...
            self.assertEqual(
                mockresponse.selector.one("#hint_id_users_blob").alltext_normalized,
                'Type or paste in usernames separated by comma (","), space or one user on each line.',
            )
    ...
    def test_get_render_form_placeholder_email_backend(self):
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=True):
            mockresponse = self.mock_http200_getrequest_htmls(cradmin_instance=self.mock_crinstance_with_devilry_role())
            self.assertEqual(
                mockresponse.selector.one("#id_users_blob")["placeholder"], "jane@example.com\njohn@example.com"
            )

    def test_get_render_form_placeholder_username_backend(self):
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=False):
            mockresponse = self.mock_http200_getrequest_htmls(cradmin_instance=self.mock_crinstance_with_devilry_role())
            self.assertEqual(mockresponse.selector.one("#id_users_blob")["placeholder"], "jane\njohn")

    def test_post_blank(self):
        mockresponse = self.mock_http200_postrequest_htmls(cradmin_instance=self.mock_crinstance_with_devilry_role())
        self.assertEqual(
            "This field is required.", mockresponse.selector.one("#error_1_id_users_blob").alltext_normalized
        )
    ...
    def test_post_invalid_emails_simple(self):
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=True):
            ...
            self.assertEqual(
                "Invalid email addresses: test", mockresponse.selector.one("#error_1_id_users_blob").alltext_normalized
            )
    ...
    def test_post_invalid_emails_multiple(self):
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=True):
            mockresponse = self.mock_http200_postrequest_htmls(
                cradmin_instance=self.mock_crinstance_with_devilry_role(),
                requestkwargs=dict(data={"users_blob": "test,test2@example.com,test3@@example.com"}),
            )
            self.assertEqual(
                "Invalid email addresses: test, test3@@example.com",
    ...
    def test_post_invalid_usernames_simple(self):
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=False):
            ...
            self.assertEqual(
                "Invalid usernames: Test", mockresponse.selector.one("#error_1_id_users_blob").alltext_normalized
            )
    ...
    def test_post_invalid_usernames_multiple(self):
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=False):
            mockresponse = self.mock_http200_postrequest_htmls(
                cradmin_instance=self.mock_crinstance_with_devilry_role(),
                requestkwargs=dict(data={"users_blob": "test,test2@example.com,test3.ing,Test4,test5"}),
            )
            self.assertEqual(
                "Invalid usernames: Test4, test2@example.com, test3.ing",
                mockresponse.selector.one("#error_1_id_users_blob").alltext_normalized,
            )
...
class TestAbstractTypeInUsersView(TestCase, AbstractTypeInUsersViewTestMixin):
    ...
    def test_split_users_blob_single(self):
        self.assertEqual({"test@example.com"}, AbstractTypeInUsersView.split_users_blob("test@example.com"))
    ...
    def test_split_users_blob_messy_prefix(self):
        self.assertEqual({"test@example.com"}, AbstractTypeInUsersView.split_users_blob(", ;\n\n,\ntest@example.com"))
