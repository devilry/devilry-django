...
class TestCreateView(TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_get_render_title(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=subject)
        self.assertEqual("Create new semester - testcourse", mockresponse.selector.one("title").alltext_normalized)

    def test_get_render_h1(self):
        subject = baker.make("core.Subject")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=subject)
        self.assertEqual("Create new semester", mockresponse.selector.one("h1").alltext_normalized)
    ...
    def test_get_suggested_name_previous_period_not_suffixed_with_number(self):
        subject = baker.make_recipe("devilry.apps.core.period_old", long_name="Test", short_name="test").subject
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=subject)
        self.assertEqual(mockresponse.selector.one("input[name=long_name]").get("value", ""), "")
        self.assertEqual(mockresponse.selector.one("input[name=short_name]").get("value", ""), "")

    def test_get_suggested_name_previous_period_suffixed_with_number(self):
        subject = baker.make_recipe("devilry.apps.core.period_old", long_name="Test1", short_name="test1").subject
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=subject)
    ...
    def test_get_suggested_name_previous_period_suffixed_with_number_namecollision_strange_order(self):
        subject = baker.make("core.Subject")
        baker.make_recipe("devilry.apps.core.period_active", parentnode=subject, long_name="Test1", short_name="test1")
        baker.make_recipe("devilry.apps.core.period_old", parentnode=subject, long_name="Test2", short_name="test2")
    ...
    def test_post_missing_start_time(self):
        ...
        self.assertEqual(Period.objects.count(), 0)
        self.assertEqual(
            "This field is required.", mockresponse.selector.one("#error_1_id_start_time").alltext_normalized
        )
    ...
    def test_post_missing_end_time(self):
        ...
        self.assertEqual(Period.objects.count(), 0)
        self.assertEqual(
            "This field is required.", mockresponse.selector.one("#error_1_id_end_time").alltext_normalized
        )
    ...
    def test_post_start_time_before_end_time(self):
        ...
        self.assertEqual(Period.objects.count(), 0)
        self.assertEqual(
            "Start time must be before end time.", mockresponse.selector.one(".alert-danger").alltext_normalized
        )

    def __valid_post_request(self, subject=None, start_time=ACTIVE_PERIOD_START, end_time=ACTIVE_PERIOD_END):
        if not subject:
            subject = baker.make("core.Subject")
    ...
    def test_post_sanity(self):
        ...
        created_period, mockresponse = self.__valid_post_request(
            start_time=ACTIVE_PERIOD_START, end_time=ACTIVE_PERIOD_END
        )
    ...
    def test_post_success_redirect(self):
        ...
        self.assertEqual(
            ...
            crinstance.reverse_cradmin_url(
                instanceid="devilry_admin_periodadmin", appname="overview", roleid=created_period.id
            ),
        )
