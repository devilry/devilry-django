...
class TestUpdateView(TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_get_render_title(self):
        ...
        self.assertIn("Edit testsubject", mockresponse.selector.one("title").alltext_normalized)

    def test_get_render_h1(self):
        testsubject = baker.make("core.Subject", short_name="testsubject")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testsubject)
        self.assertEqual("Edit testsubject", mockresponse.selector.one("h1").alltext_normalized)

    def test_get_render_formfields(self):
        testsubject = baker.make("core.Subject", long_name="Test subject", short_name="testsubject")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testsubject)
        self.assertEqual("Test subject", mockresponse.selector.one("input[name=long_name]")["value"])
        self.assertEqual("testsubject", mockresponse.selector.one("input[name=short_name]")["value"])
    ...
    def __valid_post_request(self, testsubject, **kwargs):
    ...
    def test_post_sanity(self):
        testsubject = baker.make("core.Subject")
        updated_period, mockresponse = self.__valid_post_request(testsubject=testsubject)
    ...
    def test_post_success_redirect(self):
        ...
        self.__valid_post_request(testsubject=testsubject, cradmin_instance=mock_cradmin_instance)
