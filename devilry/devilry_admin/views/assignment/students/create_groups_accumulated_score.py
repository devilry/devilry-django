...
class SelectAssignmentItemValuePreMixin(object):
    ...
    def grading_plugin_label(self):
        if self.assignment.grading_system_plugin_id == Assignment.GRADING_SYSTEM_PLUGIN_ID_PASSEDFAILED:
            return pgettext_lazy("admin create_groups_accumulated_score_on_assignments", "Passed/failed")
        if self.assignment.grading_system_plugin_id == Assignment.GRADING_SYSTEM_PLUGIN_ID_POINTS:
            return pgettext_lazy("admin create_groups_accumulated_score_on_assignments", "Points")
        return pgettext_lazy("admin create_groups_accumulated_score_on_assignments", "Unknown grading plugin.")

    def get_description(self):
        return pgettext_lazy("admin create_groups_accumulated_score_on_assignments", "No information available")
...
class AssignmentItemTargetRenderer(multiselect2.target_renderer.Target):
    def get_with_items_title(self):
        return pgettext_lazy("admin create_groups_accumulated_score_on_assignments", "Selected assignments")

    def get_without_items_text(self):
        return pgettext_lazy("admin create_groups_accumulated_score_on_assignments", "No assignments selected")

    def get_submit_button_text(self):
        return pgettext_lazy("admin create_groups_accumulated_score_on_assignments", "Use selected assignments")
...
class SelectAssignmentsForm(forms.Form):
    selected_items = forms.ModelMultipleChoiceField(
        widget=forms.MultipleHiddenInput, queryset=Assignment.objects.none()
    )
    ...
    points_threshold = forms.IntegerField(
        min_value=0,
        required=True,
        label=pgettext_lazy("admin create_groups_accumulated_score_on_assignments", "Points threshold"),
        help_text=pgettext_lazy(
            "admin create_groups_accumulated_score_on_assignments",
            "The minimum score the students need to have accumulated across the selected assignments.",
        ),
    )
...
class SelectAssignmentsView(multiselect2view.ListbuilderView):
    ...
    def get_queryset_for_role(self, role):
        return (
            Assignment.objects.exclude(id=role.id).filter(parentnode_id=role.parentnode.id).order_by("-publishing_time")
        )
...
class PreviewRelatedstudentsListView(listbuilderview.View):
    ...
    def dispatch(self, request, *args, **kwargs):
        if (
            "from_select_assignment_view" not in request.session
            or "selected_assignment_ids" not in request.session
            or "points_threshold" not in request.session
        ):
            raise Http404()
    ...
    def __get_relatedstudent_ids_already_on_assignment(self):
        ...
        return RelatedStudent.objects.filter(
            period=assignment.parentnode, candidate__assignment_group__parentnode=assignment
    ...
    def __get_relatedstudents(self, selected_assignment_ids, points_threshold):
        ...
        """
        queryset = (
            RelatedStudent.objects.filter(candidate__assignment_group__parentnode_id__in=selected_assignment_ids)
            .filter(active=True)
            .distinct()
        )
        ...
        queryset = (
            queryset.exclude(id__in=self.__get_relatedstudent_ids_already_on_assignment())
    ...
    def __get_selected_assignments_queryset(self):
        return Assignment.objects.filter(id__in=self.selected_assignment_ids)

    def get_queryset_for_role(self, role):
        return self.__get_relatedstudents(
            selected_assignment_ids=self.selected_assignment_ids, points_threshold=self.points_threshold
        )
