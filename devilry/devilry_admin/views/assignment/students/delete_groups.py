...
class ChooseManualSelectItemValue(listbuilder.itemvalue.TitleDescription):
    ...
    def get_description(self):
        return pgettext_lazy("admin delete_groups", "You can delete some students on this assignment, or all of them")
...
class ChooseAssignmentItemValue(listbuilder.itemvalue.TitleDescription):
    ...
    def get_title(self):
        return pgettext_lazy("admin delete_groups", "Students that failed %(assignment)s") % {
            "assignment": self.assignment.long_name
        }

    def get_description(self):
        return pgettext_lazy("admin delete_groups", "Delete students that did not pass %(assignment)s") % {
            "assignment": self.assignment.long_name
        }
...
class ChooseMethod(TemplateView):
    ...
    def __make_listbuilder_list(self):
        ...
        listbuilder_list.append(ManuallyDeleteMethodLinkFrame(ChooseManualSelectItemValue(value=current_assignment)))
        ...
        for assignment in assignments:
            listbuilder_list.append(
                FromAssignmentDeleteMethodLinkFrame(
                    current_assignment=current_assignment, inneritem=ChooseAssignmentItemValue(value=assignment)
                )
            )
...
class DeleteGroupsView(groupview_base.BaseMultiselectView):
    ...
    def get_unfiltered_queryset_for_role(self, role):
        ...
        if self.has_delete_with_content_permission():
            ...
            return (
                queryset.exclude(cached_data__public_student_comment_count__gt=0)
    ...
    def get_success_message(self, candidatecount):
        return gettext_lazy("Deleted %(count)s students from this assignment.") % {"count": candidatecount}

    def __count_candidates_in_assignmentgroups(self, groupqueryset):
        return Candidate.objects.filter(assignment_group__in=groupqueryset).count()
    ...
    def form_valid(self, form):
        ...
        candidatecount = self.__count_candidates_in_assignmentgroups(groupqueryset=groupqueryset)
...
class SelectedAssignmentGroupsForm(forms.Form):
    selected_items = forms.ModelMultipleChoiceField(
        widget=forms.MultipleHiddenInput, queryset=AssignmentGroup.objects.none()
    )
...
class BulkSelectionDeleteGroupsViewMixin(object):
    ...
    def get_form_kwargs(self):
        kwargs = {"assignmentgroup_queryset": self.get_unfiltered_queryset_for_role(role=self.request.cradmin_role)}
...
class ConfirmView(BulkSelectionDeleteGroupsViewMixin, listbuilder_assignmentgroup.VerticalFilterListView):
    """
    ...
    def get_pagetitle(self):
        return pgettext_lazy(
            "admin delete_groups", "Confirm that you want to delete students that failed %(from_assignment_name)s"
    ...
    def get_filterlist_url(self, filters_string):
        return self.request.cradmin_app.reverse_appurl(
            viewname="confirm_delete",
            kwargs={"from_assignment_id": self.from_assignment.id, "filters_string": filters_string},
        )
    ...
    def __failed_from_group_queryset(self):
        ...
        """
        return (
            AssignmentGroup.objects.filter(parentnode_id=self.from_assignment.id)
    ...
    def get_unfiltered_queryset_for_role(self, role):
        ...
        """
        failed_group_ids = self.__failed_from_group_queryset().values_list("id", flat=True)
        relatedstudents_from_failed_groups_ids = Candidate.objects.filter(
            assignment_group_id__in=failed_group_ids
        ).values_list("relatedstudent_id", flat=True)
        return (
            AssignmentGroup.objects.filter(parentnode_id=role.id)
            .prefetch_related(
                models.Prefetch(
                    "candidates",
    ...
    def __get_formhelper(self):
        ...
        helper.layout = layout.Layout(
            "selected_items", PrimarySubmit("delete_groups", pgettext_lazy("admin delete_groups", "Delete students"))
        )
        helper.form_action = self.request.get_full_path()
        return helper

    def __get_total_candidate_count(self):
        return Candidate.objects.filter(assignment_group__parentnode=self.request.cradmin_role).count()

    def get_success_message(self, delete_group_count):
        return pgettext_lazy(
            "admin delete groups confirm", "Successfully deleted %(delete_group_count)s students from the assignment"
        ) % {"delete_group_count": delete_group_count}

    def get_context_data(self, **kwargs):
        context = super(ConfirmView, self).get_context_data(**kwargs)
        context["from_assignment"] = self.from_assignment
        context["no_groups_found"] = not self.get_unfiltered_queryset_for_role(role=self.request.cradmin_role).exists()
        num_candidates_total = self.__get_total_candidate_count()
        num_to_be_deleted = self.get_unfiltered_queryset_for_role(role=self.request.cradmin_role).count()
...
class App(crapp.App):
    appurls = [
        ...
        crapp.Url(r"^manual-select$", DeleteGroupsView.as_view(), name="manual_select"),
        crapp.Url(r"^manual-select/filter/(?P<filters_string>.+)?$", DeleteGroupsView.as_view(), name="filter"),
        crapp.Url(
