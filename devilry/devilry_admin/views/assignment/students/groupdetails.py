...
class GroupDetailsView(detail.DetailView):
    ...
    def get_queryset_for_role(self, role):
        assignment = role
        candidatequeryset = Candidate.objects.select_related("relatedstudent__user").order_by(
            Lower(
                Concat(
        ...
        examinerqueryset = Examiner.objects.select_related("relatedexaminer__user").order_by(
            Lower(
                Concat(
        ...
        return (
            AssignmentGroup.objects.filter(parentnode=assignment)
    ...
    def get_context_data(self, **kwargs):
        ...
        context["groupdetails"] = GroupDetailsRenderable(value=self.group, assignment=self.assignment)
        return context


class App(crapp.App):
    appurls = [
        crapp.Url(r"^(?P<pk>\d+)$", GroupDetailsView.as_view(), name="groupdetails"),
    ]
