...
class MergeGroupsView(groupview_base.BaseMultiselectView):
    ...
    def __get_merged_candidates_short_name(self, group):
        candidates = Candidate.objects.filter(assignment_group=group).select_related("relatedstudent__user")
        ret = ""
    ...
    def form_valid(self, form):
        try:
            ...
            messages.warning(self.request, gettext_lazy(str(e.message)))
        else:
            candidates_short_name = self.__get_merged_candidates_short_name(self.target_group)
            messages.success(
                self.request, gettext_lazy("A group with %(what)s has been created!") % {"what": candidates_short_name}
            )
...
class App(crapp.App):
    appurls = [
        crapp.Url(r"^$", MergeGroupsView.as_view(), name=crapp.INDEXVIEW_NAME),
        crapp.Url(r"^filter/(?P<filters_string>.+)?$", MergeGroupsView.as_view(), name="filter"),
    ]
