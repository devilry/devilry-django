...
class ChoosePeriodItemValue(listbuilder.itemvalue.TitleDescription):
    ...
    def get_title(self):
        return pgettext_lazy("admin create_groups", "Select students from %(subject)s %(period)s") % {
...
class ChooseAssignmentItemValue(listbuilder.itemvalue.TitleDescription):
    ...
    def get_title(self):
        return pgettext_lazy("admin create_groups", "Select students from %(assignment)s") % {
...
class ChooseAccumulatedAssignmentScore(listbuilder.itemvalue.TitleDescription):
    ...
    def get_title(self):
        return pgettext_lazy(
            "admin create_groups_accumulated_score_on_assignments", "Add students based on their total score"
        )

    def get_description(self):
        return pgettext_lazy(
            "admin create_groups_accumulated_score_on_assignments",
            "Add students based on their total score across assignments you select from this semester.",
        )
...
class ChooseMethod(TemplateView):
    ...
    def __make_listbuilder_list(self):
        ...
        listbuilder_list.append(listbuilder.itemframe.DefaultSpacingItemFrame(ChoosePeriodItemValue(value=self.period)))

        # Choices for adding all students or only students with passing grade from assignments.
        assignments = self.period.assignments.order_by("-publishing_time").exclude(pk=self.assignment.pk)
        for assignment in assignments:
            listbuilder_list.append(
                listbuilder.itemframe.DefaultSpacingItemFrame(ChooseAssignmentItemValue(value=assignment))
            )
...
class CreateGroupsViewMixin(object):
    ...
    def __get_relatedstudents_in_group_on_assignment(self):
        assignment = self.request.cradmin_role
        return Candidate.objects.filter(assignment_group__parentnode=assignment).values_list(
            "relatedstudent_id", flat=True
        )

    def get_unfiltered_queryset_for_role(self, role):
        queryset = (
            self.period.relatedstudent_set.prefetch_syncsystemtag_objects().filter(active=True).select_related("user")
        )
    ...
    def get_form_kwargs(self):
        available_relatedstudents_queryset = self.get_unfiltered_queryset_for_role(role=self.request.cradmin_role)
    ...
    def create_groups_with_candidate_and_feedbackset(self, relatedstudent_queryset):
        ...
        return AssignmentGroup.objects.bulk_create_groups(
            created_by_user=self.request.user, assignment=assignment, relatedstudents=list(relatedstudent_queryset)
        )
...
class ConfirmView(CreateGroupsViewMixin, listbuilder_relatedstudent.VerticalFilterListView):
    ...
    def get_pagetitle(self):
        return pgettext_lazy(
            "admin create_groups", "Confirm that you want to add the following students to %(assignment)s"
    ...
    def get_filterlist_url(self, filters_string):
        return self.request.cradmin_app.reverse_appurl(
            "confirm", kwargs={"selected_students": self.kwargs["selected_students"], "filters_string": filters_string}
        )
    ...
    def __get_assignment_id_form_class(self):
        ...
        class AssignmentIdForm(forms.Form):
            assignment = forms.ModelChoiceField(queryset=assignment_queryset, required=True)
    ...
    def __filter_students_on_assignment(self, assignment, relatedstudents_queryset, only_passing_grade):
        matching_candidates = Candidate.objects.filter(assignment_group__parentnode=assignment)
        if only_passing_grade:
            matching_candidates = matching_candidates.filter_has_passing_grade(assignment=assignment)
        matching_relatedstudent_ids = matching_candidates.values_list("relatedstudent_id", flat=True)
    ...
    def __filter_selected_students_on_assignment(self, relatedstudents_queryset, only_passing_grade):
    ...
    def get_unfiltered_queryset_for_role(self, role):
        if self.request.method == "POST":
            return super(ConfirmView, self).get_unfiltered_queryset_for_role(role=role)
        else:
            selected_students = self.kwargs["selected_students"]
            relatedstudents_queryset = super(ConfirmView, self).get_unfiltered_queryset_for_role(role=role)
            if selected_students == self.SELECTED_STUDENTS_ALL_ON_ASSIGNMENT:
                queryset = self.__filter_selected_students_on_assignment(
                    relatedstudents_queryset=relatedstudents_queryset, only_passing_grade=False
                )
            elif selected_students == self.SELECTED_STUDENTS_PASSING_GRADE_ON_ASSIGNMENT:
                queryset = self.__filter_selected_students_on_assignment(
                    relatedstudents_queryset=relatedstudents_queryset, only_passing_grade=True
                )
    ...
    def __get_formhelper(self):
        ...
        helper.layout = layout.Layout(
            "selected_items", PrimarySubmit("add_students", pgettext_lazy("admin create_groups", "Add students"))
        )
...
class ManualSelectStudentsView(
    listbuilder_relatedstudent.AddFilterListItemsMixin, CreateGroupsViewMixin, multiselect2view.ListbuilderFilterView
):
    """
    ...
    def get_pagetitle(self):
        return pgettext_lazy("admin create_groups", "Select the students you want to add to %(assignment)s") % {
            "assignment": self.assignment.get_path()
        }
    ...
    def get_filterlist_url(self, filters_string):
        return self.request.cradmin_app.reverse_appurl("manual-select", kwargs={"filters_string": filters_string})
...
class App(crapp.App):
    appurls = [
        ...
        crapp.Url(
            r"^confirm/(?P<selected_students>\w+)/(?P<filters_string>.+)?$", ConfirmView.as_view(), name="confirm"
        ),
        crapp.Url(r"^manual-select/(?P<filters_string>.+)?$", ManualSelectStudentsView.as_view(), name="manual-select"),
        crapp.Url(
        ...
        crapp.Url(
            r"^accumulated-score/preview", PreviewRelatedstudentsListView.as_view(), name="accumulated-score-preview"
        ),
    ]
