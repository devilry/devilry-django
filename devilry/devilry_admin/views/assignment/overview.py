...
class Overview(DetailRoleView):
    ...
    def get_distinct_examiners_count(self):
        ...
        """
        return (
            Examiner.objects.filter(assignmentgroup__parentnode=self.assignment)
            .distinct("relatedexaminer__user")
            .count()
        )
    ...
    def get_related_students_count(self):
        return RelatedStudent.objects.filter(period=self.assignment.period, active=True).distinct("user").count()

    def get_related_examiners_count(self):
        return RelatedExaminer.objects.filter(period=self.assignment.period, active=True).distinct("user").count()
    ...
    def get_user_is_subjectadmin_or_higher(self):
        return (
            SubjectPermissionGroup.objects.get_devilryrole_for_user_on_subject(
                user=self.request.user, subject=self.assignment.parentnode.parentnode
            )
            is not None
        )
