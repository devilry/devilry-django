...
class App(crapp.App):
    appurls = [
        crapp.Url(r"^$", overview.Overview.as_view(), name=crapp.INDEXVIEW_NAME),
        # Auto pass students on selected period.
        crapp.Url(r"^select-period$", passed_previous_period.SelectPeriodView.as_view(), name="select_period"),
        crapp.Url(
        ...
        crapp.Url(
            r"^confirm/(?P<period_id>\d+)$", passed_previous_period.ApprovePreviousAssignments.as_view(), name="confirm"
        ),
