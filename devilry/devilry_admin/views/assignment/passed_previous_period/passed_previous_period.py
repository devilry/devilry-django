...
from devilry.utils.passed_in_previous_period import (
    PassedInPreviousPeriod,
    SomeCandidatesDoesNotQualifyToPass,
    NoCandidatesPassed,
)
...
class SelectPeriodView(formbase.FormView):
    ...
    def __get_period_queryset(self):
        return (
            Period.objects.filter(
                parentnode=self.assignment.parentnode.parentnode, assignments__short_name=self.assignment.short_name
            )
    ...
    def get_buttons(self):
        return [PrimarySubmit("Next", gettext_lazy("Next"))]

    def get_field_layout(self):
        return [layout.Div("semester", css_class="cradmin-globalfields")]
    ...
    def get_redirect_url(self, period):
        return self.request.cradmin_app.reverse_appurl("assignments", kwargs={"period_id": period.id})
...
class CandidateItemValue(listbuilder.itemvalue.TitleDescription):
    ...
    def __init__(self, **kwargs):
        ...
        self.assignment = Assignment.objects.prefetch_point_to_grade_map().get(
            id=self.value.assignment_group.parentnode.id
        )
...
class ApprovePreviousAssignments(formbase.FormView):
    ...
    def get_buttons(self):
        return [PrimarySubmit("Confirm", gettext_lazy("Confirm"))]

    def get_field_layout(self):
        return [layout.Hidden("candidates", self.__get_candidate_ids())]
    ...
    def form_valid(self, form):
        try:
            candidates = Candidate.objects.filter(id__in=json.loads(form.data["candidates"])).select_related(
                "relatedstudent__user"
            )
            self.util_class.set_passed_in_current_period(candidates, self.request.user)
        except SomeCandidatesDoesNotQualifyToPass as e:
            messages.warning(self.request, gettext_lazy("Some students does not qualify to pass the assignment."))
            ...
            messages.warning(self.request, gettext_lazy("An error occurred."))
        else:
            messages.success(
                self.request,
                gettext_lazy("%(students)s was marked as approved for this assignment.")
