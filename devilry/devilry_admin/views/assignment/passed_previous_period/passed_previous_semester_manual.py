...
class PassAssignmentGroupsView(groupview_base.BaseMultiselectView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if self.get_unfiltered_queryset_for_role(role=self.request.cradmin_role).exists():
            ...
            messages.info(self.request, gettext_lazy("There are no students on this assignment."))
    ...
    def get_filterlist_url(self, filters_string):
        return self.request.cradmin_app.reverse_appurl(
            viewname="manually_select_groups", kwargs={"filters_string": filters_string}
        )
    ...
    def form_valid(self, form):
        ...
        self.__publish_grading_on_current_assignment(
            queryset=queryset, published_by=self.request.user, comment_text=form.cleaned_data["feedback_comment_text"]
        )
        return redirect(str(self.get_success_url()))
