...
class GradingConfigurationForm(forms.Form):
    error_messages = {
        "point_to_grade_map_json_invalid_format": gettext_lazy(
            "The grade to points table must have at least 2 rows. The first row must have 0 as points."
        ),
        "max_points_too_small_for_point_to_grade_map": gettext_lazy(
            "The grade to points table has points that is larger than the maximum number of points."
        ),
    ...
    grading_system_plugin_id = forms.ChoiceField(
        ...
        label=pgettext_lazy("assignment config", "Examiner chooses"),
    )
    ...
    max_points = forms.IntegerField(min_value=0, required=True, label="...", help_text="...")
...
class AssignmentGradingConfigurationUpdateView(OnlySaveButtonMixin, formbase.FormView):
    ...
    template_name = "devilry_admin/assignment/gradingconfiguration-update/gradingconfiguration-update.django.html"
        ...
        "data-ievv-jsbase-widget-config": json.dumps(
            {
                Assignment.GRADING_SYSTEM_PLUGIN_ID_PASSEDFAILED: {
                    "max_points_label": pgettext("assignment config", "Points awarded for passing grade"),
                },
                Assignment.GRADING_SYSTEM_PLUGIN_ID_POINTS: {
                    "max_points_label": pgettext("assignment config", "Maximum number of points"),
                    "max_points_help_text": pgettext(
                        "assignment config", "The maximum number of points possible for this assignment."
                    ),
                },
            }
        ),
    ...
    def get_pagetitle(self):
        return pgettext_lazy("assignment config", "Edit grading configuration")
    ...
    def assignment(self):
        if not hasattr(self, "_assignment"):
            queryset = Assignment.objects.filter(id=self.request.cradmin_role.id).prefetch_point_to_grade_map()
    ...
    def get_initial(self):
        ...
        if self.assignment.prefetched_point_to_grade_map is not None:
            initial["point_to_grade_map_json"] = json.dumps(self.assignment.prefetched_point_to_grade_map.as_choices())
        return initial
    ...
    def __create_point_to_grade_map(self, form, assignment):
        ...
        if form.cleaned_data["points_to_grade_mapper"] != Assignment.POINTS_TO_GRADE_MAPPER_CUSTOM_TABLE:
            return
        point_to_grade_map = PointToGradeMap.objects.create(assignment=assignment, invalid=False)
    ...
    def form_valid(self, form):
        ...
        assignment.setup_grading(grading_system_plugin_id, points_to_grade_mapper, passing_grade_min_points, max_points)
        assignment.full_clean()
        with transaction.atomic():
            assignment.save()
            self.__create_point_to_grade_map(form=form, assignment=assignment)
            messages.success(request=self.request, message=gettext_lazy("Saved Grading configuration for assignment"))
