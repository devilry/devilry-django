...
class RemoveGroupsToExaminerView(groupview_base.BaseMultiselectView, base_single_examinerview.SingleExaminerViewMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if self.get_unfiltered_queryset_for_role(role=self.request.cradmin_role).exists():
            ...
            messages.info(self.request, gettext_lazy("No students to remove."))
    ...
    def get_unfiltered_queryset_for_role(self, role):
        return (
            super(RemoveGroupsToExaminerView, self)
    ...
    def get_success_message(self, groupcount, candidatecount):
        if groupcount == candidatecount:
            return gettext_lazy("Removed %(count)s students.") % {"count": candidatecount}
        else:
    ...
    def __remove_examiner_objects(self, groupqueryset):
        ...
        examinerqueryset = Examiner.objects.filter(
            assignmentgroup__in=groupqueryset, relatedexaminer=self.get_relatedexaminer()
        )
