...
class OrganizeByTagListbuilderView(listbuilderview.View):
    ...
    def __relatedstudent_queryset(self):
        return RelatedStudent.objects.select_related("user", "period", "period__parentnode").order_by("user__shortname")

    def __relatedexaminer_queryset(self):
        return RelatedExaminer.objects.select_related("user", "period", "period__parentnode").order_by(
            "user__shortname"
        )

    def __get_groups_from_candidates(self, candidates, periodtag):
        candidates = candidates.filter(relatedstudent__in=periodtag.relatedstudents.all()).prefetch_related(
            "assignment_group__candidates__relatedstudent__user"
        )
    ...
    def get_listbuilder_list(self, context):
        ...
        candidate_queryset = Candidate.objects.filter(assignment_group__parentnode=assignment).select_related(
            "assignment_group",
        ...
        for periodtag in periodtag_queryset:
            listbuilder_list.append(
                listbuilder.itemframe.DefaultSpacingItemFrame(
                    TagItemValue(
                        assignment_groups=self.__get_groups_from_candidates(
                            candidates=candidate_queryset, periodtag=periodtag
                        ),
                        value=periodtag,
                    )
                )
    ...
    def get_queryset_for_role(self, role):
        ...
        candidate_queryset = (
            Candidate.objects.filter(assignment_group__parentnode=role)
            .select_related("assignment_group")
            .values_list("relatedstudent", flat=True)
        )

        period_tag_queryset = (
            period_tag.PeriodTag.objects.filter_editable_tags_on_period(period=period)
            .select_related("period", "period__parentnode")
            .prefetch_related(models.Prefetch("relatedexaminers", queryset=self.__relatedexaminer_queryset()))
            .prefetch_related(models.Prefetch("relatedstudents", queryset=self.__relatedstudent_queryset()))
            .annotate_with_relatedexaminers_count()
            .annotate_with_relatedstudents_count()
            .filter(models.Q(annotated_relatedexaminers_count__gt=0) & models.Q(annotated_relatedstudents_count__gt=0))
    ...
    def get_context_data(self, **kwargs):
        ...
        assignment = self.request.cradmin_role
        context_data["period_tags_count"] = period_tag.PeriodTag.objects.filter_editable_tags_on_period(
    ...
    def __organize_examiners(self):
        ...
        candidates = Candidate.objects.filter(assignment_group__parentnode=assignment).select_related(
            "assignment_group", "relatedstudent"
        )
...
class RandomOrganizeForm(groupview_base.SelectedGroupsForm):
    selected_relatedexaminers_invalid_choice_message = gettext_lazy("You must select at least two examiners.")
    ...
    def __make_relatedexaminer_choices(self, relatedexaminerqueryset):
        return [
            (relatedexaminer.id, relatedexaminer.user.get_full_name()) for relatedexaminer in relatedexaminerqueryset
        ]
    ...
    def clean(self):
        ...
        if selected_relatedexaminers.count() < 2:
            self.add_error("selected_relatedexaminers", self.selected_relatedexaminers_invalid_choice_message)
...
class RandomOrganizeTargetRenderer(devilry_listbuilder.assignmentgroup.GroupTargetRenderer):
    ...
    def get_field_layout(self):
        return ["selected_relatedexaminers"]
...
class RandomView(groupview_base.BaseMultiselectView):
    ...
    def __get_relatedexaminerqueryset(self):
        ...
        queryset = RelatedExaminer.objects.filter(period=period).select_related("user").exclude(active=False)
        return queryset
    ...
    def get_success_url(self):
        return self.request.cradmin_instance.reverse_url(appname="examineroverview", viewname=crapp.INDEXVIEW_NAME)
    ...
    def form_valid(self, form):
        ...
        self.__random_organize_examiners(groupqueryset=groupqueryset, relatedexaminerqueryset=relatedexaminerqueryset)
...
class ManualAddOrReplaceExaminersForm(groupview_base.SelectedGroupsForm):
    selected_relatedexaminers_required_message = gettext_lazy("You must select at least one examiner.")
    selected_relatedexaminers = forms.ModelMultipleChoiceField(
        queryset=RelatedExaminer.objects.none(),
        widget=forms.CheckboxSelectMultiple(),
        label=gettext_lazy("Select examiners:"),
        required=True,
        error_messages={"required": selected_relatedexaminers_required_message},
    )
...
class BaseManualAddOrReplaceView(groupview_base.BaseMultiselectView):
        ...
        for group in groupqueryset:
            ...
            for relatedexaminer in relatedexaminers:
                if relatedexaminer.id not in ignored_relatedexaminers_for_group:
                    examiner = Examiner(assignmentgroup=group, relatedexaminer=relatedexaminer)
    ...
    def get_success_message(self, candidatecount, relatedexaminers):
        examinernames = [relatedexaminer.user.get_full_name() for relatedexaminer in relatedexaminers]
    ...
    def form_valid(self, form):
        ...
        groupcount, candidatecount, relatedexaminers = self.__add_examiners(
            groupqueryset=groupqueryset, relatedexaminerqueryset=relatedexaminerqueryset
        )
        messages.success(
            self.request, self.get_success_message(candidatecount=candidatecount, relatedexaminers=relatedexaminers)
        )
        return redirect(str(self.get_success_url()))
...
class ManualReplaceView(BaseManualAddOrReplaceView):
    ...
    def get_success_message_formatting_string(self):
        return gettext_lazy(
            "Made %(examinernames)s examiner for %(count)s students, replacing any previous "
...
class ExaminerMultiSelectListFilterView(multiselect2view.ListbuilderView):
    ...
    def get_pagetitle(self):
        return pgettext_lazy("examiner_bulk_delete_view pagetitle", "Select examiners")

    def get_pageheading(self):
        return pgettext_lazy("examiner_bulk_delete_view pageheading", "Select examiners")

    def get_page_subheading(self):
        return pgettext_lazy(
            "assignment_group_multiselect_list_filter_view page_subheading", "Select the examiners you want to remove."
        )
    ...
    def get_value_and_frame_renderer_kwargs(self):
        return {"assignment": self.request.cradmin_role}
...
class App(crapp.App):
    appurls = [
        crapp.Url(r"^$", SelectMethodView.as_view(), name=crapp.INDEXVIEW_NAME),
        crapp.Url(r"^random/(?P<filters_string>.+)?$", RandomView.as_view(), name="random"),
        crapp.Url(r"^manual-add/(?P<filters_string>.+)?$", ManualAddView.as_view(), name="manual-add"),
        crapp.Url(r"^manual-replace/(?P<filters_string>.+)?$", ManualReplaceView.as_view(), name="manual-replace"),
        # crapp.Url(r'^manual-remove-examiners$',
        #           ExaminerMultiSelectListFilterView.as_view(),
        #           name='manual-remove-examiners'),
        crapp.Url("^tag$", OrganizeByTagListbuilderView.as_view(), name="organize-by-tag"),
    ]
