...
class Overview(DetailRoleView):
    ...
    def __examiners_exist(self):
        return Examiner.objects.filter(assignmentgroup__parentnode=self.request.cradmin_role).exists()

    def __get_relatedexaminer_ids(self):
        return (
            RelatedExaminer.objects.select_related("user")
        ...
        default_config.update(
            {
                ...
                "points_label": pgettext("devilry_admin assignment examiner statistics", "Points"),
                "points_average_label": pgettext("devilry_admin assignment examiner statistics", "Average"),
                "points_highest_label": pgettext("devilry_admin assignment examiner statistics", "Highest"),
                "points_lowest_label": pgettext("devilry_admin assignment examiner statistics", "Lowest"),
            }
        )
    ...
    def get_examiner_percentage_grade_widget_config(self):
        ...
        widget_config.update(
            {
                "x_axes_label": pgettext("devilry_admin assignment examiner statistics", "Percentage"),
                "passed_label": pgettext("devilry_admin assignment examiner statistics", "Passed"),
                "failed_label": pgettext("devilry_admin assignment examiner statistics", "Failed"),
                "not_corrected_label": pgettext("devilry_admin assignment examiner statistics", "Not corrected"),
            }
        )
        return widget_config
    ...
    def get_context_data(self, **kwargs):
        ...
        context["examiner_average_point_chart_config"] = json.dumps(self.get_examiner_average_points_widget_config())
        context["examiner_percentage_graded_chart_config"] = json.dumps(
            self.get_examiner_percentage_grade_widget_config()
        )
        context["examiner_detail_config"] = json.dumps(self.get_examiner_detail_js_base_widget_config())
        context["examiners_exist"] = self.__examiners_exist()
        context["groups_with_multiple_examiners_exist"] = (
            AssignmentGroup.objects.filter(parentnode=self.assignment.id)
