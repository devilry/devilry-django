...
class GetCustomManagablePermissionGroupMixin(object):
    def get_custom_managable_periodpermissiongroup(self):
        ...
        try:
            return PeriodPermissionGroup.objects.get_custom_managable_periodpermissiongroup_for_period(period=period)
...
class Overview(GetCustomManagablePermissionGroupMixin, listbuilderview.View):
    ...
    def get_queryset_for_role(self, role):
        if self.custom_managable_periodpermissiongroup:
            return (
                PermissionGroupUser.objects.filter(
    ...
    def __prefetch_permissiongroupuser_queryset(self):
        return PermissionGroupUser.objects.select_related("user").order_by("user__shortname")

    def __get_periodpermissiongroups(self):
        period = self.request.cradmin_role
        queryset = (
            PeriodPermissionGroup.objects.filter(period=period)
            .select_related("period")
            .prefetch_related(
                models.Prefetch(
                    "permissiongroup__permissiongroupuser_set", queryset=self.__prefetch_permissiongroupuser_queryset()
                )
            )
        )
        if self.custom_managable_periodpermissiongroup:
    ...
    def __get_subjectpermissiongroups(self):
        period = self.request.cradmin_role
        return (
            SubjectPermissionGroup.objects.filter(subject=period.subject)
    ...
    def __make_permissiongroup_listbuilderlist(self):
        ...
        listbuilderlist.add_periodpermissiongroups(periodpermissiongroups=self.__get_periodpermissiongroups())
        listbuilderlist.add_subjectpermissiongroups(subjectpermissiongroups=self.__get_subjectpermissiongroups())
...
class AddView(GetCustomManagablePermissionGroupMixin, devilry_multiselect2.user.BaseMultiselectUsersView):
    ...
    def __get_userids_already_admin_queryset(self):
        ...
        periodpermissiongroups_with_access_to_period = PeriodPermissionGroup.objects.filter(period=period).values_list(
            "permissiongroup_id", flat=True
        )
        subjectpermissiongroups_with_access_to_period = SubjectPermissionGroup.objects.filter(
            subject=subject
        ).values_list("permissiongroup_id", flat=True)
        return (
            PermissionGroupUser.objects.filter(
                models.Q(permissiongroup__in=periodpermissiongroups_with_access_to_period)
    ...
    def __create_permissiongroup_if_it_does_not_exist(self):
        ...
        periodpermissiongroup = PeriodPermissionGroup(permissiongroup=permissiongroup, period=period)
    ...
    def __create_permissiongroupusers(self, selected_users, permissiongroup):
        ...
        for user in selected_users:
            permissiongroupuser = PermissionGroupUser(permissiongroup=permissiongroup, user=user)
    ...
    def form_valid(self, form):
        ...
        self.__create_permissiongroupusers(
            selected_users=selected_users, permissiongroup=periodpermissiongroup.permissiongroup
        )
...
class DeleteView(GetCustomManagablePermissionGroupMixin, devilry_confirmview.View):
    ...
    def __get_permissiongroupuser(self, permissiongroup):
        ...
        try:
            ...
            raise Http404(
                "No PermissionGroupUser in custom managable permissiongroup for Period#{} found.".format(period.id)
            )
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.custom_managable_periodpermissiongroup is None:
            raise Http404("No custom managable permissiongroup for Period#{} found.".format(period.id))
...
class App(crapp.App):
    appurls = [
        ...
        crapp.Url(r"^delete/(?P<pk>\d+)$", DeleteView.as_view(), name="delete"),
    ]
