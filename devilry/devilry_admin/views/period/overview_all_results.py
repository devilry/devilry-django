...
class StudentRowList(base_new.AbstractRowList):
    ...
    def __init__(self, assignments, related_student_result):
        ...
        self.renderable_list.append(RelatedStudentItemValue(related_student=related_student_result.relatedstudent))
...
class RelatedStudentsAllResultsOverview(FilterListMixin, listbuilderview.View):
    ...
    def get_unfiltered_queryset_for_role(self, role):
        period = self.request.cradmin_role
        related_student_queryset = core_models.RelatedStudent.objects.filter(period=period)
        ...
        context["report_options"] = json.dumps(
            {
                ...
                "generator_options": {"period_id": self.request.cradmin_role.id},
            }
        )
        return context
...
class App(crapp.App):
    appurls = [
        crapp.Url(r"^all-results-overview$", RelatedStudentsAllResultsOverview.as_view(), name=crapp.INDEXVIEW_NAME),
        crapp.Url(
            r"^all-results-overview/filter/(?P<filters_string>.+)?$",
            RelatedStudentsAllResultsOverview.as_view(),
            name="filter",
        ),
        crapp.Url(r"^download-report$", DownloadReportView.as_view(), name="download_report"),
    ]
