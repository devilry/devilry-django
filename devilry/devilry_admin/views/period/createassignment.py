...
class CreateForm(forms.ModelForm):
    ...
    student_import_option = forms.ChoiceField(label=gettext_lazy("Import students"), required=True, choices=[])

    class Meta:
        model = Assignment
        fields = ["long_name", "short_name", "student_import_option", "first_deadline", "publishing_time", "parentnode"]

    def __init__(self, *args, **kwargs):
        self.period = kwargs.pop("period")
        super(CreateForm, self).__init__(*args, **kwargs)
        self.fields["long_name"].help_text = gettext_lazy("Type the name of your assignment.")
        self.fields["short_name"].help_text = gettext_lazy(
            'Up to 20 letters of lowercase english letters (a-z), numbers, underscore ("_") and hyphen ("-").'
        )

        # Set student import options data.
        self.fields["student_import_option"].help_text = gettext_lazy(
        ...
        self.fields["first_deadline"].widget = DateTimePickerWidget(
            ...
            + timedelta(minutes=settings.DEVILRY_ASSIGNMENT_PUBLISHING_TIME_DELAY_MINUTES),
        ...
        self.fields["first_deadline"].help_text = gettext_lazy(
            "The first deadline for this assignment. This is shared by all the students on the assignment."
        )
    ...
    def default_import_options(self):
        return [("", "----"), ("all", gettext_lazy("All students on semester")), ("none", gettext_lazy("No students"))]
    ...
    def __create_student_import_choices(self):
        assignment_queryset = (
            Assignment.objects.filter_is_active().filter(parentnode=self.period).order_by("-first_deadline")
        )
    ...
    def __raise_validation_error(self, start_time):
        publishing_time_naturaltime = naturaltime(
            start_time
            + timedelta(
                minutes=settings.DEVILRY_ASSIGNMENT_PUBLISHING_TIME_DELAY_MINUTES,
...
class CreateView(crudbase.OnlySaveButtonMixin, create.CreateView):
    ...
    def dispatch(self, *args, **kwargs):
        ...
        self.previous_assignment = (
            self.period.assignments.exclude(first_deadline=None).order_by("first_deadline").last()
        )
    ...
    def get_initial(self):
        ...
        if self.previous_assignment:
            namesuggestion = nodenamesuggestor.Suggest(
                long_name=self.previous_assignment.long_name, short_name=self.previous_assignment.short_name
            )
    ...
    def __render_suggested_deadlines_box(self):
        return render_to_string(
            self.suggested_deadlines_template_name, {"suggested_deadlines": self.__get_suggested_deadlines()}
        )
    ...
    def get_field_layout(self):
        return [
            layout.Div(
                layout.Field("long_name", placeholder=gettext_lazy("Example: Assignment 1"), focusonme="focusonme"),
                ...
                layout.Div(
                    layout.Div(layout.Field("first_deadline"), css_class="col-sm-6"),
    ...
    def get_success_url(self):
        return crinstance.reverse_cradmin_url(
            instanceid="devilry_admin_assignmentadmin", appname="overview", roleid=self.created_assignment.id
        )
    ...
    def add_success_messages(self, object):
        ...
        messages.success(
            self.request, render_to_string(self.success_message_template_name, context=messages_template_context)
        )
    ...
    def __get_selected_assignment_from_import_option(self, student_import_option):
        ...
        try:
            assignment = Assignment.objects.filter_is_active().filter(parentnode=self.period).get(id=assignment_id)
    ...
    def __copy_all_students_from_assignment(self, created_assignment, student_import_option):
        ...
        created_assignment.copy_groups_from_another_assignment(sourceassignment=import_from_assignment)
    ...
    def get_backlink_url(self):
        return crinstance.reverse_cradmin_url(
            instanceid="devilry_admin_periodadmin", appname="overview", roleid=self.request.cradmin_role.id
        )

    def get_context_data(self, **kwargs):
        context = super(CreateView, self).get_context_data(**kwargs)
        context["backlink_url"] = self.get_backlink_url()
        context["import_option_not_selected_text"] = gettext_lazy(
            "Choose how you would like to set up students for the new "
            "assignment. You can add all students from the semester, no "
            "students or copy students and examiners from previous "
            "assignments."
        ...
        context["import_option_none_selected_text"] = gettext_lazy(
            "Will not import any students. You have to manually configure students from the assignment-dashboard."
        )
        context["import_option_assignment_all_selected_text"] = gettext_lazy(
            "Copy all students from this assignment. "
            "This will copy the group and examiner setup from "
            "the selected assignment."
        )
        context["import_option_assignment_passing_selected_text"] = gettext_lazy(
            "Copy students from this assignment with passing "
