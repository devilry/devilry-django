...
class AllResultsExcelReportGenerator(AbstractExcelReportGenerator):
    ...
    def get_object_iterable(self):
        related_student_queryset = RelatedStudent.objects.filter(period=self.period)
    ...
    def __get_all_assignments_for_period(self):
        ...
        """
        return Assignment.objects.prefetch_point_to_grade_map().filter(parentnode_id=self.period.id)
