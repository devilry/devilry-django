...
class ActivateView(SingleRelatedStudentMixin, devilry_confirmview.View):
    ...
    def get_pagetitle(self):
        return gettext_lazy("Re-activate student: %(user)s?") % {"user": self.relatedstudent.user.get_full_name()}
    ...
    def __get_success_message(self):
        return gettext_lazy("%(user)s was re-activated.") % {"user": self.relatedstudent.user.get_full_name()}
...
def students_not_added_to_assignments_warning(request, period):
    ...
    message = render_to_string(
        "devilry_admin/period/students/students-not-added-to-assignments-warning.django.html",
...
class AddView(devilry_multiselect2.user.BaseMultiselectUsersView):
    ...
    def __get_userids_already_relatedstudent_queryset(self):
        period = self.request.cradmin_role
        return RelatedStudent.objects.filter(period=period).values_list("user_id", flat=True)
    ...
    def __create_relatedstudents(self, selected_users):
        ...
        for user in selected_users:
            relatedstudent = RelatedStudent(period=period, user=user)
    ...
    def form_valid(self, form):
        ...
        students_not_added_to_assignments_warning(request=self.request, period=self.request.cradmin_role)
...
class ImportStudentsView(bulkimport_users_common.AbstractTypeInUsersView):
    ...
    def import_users_from_emails(self, emails):
        ...
        if result.new_relatedusers_was_created():
            messages.success(
                self.request,
                gettext_lazy("Added %(count)s new students to %(period)s.")
        ...
        if result.existing_relateduser_emails_set:
            messages.info(
                self.request,
                gettext_lazy("%(count)s users was already student on %(period)s.")
...
class App(crapp.App):
    appurls = [
        ...
        crapp.Url(r"^importstudents", ImportStudentsView.as_view(), name="importstudents"),
    ]
