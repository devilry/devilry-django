...
class Overview(listbuilderview.FilterListMixin, listbuilderview.View):
    ...
    def __get_relatedstudent_count(self):
        period = self.request.cradmin_role
        return RelatedStudent.objects.filter(period=period).count()

    def __get_relatedexaminer_count(self):
        period = self.request.cradmin_role
        return RelatedExaminer.objects.filter(period=period).count()
    ...
    def get_context_data(self, **kwargs):
        ...
        context["period_admin_access_restricted"] = (
            self.request.cradmin_instance.period_admin_access_semi_anonymous_assignments_restricted()
        )
        return context
