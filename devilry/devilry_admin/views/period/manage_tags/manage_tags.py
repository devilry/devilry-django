...
class TagListBuilderListView(listbuilderview.FilterListMixin, listbuilderview.View):
    ...
    def get_unfiltered_queryset_for_role(self, role):
        queryset = (
            self.model.objects.filter(period=role)
            .prefetch_related(
                models.Prefetch(
                    "relatedstudents",
                    queryset=RelatedStudent.objects.all().select_related("user").order_by("user__shortname"),
                )
            )
            .prefetch_related(
                models.Prefetch(
                    "relatedexaminers",
                    queryset=RelatedExaminer.objects.all().select_related("user").order_by("user__shortname"),
                )
            )
        )
        return queryset
    ...
    def get_no_items_message(self):
        return pgettext_lazy("TagListBuilderListView get_no_items_message", "No period tags")
...
class CreatePeriodTagForm(forms.Form):
    ...
    def get_added_tags_list(self):
        ...
        """
        return [tag.strip() for tag in self.cleaned_data["tag_text"].split(",") if len(tag.strip()) > 0]
    ...
    def clean(self):
        ...
        if len(tags_list) == 0:
            if len(tags_list) > 15:
                raise ValidationError({"tag_text": gettext_lazy("Wrong format. Example: tag1, tag2, tag3")})
        for tag in tags_list:
            if len(tag) > 15:
                raise ValidationError({"tag_text": gettext_lazy("One or more tags exceed the limit of 15 characters.")})
...
class AddTagsView(formbase.FormView):
    ...
    def get_field_layout(self):
        return [layout.Div(layout.Field("tag_text", focusonme="focusonme"), css_class="cradmin-globalfields")]

    def get_buttons(self):
        return [PrimarySubmit("add_tags", gettext_lazy("Add tags"))]
    ...
    def form_valid(self, form):
        ...
        excluded_tags = (
            PeriodTag.objects.filter_editable_tags_on_period(period=self.request.cradmin_role)
        ...
        if excluded_tags.count() > 0:
            message += gettext_lazy(", %(excluded)d tag(s) already existed and were ignored.") % {
...
class EditPeriodTagForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["tag"].help_text = gettext_lazy(
            "Rename the tag here. Up to 15 characters. Can contain any character except comma(,)"
        )

    def clean(self):
        cleaned_data = super(EditPeriodTagForm, self).clean()
        if "tag" not in self.cleaned_data or len(self.cleaned_data["tag"]) == 0:
            raise ValidationError({"tag": gettext_lazy("Tag cannot be empty.")})
        ...
        if "," in tag:
            raise ValidationError({"tag": gettext_lazy("Tag contains a comma(,).")})
        return cleaned_data
...
class EditTagView(crudbase.OnlySaveButtonMixin, EditDeleteViewMixin, update.UpdateView):
    ...
    def get_pagetitle(self):
        return gettext_lazy("Edit %(what)s") % {"what": self.tag.displayname}

    def get_field_layout(self):
        return [layout.Div(layout.Field("tag", focusonme="focusonme"), css_class="cradmin-globalfields")]
    ...
    def get_context_data(self, **kwargs):
        ...
        context_data["period"] = period
        context_data["period_tags"] = PeriodTag.objects.filter_editable_tags_on_period(period=period)
        return context_data
...
class SelectedRelatedUsersForm(forms.Form):
    ...
    selected_items = forms.ModelMultipleChoiceField(
        queryset=None, error_messages={"invalid_choice": invalid_item_selected_message}
    )
...
class SelectedItemsTarget(multiselect2.target_renderer.Target):
    ...
    def get_with_items_title(self):
        return pgettext_lazy("admin multiselect2_relateduser", "Selected %(what)s") % {"what": self.relateduser_type}

    def get_without_items_text(self):
        return pgettext_lazy("admin multiselect2_relateduser", "No %(what)s selected") % {"what": self.relateduser_type}
...
class AddRelatedUserToTagMultiSelectView(BaseRelatedUserMultiSelectView):
    ...
    def get_pagetitle(self):
        ...
        return gettext_lazy("Add %(user)s to %(tag)s") % {"user": self.relateduser_string, "tag": tag_displayname}

    def get_queryset_for_role(self, role):
        return (
            super(AddRelatedUserToTagMultiSelectView, self)
    ...
    def form_valid(self, form):
        ...
        self.add_success_message(
            message=gettext_lazy("%(number_users)d %(user_string)s added successfully.")
            % {
...
class RemoveRelatedUserFromTagMultiSelectView(BaseRelatedUserMultiSelectView):
    ...
    def get_pagetitle(self):
        tag_displayname = self.get_period_tag().displayname
        return gettext_lazy("Remove %(user)s from %(tag)s") % {"user": self.relateduser_string, "tag": tag_displayname}

    def get_queryset_for_role(self, role):
        return (
            super(RemoveRelatedUserFromTagMultiSelectView, self)
            .get_queryset_for_role(role=role)
    ...
    def form_valid(self, form):
        ...
        self.add_success_message(
            message=gettext_lazy("%(number_users)d %(user_string)s removed successfully")
            % {"number_users": len(related_users), "user_string": self.relateduser_string}
        )
...
class RelatedExaminerAddView(ExaminerMultiSelectViewMixin, AddRelatedUserToTagMultiSelectView):
    ...
    def get_filterlist_url(self, filters_string):
        return self.request.cradmin_app.reverse_appurl(
            "add_examiners_filter", kwargs={"tag_id": self.tag_id, "filters_string": filters_string}
        )
...
class RelatedExaminerRemoveView(ExaminerMultiSelectViewMixin, RemoveRelatedUserFromTagMultiSelectView):
    ...
    def get_filterlist_url(self, filters_string):
        return self.request.cradmin_app.reverse_appurl(
            "remove_examiners_filter", kwargs={"tag_id": self.tag_id, "filters_string": filters_string}
        )
...
class RelatedStudentAddView(StudentMultiSelectViewMixin, AddRelatedUserToTagMultiSelectView):
    ...
    def get_filterlist_url(self, filters_string):
        return self.request.cradmin_app.reverse_appurl(
            "add_students_filter", kwargs={"tag_id": self.tag_id, "filters_string": filters_string}
        )
...
class RelatedStudentRemoveView(StudentMultiSelectViewMixin, RemoveRelatedUserFromTagMultiSelectView):
    ...
    def get_filterlist_url(self, filters_string):
        return self.request.cradmin_app.reverse_appurl(
            "remove_students_filter", kwargs={"tag_id": self.tag_id, "filters_string": filters_string}
        )


class App(crapp.App):
    appurls = [
        crapp.Url(r"^$", TagListBuilderListView.as_view(), name=crapp.INDEXVIEW_NAME),
        crapp.Url(r"^filter/(?P<filters_string>.+)?$", TagListBuilderListView.as_view(), name="filter"),
        crapp.Url(r"^add$", AddTagsView.as_view(), name="add_tag"),
        crapp.Url(r"^edit/(?P<pk>\d+)$", EditTagView.as_view(), name="edit"),
        crapp.Url(r"^delete/(?P<pk>\d+)$", DeleteTagView.as_view(), name="delete"),
        crapp.Url(r"^toggle-visibility$", HideShowPeriodTag.as_view(), name="toggle_visibility"),
        crapp.Url("^add-examiners/(?P<tag_id>\d+)$", RelatedExaminerAddView.as_view(), name="add_examiners"),
        crapp.Url(
            "^add-examiners/(?P<tag_id>\d+)/(?P<filters_string>.+)?$",
            RelatedExaminerAddView.as_view(),
            name="add_examiners_filter",
        ),
        crapp.Url("^remove-examiners/(?P<tag_id>\d+)$", RelatedExaminerRemoveView.as_view(), name="remove_examiners"),
        crapp.Url(
            "^remove-examiners/(?P<tag_id>\d+)/(?P<filters_string>.+)?$",
            RelatedExaminerRemoveView.as_view(),
            name="remove_examiners_filter",
        ),
        crapp.Url("^add-students/(?P<tag_id>\d+)$", RelatedStudentAddView.as_view(), name="add_students"),
        crapp.Url(
            "^add-students/(?P<tag_id>\d+)/(?P<filters_string>.+)?$",
            RelatedStudentAddView.as_view(),
            name="add_students_filter",
        ),
        crapp.Url("^remove-students/(?P<tag_id>\d+)$", RelatedStudentRemoveView.as_view(), name="remove_students"),
        crapp.Url(
            "^remove-students/(?P<tag_id>\d+)/(?P<filters_string>.+)?$",
