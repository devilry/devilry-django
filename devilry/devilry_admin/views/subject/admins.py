...
class GetCustomManagablePermissionGroupMixin(object):
    def get_custom_managable_subjectpermissiongroup(self):
        ...
        try:
            return SubjectPermissionGroup.objects.get_custom_managable_subjectpermissiongroup_for_subject(
                subject=subject
            )
...
class Overview(GetCustomManagablePermissionGroupMixin, listbuilderview.View):
    ...
    def __get_subjectpermissiongroups(self):
        subject = self.request.cradmin_role
        queryset = (
            SubjectPermissionGroup.objects.filter(subject=subject)
...
class AddView(GetCustomManagablePermissionGroupMixin, devilry_multiselect2.user.BaseMultiselectUsersView):
    ...
    def __get_userids_already_admin_queryset(self):
        subject = self.request.cradmin_role
        subjectpermissiongroups_with_access_to_subject = SubjectPermissionGroup.objects.filter(
            subject=subject
        ).values_list("permissiongroup_id", flat=True)
        return (
            PermissionGroupUser.objects.filter(permissiongroup__in=subjectpermissiongroups_with_access_to_subject)
    ...
    def __create_permissiongroup_if_it_does_not_exist(self):
        ...
        subjectpermissiongroup = SubjectPermissionGroup(permissiongroup=permissiongroup, subject=subject)
    ...
    def form_valid(self, form):
        ...
        self.__create_permissiongroupusers(
            selected_users=selected_users, permissiongroup=subjectpermissiongroup.permissiongroup
        )
...
class DeleteView(GetCustomManagablePermissionGroupMixin, devilry_confirmview.View):
    ...
    def __get_permissiongroupuser(self, permissiongroup):
        ...
        try:
            ...
            raise Http404(
                "No PermissionGroupUser in custom managable permissiongroup for Subject#{} found.".format(subject.id)
            )
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.custom_managable_subjectpermissiongroup is None:
            raise Http404("No custom managable permissiongroup for Subject#{} found.".format(subject.id))
