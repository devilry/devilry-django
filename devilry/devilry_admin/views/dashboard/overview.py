...
class OverviewSubjectListView(listbuilderview.FilterListMixin, listbuilderview.View):
    ...
    def __get_all_subjects_where_user_is_subjectadmin(self):
        return Subject.objects.filter_user_is_admin(user=self.request.user).order_by("long_name").distinct()

    def __get_all_periods_where_user_is_subjectadmin_or_periodadmin(self):
        groups = []
        periods = (
            Period.objects.filter_user_is_admin(user=self.request.user)
    ...
    def add_filterlist_items(self, filterlist):
        ...
        filterlist.append(
            listfilter.django.single.textinput.Search(
                slug="search", label="Search", label_is_screenreader_only=True, modelfields=["long_name"]
            )
        )
        filterlist.append(OrderSubjectFilter(slug="short_name", label=gettext_lazy("Short name")))
    ...
    def get_unfiltered_queryset_for_role(self, site):
        ...
        queryset = coremodels.Subject.objects.filter_user_is_admin_for_any_periods_within_subject(
            self.request.user
    ...
    def get_context_data(self, **kwargs):
        ...
        context["subjects_where_user_is_subjectadmin"] = self.__get_all_subjects_where_user_is_subjectadmin()
...
class App(crapp.App):
    appurls = [
        ...
        crapp.Url(r"^filter/(?P<filters_string>.+)?$", OverviewSubjectListView.as_view(), name="filter"),
    ]
