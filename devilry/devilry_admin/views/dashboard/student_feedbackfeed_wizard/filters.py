...
class UserSearchExtension(devilry_listfilter.user.Search):
    def get_placeholder(self):
        return pgettext_lazy("devilry_admin studentfeedbackfeed wizard search users", "Search users...")


class SearchExtension(devilry_listfilter.assignmentgroup.SearchNotAnonymous):
    def get_placeholder(self):
        return pgettext_lazy("devilry_admin studentfeedbackfeed wizard search assignments", "Search assignments...")
...
class IsActiveSemester(abstractselect.AbstractBoolean):
    ...
    def get_label(self):
        return pgettext_lazy("group in active semester filter", "Active semesters?")

    def filter(self, queryobject):
        cleaned_value = self.get_cleaned_value()
        now = timezone.now()
        if cleaned_value in ("true", "false"):
            query = models.Q(parentnode__parentnode__start_time__lt=now, parentnode__parentnode__end_time__gt=now)
