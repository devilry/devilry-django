...
class SelectedItem(
    listbuilder_relatedstudent.RelatedStudentItemValueTitleDescriptionMixin,
...
class Target(multiselect2.target_renderer.Target):
    def get_with_items_title(self):
        return pgettext_lazy("admin multiselect2_relatedstudent", "Selected students")

    def get_without_items_text(self):
        return pgettext_lazy("admin multiselect2_relatedstudent", "No students selected")


class SelectRelatedStudentsForm(forms.Form):
    selected_items = forms.ModelMultipleChoiceField(
        widget=forms.MultipleHiddenInput, queryset=RelatedStudent.objects.none()
    )
