...
class TestIndexView(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_get_title(self):
        requestuser = baker.make(settings.AUTH_USER_MODEL, shortname="test", fullname="Test")
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=requestuser)
        self.assertEqual(mockresponse.selector.one("title").alltext_normalized, "Test (test) - Account")

    def test_get_h1(self):
        requestuser = baker.make(settings.AUTH_USER_MODEL)
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=requestuser)
        self.assertEqual(mockresponse.selector.one(".test-primary-h1").alltext_normalized, "Account overview")

    def test_get_fullname_none(self):
        requestuser = baker.make(settings.AUTH_USER_MODEL)
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=requestuser)
        self.assertEqual(
            mockresponse.selector.one(".test-fullname").alltext_normalized, "Name not registered for account"
        )

    def test_get_fullname(self):
        requestuser = baker.make(settings.AUTH_USER_MODEL, fullname="Test User")
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=requestuser)
        self.assertEqual(mockresponse.selector.one(".test-fullname").alltext_normalized, "Test User")

    def test_get_shortname(self):
        requestuser = baker.make(settings.AUTH_USER_MODEL, shortname="testuser")
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=requestuser)
        self.assertEqual(mockresponse.selector.one(".test-shortname").alltext_normalized, "testuser")
    ...
    def test_get_email_addresses_single(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=requestuser)
        self.assertEqual(self._get_emails_from_selector(mockresponse.selector), ["test@example.com"])
        self.assertEqual(mockresponse.selector.one(".test-emails-title").alltext_normalized, "Email address")

    def test_get_email_addresses_multiple(self):
        requestuser = baker.make(settings.AUTH_USER_MODEL)
        baker.make("devilry_account.UserEmail", email="test1@example.com", user=requestuser)
        baker.make(
            "devilry_account.UserEmail", email="test2@example.com", user=requestuser, use_for_notifications=False
        )
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=requestuser)
        self.assertEqual(
            set(self._get_emails_from_selector(mockresponse.selector)),
            {"test1@example.com (use for notifications)", "test2@example.com"},
        )
        self.assertEqual(mockresponse.selector.one(".test-emails-title").alltext_normalized, "Email addresses")
    ...
    def test_get_usernames_email_auth_backend_true(self):
        requestuser = baker.make(settings.AUTH_USER_MODEL)
        baker.make("devilry_account.UserName", username="test", user=requestuser)
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=requestuser)
    ...
    def test_get_usernames_single(self):
        ...
        baker.make("devilry_account.UserName", username="test", user=requestuser)
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=requestuser)
        self.assertEqual(self._get_usernames_from_selector(mockresponse.selector), ["test"])
        self.assertEqual(mockresponse.selector.one(".test-usernames-title").alltext_normalized, "Username")

    @override_settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=False)
    def test_get_usernames_multiple(self):
        requestuser = baker.make(settings.AUTH_USER_MODEL)
        baker.make("devilry_account.UserName", username="test1", user=requestuser)
        baker.make("devilry_account.UserName", username="test2", user=requestuser)
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=requestuser)
        self.assertEqual(set(self._get_usernames_from_selector(mockresponse.selector)), {"test1", "test2"})
        self.assertEqual(mockresponse.selector.one(".test-usernames-title").alltext_normalized, "Usernames")

    def __make_period_and_semester(
        self, subject_longname, subject_shortname, period_longname, period_shortname, relative_to_datetime, active=True
    ):
        if active:
    ...
    def test_no_role_no_active_semester(self):
        ...
        self.__make_period_and_semester(
            subject_longname="Subject",
            subject_shortname="subject",
            period_longname="Period",
            period_shortname="period",
    ...
    def test_inactive_role_active_semester_examiner(self):
        ...
        period = self.__make_period_and_semester(
            subject_longname="Subject1",
            subject_shortname="subject1",
            period_longname=f"Semester1",
            period_shortname=f"semester1",
            relative_to_datetime=timezone.now(),
        )
    ...
    def test_role_active_semester_examiner_single_semester(self):
        ...
        self.assertEqual(mockresponse.selector.one(".test-examiner-role-heading").alltext_normalized, "As examiner")
        roleperiodlist = mockresponse.selector.list(".test-examiner-role-list-item")
        self.assertEqual(len(roleperiodlist), 1)
        self.assertEqual(roleperiodlist[0].alltext_normalized, f"{period.parentnode.long_name}: {period.long_name}")
    ...
    def test_role_active_semester_examiner_multiple_semesters(self):
        ...
        period2 = self.__make_period_and_semester(
            subject_longname="Subject2",
            subject_shortname="subject2",
            period_longname=f"Semester1",
            period_shortname=f"semester1",
            relative_to_datetime=timezone.now(),
        )
        ...
        self.assertEqual(roleperiodlist[0].alltext_normalized, f"{period1.parentnode.long_name}: {period1.long_name}")
        self.assertEqual(roleperiodlist[1].alltext_normalized, f"{period2.parentnode.long_name}: {period2.long_name}")
    ...
    def test_role_active_semester_student_single_semester(self):
        ...
        self.assertEqual(mockresponse.selector.one(".test-student-role-heading").alltext_normalized, "As student")
    ...
    def test_role_active_semester_examiner_and_student(self):
        ...
        self.assertEqual(
            roleperiodlist_examiner[0].alltext_normalized, f"{period1.parentnode.long_name}: {period1.long_name}"
        )
        self.assertEqual(mockresponse.selector.one(".test-student-role-heading").alltext_normalized, "As student")
        ...
        self.assertEqual(
            roleperiodlist_student[0].alltext_normalized, f"{period2.parentnode.long_name}: {period2.long_name}"
        )
