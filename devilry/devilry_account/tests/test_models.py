...
class TestUser(TestCase):
    ...
    def test_get_displayname_has_fullname(self):
        user = baker.make("devilry_account.User", shortname="elvis", fullname="Elvis Aron Presley")
    ...
    def test_is_not_active(self):
        user = baker.make("devilry_account.User", suspended_datetime=timezone.now())
        self.assertFalse(user.is_active)

    def test_clean_suspended_reason_with_blank_suspended_datetime(self):
        user = baker.make("devilry_account.User", suspended_datetime=None, suspended_reason="Test")
        with self.assertRaisesMessage(
            ValidationError, "Can not provide a reason for suspension when suspension time is blank."
        ):
            user.clean()

    def test_clean_suspended_reason_with_suspended_datetime(self):
        user = baker.make("devilry_account.User", suspended_datetime=timezone.now(), suspended_reason="Test")
        user.clean()
    ...
    def test_clean_unset_lastname_when_no_fullname(self):
        user = baker.make("devilry_account.User", fullname="The Test User")
        user.fullname = ""
        user.clean()
...
class TestUserQuerySet(TestCase):
    def test_prefetch_related_notification_emails(self):
        user = baker.make("devilry_account.User")
        notification_useremail1 = baker.make(
            "devilry_account.UserEmail", user=user, use_for_notifications=True, email="test1@example.com"
        )
        notification_useremail2 = baker.make(
            "devilry_account.UserEmail", user=user, use_for_notifications=True, email="test2@example.com"
        )
        baker.make("devilry_account.UserEmail", user=user, use_for_notifications=False, email="unused@example.com")
        with self.assertNumQueries(2):
            user_with_prefetch = User.objects.prefetch_related_notification_emails().first()
            self.assertEqual(len(user_with_prefetch.notification_useremail_objects), 2)
            self.assertTrue(isinstance(user_with_prefetch.notification_useremail_objects, list))
            self.assertEqual(
                {notification_useremail1, notification_useremail2},
                set(user_with_prefetch.notification_useremail_objects),
            )
            self.assertEqual({"test1@example.com", "test2@example.com"}, set(user_with_prefetch.notification_emails))

    def test_prefetch_related_primary_email(self):
        user = baker.make("devilry_account.User")
        primary_useremail = baker.make(
            "devilry_account.UserEmail", user=user, email="test@example.com", is_primary=True
        )
        baker.make("devilry_account.UserEmail", user=user, is_primary=None)
        baker.make("devilry_account.UserEmail", user=user, is_primary=None)
        with self.assertNumQueries(2):
            user_with_prefetch = User.objects.prefetch_related_primary_email().first()
            ...
            self.assertEqual(primary_useremail, user_with_prefetch.primary_useremail_objects[0])
            self.assertEqual(primary_useremail, user_with_prefetch.primary_useremail_object)
            self.assertEqual("test@example.com", user_with_prefetch.primary_email)

    def test_prefetch_related_primary_username(self):
        user = baker.make("devilry_account.User")
        primary_username = baker.make("devilry_account.UserName", user=user, username="testuser", is_primary=True)
        baker.make("devilry_account.UserEmail", user=user, is_primary=None)
        baker.make("devilry_account.UserEmail", user=user, is_primary=None)
        with self.assertNumQueries(2):
            user_with_prefetch = User.objects.prefetch_related_primary_username().first()
            ...
            self.assertEqual(primary_username, user_with_prefetch.primary_username_objects[0])
            self.assertEqual(primary_username, user_with_prefetch.primary_username_object)
            self.assertEqual("testuser", user_with_prefetch.primary_username)
    ...
    def test_filter_by_usernames(self):
        ...
        self.assertEqual({user1, user2, user3}, set(User.objects.filter_by_usernames(["user1", "user2", "user3"])))
...
class TestUserManager(TestCase):
    ...
    def test_create_user_fullname(self):
        user = User.objects.create_user(email="testuser@example.com", fullname="The Test User")
    ...
    def test_create_user_unset_lastname_when_no_fullname(self):
        user = User.objects.create_user(email="testuser@example.com", fullname="")
        self.assertEqual(user.lastname, "")
    ...
    def test_get_by_email(self):
        ...
        self.assertEqual(User.objects.get_by_email(email="test@example.com"), user)
    ...
    def test_get_by_username(self):
        ...
        self.assertEqual(User.objects.get_by_username(username="test@example.com"), user)
    ...
    def test_bulk_create_from_usernames_create_no_useremail_if_default_email_suffix_is_not_defined(self):
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=False, DEVILRY_DEFAULT_EMAIL_SUFFIX=""):
    ...
    def test_bulk_create_from_usernames_create_useremail_if_a_default_email_suffix_is_defined(self):
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=False, DEVILRY_DEFAULT_EMAIL_SUFFIX="@example.com"):
            User.objects.bulk_create_from_usernames(["testuser"])
    ...
    def test_bulk_create_from_usernames_multiple_new_users(self):
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=False):
            ...
            self.assertEqual(
                {"testuser1", "testuser2", "testuser3"}, set(User.objects.all().values_list("shortname", flat=True))
            )

            self.assertEqual(UserName.objects.count(), 3)
            self.assertEqual(
                {"testuser1", "testuser2", "testuser3"}, set(UserName.objects.all().values_list("username", flat=True))
            )
    ...
    def test_bulk_create_from_emails_single_new_user(self):
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=True):
            created_users, existing_emails = User.objects.bulk_create_from_emails(["testuser@example.com"])
...
class TestUserEmail(TestCase):
    ...
    def test_clean_is_primary_can_not_be_false(self):
        ...
        with self.assertRaisesMessage(ValidationError, "is_primary can not be False. Valid values are: True, None."):
            useremail.clean()

    def test_clean_useremail_set_is_primary_unsets_other(self):
        user = baker.make("devilry_account.User")
        useremail1 = baker.make("devilry_account.UserEmail", user=user, is_primary=True)
        useremail2 = baker.make("devilry_account.UserEmail", user=user, is_primary=None)
...
class TestUserName(TestCase):
    ...
    def test_clean_is_primary_can_not_be_false(self):
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=False):
            ...
            with self.assertRaisesMessage(
                ValidationError, "is_primary can not be False. Valid values are: True, None."
            ):
                usernameobject.clean()
    ...
    def test_clean_set_is_primary_unsets_other(self):
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=False):
            user = baker.make("devilry_account.User")
            usernameobject1 = baker.make("devilry_account.UserName", user=user, is_primary=True)
            usernameobject2 = baker.make("devilry_account.UserName", user=user, is_primary=None)
...
class TestPermissionGroup(TestCase):
    def test_grouptype_departmentadmin_can_not_be_editable(self):
        permissiongroup = baker.prepare(
            "devilry_account.PermissionGroup",
            is_custom_manageable=True,
            grouptype=PermissionGroup.GROUPTYPE_DEPARTMENTADMIN,
        )
        with self.assertRaisesMessage(ValidationError, "Department administrator groups can not be custom manageable."):
    ...
    def test_grouptype_can_not_be_changed_for_existing_group(self):
        ...
        with self.assertRaisesMessage(ValidationError, "Permission group type can not be changed."):
            permissiongroup.clean()
...
class TestPeriodPermissionGroupQuerySet(TestCase):
    def test_user_is_periodadmin_for_period_false_not_in_any_periodpermissiongroup(self):
        ...
        self.assertFalse(PeriodPermissionGroup.objects.user_is_periodadmin_for_period(period=testperiod, user=testuser))
    ...
    def test_user_is_periodadmin_for_period_true_for_periodadmin(self):
        ...
        baker.make(
            ...
            user=testuser,
            permissiongroup=baker.make("devilry_account.PeriodPermissionGroup", period=testperiod).permissiongroup,
        )
        self.assertTrue(PeriodPermissionGroup.objects.user_is_periodadmin_for_period(period=testperiod, user=testuser))

    def test_get_devilryrole_for_user_on_period_not_in_any_permissiongroup(self):
        testperiod = baker.make("core.Period")
        testuser = baker.make(settings.AUTH_USER_MODEL)
        self.assertEqual(
            None, PeriodPermissionGroup.objects.get_devilryrole_for_user_on_period(period=testperiod, user=testuser)
        )
    ...
    def test_get_devilryrole_for_user_on_period_superuser(self):
        ...
        self.assertEqual(
            "departmentadmin",
            PeriodPermissionGroup.objects.get_devilryrole_for_user_on_period(period=testperiod, user=testuser),
        )
    ...
    def test_get_custom_managable_periodpermissiongroup_for_period_only_custom_managable(self):
        testperiod = baker.make("core.Period")
        baker.make(
            "devilry_account.PeriodPermissionGroup", permissiongroup__is_custom_manageable=False, period=testperiod
        )
        with self.assertRaises(PeriodPermissionGroup.DoesNotExist):
            PeriodPermissionGroup.objects.get_custom_managable_periodpermissiongroup_for_period(period=testperiod)

    def test_get_custom_managable_periodpermissiongroup_for_period_only_in_period(self):
        testperiod = baker.make("core.Period")
        baker.make("devilry_account.PeriodPermissionGroup", permissiongroup__is_custom_manageable=True)
        with self.assertRaises(PeriodPermissionGroup.DoesNotExist):
            PeriodPermissionGroup.objects.get_custom_managable_periodpermissiongroup_for_period(period=testperiod)
    ...
    def test_get_custom_managable_periodpermissiongroup_for_period(self):
        ...
        periodpermissiongroup = baker.make(
            "devilry_account.PeriodPermissionGroup", permissiongroup__is_custom_manageable=True, period=testperiod
        )
        self.assertEqual(
            periodpermissiongroup,
            PeriodPermissionGroup.objects.get_custom_managable_periodpermissiongroup_for_period(period=testperiod),
...
class TestPeriodPermissionGroup(TestCase):
    def test_grouptype_must_be_periodadmin(self):
        periodpermissiongroup = baker.make(
            "devilry_account.PeriodPermissionGroup", permissiongroup__grouptype=PermissionGroup.GROUPTYPE_SUBJECTADMIN
        )
        with self.assertRaisesMessage(
            ValidationError, "Only semesters can be added to semester administrator permission groups."
        ):
    ...
    def test_only_single_editable_group_for_each_period_id_none(self):
        ...
        baker.make("devilry_account.PeriodPermissionGroup", permissiongroup=permissiongroup, period=period1)
        with self.assertRaisesMessage(
            ValidationError, "Only a single editable permission group is allowed for a semester."
        ):
            baker.prepare(
                "devilry_account.PeriodPermissionGroup", permissiongroup=permissiongroup, period=period2
            ).clean()
    ...
    def test_only_single_editable_group_for_each_period_id_not_none(self):
        ...
        baker.make("devilry_account.PeriodPermissionGroup", permissiongroup=permissiongroup, period=period1)
        periodpermissiongroup = baker.make("devilry_account.PeriodPermissionGroup", period=period2)
...
class TestSubjectPermissionGroupQuerySet(TestCase):
    def test_user_is_departmentadmin_for_subject_false_not_in_any_subjectpermissiongroup(self):
        ...
        testuser = baker.make(settings.AUTH_USER_MODEL)
        self.assertFalse(
            SubjectPermissionGroup.objects.user_is_departmentadmin_for_subject(subject=testsubject, user=testuser)
        )
    ...
    def test_user_is_subjectadmin_for_subject_false_not_in_any_subjectpermissiongroup(self):
        ...
        testuser = baker.make(settings.AUTH_USER_MODEL)
        self.assertFalse(
            SubjectPermissionGroup.objects.user_is_subjectadmin_for_subject(subject=testsubject, user=testuser)
        )
    ...
    def test_get_devilryrole_for_user_on_subject_not_in_any_subjectpermissiongroup(self):
        ...
        testuser = baker.make(settings.AUTH_USER_MODEL)
        self.assertEqual(
            None, SubjectPermissionGroup.objects.get_devilryrole_for_user_on_subject(subject=testsubject, user=testuser)
        )
    ...
    def test_get_devilryrole_for_user_on_subject_superuser(self):
        ...
        self.assertEqual(
            "departmentadmin",
            SubjectPermissionGroup.objects.get_devilryrole_for_user_on_subject(subject=testsubject, user=testuser),
        )
    ...
    def test_get_custom_managable_periodpermissiongroup_for_subject_only_custom_managable(self):
        testsubject = baker.make("core.Subject")
        baker.make(
            "devilry_account.SubjectPermissionGroup", permissiongroup__is_custom_manageable=False, subject=testsubject
        )
        with self.assertRaises(SubjectPermissionGroup.DoesNotExist):
            SubjectPermissionGroup.objects.get_custom_managable_subjectpermissiongroup_for_subject(subject=testsubject)

    def test_get_custom_managable_subjectpermissiongroup_for_subject_only_in_subject(self):
        testsubject = baker.make("core.Subject")
        baker.make("devilry_account.SubjectPermissionGroup", permissiongroup__is_custom_manageable=True)
        with self.assertRaises(SubjectPermissionGroup.DoesNotExist):
            SubjectPermissionGroup.objects.get_custom_managable_subjectpermissiongroup_for_subject(subject=testsubject)
    ...
    def test_get_custom_managable_subjectpermissiongroup_for_subject(self):
        ...
        subjectpermissiongroup = baker.make(
            "devilry_account.SubjectPermissionGroup", permissiongroup__is_custom_manageable=True, subject=testsubject
        )
        self.assertEqual(
            subjectpermissiongroup,
            SubjectPermissionGroup.objects.get_custom_managable_subjectpermissiongroup_for_subject(subject=testsubject),
...
class TestSubjectPermissionGroup(TestCase):
    def test_grouptype_must_be_subjectadmin(self):
        subjectpermissiongroup = baker.make(
            "devilry_account.SubjectPermissionGroup", permissiongroup__grouptype=PermissionGroup.GROUPTYPE_PERIODADMIN
        )
        with self.assertRaisesMessage(
            ValidationError, "Courses can only be added to course and department administrator permission groups"
        ):
    ...
    def test_only_single_editable_group_for_each_subject_id_none(self):
        ...
        baker.make("devilry_account.SubjectPermissionGroup", permissiongroup=permissiongroup, subject=subject1)
        with self.assertRaisesMessage(
            ValidationError, "Only a single editable permission group is allowed for a course."
        ):
            baker.prepare(
                "devilry_account.SubjectPermissionGroup", permissiongroup=permissiongroup, subject=subject2
            ).clean()
    ...
    def test_only_single_editable_group_for_each_subject_id_not_none(self):
        ...
        baker.make("devilry_account.SubjectPermissionGroup", permissiongroup=permissiongroup, subject=subject1)
        subjectpermissiongroup = baker.make("devilry_account.SubjectPermissionGroup", subject=subject2)
