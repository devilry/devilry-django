...
class TestUserMerger(test.TestCase):
    def test_merge_permission_group_user_objects(self):
        ...
        UserMerger(
            source_user=source_user, target_user=target_user, pretend=False
    ...
    def test_merge_relatedstudent_objects(self):
        ...
        source_relatedstudent1_candidate1 = baker.make(
            Candidate, assignment_group__assignment__period=period1, relatedstudent=source_relatedstudent1
        )
        source_relatedstudent1_candidate2 = baker.make(
            Candidate, assignment_group__assignment__period=period1, relatedstudent=source_relatedstudent1
        )
        source_relatedstudent2_candidate1 = baker.make(
            Candidate, assignment_group__assignment__period=period2, relatedstudent=source_relatedstudent2
        )
        target_relatedstudent1_candidate1 = baker.make(
            Candidate, assignment_group__assignment__period=period2, relatedstudent=target_relatedstudent1
        )
        target_relatedstudent2_candidate1 = baker.make(
            Candidate, assignment_group__assignment__period=period3, relatedstudent=target_relatedstudent2
        )

        source_relatedstudent1_qualifies_for_final_exam = baker.make(
            QualifiesForFinalExam, relatedstudent=source_relatedstudent1
        )
        source_relatedstudent2_qualifies_for_final_exam = baker.make(
            QualifiesForFinalExam, relatedstudent=source_relatedstudent2
        )
        target_relatedstudent1_qualifies_for_final_exam = baker.make(
            QualifiesForFinalExam, relatedstudent=target_relatedstudent1
        )
        ...
        UserMerger(source_user=source_user, target_user=target_user, pretend=False)._merge_relatedstudent_objects()
        ...
        self.assertEqual(
            Candidate.objects.filter(relatedstudent=target_converted_from_source_relatedstudent1).count(), 2
        )
        self.assertEqual(
        ...
        self.assertEqual(
            Candidate.objects.filter(relatedstudent=target_relatedstudent2).first(), target_relatedstudent2_candidate1
        )

        # Detailed cheks for QualifiedForFinalExam
        self.assertEqual(
            QualifiesForFinalExam.objects.filter(relatedstudent=target_converted_from_source_relatedstudent1).count(), 1
        )
        self.assertEqual(
    ...
    def test_merge_relatedexaminer_objects(self):
        ...
        source_relatedexaminer1_examiner1 = baker.make(
            Examiner, assignmentgroup__assignment__period=period1, relatedexaminer=source_relatedexaminer1
        )
        source_relatedexaminer1_examiner2 = baker.make(
            Examiner, assignmentgroup__assignment__period=period1, relatedexaminer=source_relatedexaminer1
        )
        source_relatedexaminer2_examiner1 = baker.make(
            Examiner, assignmentgroup__assignment__period=period2, relatedexaminer=source_relatedexaminer2
        )
        target_relatedexaminer1_examiner1 = baker.make(
            Examiner, assignmentgroup__assignment__period=period2, relatedexaminer=target_relatedexaminer1
        )
        target_relatedexaminer2_examiner1 = baker.make(
            Examiner, assignmentgroup__assignment__period=period3, relatedexaminer=target_relatedexaminer2
        )
        ...
        UserMerger(source_user=source_user, target_user=target_user, pretend=False)._merge_relatedexaminer_objects()
        ...
        self.assertEqual(
            Examiner.objects.filter(relatedexaminer=target_converted_from_source_relatedexaminer1).count(), 2
        )
        self.assertEqual(
        ...
        self.assertEqual(
            Examiner.objects.filter(relatedexaminer=target_relatedexaminer2).first(), target_relatedexaminer2_examiner1
        )
    ...
    def test_merge_comment_objects(self):
        ...
        UserMerger(source_user=source_user, target_user=target_user, pretend=False)._merge_comment_objects()
    ...
    def test_merge_feedbackset_objects_created_by(self):
        ...
        UserMerger(source_user=source_user, target_user=target_user, pretend=False)._merge_feedbackset_objects()
        ...
        self.assertEqual(
            set(FeedbackSet.objects.filter(created_by=target_user)), {feedbackset1, feedbackset2, feedbackset3}
        )
    ...
    def test_merge_feedbackset_objects_last_updated_by(self):
        ...
        self.assertEqual(
            set(FeedbackSet.objects.filter(last_updated_by=target_user)), {feedbackset1, feedbackset2, feedbackset3}
        )
    ...
    def test_merge_feedbackset_objects_grading_published_by(self):
        ...
        self.assertEqual(
            set(FeedbackSet.objects.filter(grading_published_by=target_user)),
            {feedbackset1, feedbackset2, feedbackset3},
        )

    def test_merge_sanity(self):
        source_user = baker.make(settings.AUTH_USER_MODEL)
        target_user = baker.make(settings.AUTH_USER_MODEL)
        merger = UserMerger(source_user=source_user, target_user=target_user, pretend=False)
