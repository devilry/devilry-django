...
class MergedUserAdmin(admin.ModelAdmin):
    ...
    def get_summary_json_pretty(self, obj):
        return format_html("<pre>{}</pre>", json.dumps(obj.summary_json, indent=2, sort_keys=True))
...
class PermissionGroupAdmin(admin.ModelAdmin):
    ...
    inlines = [PermissionGroupUserInline, SubjectPermissionGroupInline, PeriodPermissionGroupInline]

    def get_queryset(self, request):
        return super(PermissionGroupAdmin, self).get_queryset(request).prefetch_related("users")
    ...
    def get_inline_instances(self, request, obj=None):
        inline_instances = [PermissionGroupUserInline(self.model, self.admin_site)]
        if obj:
...
class SubjectPermissionGroupAdmin(admin.ModelAdmin):
    ...
    raw_id_fields = ["permissiongroup", "subject"]
...
class PeriodPermissionGroupAdmin(admin.ModelAdmin):
    ...
    raw_id_fields = ["permissiongroup", "period"]
