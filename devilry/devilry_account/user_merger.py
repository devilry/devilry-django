...
class UserMerger:
    ...
    def __init__(self, source_user, target_user, pretend=True):
        ...
        self.merged_user = MergedUser(source_user=source_user, target_user=target_user, summary_json={})
        ...
        self.summary["PermissionGroupUser"] = {
            ...
            "details": {
                "permission_group_users_updated": [
                    ...
                    for permissiongroupuser in permissiongroupusers.order_by("permissiongroup__name").select_related(
                        "permissiongroup"
                    )
                ]
            },
    ...
    def _merge_relatedstudent_objects(self):
        ...
        for source_relatedstudent in RelatedStudent.objects.filter(user=self.source_user).select_related("period"):
            target_relatedstudent = RelatedStudent.objects.filter(
                user=self.target_user, period=source_relatedstudent.period
            ).first()
            if target_relatedstudent:
                # Candidates
                candidates = Candidate.objects.filter(relatedstudent=source_relatedstudent).select_related(
                    "assignment_group__parentnode"
                )
                if not self.pretend:
                    candidates.update(relatedstudent=target_relatedstudent)
                for candidate in candidates:
                    summary["details"]["merged_candidates"].append(
                        {"candidate_id": candidate.id, "assignment": candidate.assignment_group.assignment.get_path()}
                    )

                # QualifiesForFinalExam
                qualifiesforfinalexams = QualifiesForFinalExam.objects.filter(
                    relatedstudent=source_relatedstudent
                ).select_related("relatedstudent__period")
                if not self.pretend:
                    qualifiesforfinalexams.update(relatedstudent=target_relatedstudent)
                ...
                summary["details"]["relatedstudents_with_user_fk_changed"].append(
                    {"relatedstudent_id": source_relatedstudent.id, "period": source_relatedstudent.period.get_path()}
                )
    ...
    def _merge_relatedexaminer_objects(self):
        ...
        for source_relatedexaminer in RelatedExaminer.objects.filter(user=self.source_user).select_related("period"):
            target_releatedexaminer = RelatedExaminer.objects.filter(
                user=self.target_user, period=source_relatedexaminer.period
            ).first()
            if target_releatedexaminer:
                # Candidates
                examiners = Examiner.objects.filter(relatedexaminer=source_relatedexaminer).select_related(
                    "assignmentgroup__parentnode"
                )
                if not self.pretend:
                    examiners.update(relatedexaminer=target_releatedexaminer)
                for examiner in examiners:
                    summary["details"]["merged_examiners"].append(
                        {"examiner_id": examiner.id, "assignment": examiner.assignmentgroup.assignment.get_path()}
                    )
