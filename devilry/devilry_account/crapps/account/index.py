...
class IndexView(TemplateView):
    ...
    def get_related_examiners_for_active_periods(self):
        now = timezone.now()
        return (
            RelatedExaminer.objects.select_related("user", "period", "period__parentnode")
            .filter(active=True, user=self.request.user, period__start_time__lte=now, period__end_time__gt=now)
    ...
    def get_related_students_for_active_periods(self):
        now = timezone.now()
        return (
            RelatedStudent.objects.select_related("user", "period", "period__parentnode")
            .filter(active=True, user=self.request.user, period__start_time__lte=now, period__end_time__gt=now)
            .order_by("period__parentnode__long_name")
        )
