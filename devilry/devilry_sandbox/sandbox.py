...
class Sandbox(object):
    ...
    EXAMINERS = (("donald", "Donald Duck", "group1"), ("scrooge", "Scrooge McDuck", "group2"))
    ...
    def _create_rootnode_if_not_exists(self):
        if not self._rootnode_exists():
            node = Node.objects.create(short_name=self.nodename, long_name=self.nodename)
        else:
    ...
    def create_user(self, username, fullname):
        ...
        user = User.objects.create(username=username, email="{0}@example.com".format(username))
    ...
    def add_relatedstudents(self, period):
        for username, fullname, tags in self.STUDENTS:
            period.relatedstudent_set.create(
                user=self.add_or_get_user(username, fullname),
    ...
    def add_relatedexaminers(self, period):
        for username, fullname, tags in self.EXAMINERS:
            period.relatedexaminer_set.create(user=self.add_or_get_user(username, fullname), tags=tags)
    ...
    def _create_subject(self, short_name, long_name):
        ...
        subject = node.subjects.create(short_name=short_name, long_name=long_name)
        return subject
