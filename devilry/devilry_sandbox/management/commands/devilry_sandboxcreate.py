...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
        make_option("-s", "--subject-short", dest="subject_shortname", help="Short name of subject."),
        make_option("-l", "--subject-long", dest="subject_longname", help="Long name of subject."),
        make_option(
            "--subjectadmin",
            action="store_true",
            default=False,
    ...
    def handle(self, *args, **options):
        with transaction.commit_on_success():
            ...
            subject = sandbox.create_subject(
                short_name=options["subject_shortname"], long_name=options["subject_longname"]
            )

            period = sandbox.create_period(subject, short_name="fall2012", long_name="Fall 2012")
