...
class TestMessage(test.TestCase):
    ...
    def test_validate_user_ids_not_in_virtual_message_receivers(self):
        message = baker.make(
            "devilry_message.Message", virtual_message_receivers={"test": "lol"}, message_type=["email"]
        )
    ...
    def test_validate_virtual_message_receivers_not_list(self):
        message = baker.make(
            "devilry_message.Message", virtual_message_receivers={"user_ids": {}}, message_type=["email"]
        )
    ...
    def test_validate_virtual_message_receivers_contains_non_integer_values(self):
        message = baker.make(
            "devilry_message.Message", virtual_message_receivers={"user_ids": [1, "23"]}, message_type=["email"]
        )
        with self.assertRaisesMessage(
            ValueError, "'virtual_message_receivers[\"user_ids\"]' contains a non-integer value.: 23"
        ):
            message.validate_virtual_message_receivers()
    ...
    def test_prepare_single_message_receiver(self):
        ...
        message = baker.make(
            "devilry_message.Message", virtual_message_receivers={"user_ids": [user.id]}, message_type=["email"]
        )
    ...
    def test_prepare_and_send_message_not_draft(self):
        user = self.__make_email_for_user(baker.make(settings.AUTH_USER_MODEL), "testuser@example.com").user
        message_preparing = baker.make(
            "devilry_message.Message",
            virtual_message_receivers={"user_ids": [user.id]},
            message_type=["email"],
            status="preparing",
        )
        message_sending = baker.make(
            "devilry_message.Message",
            virtual_message_receivers={"user_ids": [user.id]},
            message_type=["email"],
            status="sending",
        )
        message_error = baker.make(
            "devilry_message.Message",
            virtual_message_receivers={"user_ids": [user.id]},
            message_type=["email"],
            status="error",
        )
        message_sent = baker.make(
            "devilry_message.Message",
            virtual_message_receivers={"user_ids": [user.id]},
            message_type=["email"],
            status="sent",
    ...
    def test_prepare_and_send_create_message_receivers_raises_error(self):
        ...
        with mock.patch(
            "devilry.devilry_message.models.Message.create_message_receivers", mock_create_message_receivers
        ):
            message.prepare_and_send(
                subject_generator=test_utils.SubjectTextTestGenerator(),
                template_name="devilry_message/for_test.django.html",
    ...
    def test_prepare_and_send_query_count(self):
        ...
        for i in range(1, 10):
            user = self.__make_email_for_user(
                baker.make(settings.AUTH_USER_MODEL), "testuser{}@example.com".format(i)
            ).user
            user_ids.append(user.id)
        message = baker.make(
            "devilry_message.Message", virtual_message_receivers={"user_ids": user_ids}, message_type=["email"]
        )
