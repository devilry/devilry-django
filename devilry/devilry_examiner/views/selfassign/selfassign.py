...
class ExaminerSelfAssignGroupItemValue(devilry_listbuilder.assignmentgroup.ExaminerItemValue):
    ...
    def is_assigned_to_group(self):
        return Examiner.objects.filter(
            assignmentgroup=self.group, relatedexaminer__user=self.kwargs["request"].user
        ).exists()
    ...
    def attributes(self):
        ...
        attributes = " ".join(
            ...
            for key, value in {
                ...
                "assignText": pgettext("examiner selfassign group item value", "Add me"),
                "assignProgressText": pgettext("examiner selfassign group item value", "Adding..."),
                "unassignText": pgettext("examiner selfassign group item value", "Remove me"),
                "unassignProgressText": pgettext("examiner selfassign group item value", "Removing..."),
                "unavailableText": pgettext("examiner selfassign group item value", "UNAVAILABLE"),
            }.items()
        )
        return attributes
...
class SelfAssignGroupListView(listbuilderview.FilterListMixin, listbuilderview.View):
    ...
    def get_pagetitle(self):
        return pgettext_lazy("examiner selfassign view", "%(period_name)s - self-assign") % {
    ...
    def get_value_and_frame_renderer_kwargs(self):
        return {"request": self.request, "period": self.period}
    ...
    def get_unfiltered_queryset_for_role(self, role):
        ...
        queryset = (
            assignment_groups_available_for_self_assign(period=role, user=self.request.user)
            ...
            .annotate(
                is_examiner=models.Exists(
                    Examiner.objects.filter(
                        relatedexaminer__user=self.request.user, assignmentgroup_id=models.OuterRef("id")
                    )
                )
            )
    ...
    def get_distinct_assignments_queryset(self):
        if not hasattr(self, "_distinct_assignment_ids"):
            self._distinct_assignment_ids = self.__get_unfiltered_queryset_for_role().values_list(
                "parentnode_id", flat=True
            )
        return (
            coremodels.Assignment.objects.filter(id__in=self._distinct_assignment_ids).distinct().order_by("short_name")
        )
...
class App(crapp.App):
    appurls = [
        crapp.Url(r"^$", SelfAssignGroupListView.as_view(), name=crapp.INDEXVIEW_NAME),
        crapp.Url(r"^filter/(?P<filters_string>.+)?$", SelfAssignGroupListView.as_view(), name="filter"),
    ]
