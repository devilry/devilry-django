...
def get_assignments_available_for_selfassign():
    ...
    return (
        Assignment.objects.select_related("parentnode")
        .filter_is_active()
        .filter(anonymizationmode=Assignment.ANONYMIZATIONMODE_OFF, examiners_can_self_assign=True)
    )
...
def assignment_groups_available_for_self_assign(period, user):
    ...
    assignment_queryset = get_assignments_available_for_selfassign().filter(
        parentnode_id__in=RelatedExaminer.objects.select_related("period")
        .filter(user=user, period=period, active=True)
        .values_list("period_id", flat=True)
    )

    # AssignmentGroups
    assignmentgroup_queryset = (
        AssignmentGroup.objects.select_related("parentnode")
...
def selfassign_available_periods(user):
    return (
        Period.objects.filter_active()
        .filter(
            id__in=RelatedExaminer.objects.select_related("period")
        ...
        .filter(has_assignments_with_selfassign_enabled=True)
        .distinct()
    )
