...
class ExaminerSelfAssignApi(APIView):
    ...
    def is_assigned_to_group(self):
        return Examiner.objects.filter(
            assignmentgroup=self.assignment_group, relatedexaminer__user=self.request.user
        ).exists()

    def assign_to_group(self):
        if not self.is_assigned_to_group():
            relatedexaminer = RelatedExaminer.objects.filter(active=True).get(
                period=self.period, user=self.request.user
            )
    ...
    def unassign_from_group(self):
        if self.is_assigned_to_group():
            examiner = Examiner.objects.filter(
                assignmentgroup=self.assignment_group, relatedexaminer__user=self.request.user
            ).get()
            examiner.delete()
    ...
    def post(self, request, *args, **kwargs):
        try:
            self.period = Period.objects.filter_active().get(id=self.kwargs["period_id"])
        ...
        try:
            self.assignment_group = selfassign_utils.assignment_groups_available_for_self_assign(
                period=self.period, user=self.request.user
            ).get(id=group_id)
