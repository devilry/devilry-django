...
class AssignmentListView(listbuilderview.FilterListMixin, listbuilderview.View):
    ...
    def get_pagetitle(self):
        return pgettext_lazy("examiner assignmentlist", "Examiner dashboard")
    ...
    def get_unfiltered_queryset_for_role(self, role):
        return (
            coremodels.Assignment.objects.filter_examiner_has_access(user=self.request.user)
    ...
    def get_context_data(self, **kwargs):
        ...
        context["selfassign_available_periods"] = selfassign_utils.selfassign_available_periods(user=self.request.user)
        return context


class App(crapp.App):
    appurls = [
        crapp.Url(r"^$", AssignmentListView.as_view(), name=crapp.INDEXVIEW_NAME),
        crapp.Url(r"^filter/(?P<filters_string>.+)?$", AssignmentListView.as_view(), name="filter"),
    ]
