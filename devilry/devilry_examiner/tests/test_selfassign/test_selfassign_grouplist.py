...
class TestSelfassignGrouplistView(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def __make_group(self, student_user=None, num_other_examiners=0, **assignmentgroup_kwargs):
        ...
        for num in range(num_other_examiners):
            baker.make(
                ...
                relatedexaminer=baker.make("core.RelatedExaminer", period=group.parentnode.parentnode),
            )
    ...
    def test_title(self):
        ...
        testperiod = baker.make_recipe(
            "devilry.apps.core.period_active", short_name="thesemester", long_name="The Semester"
        )
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testperiod, requestuser=testuser)
        self.assertIn("The Semester - self-assign", mockresponse.selector.one("title").alltext_normalized)
    ...
    def test_h1(self):
        ...
        baker.make("core.RelatedExaminer", period=testperiod, user=testuser)
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testperiod, requestuser=testuser)
        self.assertIn("The Semester - self-assign", mockresponse.selector.one("h1").alltext_normalized)
    ...
    def test_selfassign_limit_one_group_available(self):
        ...
        group_itemvalue_list = mockresponse.selector.list(
    ...
    def test_multiple_groups_available_single_assignment_sanity(self):
        ...
        self.__make_group(
            student_user=baker.make(
                settings.AUTH_USER_MODEL, fullname="Student User1", shortname="student1@example.com"
            ),
            parentnode=testassignment,
        )
        self.__make_group(
            student_user=baker.make(
                settings.AUTH_USER_MODEL, fullname="Student User2", shortname="student2@example.com"
            ),
            parentnode=testassignment,
        )
        baker.make("core.RelatedExaminer", period=testperiod, user=testuser)
    ...
    def test_multiple_groups_available_multiple_assignments_sanity(self):
        ...
        self.__make_group(
            student_user=baker.make(
                settings.AUTH_USER_MODEL, fullname="Student Assignment1", shortname="studentassignment1@example.com"
            ),
            parentnode=testassignment1,
        )
        self.__make_group(
            student_user=baker.make(
                settings.AUTH_USER_MODEL, fullname="Student Assignment2", shortname="studentassignment2@example.com"
            ),
            parentnode=testassignment2,
        )
        baker.make("core.RelatedExaminer", period=testperiod, user=testuser)
...
class TestSelfassignGrouplistViewFilters(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_assignment_filter_label_sanity(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=self.period, requestuser=self.examiner_user)
        self.assertEqual(
    ...
    def test_assignment_filter_name_single_assignment_sanity(self):
        ...
        self.assertEqual(
            mockresponse.selector.one(
                f"#cradmin_legacy_listfilter_assignmentname_input_{assignment.short_name}_label"
            ).alltext_normalized,
            "Assignment",
        )
    ...
    def test_assignment_filter_name_multiple_assignments_sanity(self):
        ...
        self.assertEqual(
            mockresponse.selector.one(
                f"#cradmin_legacy_listfilter_assignmentname_input_{assignment1.short_name}_label"
            ).alltext_normalized,
            "Assignment 1",
        )
        self.assertEqual(
            mockresponse.selector.one(
                f"#cradmin_legacy_listfilter_assignmentname_input_{assignment2.short_name}_label"
            ).alltext_normalized,
            "Assignment 2",
        )
        self.assertEqual(
            mockresponse.selector.one(
                f"#cradmin_legacy_listfilter_assignmentname_input_{assignment3.short_name}_label"
            ).alltext_normalized,
            "Assignment 3",
        )
    ...
    def test_filter_no_filter_sanity(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=self.period, requestuser=self.examiner_user, viewkwargs={"filters_string": ""}
        )
        group_itemvalue_list = mockresponse.selector.list(".cradmin-legacy-listbuilder-itemvalue")
        self.assertEqual(len(group_itemvalue_list), 3)
    ...
    def test_assignment_filter_all_assignments_sanity(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(
            ...
            requestuser=self.examiner_user,
            viewkwargs={
                "filters_string": f"assignmentname-{assignment1.short_name}%2C{assignment2.short_name}%2C{assignment3.short_name}"
            },
        )
        group_itemvalue_list = mockresponse.selector.list(".cradmin-legacy-listbuilder-itemvalue")
        self.assertEqual(len(group_itemvalue_list), 3)
    ...
    def test_assignstatus_filter_all_groups(self):
        ...
        baker.make(
            "core.Examiner",
            relatedexaminer=self.related_examiner,
            assignmentgroup=AssignmentGroup.objects.get(parentnode=assignment1),
        )
        baker.make(
            "core.Examiner",
            relatedexaminer=self.related_examiner,
            assignmentgroup=AssignmentGroup.objects.get(parentnode=assignment2),
        )
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=self.period, requestuser=self.examiner_user, viewkwargs={"filters_string": ""}
        )
