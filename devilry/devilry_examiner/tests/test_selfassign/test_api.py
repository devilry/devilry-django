...
class TestSelfAssignApi(test.TestCase, api_test_mixin.ApiTestMixin):
    ...
    def test_method_get_404(self):
        response = self.make_get_request(requestuser=baker.make(settings.AUTH_USER_MODEL))
    ...
    def test_missing_group_id_in_post_data(self):
        ...
        response = self.make_post_request(
            requestuser=self.examiner_user, viewkwargs={"period_id": self.period.id}, data={"action": "ASSIGN"}
        )
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data, {"error": 'Missing "group_id".'})

    def test_missing_action_in_post_data(self):
        assignment = baker.make("core.Assignment", examiners_can_self_assign=True, parentnode=self.period)
        group = self.__make_group(assignment=assignment)
        response = self.make_post_request(
            requestuser=self.examiner_user, viewkwargs={"period_id": self.period.id}, data={"group_id": group.id}
        )
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data, {"error": 'Missing "action".'})
    ...
    def test_invalid_action_in_post_data(self):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data, {"error": 'Unknown action "SOMEACTION".'})
    ...
    def test_not_ok_selfassign_limit_reached(self):
        ...
        baker.make(
            ...
            relatedexaminer=baker.make("core.RelatedExaminer", period=self.period),
        )
        response = self.make_post_request(
            requestuser=self.examiner_user,
            viewkwargs={"period_id": self.period.id},
    ...
    def test_ok_assign_sanity(self):
        assignment = baker.make("core.Assignment", examiners_can_self_assign=True, parentnode=self.period)
        group = self.__make_group(assignment=assignment)
        self.assertFalse(Examiner.objects.filter(relatedexaminer=self.related_examiner, assignmentgroup=group).exists())
        response = self.make_post_request(
            requestuser=self.examiner_user,
            viewkwargs={"period_id": self.period.id},
            data={"group_id": group.id, "action": "ASSIGN"},
        ...
        self.assertTrue(Examiner.objects.filter(relatedexaminer=self.related_examiner, assignmentgroup=group).exists())
