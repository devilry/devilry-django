...
class TestGroupsAvailableForSelfAssign(test.TestCase):
    ...
    def test_selfassign_available_limit_reached_but_user_is_examiner(self):
        ...
        group = baker.make("core.AssignmentGroup", parentnode=assignment)
        baker.make("core.Examiner", assignmentgroup=group, relatedexaminer=related_examiner)
        groupqueryset = utils.assignment_groups_available_for_self_assign(
            period=assignment.parentnode, user=examiner_user
        )
        self.assertEqual(groupqueryset.count(), 1)
...
class TestSelfAssignAvailablePeriods(test.TestCase):
    ...
    def test_multiple_periods_mixed_sanity(self):
        ...
        baker.make(
            "core.Assignment", parentnode=period_no_selfassign_assignments, examiners_can_self_assign=False, _quantity=2
        )
        ...
        baker.make(
            "core.Assignment",
            parentnode=period_only_selfassign_assignments,
            examiners_can_self_assign=True,
            _quantity=2,
        )
        ...
        baker.make("core.Assignment", parentnode=period_mixed, examiners_can_self_assign=True, _quantity=2)
        baker.make("core.Assignment", parentnode=period_mixed, examiners_can_self_assign=False, _quantity=2)
