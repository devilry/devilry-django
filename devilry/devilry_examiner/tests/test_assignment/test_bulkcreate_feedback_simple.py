...
class TestBulkCreateFeedbackSimplePassedFailedPlugin(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_get_form_tags(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=user, cradmin_role=testassignment)
    ...
    def test_group_single_candidate_display_names(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=user, cradmin_role=testassignment)
        self.assertEqual(
            "Candidate(candidate)", mockresponse.selector.one(".devilry-user-verbose-inline-both").alltext_normalized
        )
    ...
    def test_group_three_candidates_display_names(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=user, cradmin_role=testassignment)
        self.assertEqual(
            "Candidate1(candidate1) , Candidate2(candidate2) , Candidate3(candidate3)",
    ...
    def test_group_anonymization_semi_single_candidate(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=user, cradmin_role=testassignment)
        self.assertEqual(
            "Automatic anonymous ID missing",
    ...
    def test_group_anonymization_semi_multiple_candidates(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=user, cradmin_role=testassignment)
        candidate_names = [
            elem.alltext_normalized for elem in mockresponse.selector.list(".devilry-core-candidate-anonymous-name")
        ]
    ...
    def test_post_without_grading_not_saved(self):
        ...
        self.mock_http302_postrequest(
            ...
            cradmin_role=testassignment,
            requestkwargs={
                "data": {"grade_{}".format(testgroup.id): "", "comment_text_{}".format(testgroup.id): "you passed"}
            },
        )
    ...
    def test_get_query_count(self):
        ...
        with self.assertNumQueries(2):
            self.mock_http200_getrequest_htmls(requestuser=user, cradmin_role=testassignment)
...
class TestBulkCreateFeedbackSimplePointsPlugin(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_post_comment_attributes(self):
        ...
        self.mock_http302_postrequest(
            ...
            cradmin_role=testassignment,
            requestkwargs={
                "data": {"grade_{}".format(testgroup.id): 100, "comment_text_{}".format(testgroup.id): "you passed"}
            },
        )
        group_comments = group_models.GroupComment.objects.all()
        self.assertEqual(1, group_comments.count())
    ...
    def test_post_bulk_feedback_single_candidate_with_comment_text(self):
        ...
        self.mock_http302_postrequest(
            ...
            cradmin_role=testassignment,
            requestkwargs={
                "data": {"grade_{}".format(testgroup.id): 60, "comment_text_{}".format(testgroup.id): "you passed"}
            },
        )
        cached_data = AssignmentGroupCachedData.objects.get(group_id=testgroup.id)
        group_comments = group_models.GroupComment.objects.all()
