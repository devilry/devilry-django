...
class TestStudentRoleItemValue(test.TestCase):
    def test_sanity(self):
        ...
        self.assertTrue(selector.exists(".devilry-frontpage-listbuilder-roleselect-itemvalue"))

    def test_correct_devilryrole(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        selector = htmls.S(listbuilder_role.StudentRoleItemValue(value=testuser).render())
        self.assertTrue(selector.exists(".devilry-frontpage-listbuilder-roleselect-itemvalue-student"))

    def test_title(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        selector = htmls.S(listbuilder_role.StudentRoleItemValue(value=testuser).render())
        self.assertEqual(
            "Student", selector.one(".cradmin-legacy-listbuilder-itemvalue-titledescription-title").alltext_normalized
        )
...
class TestExaminerRoleItemValue(test.TestCase):
    ...
    def test_correct_devilryrole(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        selector = htmls.S(listbuilder_role.ExaminerRoleItemValue(value=testuser).render())
        self.assertTrue(selector.exists(".devilry-frontpage-listbuilder-roleselect-itemvalue-examiner"))

    def test_title(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        selector = htmls.S(listbuilder_role.ExaminerRoleItemValue(value=testuser).render())
        self.assertEqual(
            "Examiner", selector.one(".cradmin-legacy-listbuilder-itemvalue-titledescription-title").alltext_normalized
        )
...
class TestStudentRoleItemFrame(test.TestCase):
    def test_sanity(self):
        ...
        self.assertTrue(selector.exists(".devilry-frontpage-listbuilder-roleselect-itemframe"))

    def test_correct_devilryrole(self):
        selector = htmls.S(listbuilder_role.StudentRoleItemFrame(inneritem=mock.MagicMock()).render())
        self.assertTrue(selector.exists(".devilry-frontpage-listbuilder-roleselect-itemframe-student"))

    def test_url(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        selector = htmls.S(listbuilder_role.StudentRoleItemFrame(inneritem=mock.MagicMock(value=testuser)).render())
        self.assertEqual("/devilry_student/", selector.one(".cradmin-legacy-listbuilder-itemframe-link")["href"])
...
class TestExaminerRoleItemFrame(test.TestCase):
    ...
    def test_correct_devilryrole(self):
        selector = htmls.S(listbuilder_role.ExaminerRoleItemFrame(inneritem=mock.MagicMock()).render())
        self.assertTrue(selector.exists(".devilry-frontpage-listbuilder-roleselect-itemframe-examiner"))

    def test_url(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        selector = htmls.S(listbuilder_role.ExaminerRoleItemFrame(inneritem=mock.MagicMock(value=testuser)).render())
        self.assertEqual("/devilry_examiner/", selector.one(".cradmin-legacy-listbuilder-itemframe-link")["href"])
...
class TestAnyAdminRoleItemFrame(test.TestCase):
    ...
    def test_correct_devilryrole(self):
        selector = htmls.S(listbuilder_role.AnyAdminRoleItemFrame(inneritem=mock.MagicMock()).render())
        self.assertTrue(selector.exists(".devilry-frontpage-listbuilder-roleselect-itemframe-anyadmin"))

    def test_url(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        selector = htmls.S(listbuilder_role.AnyAdminRoleItemFrame(inneritem=mock.MagicMock(value=testuser)).render())
        self.assertEqual("/devilry_admin/", selector.one(".cradmin-legacy-listbuilder-itemframe-link")["href"])
...
class TestRoleSelectList(test.TestCase):
    def test_user_is_student(self):
        ...
        self.assertTrue(selector.exists(".devilry-frontpage-listbuilder-roleselect-itemvalue-student"))
        self.assertFalse(selector.exists(".devilry-frontpage-listbuilder-roleselect-itemvalue-examiner"))
    ...
    def test_user_is_examiner(self):
        ...
        selector = htmls.S(listbuilder_role.RoleSelectList(user=testuser).render())
        self.assertFalse(selector.exists(".devilry-frontpage-listbuilder-roleselect-itemvalue-student"))
        self.assertTrue(selector.exists(".devilry-frontpage-listbuilder-roleselect-itemvalue-examiner"))
        self.assertFalse(selector.exists(".devilry-frontpage-listbuilder-roleselect-itemvalue-anyadmin"))
