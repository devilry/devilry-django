...
class FeedbackSetCompressAction(AbstractBaseBatchAction, FeedbackSetBatchMixin):
    ...
    def execute(self):
        ...
        try:
            ...
            feedback_sets_with_public_student_comments = (
                group_models.FeedbackSet.objects.filter_public_comment_files_from_students().filter(id=feedback_set.id)
            )
            ...
            archive_name = "feedbackset-{}-{}-delivery".format(
                feedback_set.id, timezone.now().strftime("%Y-%m-%d_%H-%M-%S.%f")
            )
            ...
            CompressedArchiveMeta.objects.create_meta(
                instance=feedback_set, zipfile_backend=zipfile_backend, user=started_by_user
            )

            print_memory_usage("End of RQ task")
        except Exception:
            logger.error("Failed to generate zip archive from feedbackset %s", feedback_set.pk)
            raise
