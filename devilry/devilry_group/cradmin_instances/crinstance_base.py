...
class DevilryGroupCrInstanceMixin(object):
    ...
    def _get_base_rolequeryset(self):
        ...
        """
        return (
            AssignmentGroup.objects.annotate_with_is_waiting_for_feedback_count()
            ...
            .prefetch_related(models.Prefetch("candidates", queryset=self._get_candidatequeryset()))
            .prefetch_related(models.Prefetch("examiners", queryset=self._get_examinerqueryset()))
            .prefetch_related(models.Prefetch("cached_data", queryset=self._get_assignment_group_cacheddata_queryset()))
        )
    ...
    def _get_candidatequeryset(self):
        ...
        """
        return Candidate.objects.select_related("relatedstudent").order_by(
            Lower(
                Concat(
                    "relatedstudent__user__fullname", "relatedstudent__user__shortname", output_field=models.CharField()
                )
            )
        )
    ...
    def _get_examinerqueryset(self):
        ...
        """
        return Examiner.objects.select_related("relatedexaminer").order_by(
            Lower(
                Concat(
    ...
    def _get_assignment_group_cacheddata_queryset(self):
        return AssignmentGroupCachedData.objects.select_related(
            "group", "first_feedbackset", "last_feedbackset", "last_published_feedbackset"
        )
