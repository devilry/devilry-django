...
class TestFeedbackSetModel(TestCase):
    ...
    def test_feedbackset_created_by(self):
        ...
        feedbackset = group_baker.make_first_feedbackset_in_group(created_by=testuser)
    ...
    def test_feedbackset_grading_published_datetime(self):
        feedbackset = group_baker.make_first_feedbackset_in_group(grading_published_datetime=timezone.now())
    ...
    def test_feedbackset_current_deadline_first_attempt(self):
        ...
        test_feedbackset = group_baker.make_first_feedbackset_in_group(group__parentnode=test_assignment)
    ...
    def test_feedback_set_publish_multiple_feedbackcomments_order(self):
        ...
        groupcomments = group_models.GroupComment.objects.filter(feedback_set__id=testfeedbackset.id).order_by(
            "published_datetime"
        )
    ...
    def test_feedbackset_not_ignored_with_reason(self):
        test_feedbackset = group_baker.make_first_feedbackset_in_group(ignored_reason="dewey was sick!")
        with self.assertRaisesMessage(
            ValidationError, "FeedbackSet can not have a ignored reason without being set to ignored."
        ):
            test_feedbackset.full_clean()

    def test_feedbackset_ignored_with_grading_published(self):
        test_feedbackset = group_baker.make_first_feedbackset_in_group(
            ignored=True, ignored_reason="test", grading_published_datetime=timezone.now()
        )
        with self.assertRaisesMessage(
            ValidationError,
            "Ignored FeedbackSet can not have grading_published_datetime, grading_points or grading_published_by set.",
        ):
            test_feedbackset.full_clean()
    ...
    def test_feedbackset_publish_published_by_is_none(self):
        ...
        with self.assertRaisesMessage(
            ValidationError, "A FeedbackSet can not be published without being published by someone."
        ):
    ...
    def test_can_add_comment_student_assignment_deadline_hard_expired(self):
        ...
        with self.assertRaises(group_models.HardDeadlineExpiredException):
            test_feedbackset.can_add_comment(
                assignment=test_assignment, comment_user_role=group_models.GroupComment.USER_ROLE_STUDENT
            )
    ...
    def test_can_add_comment_examiner_assignment_deadline_hard_expired_exception_not_raised(self):
        ...
        test_feedbackset = baker.make("devilry_group.FeedbackSet", group__parentnode=test_assignment)
        test_feedbackset.can_add_comment(
            assignment=test_assignment, comment_user_role=group_models.GroupComment.USER_ROLE_EXAMINER
        )
    ...
    def test_can_add_comment_admin_assignment_deadline_hard_expired_exception_not_raised(self):
        ...
        test_feedbackset = baker.make("devilry_group.FeedbackSet", group__parentnode=test_assignment)
        test_feedbackset.can_add_comment(
            assignment=test_assignment, comment_user_role=group_models.GroupComment.USER_ROLE_ADMIN
        )
    ...
    def test_filter_public_comment_files_from_students_multiple_feedbacksets_with_and_without_comments(self):
        ...
        group_comment1 = baker.make(
            "devilry_group.GroupComment",
            feedback_set=test_feedbackset1,
            user_role=comment_models.Comment.USER_ROLE_STUDENT,
        )
        baker.make("devilry_comment.CommentFile", comment=group_comment1)
        group_comment2 = baker.make(
            "devilry_group.GroupComment",
            feedback_set=test_feedbackset1,
            user_role=comment_models.Comment.USER_ROLE_STUDENT,
        )
        ...
        group_comment5 = baker.make(
            "devilry_group.GroupComment",
            feedback_set=test_feedbackset3,
            user_role=comment_models.Comment.USER_ROLE_STUDENT,
        )
    ...
    def test_filter_public_comment_files_from_students_no_public_student_files(self):
        test_feedbackset = baker.make("devilry_group.FeedbackSet")
        group_comment_admin = baker.make(
            "devilry_group.GroupComment",
            feedback_set=test_feedbackset,
            user_role=comment_models.Comment.USER_ROLE_ADMIN,
        )
        group_comment_examiner = baker.make(
            "devilry_group.GroupComment",
            feedback_set=test_feedbackset,
            user_role=comment_models.Comment.USER_ROLE_EXAMINER,
        )
    ...
    def test_filter_public_comment_files_from_students_query_count(self):
        ...
        with self.assertNumQueries(1):
            self.assertIn(
                test_feedbackset, group_models.FeedbackSet.objects.filter_public_comment_files_from_students()
            )
