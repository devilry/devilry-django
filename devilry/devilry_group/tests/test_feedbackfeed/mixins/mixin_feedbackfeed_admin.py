...
class MixinTestFeedbackfeedAdmin(mixin_feedbackfeed_common.MixinTestFeedbackFeed):
    ...
    def test_get_feedbackfeed_event_delivery_passed(self):
        ...
        feedbackset = group_baker.feedbackset_first_attempt_published(group=testgroup, grading_points=7)
    ...
    def test_get_feedbackfeed_event_delivery_failed(self):
        ...
        testgroup = baker.make("core.AssignmentGroup", parentnode=assignment)
        feedbackset = group_baker.feedbackset_first_attempt_published(group=testgroup, grading_points=0)
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=feedbackset.group)
    ...
    def test_get_feedbackfeed_periodadmin(self):
        ...
        self.assertEqual(
            "periodadmin", PeriodPermissionGroup.objects.get_devilryrole_for_user_on_period(period=period, user=admin)
        )
    ...
    def test_get_feedbackfeed_periodadmin_raise_404_semi_anonymous(self):
        ...
        with self.assertRaisesMessage(http.Http404, ""):
            self.mock_getrequest(
                requestuser=testuser, cradmin_role=testgroup, cradmin_instance=mockrequest.cradmin_instance
            )
        self.assertEqual(1, group_models.FeedbackSet.objects.count())
    ...
    def test_get_feedbackfeed_subjectadmin_can_see_student_name_semi_anonymous(self):
        ...
        candidate = baker.make(
            "core.Candidate", assignment_group=testgroup, relatedstudent__user__shortname="teststudent"
        )
        baker.make(
            "devilry_group.GroupComment",
        ...
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=testgroup, cradmin_instance=mockrequest.cradmin_instance
        )
    ...
    def test_get_periodadmin_no_access(self):
        ...
        baker.make("devilry_account.PermissionGroupUser", user=admin, permissiongroup=permissiongroup.permissiongroup)
    ...
    def test_get_feedbackfeed_download_visible_public_commentfiles_exist(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=testgroup, cradmin_instance=mock_cradmininstance, requestuser=testuser
        )
        self.assertTrue(
    ...
    def test_get_no_edit_link_for_other_users_comments(self):
        ...
        baker.make("devilry_group.GroupComment", user_role="examiner", feedback_set=feedbackset)
        baker.make("devilry_group.GroupComment", user_role="student", feedback_set=feedbackset)
    ...
    def test_get_edit_link(self):
        ...
        testgroup = baker.make("core.AssignmentGroup", parentnode__parentnode=period)
        feedbackset = group_baker.feedbackset_first_attempt_unpublished(group=testgroup)
        baker.make("devilry_group.GroupComment", user=admin, user_role="admin", feedback_set=feedbackset)
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testgroup, requestuser=admin)
        self.assertTrue(mockresponse.selector.exists(".devilry-group-comment-edit-link__admin"))
        self.assertTrue("Edit", mockresponse.selector.one(".devilry-group-comment-edit-link__admin").alltext_normalized)
    ...
    def test_get_edit_link_url(self):
        ...
        testgroup = baker.make("core.AssignmentGroup", parentnode__parentnode=period)
        feedbackset = group_baker.feedbackset_first_attempt_unpublished(group=testgroup)
        groupcomment = baker.make("devilry_group.GroupComment", user=admin, user_role="admin", feedback_set=feedbackset)
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testgroup, requestuser=admin)
        self.assertTrue(mockresponse.selector.exists(".devilry-group-comment-edit-link__admin"))
        self.assertEqual(
            mockresponse.selector.one(".devilry-group-comment-edit-link__admin").get("href"),
            "/devilry_group/admin/{}/feedbackfeed/groupcomment-edit/{}".format(testgroup.id, groupcomment.id),
        )
    ...
    def test_get_num_queries(self):
        ...
        with self.assertNumQueries(18):
            self.mock_http200_getrequest_htmls(
                cradmin_role=testgroup, requestuser=admin, cradmin_instance=mock_cradmininstance
            )
        self.assertEqual(1, group_models.FeedbackSet.objects.count())
