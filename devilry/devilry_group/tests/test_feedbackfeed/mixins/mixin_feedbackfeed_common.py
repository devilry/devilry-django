...
class MixinTestFeedbackFeedGroupComment(cradmin_testhelpers.TestCaseMixin):
    ...
    def test_get_feedbackfeed_candidate_user_deleted(self):
        ...
        baker.make("devilry_group.GroupComment", user_role="student", user=None, feedback_set=testfeedbackset)
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testgroup)
        self.assertEqual(
            "USER DELETED", mockresponse.selector.one(".devilry-group-comment-user-deleted").alltext_normalized
        )
    ...
    def test_get_feedbackfeed_examiner_user_deleted(self):
        ...
        testgroup = baker.make("core.AssignmentGroup", parentnode=testassignment)
        testfeedbackset = group_baker.feedbackset_first_attempt_unpublished(group=testgroup)
        baker.make("devilry_group.GroupComment", user_role="examiner", user=None, feedback_set=testfeedbackset)
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testgroup)
        self.assertEqual(
            "USER DELETED", mockresponse.selector.one(".devilry-group-comment-user-deleted").alltext_normalized
        )
    ...
    def test_get_feedbackfeed_admin_user_deleted(self):
        ...
        testgroup = baker.make("core.AssignmentGroup", parentnode=testassignment)
        testfeedbackset = group_baker.feedbackset_first_attempt_unpublished(group=testgroup)
        baker.make("devilry_group.GroupComment", user_role="admin", user=None, feedback_set=testfeedbackset)
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testgroup)
        self.assertEqual(
            "USER DELETED", mockresponse.selector.one(".devilry-group-comment-user-deleted").alltext_normalized
        )
    ...
    def test_get_comment_student(self):
        ...
        candidate = baker.make(
            "core.Candidate", relatedstudent=baker.make("core.RelatedStudent"), assignment_group=group
        )
        baker.make(
        ...
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=group, cradmin_instance=_get_mock_cradmin_instance()
        )
    ...
    def test_get_comment_poster_fullname(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=comment.feedback_set.group, cradmin_instance=_get_mock_cradmin_instance()
        )
...
class MixinTestFeedbackFeed(MixinTestFeedbackFeedHeader, MixinTestFeedbackFeedGroupComment):
    ...
    def test_get(self):
        group = baker.make("core.AssignmentGroup")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=group)
        self.assertEqual(mockresponse.selector.one("title").alltext_normalized, group.assignment.get_path())
    ...
    def test_semester_expired_comment_form_not_rendered(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=test_feedbackset.group, requestuser=testuser)
    ...
    def test_get_event_with_feedbackset_deadline_datetime_expired(self):
        ...
        feedbackset = baker.make(
            "devilry_group.FeedbackSet", deadline_datetime=timezone.now() - timezone.timedelta(days=1)
        )
    ...
    def test_get_event_two_feedbacksets_deadlines_expired_assignment_firstdeadline(self):
        ...
        group_baker.feedbackset_new_attempt_unpublished(
            group=testgroup, deadline_datetime=timezone.now() - timezone.timedelta(days=2)
        )
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testgroup)
    ...
    def test_get_feedbackset_header_attempt(self):
        ...
        self.assertEqual(mockresponse.selector.one(".header-attempt-number").alltext_normalized, "Attempt 1")
    ...
    def test_get_feedbackset_header_grading_info_waiting_for_deliveries_for_feedback(self):
        ...
        self.assertEqual(mockresponse.selector.one(".header-grading-info").alltext_normalized, "waiting for deliveries")
    ...
    def test_get_feedbackset_header_two_attempts(self):
        ...
        self.assertEqual(mockresponse.selector.list(".header-attempt-number")[0].alltext_normalized, "Attempt 1")
        self.assertEqual(mockresponse.selector.list(".header-attempt-number")[1].alltext_normalized, "Attempt 2")
    ...
    def test_get_feedbackset_deadline_history_username_rendered(self):
        ...
        self.assertEqual(
            mockresponse.selector.one(
                ".devilry-group-feedbackfeed-event-message__user_display_name"
            ).alltext_normalized,
