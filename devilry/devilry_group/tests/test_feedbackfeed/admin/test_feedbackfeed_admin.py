...
class TestFeedbackfeedAdminDiscussPublicView(TestCase, mixin_feedbackfeed_admin.MixinTestFeedbackfeedAdmin):
    ...
    def test_post_feedbackset_comment_visible_to_everyone(self):
        ...
        self.mock_http302_postrequest(
            ...
            requestkwargs={"data": {"text": "This is a comment"}},
        )
    ...
    def test_upload_single_file_visibility_everyone(self):
        ...
        testuser = baker.make(settings.AUTH_USER_MODEL)
        temporary_filecollection = group_baker.temporary_file_collection_with_tempfile(user=testuser)
        self.mock_http302_postrequest(
            cradmin_role=testgroup,
...
class TestAdminEditGroupCommentView(TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def __make_user_admin(self, user, assignment, permissiongroup_type="period"):
        if permissiongroup_type == "period":
            permissiongroup = baker.make("devilry_account.PeriodPermissionGroup", period=assignment.period)
        else:
            permissiongroup = baker.make("devilry_account.SubjectPermissionGroup", period=assignment.period)
        baker.make("devilry_account.PermissionGroupUser", user=user, permissiongroup=permissiongroup.permissiongroup)
    ...
    def test_no_comment_raises_404(self):
        ...
        with self.assertRaises(Http404):
            self.mock_http200_getrequest_htmls(cradmin_role=testgroup, requestuser=testuser, viewkwargs={"pk": 1})
    ...
    def test_raise_404_admin_can_not_edit_other_admins_comments_sanity(self):
        ...
        with self.assertRaises(Http404):
            self.mock_http200_getrequest_htmls(
                cradmin_role=testgroup, requestuser=testuser, viewkwargs={"pk": groupcomment.id}
            )
    ...
    def test_post_initial_empty_comment_can_be_edited(self):
        ...
        self.mock_http302_postrequest(
            ...
            viewkwargs={"pk": groupcomment.id},
            messagesmock=messagesmock,
            requestkwargs={"data": {"text": "edited"}},
        )
        db_comment = group_models.GroupComment.objects.get(id=groupcomment.id)
