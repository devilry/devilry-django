...
class TestStudentCommentEditHistoryView(TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_comment_not_visible_to_everyone_raises_404(self):
        ...
        groupcomment = baker.make(
            "devilry_group.GroupComment", visibility=group_models.GroupComment.VISIBILITY_VISIBLE_TO_EXAMINER_AND_ADMINS
        )
    ...
    def test_students_can_not_see_other_users_comment_edit_history(self):
        ...
        baker.make(
            "devilry_group.GroupCommentEditHistory",
    ...
    def test_comment_no_history_no_history_items_rendered(self):
        ...
        self.assertEqual(
            mockresponse.selector.one(".devilry-comment-history-no-items").alltext_normalized,
    ...
    def test_history_entry_content(self):
        ...
        self.assertEqual(
            mockresponse.selector.one(".devilry-comment-history-item__title").alltext_normalized,
            "{}".format(defaultfilters.date(timezone.localtime(history_entry.edited_datetime), "DATETIME_FORMAT")),
        )
        self.assertEqual(
            mockresponse.selector.one(".devilry-comment-history-item__description").alltext_normalized, "Pre edit text"
        )
    ...
    def test_history_entries_render_order(self):
        ...
        self.assertEqual(
            mockresponse.selector.list(".devilry-comment-history-item__title")[0].alltext_normalized,
            "{}".format(defaultfilters.date(timezone.localtime(history_entry3.edited_datetime), "DATETIME_FORMAT")),
        )
        self.assertEqual(
            mockresponse.selector.list(".devilry-comment-history-item__title")[1].alltext_normalized,
            "{}".format(defaultfilters.date(timezone.localtime(history_entry1.edited_datetime), "DATETIME_FORMAT")),
        )
        self.assertEqual(
            mockresponse.selector.list(".devilry-comment-history-item__title")[2].alltext_normalized,
            "{}".format(defaultfilters.date(timezone.localtime(history_entry2.edited_datetime), "DATETIME_FORMAT")),
        )
