...
class MixinTestFeedbackfeedExaminerDiscuss(mixin_feedbackfeed_examiner.MixinTestFeedbackfeedExaminer):
    ...
    def test_get_examiner_new_attempt_feedback_tab_does_not_exist_if_last_feedbackset_is_published(self):
        ...
        group_baker.feedbackset_new_attempt_published(
            group=testgroup, deadline_datetime=timezone.now() + timedelta(days=3)
        )
        examiner = baker.make("core.Examiner", assignmentgroup=testgroup)
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=testgroup, requestuser=examiner.relatedexaminer.user
    ...
    def test_post_comment_always_to_last_feedbackset(self):
        ...
        examiner = baker.make(
            "core.Examiner", assignmentgroup=group, relatedexaminer=baker.make("core.RelatedExaminer")
        )
    ...
    def test_get_feedbackset_published_move_deadline_button_not_rendered(self):
        ...
        self.assertEqual(
            mockresponse.selector.one(".devilry-group-event__grade-last-edit-button").alltext_normalized, "Edit grade"
        )
        self.assertEqual(
    ...
    def test_get_feedbackset_grading_updated_multiple_events_rendered(self):
        ...
        self.assertIn(
            "The grade was changed from passed (1/1) to failed (0/1) by Test User(test@example.com)", event_text_list[0]
        )
        self.assertIn(
            "The grade was changed from failed (0/1) to passed (1/1) by Test User(test@example.com)", event_text_list[1]
        )
        self.assertIn(
            "The grade was changed from passed (1/1) to failed (0/1) by Test User(test@example.com)", event_text_list[2]
        )
        self.assertIn(
            "The grade was changed from failed (0/1) to passed (1/1) by Test User(test@example.com)", event_text_list[3]
        )
...
class TestFeedbackfeedExaminerPublicDiscuss(TestCase, MixinTestFeedbackfeedExaminerDiscuss):
    ...
    def test_get_examiner_add_comment_button(self):
        testgroup = baker.make(
            "core.AssignmentGroup", parentnode__parentnode=baker.make_recipe("devilry.apps.core.period_active")
        )
        examiner = baker.make("core.Examiner", assignmentgroup=testgroup)
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=testgroup, requestuser=examiner.relatedexaminer.user
        ...
        self.assertEqual(
            "Add comment", mockresponse.selector.one("#submit-id-examiner_add_public_comment").alltext_normalized
        )
    ...
    def test_post_comment_mail_sent_to_everyone_in_group_sanity(self):
        ...
        group_baker.feedbackset_first_attempt_unpublished(group=testgroup)
        examiner = baker.make("core.Examiner", assignmentgroup=testgroup)
        examiner_email = baker.make(
            "devilry_account.UserEmail", user=examiner.relatedexaminer.user, email="examiner@example.com"
        )
    ...
    def test_post_first_attempt_unpublished_comment_with_text(self):
        ...
        self.assertEqual(group_models.GroupComment.VISIBILITY_VISIBLE_TO_EVERYONE, posted_comment.visibility)
...
class TestFeedbackfeedExaminerWithAdminDiscuss(TestCase, MixinTestFeedbackfeedExaminerDiscuss):
    ...
    def test_post_first_attempt_unpublished_comment_with_text(self):
        ...
        self.assertEqual(1, group_models.GroupComment.objects.count())
        posted_comment = group_models.GroupComment.objects.all()[0]
        self.assertEqual(group_models.GroupComment.VISIBILITY_VISIBLE_TO_EXAMINER_AND_ADMINS, posted_comment.visibility)
        self.assertEqual("This is a comment", posted_comment.text)
...
class TestFeedbackfeedPublicDiscussFileUploadExaminer(
    TestCase, mixin_feedbackfeed_examiner.MixinTestFeedbackfeedExaminer
):
    viewclass = feedbackfeed_examiner.ExaminerPublicDiscussView
    ...
    def test_comment_without_text_or_file_visibility_everyone(self):
        ...
        mockresponse = self.mock_http200_postrequest_htmls(
            ...
            requestkwargs={"data": {"text": "", "examiner_add_public_comment": "unused value"}},
        )
    ...
    def test_upload_single_file_visibility_everyone(self):
        ...
        self.mock_http302_postrequest(
            ...
            requestuser=testexaminer.relatedexaminer.user,
            viewkwargs={"pk": testfeedbackset.group.id},
            requestkwargs={"data": {"text": "", "temporary_file_collection_id": temporary_filecollection.id}},
        )
        self.assertEqual(1, group_models.GroupComment.objects.count())
    ...
    def test_upload_files_and_comment_text(self):
        ...
        self.mock_http302_postrequest(
            ...
            viewkwargs={"pk": testfeedbackset.group.id},
            requestkwargs={
                "data": {"text": "Test comment", "temporary_file_collection_id": temporary_filecollection.id}
            },
        )
...
class TestFeedbackfeedExaminerWithAdminDiscussFileUpload(
    TestCase, mixin_feedbackfeed_examiner.MixinTestFeedbackfeedExaminer
):
    viewclass = feedbackfeed_examiner.ExaminerWithAdminsDiscussView
