...
class TestFeedbackfeedExaminerFeedback(TestCase, mixin_feedbackfeed_examiner.MixinTestFeedbackfeedExaminer):
    ...
    def test_no_404_on_last_feedbackset_unpublished(self):
        ...
        examiner = baker.make(
            "core.Examiner", assignmentgroup=testgroup, relatedexaminer=baker.make("core.RelatedExaminer")
        )
    ...
    def test_get_feedbackfeed_examiner_wysiwyg_get_comment_choice_add_comment_for_examiners_and_admins_button(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=feedbackset.group, requestuser=examiner.relatedexaminer.user
        )
    ...
    def test_get_num_queries(self):
        ...
        with self.assertNumQueries(18):
            self.mock_http200_getrequest_htmls(cradmin_role=testgroup, requestuser=examiner.relatedexaminer.user)
...
class TestFeedbackFeedExaminerPublishFeedback(TestCase, mixin_feedbackfeed_examiner.MixinTestFeedbackfeedExaminer):
    ...
    def test_post_first_attempt_draft_appear_before_grading_event(self):
        ...
        class_list = [
            list(item.cssclasses_set) for item in mockresponse.selector.list(".devilry-group-feedbackfeed-itemvalue")
        ]
    ...
    def test_post_publish_feedbackset_after_deadline(self):
        ...
        feedbackset = group_baker.feedbackset_first_attempt_unpublished(group=testgroup)
        examiner = baker.make(
            "core.Examiner", assignmentgroup=feedbackset.group, relatedexaminer=baker.make("core.RelatedExaminer")
        )
        self.mock_http302_postrequest(
            cradmin_role=examiner.assignmentgroup,
            requestuser=examiner.relatedexaminer.user,
    ...
    def test_post_publish_feedbackset_drafts_on_last_feedbackset_only(self):
        ...
        baker.make("core.Candidate", assignment_group=testgroup, relatedstudent=baker.make("core.RelatedStudent"))
        comment1 = baker.make(
            "devilry_group.GroupComment",
            text="test text 1",
            user=examiner.relatedexaminer.user,
        ...
        cached_group = cache_models.AssignmentGroupCachedData.objects.get(group=testgroup)
        feedback_comments = (
            group_models.GroupComment.objects.all()
    ...
    def test_examiner_publishes_email_is_sent(self):
        ...
        feedbackset = group_baker.feedbackset_first_attempt_unpublished(
            group=testgroup, deadline_datetime=timezone.now() + timezone.timedelta(days=30)
        )
        examiner_user = baker.make(
            settings.AUTH_USER_MODEL, fullname="God of thunder and Battle", shortname="thor@example.com"
        )
        examiner = baker.make(
            "core.Examiner",
