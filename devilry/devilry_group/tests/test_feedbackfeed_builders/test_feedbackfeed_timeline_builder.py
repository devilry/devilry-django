...
class TimelineBuilderTestMixin:
    ...
    def __build_timeline(self, group, user, assignment):
        ...
        timeline_builder = FeedbackFeedTimelineBuilder(
            assignment=assignment, feedbacksets=feedbackset_queryset, group=group
        )
    ...
    def test_one_feedbackset_unpublished_event(self):
        ...
        timeline_builder = FeedbackFeedTimelineBuilder(
            assignment=testassignment, feedbacksets=feedbackset_queryset, group=testgroup
        )
        timeline_builder.build()
        timeline_list = timeline_builder.get_as_list()
    ...
    def test_feedbackset_published_grading_points_same_as_first_updated_grading_points(self):
        ...
        testgroup = baker.make("core.AssignmentGroup", parentnode=testassignment)
        testfeedbackset = group_baker.feedbackset_first_attempt_published(group=testgroup)
        first_grading_points_update = baker.make(
            "devilry_group.FeedbackSetGradingUpdateHistory",
            old_grading_points=testfeedbackset.grading_points,
            feedback_set=testfeedbackset,
        )
        last_grading_points_update = baker.make(
            "devilry_group.FeedbackSetGradingUpdateHistory", old_grading_points=0, feedback_set=testfeedbackset
        )
        feedbackset_queryset = builder_base.get_feedbackfeed_builder_queryset(
            group=testgroup, requestuser=testuser, devilryrole=self.devilryrole
        )
        ...
        self.assertEqual(timeline_list[0]["feedbackset_events"][0]["type"], "grade")
        self.assertEqual(
            timeline_list[0]["feedbackset_events"][0]["grade_points"], first_grading_points_update.old_grading_points
        )
        self.assertNotEqual(
            timeline_list[0]["feedbackset_events"][0]["grade_points"], last_grading_points_update.old_grading_points
        )
    ...
    def test_updated_grading_points_event_one_update(self):
        ...
        self.assertEqual(
            timeline_list[0]["feedbackset_events"][1]["obj"].old_grading_points, testfeedbackset.grading_points
        )
    ...
    def test_updated_grading_points_event_multiple_updates(self):
        ...
        baker.make("devilry_group.FeedbackSetGradingUpdateHistory", old_grading_points=0, feedback_set=testfeedbackset)
        baker.make("devilry_group.FeedbackSetGradingUpdateHistory", old_grading_points=1, feedback_set=testfeedbackset)
        timeline_list = self.__build_timeline(group=testgroup, user=testuser, assignment=testassignment)
