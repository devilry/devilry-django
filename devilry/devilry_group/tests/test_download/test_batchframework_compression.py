...
class TestFeedbackSetBatchTask(TestCompressed):
    def __make_comment_file(self, feedback_set, file_name, file_content, user_role="student", **comment_kwargs):
        comment = baker.make(
            "devilry_group.GroupComment", feedback_set=feedback_set, user_role=user_role, **comment_kwargs
        )
    ...
    def test_batchframework_feedbackset_student_after_deadline(self):
        ...
        baker.make("core.Candidate", assignment_group=testfeedbackset.group, relatedstudent__user=studentuser)
        self.__make_comment_file(
            feedback_set=testfeedbackset, file_name="testfile.txt", file_content="testcontent", user=studentuser
        )
        ...
        self.assertEqual(
            zipfileobject.extractfile("after_deadline_not_part_of_delivery/testfile.txt").read(), b"testcontent"
        )
    ...
    def test_batchframework_examiner_files_not_uploaded(self):
        ...
        testfeedbackset = group_baker.feedbackset_first_attempt_unpublished()
        self.__make_comment_file(
            feedback_set=testfeedbackset, file_name="testfile.txt", file_content="testcontent", user_role="examiner"
        )
    ...
    def test_batchframework_files_from_examiner_and_student(self):
        ...
        self.__make_comment_file(
            feedback_set=testfeedbackset,
            file_name="testfile_examiner.txt",
            file_content="examiner testcontent",
            user_role="examiner",
        )
        self.__make_comment_file(
            feedback_set=testfeedbackset,
            file_name="testfile_student.txt",
            file_content="student testcontent",
            user_role="student",
        )
