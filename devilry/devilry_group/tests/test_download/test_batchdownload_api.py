...
class TestHelper(object):
    ...
    def _register_and_run_actiongroup(self, actiongroup_name, task, context_object):
        ...
        batchregistry.Registry.get_instance().run(
            actiongroup_name=actiongroup_name, context_object=context_object, test="test"
        )
...
class TestFeedbackSetBatchDownloadApi(test.TestCase, TestHelper, TestCaseMixin):
    ...
    def test_get_status_no_files(self):
        ...
        mockresponse = self.mock_getrequest(viewkwargs={"content_object_id": testfeedbackset.id})
    ...
    def test_get_status_not_started_unprocessed(self):
        ...
        mockresponse = self.mock_getrequest(
            cradmin_app=self.__mock_cradmin_app(), viewkwargs={"content_object_id": testfeedbackset.id}
        )
    ...
    def test_get_status_not_created_when_new_file_is_added(self):
        ...
        testgroup = baker.make("core.AssignmentGroup")
        testfeedbackset = devilry_group_baker_factories.feedbackset_first_attempt_unpublished(group=testgroup)
        commentuser = baker.make(settings.AUTH_USER_MODEL)
        testcomment = baker.make(
            "devilry_group.GroupComment", feedback_set=testfeedbackset, user_role="student", user=commentuser
        )
        commentfile = baker.make("devilry_comment.CommentFile", comment=testcomment, filename="testfile.txt")
        commentfile.file.save("testfile.txt", ContentFile("testcontent"))
        self._register_and_run_actiongroup(
        ...
        testcomment_added_after = baker.make(
            "devilry_group.GroupComment", feedback_set=testfeedbackset, user_role="student", user=commentuser
        )
        commentfile_added_after = baker.make(
            "devilry_comment.CommentFile", comment=testcomment_added_after, filename="testfile.txt"
        )
    ...
    def test_get_status_finished_with_compressed_archive_meta(self):
        ...
        commentfile = baker.make("devilry_comment.CommentFile", comment=testcomment, filename="testfile.txt")
        commentfile.file.save("testfile.txt", ContentFile("testcontent"))
        baker.make("devilry_compressionutil.CompressedArchiveMeta", content_object=testfeedbackset)
        mock_cradmin_app = mock.MagicMock()
        mock_cradmin_app.reverse_appurl.return_value = "url-to-downloadview"
        mockresponse = self.mock_getrequest(
            cradmin_app=mock_cradmin_app, viewkwargs={"content_object_id": testfeedbackset.id}
        )
    ...
    def test_post_marks_archive_as_deleted_if_new_files_are_added(self):
        ...
        compressed_archive_meta = baker.make(
            "devilry_compressionutil.CompressedArchiveMeta", content_object=testfeedbackset
        )
        testcomment = baker.make(
            "devilry_group.GroupComment",
            feedback_set=testfeedbackset,
