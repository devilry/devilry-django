...
class AdminPublicDiscussView(AdminBaseFeedbackFeedView):
    ...
    def get_buttons(self):
        ...
        buttons.extend(
            [PrimarySubmit("admin_add_public_comment", gettext_lazy("Add comment"), css_class="btn btn-default")]
        )
        return buttons
...
class AdminWithExaminersDiscussView(AdminBaseFeedbackFeedView):
    ...
    def get_buttons(self):
        ...
        buttons.extend([PrimarySubmit("admin_add_comment_for_examiners_and_admins", gettext_lazy("Add note"))])
        return buttons
...
class App(crapp.App):
    appurls = [
        crapp.Url(r"^$", ensure_csrf_cookie(AdminPublicDiscussView.as_view()), name=crapp.INDEXVIEW_NAME),
        crapp.Url(
