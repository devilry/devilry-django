...
class FeedbackSetAdmin(admin.ModelAdmin):
    ...
    raw_id_fields = ["group", "created_by", "last_updated_by", "grading_published_by"]
    readonly_fields = ["feedbackset_type", "gradeform_data_json"]
    search_fields = [
        "=group__id",
...
class GroupCommentAdmin(admin.ModelAdmin):
    list_display = ["id", "user", "part_of_grading"]

    raw_id_fields = ["feedback_set"]
...
class FeedbackSetPassedPreviousPeriodAdmin(admin.ModelAdmin):
    ...
    search_fields = ["feedbackset_id", "created_by_id", "passed_previous_period_type"]
    list_display = ["feedbackset"]
...
class FeedbackSetGradingUpdateHistoryAdmin(admin.ModelAdmin):
    ...
    search_fields = ["feedbackset_id", "updated_by_id", "old_grading_published_by_id"]
    list_display = ["feedback_set", "updated_by"]
...
class FeedbackSetDeadlineHistoryAdmin(admin.ModelAdmin):
    readonly_fields = ["feedback_set", "changed_by", "changed_datetime", "deadline_old", "deadline_new"]
    search_fields = ["feedback_set_id", "changed_by_id"]
    list_display = ["feedback_set", "changed_datetime"]
...
class GroupCommentEditHistoryAdmin(admin.ModelAdmin):
    list_display = ["visibility", "edited_datetime", "edited_by"]
