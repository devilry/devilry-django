...
class DataPortenUserUpdater(AbstractUserUpdater):
    ...
    def make_shortname(cls, socialaccount):
        ...
        if userid_sec.startswith("feide:"):
            ...
            feide_userid_sec_to_username_suffix = getattr(settings, "DEVILRY_FEIDE_USERID_SEC_TO_USERNAME_SUFFIX", None)

            # Debug log
            logger.debug("DEVILRY_FEIDE_USERID_SEC_TO_USERNAME_SUFFIX: {}".format(feide_userid_sec_to_username_suffix))

            if feide_userid_sec_to_username_suffix and userid_sec.endswith(feide_userid_sec_to_username_suffix):
                # Debug log
                logger.debug(
                    "feide_userid_sec_to_username_suffix is not None, and userid_sec ends with {}".format(
                ...
                # Debug log
                logger.debug(
                    "feide_userid_sec_to_username_suffix is None, or userid_sec does not end with {}".format(
                        feide_userid_sec_to_username_suffix
                    )
                )
                logger.debug("...")
...
socialaccount_updaters = {DevilryDataportenProvider.id: DataPortenUserUpdater}
