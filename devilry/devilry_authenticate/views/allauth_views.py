...
class AllauthLoginView(LoginView):
    ...
    def get(self, *args, **kwargs):
        ...
        if len(settings.AUTHENTICATION_BACKENDS) == 1:
            if allauth_backend in settings.AUTHENTICATION_BACKENDS and len(all_providers) == 1:
                provider = all_providers[0]
                return HttpResponseRedirect(provider.get_login_url(request=self.request, process="login"))
            else:
                return HttpResponseRedirect(self._get_cradmin_auth_url())
