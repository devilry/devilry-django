...
class TestPeriodOverviewView(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_title(self):
        ...
        testperiod = baker.make_recipe(
            "devilry.apps.core.period_active", parentnode__long_name="Test Subject", long_name="Test Period"
        )
        ...
        self.assertIn("Test Subject - Test Period", mockresponse.selector.one("title").alltext_normalized)
    ...
    def test_h1(self):
        ...
        baker.make("core.Candidate", relatedstudent__user=testuser)
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=testuser, cradmin_role=testperiod)
        self.assertEqual("Test Subject - Test Period", mockresponse.selector.one("h1").alltext_normalized)
    ...
    def test_grouplist_not_future_periods(self):
        ...
        baker.make(
            ...
            assignment_group__parentnode=baker.make_recipe(
                "devilry.apps.core.assignment_futureperiod_start", parentnode=testperiod
            ),
        )
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=testuser, cradmin_role=testperiod)
        self.assertEqual(0, self.__get_assignment_count(selector=mockresponse.selector))
    ...
    def test_grouplist_include_old_periods(self):
        ...
        baker.make(
            ...
            relatedstudent__user=testuser,
            assignment_group__parentnode=baker.make_recipe(
                "devilry.apps.core.assignment_oldperiod_end", parentnode=testperiod
            ),
        )
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=testuser, cradmin_role=testperiod)
    ...
    def test_grouplist_include_active_periods(self):
        ...
        baker.make(
            ...
            relatedstudent__user=testuser,
            assignment_group__parentnode=baker.make_recipe(
                "devilry.apps.core.assignment_activeperiod_start", parentnode=testperiod
            ),
        )
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=testuser, cradmin_role=testperiod)
        self.assertEqual(1, self.__get_assignment_count(selector=mockresponse.selector))
    ...
    def test_no_active_assignments_message(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=testuser, cradmin_role=testperiod)
        self.assertEqual(
            "No assignments.", mockresponse.selector.one(".cradmin-legacy-listing-no-items-message").alltext_normalized
        )

    def test_assignment_url(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        testperiod = baker.make_recipe("devilry.apps.core.period_active")
        testassignment = baker.make_recipe("devilry.apps.core.assignment_activeperiod_start", parentnode=testperiod)
    ...
    def test_grouplist_search_nomatch(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(
            viewkwargs={"filters_string": "search-nomatch"}, requestuser=testuser, cradmin_role=testperiod
        )
        self.assertEqual(0, self.__get_assignment_count(selector=mockresponse.selector))

    def test_grouplist_search_match_subject_short_name(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        testperiod = baker.make_recipe("devilry.apps.core.period_active", parentnode__short_name="testsubject")
        baker.make(
            "core.Candidate",
            relatedstudent__user=testuser,
            assignment_group__parentnode=baker.make_recipe(
        ...
        mockresponse = self.mock_http200_getrequest_htmls(
            viewkwargs={"filters_string": "search-testsubject"}, requestuser=testuser, cradmin_role=testperiod
        )
        self.assertEqual(1, self.__get_assignment_count(selector=mockresponse.selector))

    def test_grouplist_search_match_subject_long_name(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        testperiod = baker.make_recipe("devilry.apps.core.period_active", parentnode__long_name="Testsubject")
        baker.make(
            "core.Candidate",
            relatedstudent__user=testuser,
            assignment_group__parentnode=baker.make_recipe(
    ...
    def test_grouplist_search_match_period_short_name(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        testperiod = baker.make_recipe("devilry.apps.core.period_active", short_name="testperiod")
        baker.make(
            "core.Candidate",
            relatedstudent__user=testuser,
            assignment_group__parentnode=baker.make_recipe(
        ...
        mockresponse = self.mock_http200_getrequest_htmls(
            viewkwargs={"filters_string": "search-testperiod"}, requestuser=testuser, cradmin_role=testperiod
        )
        self.assertEqual(1, self.__get_assignment_count(selector=mockresponse.selector))

    def test_grouplist_search_match_period_long_name(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        testperiod = baker.make_recipe("devilry.apps.core.period_active", long_name="Testperiod")
        baker.make(
            "core.Candidate",
            relatedstudent__user=testuser,
            assignment_group__parentnode=baker.make_recipe(
    ...
    def test_grouplist_search_match_assignment_short_name(self):
        ...
        baker.make(
            ...
            relatedstudent__user=testuser,
            assignment_group__parentnode=baker.make_recipe(
                "devilry.apps.core.assignment_activeperiod_start", short_name="testassignment", parentnode=testperiod
            ),
        )
        mockresponse = self.mock_http200_getrequest_htmls(
            viewkwargs={"filters_string": "search-testassignment"}, requestuser=testuser, cradmin_role=testperiod
        )
        self.assertEqual(1, self.__get_assignment_count(selector=mockresponse.selector))
    ...
    def test_grouplist_search_match_assignment_long_name(self):
        ...
        baker.make(
            ...
            relatedstudent__user=testuser,
            assignment_group__parentnode=baker.make_recipe(
                "devilry.apps.core.assignment_activeperiod_start", long_name="Testassignment", parentnode=testperiod
            ),
        )
        mockresponse = self.mock_http200_getrequest_htmls(
            viewkwargs={"filters_string": "search-Testassignment"}, requestuser=testuser, cradmin_role=testperiod
    ...
    def test_grouplist_title_sanity(self):
        ...
        testgroup = baker.make(
            ...
            parentnode=baker.make_recipe(
                "devilry.apps.core.assignment_activeperiod_start", parentnode=testperiod, long_name="Test Assignment"
            ),
        )
    ...
    def test_grouplist_no_examiners(self):
        ...
        testgroup = baker.make(
            "core.AssignmentGroup",
            parentnode=baker.make_recipe("devilry.apps.core.assignment_activeperiod_start", parentnode=testperiod),
        )
        baker.make("core.Candidate", relatedstudent__user=testuser, assignment_group=testgroup)
    ...
    def test_querycount(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        testperiod = baker.make_recipe("devilry.apps.core.period_active")
        testassignment1 = baker.make_recipe("devilry.apps.core.assignment_activeperiod_start", parentnode=testperiod)
        testassignment2 = baker.make_recipe("devilry.apps.core.assignment_activeperiod_start", parentnode=testperiod)
