...
import htmls
from cradmin_legacy import cradmin_testhelpers, crapp
...
class TestDashboardView(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_title(self):
        ...
        self.assertIn("Student dashboard", mockresponse.selector.one("title").alltext_normalized)

    def test_h1(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        baker.make("core.Candidate", relatedstudent__user=testuser)
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=testuser)
        self.assertEqual("Student dashboard", mockresponse.selector.one("h1").alltext_normalized)
    ...
    def test_one_upcoming_assignment(self):
        ...
        testperiod = baker.make_recipe(
            "devilry.apps.core.period_active", short_name="testperiod", parentnode=testsubject
        )
        testassignment1 = baker.make("core.Assignment", parentnode=testperiod, long_name="Test Assignment 1")
        testassignment2 = baker.make("core.Assignment", parentnode=testperiod, long_name="Test Assignment 2")
        self.__make_candidate_and_group_for_assignment(
    ...
    def test_one_upcoming_assignments_accross_periods(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        testsubject = baker.make("core.Subject", short_name="testsubject")
        testperiod1 = baker.make_recipe(
            "devilry.apps.core.period_active", short_name="testperiod1", parentnode=testsubject
        )
        testperiod2 = baker.make_recipe(
            "devilry.apps.core.period_active", short_name="testperiod2", parentnode=testsubject
        )
        testassignment1 = baker.make("core.Assignment", parentnode=testperiod1, long_name="Test Assignment 1")
        testassignment2 = baker.make("core.Assignment", parentnode=testperiod2, long_name="Test Assignment 2")
        self.__make_candidate_and_group_for_assignment(
            assignment=testassignment1, user=testuser, deadline_datetime=timezone.now() + timezone.timedelta(days=1)
        )
        self.__make_candidate_and_group_for_assignment(
    ...
    def test_one_upcoming_assignments_accross_subject(self):
        ...
        testperiod1 = baker.make_recipe(
            "devilry.apps.core.period_active", short_name="testperiod1", parentnode=testsubject1
        )
        testperiod2 = baker.make_recipe(
            "devilry.apps.core.period_active", short_name="testperiod2", parentnode=testsubject2
        )
        testassignment1 = baker.make("core.Assignment", parentnode=testperiod1, long_name="Test Assignment 1")
        testassignment2 = baker.make("core.Assignment", parentnode=testperiod2, long_name="Test Assignment 2")
        self.__make_candidate_and_group_for_assignment(
    ...
    def test_multiple_upcoming_assignment(self):
        ...
        testassignment1 = baker.make("core.Assignment", parentnode=testperiod, long_name="Test Assignment 1")
        testassignment2 = baker.make("core.Assignment", parentnode=testperiod, long_name="Test Assignment 2")
        testassignment3 = baker.make("core.Assignment", parentnode=testperiod, long_name="Test Assignment 3")
        self.__make_candidate_and_group_for_assignment(
            assignment=testassignment1, user=testuser, deadline_datetime=timezone.now() + timezone.timedelta(days=1)
        )
        self.__make_candidate_and_group_for_assignment(
    ...
    def test_user_can_not_see_upcoming_assignments_for_other_students_on_same_assignment(self):
        ...
        testassignment = baker.make("core.Assignment", parentnode=testperiod, long_name="Test Assignment 1")
        self.__make_candidate_and_group_for_assignment(
    ...
    def test_not_assignments_where_not_student(self):
        ...
        baker.make_recipe("devilry.apps.core.assignment_activeperiod_start")
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=testuser)
        self.assertEqual(0, self.__get_assignment_count(selector=mockresponse.selector))
    ...
    def test_grouplist_include_active_periods(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=testuser)
        self.assertEqual(1, self.__get_assignment_count(selector=mockresponse.selector))
    ...
    def test_assignment_url(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        testassignment = baker.make_recipe("devilry.apps.core.assignment_activeperiod_start")
        testcandidate = baker.make(
            "core.Candidate", relatedstudent__user=testuser, assignment_group__parentnode=testassignment
        )
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=testuser)
        self.assertEqual(
            reverse_cradmin_url(
    ...
    def test_grouplist_search_nomatch(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(
            viewkwargs={"filters_string": "search-nomatch"}, requestuser=testuser
        )
        self.assertEqual(0, self.__get_assignment_count(selector=mockresponse.selector))
    ...
    def test_grouplist_search_match_subject_short_name(self):
        ...
        baker.make(
            ...
            assignment_group__parentnode=baker.make_recipe(
                "devilry.apps.core.assignment_activeperiod_start", parentnode__parentnode__short_name="testsubject"
            ),
        )
        mockresponse = self.mock_http200_getrequest_htmls(
            viewkwargs={"filters_string": "search-testsubject"}, requestuser=testuser
        )
        self.assertEqual(1, self.__get_assignment_count(selector=mockresponse.selector))
    ...
    def test_grouplist_search_match_subject_long_name(self):
        ...
        baker.make(
            ...
            relatedstudent__user=testuser,
            assignment_group__parentnode=baker.make_recipe(
                "devilry.apps.core.assignment_activeperiod_start", parentnode__parentnode__long_name="Testsubject"
            ),
        )
        mockresponse = self.mock_http200_getrequest_htmls(
            viewkwargs={"filters_string": "search-Testsubject"}, requestuser=testuser
    ...
    def test_grouplist_search_match_period_short_name(self):
        ...
        baker.make(
            ...
            relatedstudent__user=testuser,
            assignment_group__parentnode=baker.make_recipe(
                "devilry.apps.core.assignment_activeperiod_start", parentnode__short_name="testperiod"
            ),
        )
        mockresponse = self.mock_http200_getrequest_htmls(
            viewkwargs={"filters_string": "search-testperiod"}, requestuser=testuser
        )
        self.assertEqual(1, self.__get_assignment_count(selector=mockresponse.selector))
    ...
    def test_grouplist_search_match_period_long_name(self):
        ...
        baker.make(
            ...
            relatedstudent__user=testuser,
            assignment_group__parentnode=baker.make_recipe(
                "devilry.apps.core.assignment_activeperiod_start", parentnode__long_name="Testperiod"
            ),
        )
        mockresponse = self.mock_http200_getrequest_htmls(
            viewkwargs={"filters_string": "search-Testperiod"}, requestuser=testuser
    ...
    def test_grouplist_search_match_assignment_short_name(self):
        ...
        baker.make(
            ...
            relatedstudent__user=testuser,
            assignment_group__parentnode=baker.make_recipe(
                "devilry.apps.core.assignment_activeperiod_start", short_name="testassignment"
            ),
        )
        mockresponse = self.mock_http200_getrequest_htmls(
            viewkwargs={"filters_string": "search-testassignment"}, requestuser=testuser
        )
        self.assertEqual(1, self.__get_assignment_count(selector=mockresponse.selector))
    ...
    def test_grouplist_search_match_assignment_long_name(self):
        ...
        baker.make(
            ...
            relatedstudent__user=testuser,
            assignment_group__parentnode=baker.make_recipe(
                "devilry.apps.core.assignment_activeperiod_start", long_name="Testassignment"
            ),
        )
        mockresponse = self.mock_http200_getrequest_htmls(
            viewkwargs={"filters_string": "search-Testassignment"}, requestuser=testuser
    ...
    def test_grouplist_orderby(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        testperiod1 = baker.make_recipe(
            "devilry.apps.core.period_active", parentnode__short_name="testsubject1", short_name="testperiod"
        )
        testperiod2 = baker.make_recipe(
            "devilry.apps.core.period_active", parentnode__short_name="testsubject2", short_name="testperiod"
        )
        baker.make(
            "core.Candidate",
            relatedstudent__user=testuser,
    ...
    def test_grouplist_no_examiners(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=testuser)
        self.assertTrue(mockresponse.selector.exists(".devilry-cradmin-groupitemvalue"))
    ...
    def test_grouplist_status_waiting_for_deliveries_sanity(self):
        ...
        baker.make("core.Candidate", relatedstudent__user=testuser, assignment_group=testgroup)
        devilry_group_baker_factories.feedbackset_first_attempt_published(group=testgroup, grading_points=3)
    ...
    def test_grouplist_comments_sanity(self):
        ...
        baker.make("core.Candidate", relatedstudent__user=testuser, assignment_group=testgroup)
        feedbackset = devilry_group_baker_factories.feedbackset_first_attempt_unpublished(group=testgroup)
        baker.make(
        ...
        baker.make(
            "devilry_group.GroupComment",  # Should not be part of count
            feedback_set=feedbackset,
    ...
    def test_allperiods_link_label(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=testuser)
        self.assertEqual(
            "Your courses", mockresponse.selector.one("#devilry_student_dashboard_allperiods_link").alltext_normalized
        )
    ...
    def test_querycount(self):
        ...
        for number in range(int(round(loops))):
            ...
            devilry_group_baker_factories.feedbackset_first_attempt_published(group=group1, grading_points=1)
    ...
    def test_querycount_points_to_grade_mapper_custom_table(self):
        ...
        point_to_grade_map1 = baker.make("core.PointToGradeMap", assignment=testassignment1, invalid=False)
        baker.make("core.PointRangeToGrade", point_to_grade_map=point_to_grade_map1, minimum_points=0, maximum_points=1)
        baker.make("core.PointRangeToGrade", point_to_grade_map=point_to_grade_map1, minimum_points=2, maximum_points=3)
        testassignment2 = baker.make_recipe(
            "devilry.apps.core.assignment_activeperiod_start",
            points_to_grade_mapper=Assignment.POINTS_TO_GRADE_MAPPER_CUSTOM_TABLE,
        )
        point_to_grade_map2 = baker.make("core.PointToGradeMap", assignment=testassignment2, invalid=False)
        baker.make("core.PointRangeToGrade", point_to_grade_map=point_to_grade_map2, minimum_points=0, maximum_points=1)
