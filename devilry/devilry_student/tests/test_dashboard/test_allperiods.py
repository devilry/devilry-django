...
class TestAllPeriodsView(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_title(self):
        ...
        self.assertIn("Your courses", mockresponse.selector.one("title").alltext_normalized)

    def test_h1(self):
        requestuser = baker.make(settings.AUTH_USER_MODEL)
        baker.make("core.Candidate", relatedstudent__user=requestuser)
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=requestuser)
        self.assertEqual("Your courses", mockresponse.selector.one("h1").alltext_normalized)
    ...
    def test_not_periods_where_not_student(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=requestuser)
        self.assertEqual(0, self.__get_period_count(selector=mockresponse.selector))

    def test_not_future_periods(self):
        requestuser = baker.make(settings.AUTH_USER_MODEL)
        baker.make("core.RelatedStudent", user=requestuser, period=baker.make_recipe("devilry.apps.core.period_future"))
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=requestuser)
        self.assertEqual(0, self.__get_period_count(selector=mockresponse.selector))

    def test_include_old_periods(self):
        requestuser = baker.make(settings.AUTH_USER_MODEL)
        baker.make("core.RelatedStudent", user=requestuser, period=baker.make_recipe("devilry.apps.core.period_old"))
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=requestuser)
        self.assertEqual(1, self.__get_period_count(selector=mockresponse.selector))

    def test_include_active_periods(self):
        requestuser = baker.make(settings.AUTH_USER_MODEL)
        baker.make("core.RelatedStudent", user=requestuser, period=baker.make_recipe("devilry.apps.core.period_active"))
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=requestuser)
        self.assertEqual(1, self.__get_period_count(selector=mockresponse.selector))
    ...
    def test_orderby_sanity(self):
        ...
        testperiod1 = baker.make_recipe(
            "devilry.apps.core.period_old", parentnode__long_name="Test Subject", long_name="Old Period 1"
        )
        testperiod2 = baker.make_recipe(
            "devilry.apps.core.period_old",
            parentnode__long_name="Test Subject",
            start_time=OLD_PERIOD_START + timedelta(days=2),
            long_name="Old Period 2",
        )
        testperiod3 = baker.make_recipe(
            "devilry.apps.core.period_active",
            parentnode__long_name="Test Subject",
            start_time=ACTIVE_PERIOD_START + timedelta(days=2),
            long_name="Active Period 2",
        )
        testperiod4 = baker.make_recipe(
            "devilry.apps.core.period_active", parentnode__long_name="Test Subject", long_name="Active Period 1"
        )
    ...
    def test_orderby_multiple_with_same_start_time(self):
        requestuser = baker.make(settings.AUTH_USER_MODEL)
        testperiod1 = baker.make_recipe(
            "devilry.apps.core.period_active", parentnode__long_name="Test Subject 1", long_name="Active Period"
        )
        testperiod2 = baker.make_recipe(
            "devilry.apps.core.period_active", parentnode__long_name="Test Subject 2", long_name="Active Period"
        )
        testperiod3 = baker.make_recipe(
            "devilry.apps.core.period_active", parentnode__long_name="Test Subject 3", long_name="Active Period"
        )
        baker.make("core.RelatedStudent", period=testperiod1, user=requestuser)
        baker.make("core.RelatedStudent", period=testperiod2, user=requestuser)
        baker.make("core.RelatedStudent", period=testperiod3, user=requestuser)
    ...
    def test_assignmentcount_sanity(self):
        ...
        baker.make("core.Candidate", assignment_group__parentnode=testassignment1, relatedstudent=relatedstudent)
        testassignment2 = baker.make("core.Assignment", parentnode=testperiod)
        baker.make("core.Candidate", assignment_group__parentnode=testassignment2, relatedstudent=relatedstudent)
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=requestuser)
        self.assertEqual(
            "2 assignments",
            mockresponse.selector.one(
    ...
    def test_assignmentcount_multiple_periods(self):
        ...
        baker.make("core.Candidate", assignment_group__parentnode=testassignment1, relatedstudent=relatedstudent1)
        ...
        self.assertEqual({"1 assignment", "0 assignments"}, assignmentcounts)
    ...
    def test_qualified_for_final_exam_sanity_qualified(self):
        ...
        status = baker.make("devilry_qualifiesforexam.Status", period=testperiod, status=Status.READY)
        baker.make(
