...
class PeriodOverviewView(listbuilderview.FilterListMixin, listbuilderview.View):
    ...
    def __get_assignment_id_to_assignment_map(self):
        assignmentqueryset = (
            Assignment.objects.filter_student_has_access(user=self.request.user)
    ...
    def get_filterlist_url(self, filters_string):
        return self.request.cradmin_app.reverse_appurl(crapp.INDEXVIEW_NAME, kwargs={"filters_string": filters_string})
    ...
    def get_unfiltered_queryset_for_role(self, role):
        period = role
        return (
            coremodels.AssignmentGroup.objects.filter(parentnode__parentnode=period)
    ...
    def get_no_items_message(self):
        return pgettext_lazy("student period overview", "No assignments.")
...
class App(crapp.App):
    appurls = [
        crapp.Url(r"^(?P<filters_string>.+)?$$", PeriodOverviewView.as_view(), name=crapp.INDEXVIEW_NAME),
    ]
