...
class DashboardView(listbuilderview.FilterListMixin, listbuilderview.View):
    ...
    def __get_upcoming_assignments_as_groups(self):
        ...
        queryset = (
            coremodels.AssignmentGroup.objects.filter_student_has_access(user=self.request.user)
            .filter_is_active()
            .distinct()
            .select_related(
                "parentnode",
    ...
    def get_value_and_frame_renderer_kwargs(self):
        ...
        kwargs.update({"assignment_id_to_assignment_map": self.__get_assignment_id_to_assignment_map()})
        return kwargs

    def get_pagetitle(self):
        return pgettext_lazy("student dashboard", "Student dashboard")
...
class App(crapp.App):
    appurls = [
        ...
        crapp.Url(r"^filter/(?P<filters_string>.+)?$", DashboardView.as_view(), name="filter"),
    ]
