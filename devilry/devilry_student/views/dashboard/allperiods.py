...
class AllPeriodsView(listbuilderview.FilterListMixin, listbuilderview.View):
    ...
    def get_unfiltered_queryset_for_role(self, role):
        return (
            coremodels.Period.objects.filter_user_is_relatedstudent(user=self.request.user)
            .filter_has_started()
    ...
    def get_no_items_message(self):
        return pgettext_lazy("student allperiods", "You are not registered on any courses in Devilry.")


class App(crapp.App):
    appurls = [
        crapp.Url(r"^(?P<filters_string>.+)?$", AllPeriodsView.as_view(), name=crapp.INDEXVIEW_NAME),
    ]
