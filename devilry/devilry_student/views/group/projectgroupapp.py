...
class CreateForm(forms.ModelForm):
    ...
    def _sent_to_choices(self):
        ...
        choices = [(candidate.id, candidate.relatedstudent.user.get_displayname()) for candidate in candidates]
        choices.insert(0, ("", ""))
        return choices
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout("sent_to", ButtonHolder(Submit("submit", gettext_lazy("Send invitation"))))
...
class ProjectGroupOverviewView(TemplateView):
    ...
    def get_queryset(self):
        return AssignmentGroup.objects.filter_student_has_access(self.request.user).select_related(
    ...
    def _form_kwargs(self):
        return dict(group=self.request.cradmin_role, form_action=self.request.path, sent_by=self.request.user)
    ...
    def form_valid(self, form):
        if form.cleaned_invite:
            ...
            messages.success(
                self.request,
                gettext_lazy(
                    "Invite sent to %(student)s." % {"student": form.cleaned_invite.sent_to.get_displayname()}
                ),
            )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["unanswered_received_invites"] = (
            GroupInvite.objects.filter_unanswered_received_invites(self.request.user)
...
class GroupInviteRespondView(DetailView):
    ...
    def get_queryset(self):
        return GroupInvite.objects.filter_unanswered_received_invites(self.request.user).select_related(
            "group__parentnode__parentnode__parentnode", "group__cached_data"
        )
        ...
        try:
            ...
            if accepted:
                ...
                messages.success(
                    self.request,
                    gettext_lazy(
                        "Declined group invitation from %(student)s." % {"student": invite.sent_by.get_displayname()}
                    ),
                )
...
class GroupInviteRespondViewStandalone(DetailView):
    ...
    def get_queryset(self):
        ...
        """
        return GroupInvite.objects.filter(sent_to=self.request.user).select_related(
            "group__parentnode__parentnode__parentnode", "group__cached_data"
        )
    ...
    def decline_url(self):
        ...
        """
        group = (
            AssignmentGroup.objects.filter_student_has_access(self.request.user)
            .filter(parentnode=self.group.parentnode)
            .first()
        )
        return reverse_cradmin_url(
            instanceid="devilry_group_student", appname="feedbackfeed", roleid=group.id, viewname=crapp.INDEXVIEW_NAME
        )
    ...
    def get_current_group(self, invite):
        ...
        return (
            ...
            .select_related("cached_data", "parentnode")
            .get()
        )
    ...
    def validate(self, invite):
        ...
        if invite.group.assignment.students_can_create_groups:
            if (
                invite.group.assignment.students_can_not_create_groups_after
                and invite.group.assignment.students_can_not_create_groups_after < timezone.now()
            ):
...
class GroupInviteDeleteView(DeleteView):
    ...
    def get_success_url(self):
        messages.success(
            self.request,
            gettext_lazy(
                "Removed project group invitation %(student)s."
...
class App(crapp.App):
    appurls = [
        crapp.Url(r"^$", ProjectGroupOverviewView.as_view(), name=crapp.INDEXVIEW_NAME),
        crapp.Url(r"^remove/(?P<invite_id>\d+)$", GroupInviteDeleteView.as_view(), name="delete"),
        crapp.Url(r"respond/(?P<invite_id>\d+)$", GroupInviteRespondView.as_view(), name="respond"),
    ]
