...
class TestQualificationStatusView(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_get(self):
        ...
        mockresponse = self.mock_getrequest(cradmin_role=testperiod, viewkwargs={"statusid": teststatus.id})
        self.assertEqual(mockresponse.response.status_code, 200)

    def test_get_retracted_message(self):
        testperiod = baker.make("core.Period")
        teststatus = baker.make(
        ...
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=testperiod, viewkwargs={"statusid": teststatus.id}
        )
    ...
    def test_no_retract_button_when_status_is_not_ready(self):
        testperiod = baker.make("core.Period")
        teststatus = baker.make(
            "devilry_qualifiesforexam.Status", period=testperiod, status=status_models.Status.NOTREADY
        )
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=testperiod, viewkwargs={"statusid": teststatus.id}
        )
    ...
    def test_num_queries(self):
        ...
        baker.make("devilry_qualifiesforexam.QualifiesForFinalExam", status=teststatus, qualifies=True, _quantity=10)
        baker.make("devilry_qualifiesforexam.QualifiesForFinalExam", status=teststatus, qualifies=False, _quantity=10)
        with self.assertNumQueries(5):
            self.mock_http200_getrequest_htmls(
                cradmin_role=testperiod,
                requestuser=admin_user,
                viewkwargs={"statusid": teststatus.id},
            )
...
class TestQualificationStatusPreviewTableRendering(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_table_row_is_rendered(self):
        ...
        self.__make_qualification_item(status=status, relatedstudent=relatedstudent)
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=testperiod,
            viewkwargs={"statusid": status.id},
        )
    ...
    def test_table_student_row_data_student_does_not_qualify(self):
        ...
        status = baker.make("devilry_qualifiesforexam.Status", period=testperiod)
        self.__make_qualification_item(status=status, relatedstudent=relatedstudent, qualifies=False)
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=testperiod,
            viewkwargs={"statusid": status.id},
        )
        ...
        self.assertEqual(
            studentinfo.alltext_normalized, "{} {}".format(relatedstudent.user.fullname, relatedstudent.user.shortname)
        )
    ...
    def test_table_default_ordering_lastname_if_order_by_param_is_not_supported(self):
        ...
        self.__make_qualification_item(teststatus, relatedstudent3)
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=testperiod, viewkwargs={"statusid": teststatus.id}, requestkwargs={"data": {"order_by": "asd"}}
        )
        student_list = mockresponse.selector.list(".devilry-qualifiesforexam-cell-studentinfo")
        self.assertEqual(len(student_list), 3)
        self.assertEqual(student_list[0].alltext_normalized, "C A ca@example.com")
