...
class TestStatus(test.TestCase):
    ...
    def test_get_current_status(self):
        testperiod = baker.make_recipe("devilry.apps.core.period_active")
        baker.make(
            "devilry_qualifiesforexam.Status", period=testperiod, status=status_models.Status.READY, plugin="plugin"
        )
        last_status = baker.make(
            "devilry_qualifiesforexam.Status", period=testperiod, status=status_models.Status.READY, plugin="plugin"
        )
