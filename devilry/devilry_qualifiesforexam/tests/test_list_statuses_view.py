...
class TestQualificationStatusPreviewTableRendering(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_title(self):
        ...
        self.assertEqual(mockresponse.selector.one("title").alltext_normalized, "testsubject.testperiod")
    ...
    def test_no_statuses_for_period_message(self):
        testperiod = baker.make("core.Period")
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testperiod)
        no_items_message = mockresponse.selector.one(
            ".cradmin-legacy-listbuilderview-no-items-message"
        ).alltext_normalized
    ...
    def test_status_description_in_list(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testperiod)
        self.assertEqual(
            mockresponse.selector.one(".devilry-qualifiesforexam-status-description-name").alltext_normalized, "Status"
        )
        self.assertEqual(
    ...
    def test_status_multiple_in_list(self):
        testperiod = baker.make("core.Period")
        baker.make("devilry_qualifiesforexam.Status", period=testperiod, status=status_models.Status.READY, _quantity=5)
        mockresponse = self.mock_http200_getrequest_htmls(cradmin_role=testperiod)
