...
class AbstractQualificationPreviewView(generic.FormView):
    ...
    def get_relatedstudents_queryset(self, period):
        ...
        queryset = (
            core_models.RelatedStudent.objects.filter(period=period)
            .select_related("user")
            ...
            return HttpResponseRedirect(
                str(
                    self.request.cradmin_app.reverse_appurl(
                        viewname="show-status", kwargs={"roleid": self.request.cradmin_role.id}
                    )
                )
            )
...
class PrefetchStatusInfoMixin(object):
    ...
    def _get_qualifiesforexam_queryset(self):
        ...
        """
        return status_models.QualifiesForFinalExam.objects.select_related("relatedstudent__user")
...
class QualificationStatusView(PrefetchStatusInfoMixin, AbstractQualificationPreviewView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if (
            current_status.plugin == "devilry_qualifiesforexam_plugin_approved.plugin_select_assignments"
        ...
        context_data["table"] = self._get_tablebuilder(
            relatedstudents=relatedstudents, qualifying_studentids=qualifying_studentids
        )
        return context_data
...
class DownloadCsvView(PrintStatusView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        response["Content-Disposition"] = 'attachment; filename="{}-status.csv"'.format(context_data["period"])
...
class RetractStatusForm(forms.ModelForm):
    ...
    class Meta:
        ...
        help_texts = {"message": gettext_lazy("Provide a message as to why the Status needs to be retracted.")}
...
class StatusRetractView(update.UpdateView):
    ...
    def get_buttons(self):
        buttons = [PrimarySubmit(self.get_submit_save_button_name(), self.get_submit_save_label())]
        return buttons
    ...
    def get_object(self, queryset=None):
        return status_models.Status.objects.get(id=self.kwargs.get("statusid"), period=self.request.cradmin_role)
