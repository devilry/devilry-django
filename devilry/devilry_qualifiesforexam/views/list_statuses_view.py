...
class StatusItemFrame(devilry_listbuilder.common.GoForwardLinkItemFrame):
    ...
    def get_url(self):
        return reverse_cradmin_url(
            ...
            kwargs={"statusid": self.status.id},
        )
...
class OrderStatusFilter(listfilter.django.single.select.AbstractOrderBy):
    def get_ordering_options(self):
        return [
            ("status (ready)", {"label": gettext_lazy("Status (ready)"), "order_by": ["-status"]}),
            ("status (not ready)", {"label": gettext_lazy("Status (not ready)"), "order_by": ["status"]}),
        ]
...
class ListStatusesView(listbuilderview.FilterListMixin, listbuilderview.View):
    ...
    def add_filterlist_items(self, filterlist):
        filterlist.append(OrderStatusFilter(slug="orderby", label="Order by"))
    ...
    def get_unfiltered_queryset_for_role(self, role):
        ...
        period_queryset = core_models.Period.objects.prefetch_related(
            models.Prefetch("qualifiedforexams_status", queryset=status_models.Status.objects.filter(period=period))
        ).get(id=period.id)
    ...
    def get_context_data(self, **kwargs):
        ...
        context_data["help_text"] = gettext_lazy(
            "You can either choose a plugin to create a new status, or you can select a retracted status."
        )
        return context_data
