class AssignmentGroupExtjsModelMeta:
    """ Metadata for AssignmentGroup javascript. """
    combobox_fieldgroups = ['assignment', 'period', 'subject', 'users']
    combobox_tpl = ('<section class="popuplistitem">'
<<<<<<< HEAD
                    '    <table>'
                    '        <tr>'
                    '            <td>'
                    '               <ul class="useridlist"><tpl for="candidates__identifier"><li>{.}</li></tpl></ul>'
                    '               <p><tpl if="name">{name}</tpl><p>'
                    '               <p>'
                    '                   <tpl if="is_open"><span class="group_is_uncorrected">Correction not completed</span></tpl>'
                    '                   <tpl if="!is_open"><span class="group_is_corrected">Correction completed</span></tpl>'
                    '               </p>'
                    '            </td>'
                    '            <td class="rightaligned">'
                    '               <h1>{parentnode__parentnode__parentnode__long_name:ellipsis(60)}</h1>'
                    '               <h2>{parentnode__parentnode__long_name:ellipsis(60)}</h2>'
                    '               <h3>{parentnode__long_name:ellipsis(60)}</h3>'
                    '            </td>'
                    '        </tr>'
                    '    </table>'
=======
                    '   <p class="path">'
                           '{parentnode__parentnode__parentnode__short_name:ellipsis(60)}.'
                           '{parentnode__parentnode__short_name:ellipsis(60)}.'
                           '{parentnode__short_name:ellipsis(60)}'
                    '   </p>'
                    '   <tpl if="!is_student">'
                    '       <h1><ul class="useridlist"><tpl for="candidates__identifier"><li>{.}</li></tpl></ul></h1>'
                    '   </tpl>'
                    '   <p><tpl if="name">{name}</tpl><p>'
>>>>>>> 56d4307aaca846ce3a64de2ed94cad2bb0652bc2
                   '</section>')
    combobox_displayfield = 'id'
