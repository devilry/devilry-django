...
class TestStudentItemValue(test.TestCase):
    ...
    def test_description_no_assignments(self):
        ...
        testperiod = baker.make("core.Period")
        testperiod_annotated = Period.objects.extra_annotate_with_assignmentcount_for_studentuser(user=testuser).get(
            id=testperiod.id
        )
    ...
    def test_no_qualified_for_final_exam_status(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        testperiod = baker.make("core.Period")
        testperiod_annotated = Period.objects.extra_annotate_with_user_qualifies_for_final_exam(user=testuser).get(
            id=testperiod.id
        )
        selector = htmls.S(devilry_listbuilder.period.StudentItemValue(value=testperiod_annotated).render())
