...
class TestItemValue(test.TestCase):
    def test_title_without_fullname(self):
        relatedstudent = baker.make(
            "devilry_account.PermissionGroupUser", user__shortname="test@example.com", user__fullname=""
        )
    ...
    def test_title_with_fullname(self):
        relatedstudent = baker.make(
            "devilry_account.PermissionGroupUser", user__fullname="Test User", user__shortname="test@example.com"
        )
        selector = htmls.S(devilry_listbuilder.permissiongroupuser.ItemValue(value=relatedstudent).render())
        self.assertEqual(
