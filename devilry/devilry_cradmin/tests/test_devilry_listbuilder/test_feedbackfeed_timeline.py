...
class TestStudentGroupCommentItemValue(test.TestCase):
    ...
    def __model_setup(
        self, anonymizationmode=core_models.Assignment.ANONYMIZATIONMODE_OFF, request_devilryrole="examiner"
    ):
        self.testassignment = baker.make_recipe(
            "devilry.apps.core.assignment_activeperiod_start", anonymizationmode=anonymizationmode
        )
        self.testgroup = baker.make("core.AssignmentGroup", parentnode=self.testassignment)
        self.candidate = baker.make(
            "core.Candidate",
    ...
    def test_student_comment_devilryrole_examiner_is_not_anonymized(self):
        ...
        self.assertIn(
            "Test User(testuser@example.com)", selector.one(".devilry-user-verbose-inline-both").alltext_normalized
        )
    ...
    def test_student_comment_devilryrole_student_assignment_fully_anonymized(self):
        self.__model_setup(
            anonymizationmode=core_models.Assignment.ANONYMIZATIONMODE_FULLY_ANONYMOUS, request_devilryrole="student"
        )
        selector = htmls.S(
...
class TestExaminerGroupCommentItemValue(test.TestCase):
    ...
    def __model_setup(
        self, anonymizationmode=core_models.Assignment.ANONYMIZATIONMODE_OFF, request_devilryrole="student"
    ):
        self.testassignment = baker.make_recipe(
            "devilry.apps.core.assignment_activeperiod_start", anonymizationmode=anonymizationmode
        )
        self.testgroup = baker.make("core.AssignmentGroup", parentnode=self.testassignment)
        self.relatedexaminer = baker.make(
    ...
    def test_examiner_comment_devilryrole_student_semi_anonymous(self):
        ...
        self.assertNotIn("Test User(testuser@example.com)", selector.html)
        self.assertIn("Anonymous examiner", selector.one(".devilry-core-examiner-anonymous-name").alltext_normalized)
    ...
    def test_examiner_comment_devilryrole_examiner_not_anonymized_for_examiners(self):
        self.__model_setup(
            anonymizationmode=core_models.Assignment.ANONYMIZATIONMODE_FULLY_ANONYMOUS, request_devilryrole="examiner"
        )
        selector = htmls.S(
            listbuilder.ExaminerGroupCommentItemValue(
                requestuser=baker.make(settings.AUTH_USER_MODEL),
