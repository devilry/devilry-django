...
class TestSelectedItem(test.TestCase):
    def test_title_without_fullname(self):
        user = baker.make(settings.AUTH_USER_MODEL, shortname="test@example.com", fullname="")
    ...
    def test_title_with_fullname(self):
        user = baker.make(settings.AUTH_USER_MODEL, fullname="Test User", shortname="test@example.com")
        selector = htmls.S(devilry_multiselect2.user.SelectedItem(value=user).render())
        self.assertEqual(
...
class TestTarget(test.TestCase):
    def test_with_items_title(self):
        ...
        self.assertEqual("Selected users", selector.one(".cradmin-legacy-multiselect2-target-title").alltext_normalized)
...
class TestBaseMultiselectUsersView(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_ordering(self):
        baker.make(settings.AUTH_USER_MODEL, shortname="userb")
        baker.make(settings.AUTH_USER_MODEL, shortname="usera")
        baker.make(settings.AUTH_USER_MODEL, shortname="userc")
        mockresponse = self.mock_http200_getrequest_htmls(requestuser=mock.MagicMock())
    ...
    def test_search_shortname(self):
        ...
        mockresponse = self.mock_http200_getrequest_htmls(
            requestuser=mock.MagicMock(), viewkwargs={"filters_string": "search-usera"}
        )
        self.assertEqual({"usera"}, set(self.__get_titles(mockresponse.selector)))

    def test_search_fullname(self):
        baker.make(settings.AUTH_USER_MODEL, fullname="Userb")
        baker.make(settings.AUTH_USER_MODEL, fullname="Usera")
        mockresponse = self.mock_http200_getrequest_htmls(
            requestuser=mock.MagicMock(), viewkwargs={"filters_string": "search-usera"}
        )
        self.assertEqual({"Usera"}, set(self.__get_titles(mockresponse.selector)))
    ...
    def test_search_username(self):
        baker.make("devilry_account.UserName", user__fullname="Test User 1", username="testuser1")
        baker.make("devilry_account.UserName", user__fullname="Test User 2", username="testuser2")
        mockresponse = self.mock_http200_getrequest_htmls(
            requestuser=mock.MagicMock(), viewkwargs={"filters_string": "search-testuser1"}
        )
        self.assertEqual({"Test User 1"}, set(self.__get_titles(mockresponse.selector)))

    def test_search_useremail(self):
        baker.make("devilry_account.UserEmail", user__fullname="Test User 1", email="testuser1@example.com")
        baker.make("devilry_account.UserEmail", user__fullname="Test User 2", email="testuser2@example.com")
        mockresponse = self.mock_http200_getrequest_htmls(
            requestuser=mock.MagicMock(), viewkwargs={"filters_string": "search-testuser1"}
        )
        self.assertEqual({"Test User 1"}, set(self.__get_titles(mockresponse.selector)))
