...
class AbstractOrderBy(listfilter.django.single.select.AbstractOrderBy):
    ...
    def get_ordering_options(self):
        ordering_options = [(pgettext_lazy("orderby", "By student name"), self.get_user_ordering_options())]
...
class OrderByDelivery(AbstractOrderBy):
    ...
    def filter(self, queryobject):
        cleaned_value = self.get_cleaned_value() or ""
        if cleaned_value == "":
            return self.__annotate_delivery_time(queryobject).order_by(models.F("delivery_time").desc(nulls_last=True))
        elif cleaned_value == "delivery_ascending":
            return self.__annotate_delivery_time(queryobject).order_by(models.F("delivery_time").asc(nulls_last=True))
        else:
...
class StatusRadioFilter(abstractradio.AbstractRadioFilter):
    ...
    def __make_label(self, label, count, has_count_cssclass=None):
        return mark_safe(
            "{label} {count}".format(
                label=label, count=self.__count_html(count=count, has_count_cssclass=has_count_cssclass)
            )
        )
    ...
    def get_choices(self):
        return [
            (
                "",
                self.__make_label(
                    label=pgettext("group status", "all students"), count=self.view.get_filtered_all_students_count()
                ),
            ),
            (
            ...
            (
                "corrected",
                self.__make_label(
                    label=pgettext("group status", "corrected"), count=self.view.get_filtered_corrected_count()
                ),
            ),
        ]
...
class IsPassingGradeFilter(abstractselect.AbstractBoolean):
    ...
    def get_label(self):
        return pgettext_lazy("group is passing grade filter", "Passing grade?")
...
class ExaminerFilter(abstractselect.AbstractSelectFilter):
    ...
    def __get_choices_cached(self):
        if not hasattr(self, "_choices"):
            self._choices = [
                (str(relatedexaminer.id), self.__get_examiner_name(relatedexaminer))
                for relatedexaminer in self.view.get_distinct_relatedexaminers()
            ]
        return self._choices

    def __get_valid_values(self):
        return {str(choice[0]) for choice in self.__get_choices_cached()}

    def get_choices(self):
        choices = [("", pgettext_lazy("group examiner filter", "Any"))]
...
class ActivityFilter(abstractselect.AbstractSelectFilter):
    ...
    def get_label(self):
        return pgettext_lazy("group activity", "Activity")
    ...
    def get_choices(self):
        return [
            ...
            (
                ...
                (
                    ("studentcomment", pgettext_lazy("group activity", "Has comment(s) from student")),
                    ("studentfile", pgettext_lazy("group activity", "Has file(s) from student")),
                    ("no-studentcomment", pgettext_lazy("group activity", "No comments from student")),
                    ("no-studentfile", pgettext_lazy("group activity", "No files from student")),
                ),
            ),
            (
                pgettext_lazy("group activity", "From examiner"),
                (
                    ("examinercomment", pgettext_lazy("group activity", "Has comment(s) from examiner")),
                    ("unpublishedfeedback", pgettext_lazy("group activity", "Has unpublished feedback draft")),
                    ("privatecomment", pgettext_lazy("group activity", "Has unpublished comment(s) from YOU")),
                    ("no-examinercomment", pgettext_lazy("group activity", "No comments from examiner")),
                ),
            ),
            (
                pgettext_lazy("group activity", "From administrator"),
                (("admincomment", pgettext_lazy("group activity", "Has comment(s) from administrator")),),
            ),
        ]
...
class AssignmentCheckboxFilter(listfilter.basefilters.multi.abstractcheckbox.AbstractCheckboxFilter):
    ...
    def get_choices(self):
        choices = [
            (assignment.short_name, assignment.long_name)
