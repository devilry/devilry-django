...
class AbstractSubjectOrPeriodPermissionGroupItemValue(listbuilder.itemvalue.TitleDescription):
    template_name = (
        "devilry_cradmin/devilry_listbuilder/permissiongroup/subjectorperiodpermissiongroup-itemvalue.django.html"
    )
    ...
    def get_users(self):
        users = [
            permissiongroupuser.user for permissiongroupuser in self.value.permissiongroup.permissiongroupuser_set.all()
        ]
        return users
...
class SubjectAndPeriodPermissionGroupList(listbuilder.lists.RowList):
    ...
    def add_periodpermissiongroups(self, periodpermissiongroups):
        self.extend_with_values(
            value_iterable=periodpermissiongroups, value_renderer_class=PeriodPermissionGroupItemValue
        )

    def add_subjectpermissiongroups(self, subjectpermissiongroups):
        self.extend_with_values(
            value_iterable=subjectpermissiongroups, value_renderer_class=SubjectPermissionGroupItemValue
        )
