...
class TestPermissionGroupAddSubjectCommand(test.TestCase):
    def __run_management_command(self, subject_short_name, group_name, *args):
        management.call_command("devilry_permissiongroup_add_subject", subject_short_name, group_name)
    ...
    def test_permission_group_with_type_periodadmin_error_message(self):
        baker.make("core.Subject", short_name="test")
        baker.make("devilry_account.PermissionGroup", name="Test", grouptype=PermissionGroup.GROUPTYPE_PERIODADMIN)
        with self.assertRaisesMessage(CommandError, 'PermissionGroup "Test" does not exist.'):
            self.__run_management_command("test", "Test")
    ...
    def test_subject_already_added_to_permissiongroup_error_message(self):
        ...
        permissiongroup = baker.make(
            "devilry_account.PermissionGroup", name="Test", grouptype=PermissionGroup.GROUPTYPE_SUBJECTADMIN
        )
    ...
    def test_add_subject_to_permissiongroup_with_type_subject(self):
        baker.make("core.Subject", short_name="test")
        baker.make("devilry_account.PermissionGroup", name="Test", grouptype=PermissionGroup.GROUPTYPE_SUBJECTADMIN)
        self.__run_management_command("test", "Test")
        self.assertTrue(
    ...
    def test_add_subject_to_permissiongroup_with_type_department(self):
        baker.make("core.Subject", short_name="test")
        baker.make("devilry_account.PermissionGroup", name="Test", grouptype=PermissionGroup.GROUPTYPE_DEPARTMENTADMIN)
        self.__run_management_command("test", "Test")
        self.assertTrue(
            SubjectPermissionGroup.objects.filter(subject__short_name="test", permissiongroup__name="Test").exists()
        )
    ...
    def test_add_subject_to_multiple_permissiongroups(self):
        ...
        baker.make(
            "devilry_account.PermissionGroup", name="SubjectAdmins", grouptype=PermissionGroup.GROUPTYPE_DEPARTMENTADMIN
        )
        ...
        self.assertTrue(
            SubjectPermissionGroup.objects.filter(
                subject__short_name="test", permissiongroup__name="DepartmentAdmins"
            ).exists()
        )
        self.assertTrue(
            SubjectPermissionGroup.objects.filter(
                subject__short_name="test", permissiongroup__name="SubjectAdmins"
            ).exists()
        )
