...
class TestResendFailedMessagesCommand(test.TestCase):
    def __test_standard_output(
        self, stdout, num_resent=0, exceeded_resend_limit=0, resend_limit=settings.DEVILRY_MESSAGE_RESEND_LIMIT
    ):
        self.assertEqual(
            ...
            "(current limit: {resend_limit})\n".format(
                num_resent=num_resent, exceeded_resend_limit=exceeded_resend_limit, resend_limit=resend_limit
            ),
        )
    ...
    def test_sanity_failed_message_sent_to_user(self):
        ...
        baker.make("devilry_message.MessageReceiver", user=testuser, status=MessageReceiver.STATUS_CHOICES.FAILED.value)
        out = StringIO()
        management.call_command("devilry_resend_failed_messages", stdout=out)
    ...
    def test_sanity_failed_messages_sent_to_multiple_users(self):
        ...
        receiver1 = baker.make(
            "devilry_message.MessageReceiver",
            user=testuser1,
            subject="Test 1",
            status=MessageReceiver.STATUS_CHOICES.FAILED.value,
        )
        receiver2 = baker.make(
            "devilry_message.MessageReceiver",
            user=testuser2,
            subject="Test 2",
            status=MessageReceiver.STATUS_CHOICES.FAILED.value,
    ...
    def test_stdout_no_failed_messages(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        self.__make_email_for_user(testuser, "testuser@example.com")
        baker.make("devilry_message.MessageReceiver", user=testuser, status=MessageReceiver.STATUS_CHOICES.SENT.value)
        out = StringIO()
        management.call_command("devilry_resend_failed_messages", stdout=out)
        self.__test_standard_output(stdout=out)
        out.close()
