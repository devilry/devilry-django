...
class TestPeriodDelete(TestCase):
    ...
    def test_does_not_delete_files_for_other_periods(self):
        ...
        commentfile = baker.make("devilry_comment.CommentFile", comment=testcomment, filename="notdeleted.txt")
        ...
        to_delete_commentfile = baker.make(
            "devilry_comment.CommentFile", comment=to_delete_testcomment, filename="testfile.txt"
        )
    ...
    def test_subject_not_deleted_if_not_all_periods_are_deleted(self):
        ...
        period_not_deleted = baker.make(
            "core.Period", parentnode=testsubject, end_time=timezone.now() + timezone.timedelta(days=50)
        )
    ...
    def test_sanity_delete_cascade(self):
        ...
        testperiod = baker.make(
            "core.Period", parentnode=testsubject, end_time=timezone.now() - timezone.timedelta(days=10)
        )
