...
class TestPeriodadminaddCommand(test.TestCase):
    def __run_management_command(self, subject_short_name, period_short_name, user_shortname):
        management.call_command("devilry_periodadminadd", subject_short_name, period_short_name, user_shortname)
    ...
    def test_invalid_subject_short_name(self):
        ...
        with self.assertRaisesMessage(CommandError, "Invalid subject_short_name."):
            self.__run_management_command(
                subject_short_name="invalid", period_short_name="testperiod", user_shortname="testuser"
            )

    def test_invalid_period_short_name(self):
        baker.make("core.Subject", short_name="testsubject")
        baker.make(settings.AUTH_USER_MODEL, shortname="testuser")
        with self.assertRaisesMessage(CommandError, "Invalid period_short_name."):
            self.__run_management_command(
                subject_short_name="testsubject", period_short_name="invalid", user_shortname="testuser"
            )
    ...
    def test_invalid_user_shortname(self):
        ...
        with self.assertRaisesMessage(CommandError, "Invalid user_shortname."):
            self.__run_management_command(
                subject_short_name="testsubject", period_short_name="testperiod", user_shortname="invalid"
            )
    ...
    def test_ok_new_permission_group(self):
        ...
        self.__run_management_command(
            subject_short_name="testsubject", period_short_name="testperiod", user_shortname="testuser"
        )
        ...
        self.assertEqual(created_period_permissiongroup.permissiongroup, created_permissiongroup)
    ...
    def test_ok_existing_permission_group_and_permission_group_user(self):
        ...
        baker.make("devilry_account.PermissionGroupUser", permissiongroup=permissiongroup, user=user)
        self.__run_management_command(
            subject_short_name="testsubject", period_short_name="testperiod", user_shortname="testuser"
        )
        self.assertEqual(PermissionGroup.objects.count(), 1)
