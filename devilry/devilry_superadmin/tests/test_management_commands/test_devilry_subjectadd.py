...
class TestSubjectaddCommand(test.TestCase):
    def __run_management_command(self, subject_short_name, subject_long_name, *args):
        management.call_command(
            "devilry_subjectadd", subject_short_name, "--long-name={}".format(subject_long_name), *args
        )

    def test_subject_with_short_name_exists_error_message(self):
        baker.make("core.Subject", short_name="test")
        with self.assertRaisesMessage(CommandError, 'Subject "test" already exists.'):
    ...
    def test_add_subject_with_permissiongroup_type_subject(self):
        baker.make(
            "devilry_account.PermissionGroup", name="Test group", grouptype=PermissionGroup.GROUPTYPE_SUBJECTADMIN
        )
        self.__run_management_command("test", "Test", "--permission-groups", "Test group")
        self.assertTrue(
            SubjectPermissionGroup.objects.filter(
                subject__short_name="test", permissiongroup__name="Test group"
            ).exists()
        )

    def test_add_subject_with_permissiongroup_type_department(self):
        baker.make(
            "devilry_account.PermissionGroup", name="Test group", grouptype=PermissionGroup.GROUPTYPE_DEPARTMENTADMIN
        )
        self.__run_management_command("test", "Test", "--permission-groups", "Test group")
        self.assertTrue(
            SubjectPermissionGroup.objects.filter(
    ...
    def test_add_subject_with_multiple_permission_groups(self):
        ...
        baker.make(
            "devilry_account.PermissionGroup", name="Subject admins", grouptype=PermissionGroup.GROUPTYPE_SUBJECTADMIN
        )
        self.__run_management_command("test", "Test", "--permission-groups", "Department admins", "Subject admins")
        self.assertTrue(
            SubjectPermissionGroup.objects.filter(
                subject__short_name="test", permissiongroup__name="Department admins"
            ).exists()
        )
        self.assertTrue(
            SubjectPermissionGroup.objects.filter(
                subject__short_name="test", permissiongroup__name="Subject admins"
            ).exists()
        )
