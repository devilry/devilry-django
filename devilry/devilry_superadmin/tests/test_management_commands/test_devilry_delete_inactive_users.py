...
class TestDeleteInactiveUsersCommand(test.TestCase):
    ...
    def test_aborted_no_inactive_users(self):
        ...
        mommy.make(settings.AUTH_USER_MODEL, last_login=datetimeutils.from_isoformat(iso_datetime_string))
        with self.assertRaises(SystemExit):
...
class TestInactiveUserDeleter(test.TestCase):
    ...
    def test_no_inactive_users(self):
        inactive_since = self.__create_timestamp(year=2018, hour=10)
        mommy.make(settings.AUTH_USER_MODEL, last_login=self.__create_timestamp(year=2019, hour=10))
        user_deleter = InactiveUserDeleter(inactive_since)
        self.assertEqual(user_deleter.get_users_to_delete_queryset().count(), 0)

    def test_last_login_same_as_datetime_iso_argument(self):
        inactive_since = self.__create_timestamp(year=2018, hour=10)
        mommy.make(settings.AUTH_USER_MODEL, last_login=inactive_since)
        user_deleter = InactiveUserDeleter(inactive_since)
        self.assertEqual(user_deleter.get_users_to_delete_queryset().count(), 0)
    ...
    def test_last_login_one_second_before(self):
        ...
        mommy.make(settings.AUTH_USER_MODEL, last_login=self.__create_timestamp(year=2019, hour=10, minute=0, second=0))
        user_deleter = InactiveUserDeleter(inactive_since)
    ...
    def test_last_login_one_second_after(self):
        inactive_since = self.__create_timestamp(year=2019, hour=10, minute=0, second=1)
        mommy.make(settings.AUTH_USER_MODEL, last_login=self.__create_timestamp(year=2019, hour=10, minute=0, second=2))
        user_deleter = InactiveUserDeleter(inactive_since)
        self.assertEqual(user_deleter.get_users_to_delete_queryset().count(), 0)
    ...
    def test_user_is_superuser(self):
        inactive_since = self.__create_timestamp(year=2018, hour=10)
        mommy.make(settings.AUTH_USER_MODEL, is_superuser=True, last_login=self.__create_timestamp(year=2017, hour=10))
        user_deleter = InactiveUserDeleter(inactive_since)
        self.assertEqual(user_deleter.get_users_to_delete_queryset().count(), 0)

    def test_user_registered_as_student_on_active_semester(self):
        inactive_since = self.__create_timestamp(year=2018, hour=10)
        user = mommy.make(settings.AUTH_USER_MODEL, last_login=self.__create_timestamp(year=2017, hour=10))
        period = mommy.make_recipe('devilry.apps.core.period_active')
        mommy.make('core.RelatedStudent', period=period, user=user)
        user_deleter = InactiveUserDeleter(inactive_since)
        self.assertEqual(user_deleter.get_users_to_delete_queryset().count(), 0)
    ...
    def test_user_registered_as_examiner_on_active_semester(self):
        ...
        user = mommy.make(settings.AUTH_USER_MODEL, last_login=self.__create_timestamp(year=2017, hour=10))
        period = mommy.make_recipe('devilry.apps.core.period_active')
        mommy.make('core.RelatedExaminer', period=period, user=user)
        user_deleter = InactiveUserDeleter(inactive_since)
        self.assertEqual(user_deleter.get_users_to_delete_queryset().count(), 0)

    def __make_period_admin(self, period, user):
        permission_group = mommy.make('devilry_account.PermissionGroup',
                                      grouptype=PermissionGroup.GROUPTYPE_PERIODADMIN)
        mommy.make('devilry_account.PermissionGroupUser', user=user, permissiongroup=permission_group)
        mommy.make('devilry_account.PeriodPermissionGroup', period=period, permissiongroup=permission_group)
    ...
    def test_with_students_examiners_admins_on_active_period(self):
        inactive_since = self.__create_timestamp(year=2018, hour=10)
        student_user = mommy.make(settings.AUTH_USER_MODEL, last_login=self.__create_timestamp(year=2017, hour=10))
        examiner_user = mommy.make(settings.AUTH_USER_MODEL, last_login=self.__create_timestamp(year=2017, hour=10))
        admin_user = mommy.make(settings.AUTH_USER_MODEL, last_login=self.__create_timestamp(year=2017, hour=10))
        period = mommy.make_recipe('devilry.apps.core.period_active')
        self.__make_period_admin(period=period, user=admin_user)
        mommy.make('core.RelatedStudent', period=period, user=student_user)
        mommy.make('core.RelatedExaminer', period=period, user=examiner_user)
        user_deleter = InactiveUserDeleter(inactive_since)
        self.assertEqual(user_deleter.get_users_to_delete_queryset().count(), 0)
    ...
    def test_delete_multiple_students(self):
        inactive_since = self.__create_timestamp(year=2018, hour=10)
        student_user1 = mommy.make(settings.AUTH_USER_MODEL, last_login=self.__create_timestamp(year=2017, hour=10))
        student_user2 = mommy.make(settings.AUTH_USER_MODEL, last_login=self.__create_timestamp(year=2017, hour=10))
        period = mommy.make_recipe('devilry.apps.core.period_old')
        mommy.make('core.RelatedStudent', period=period, user=student_user1)
        mommy.make('core.RelatedStudent', period=period, user=student_user2)
        user_deleter = InactiveUserDeleter(inactive_since)
    ...
    def test_delete_multiple_examiners(self):
        inactive_since = self.__create_timestamp(year=2018, hour=10)
        examiner_user1 = mommy.make(settings.AUTH_USER_MODEL, last_login=self.__create_timestamp(year=2017, hour=10))
        examiner_user2 = mommy.make(settings.AUTH_USER_MODEL, last_login=self.__create_timestamp(year=2017, hour=10))
        period = mommy.make_recipe('devilry.apps.core.period_old')
        mommy.make('core.RelatedExaminer', period=period, user=examiner_user1)
        mommy.make('core.RelatedExaminer', period=period, user=examiner_user2)
        user_deleter = InactiveUserDeleter(inactive_since)
        self.assertEqual(user_deleter.get_users_to_delete_queryset().count(), 2)
        user_deleter.delete()
        self.assertEqual(get_user_model().objects.count(), 0)
    ...
    def test_delete_multiple_period_admins(self):
        inactive_since = self.__create_timestamp(year=2018, hour=10)
        admin_user1 = mommy.make(settings.AUTH_USER_MODEL, last_login=self.__create_timestamp(year=2017, hour=10))
        admin_user2 = mommy.make(settings.AUTH_USER_MODEL, last_login=self.__create_timestamp(year=2017, hour=10))
        period = mommy.make_recipe('devilry.apps.core.period_old')
    ...
    def test_only_users_last_login_before_timestamp_deleted_sanity(self):
        inactive_since = self.__create_timestamp(year=2018, hour=10)
        user1 = mommy.make(settings.AUTH_USER_MODEL, last_login=self.__create_timestamp(year=2018, hour=10))
        user2 = mommy.make(settings.AUTH_USER_MODEL, last_login=self.__create_timestamp(year=2017, hour=10))
        user3 = mommy.make(settings.AUTH_USER_MODEL, last_login=self.__create_timestamp(year=2018, hour=11))
        user4 = mommy.make(settings.AUTH_USER_MODEL, last_login=self.__create_timestamp(year=2016, hour=10))
        user_deleter = InactiveUserDeleter(inactive_since)
        self.assertEqual(user_deleter.get_users_to_delete_queryset().count(), 2)
        user_deleter.delete()
