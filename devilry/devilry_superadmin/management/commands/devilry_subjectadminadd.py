...
class AdminAddBase(BaseCommand):
    ...
    def _permission_group_name(self):
        return "{prefix}-{path}#<subject_id>".format(
            prefix=settings.DEVILRY_SYNCSYSTEM_SHORTNAME, path=self.basenode.get_path(), id=self.basenode.id
        )
    ...
    def add_admin(self):
        ...
        permissiongroup, created = PermissionGroup.objects.create_or_update_syncsystem_permissiongroup(
            basenode=self.basenode, grouptype=self.permissiongroup_type
        )
...
class Command(AdminAddBase):
    ...
    def add_arguments(self, parser):
        parser.add_argument("subject_short_name", help="Short name of the subject you wish to add an admin to.")
