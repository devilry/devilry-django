...
class Command(BaseCommand):
    ...
    args = "<datetime>"
    help = (
        "Delete semesters and it's underlying data with end time older than specified <datetime>. "
    ...
    def add_arguments(self, parser):
        parser.add_argument("datetime", type=str, help="An ISO formatted datetime string. Example: YYYY-MM-DD HH:mm")
        parser.add_argument(
    ...
    def __confirm_delete(self):
        ...
        self.stdout.write(self.style.ERROR("This operation can NOT BE UNDONE!"))
    ...
    def handle(self, *args, **options):
        delete_older_than_datetime = datetimeutils.from_isoformat(options["datetime"]).replace(second=0, microsecond=0)
        ...
        if delete_older_than_datetime >= now:
            self.stderr.write(
                "EXITING... Given datetime {} must be earlier than now ({})".format(
                    arrow.get(delete_older_than_datetime.astimezone(timezone.get_current_timezone())).format(
                        "MMM D. YYYY HH:mm"
                    ),
