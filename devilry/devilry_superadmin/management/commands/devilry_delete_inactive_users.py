...
class InactiveUserDeleter(object):
    ...
    def get_users_to_delete_queryset(self):
        ...
        user_ids = PeriodPermissionGroup.objects.filter(
            period__start_time__lte=now, period__end_time__gte=now
        ).values_list("permissiongroup__permissiongroupuser__user_id", flat=True)

        return (
            get_user_model()
            .objects.exclude(
                models.Q(relatedstudent__period__start_time__lte=now, relatedstudent__period__end_time__gte=now)
                | models.Q(relatedexaminer__period__start_time__lte=now, relatedexaminer__period__end_time__gte=now)
            )
...
class Command(BaseCommand):
    ...
    help = (
        "Delete users that hasn't logged in since the provided date, or never logged in."
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "inactive_since_datetime", type=str, help="An ISO formatted datetime string. Example: YYYY-MM-DD HH:mm"
        )

    def __confirm_delete(self):
        confirm_string = "DELETE USERS"
        self.stdout.write(
            self.style.ERROR("Are you sure you want to delete these users? This operation can NOT BE UNDONE!")
        )
    ...
    def __preview(self, user_deleter):
        for user in user_deleter.get_users_to_delete_queryset():
            last_login = None
            if user.last_login:
                last_login = arrow.get(user.last_login.astimezone(timezone.get_current_timezone())).format(
                    "MMM D. YYYY HH:mm"
                )
            self.stdout.write("- {}: {}\n\tLast login: {}\n\n".format(user.shortname, user.get_full_name(), last_login))

    def handle(self, *args, **options):
        inactive_since_datetime = datetimeutils.from_isoformat(options["inactive_since_datetime"]).replace(
        ...
        if inactive_since_datetime >= now:
            self.stderr.write(
                "EXITING... Given datetime ({}) must be earlier than now ({})".format(
                    arrow.get(inactive_since_datetime.astimezone(timezone.get_current_timezone())).format(
                        "MMM D. YYYY HH:mm"
                    ),
        ...
        if not user_deleter.get_users_to_delete_queryset().exists():
            self.stdout.write(
                self.style.ERROR(
                    "EXITING... There are no users with last login before {}.".format(
        ...
        self.stdout.write(
            "\n\nThis action will delete {} users.".format(user_deleter.get_users_to_delete_queryset().count())
        )
