...
class Command(RelatedBaseCommand):
    help = (
        "Set related students on a period. Users are read from stdin, as "
    ...
    def _remove_compressed_archives_within_period(self):
        ...
        assignment_archives_queryset = CompressedArchiveMeta.objects.filter(
            content_object_id__in=assignment_ids,
        ...
        feedbackset_ids = FeedbackSet.objects.filter(group__parentnode__parentnode=self.period).values_list(
            "id", flat=True
        )
        feedbackset_archives_queryset = CompressedArchiveMeta.objects.filter(
            content_object_id__in=feedbackset_ids,
    ...
    def handle(self, *args, **options):
        ...
        candidate_id_set_before = set(
            self.related_user_model_class.objects.filter(period=self.period)
        ...
        with transaction.atomic():
            ...
            if candidate_id_set_before.difference(candidate_id_set_after):
                ...
                if self.verbosity > 1:
                    self.stdout.write(
                        f"Marked {deleted_count} CompressedArchiveMetas for deletion because of candidate ID changes."
                    )
