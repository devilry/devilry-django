...
class Command(UserModCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("username_or_email", help="The username if authenticating with username, and email if not.")
        parser.add_argument("--email", help="An email address that will be added if it is not registered for the user.")
        parser.add_argument("--full_name", dest="full_name", default=None, help="Full name")
        parser.add_argument(
            "--superuser",
            action="store_true",
            dest="superuser",
        ...
        try:
            ...
            if options["email"]:
                if not user.useremail_set.filter(email=options["email"]).exists():
                    useremail = UserEmail(user=user, email=options["email"])
