...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            self.stdout.write("(Dataporten) Deleting all SocialAccounts ({})".format(SocialAccount.objects.count()))
            SocialAccount.objects.all().delete()
            self.stdout.write("(Dataporten) Deleting all SocialTokens ({})".format(SocialToken.objects.count()))
            SocialToken.objects.all().delete()
            self.stdout.write("(Dataporten) Deleting all SocialApplications ({})".format(SocialApp.objects.count()))
