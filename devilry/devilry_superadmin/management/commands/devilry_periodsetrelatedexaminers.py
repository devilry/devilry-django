...
class RelatedBaseCommand(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("subject_short_name", help="Positional argument for short name of the subject.")
        parser.add_argument("period_short_name", help="Positional argument for short name of the period.")
        parser.add_argument(
            "--deactivate-missing",
            action="store_true",
    ...
    def add_users(self):
        ...
        for kw in self.input_data:
            ...
            user, created = user_model.objects.get_or_create_user(username=username, email=email)
            if not self._update_related_user(user, kw, tags_string):
                self._create_reluser(user, kw, tags_string)
        if self.verbosity > 0:
            print(
                "Added/updated {count} related {user_type}s to {subject}.{period}".format(
    ...
    def _clear_tags_for_relateduser(self, relateduser):
        kwargs = {"periodtag__prefix": self.tag_prefix, "periodtag__period": self.period}
        if self.related_user_model_class == RelatedStudent:
    ...
    def _update_tags(self, relateduser, tags_string):
        ...
        for tagname in tags_string_list:
            tag, created = PeriodTag.objects.get_or_create(prefix=self.tag_prefix, tag=tagname, period=self.period)
            self._add_relateduser_to_tag(tag=tag, relateduser=relateduser)

    def _update_related_user(self, user, kw, tags_string):
        try:
            relateduser = self.related_user_model_class.objects.get(period=self.period, user=user)
    ...
    def _create_reluser(self, user, kw, tags_string):
        relateduser = self.related_user_model_class(period=self.period, user=user, **kw)
...
class Command(RelatedBaseCommand):
    help = (
        "Set related examiners on a period. Users are read from stdin, "
