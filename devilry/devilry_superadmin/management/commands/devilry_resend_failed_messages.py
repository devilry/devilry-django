...
class Command(BaseCommand):
    ...
    help = (
        "Management command that can be used to resend messages with status `failed` and "
    ...
    def handle(self, *args, **options):
        ...
        failed_message_receivers = MessageReceiver.objects.filter(status=MessageReceiver.STATUS_CHOICES.FAILED.value)

        # Get the number of failed messages that exceeded the limit.
        exceeded_resend_limit_count = failed_message_receivers.filter(
            sending_failed_count__gt=settings.DEVILRY_MESSAGE_RESEND_LIMIT
        ).count()

        # Get all failed messaged that has not yet exceeded the resend limit
        failed_message_receivers = failed_message_receivers.filter(
