...
def get_qualifiesforfinalexam_collision_periods(user_a, user_b):
    periods = set()
    for period_id in (
        QualifiesForFinalExam.objects.filter(relatedstudent__user=user_a)
        .values_list("relatedstudent__period_id", flat=True)
        .distinct("relatedstudent__period_id")
    ):
...
def select_username_to_delete(user_a, user_b, period):
    selected_shortname = input(
        f"Select which user to DELETE qualifies for final exam results for in {period.get_path()} [{user_a.shortname}|{user_b.shortname}]: "
    ).strip()
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            dest="preview",
            default=False,
            help=("Preview all possible changes without changing anything."),
        )
    ...
    def handle(self, *args, **options):
        ...
        for period in collision_periods:
            ...
            qualifiesforfinalexam_queryset = (
                QualifiesForFinalExam.objects.filter(relatedstudent__period=period)
            ...
            if preview:
                ...
                user_to_delete = select_username_to_delete(user_a=user_a, user_b=user_b, period=period)
