...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        period = self._get_or_error(Period, short_name=period_short_name, parentnode=subject)
        assignment = self._get_or_error(Assignment, short_name=assignment_short_name, parentnode=period)
        ...
        if marked:
            for group, oldgroup in result["marked"]:
                ...
                print("     feedback: {0} ({1}) (points:{2})".format(status, feedback.grade, feedback.points))
        else:
