...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("period_path", help='The path to the period: "<subject short name>.<period short name>"')
        parser.add_argument(
            "devilryrole",
    ...
    def handle(self, *args, **options):
        ...
        period = Period.objects.get(short_name=period_short_name, parentnode__short_name=subject_short_name)
        try:
            self.stdout.write(
                self.style.SUCCESS(
                    pprint.pformat(
                        PeriodUserGuidelineAcceptance.objects.get_guidelines(period=period, devilryrole=devilryrole)
                    )
                )
            )
