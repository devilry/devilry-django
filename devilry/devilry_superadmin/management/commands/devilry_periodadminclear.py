...
class Command(BaseCommand):
    help = "Clear period admins (removes the syncsystem permission group for the period)."
    ...
    def handle(self, *args, **options):
        ...
        try:
            permissiongroup = PermissionGroup.objects.get_syncsystem_permissiongroup(
                grouptype=PermissionGroup.GROUPTYPE_PERIODADMIN, basenode=period
            )
        except PermissionGroup.DoesNotExist:
            self.stdout.write(
                "WARNING: No sync-system permission group exists for this period, so there is no admins to clear."
            )
            return
        else:
