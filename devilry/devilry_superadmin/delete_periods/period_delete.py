...
class PeriodDelete(object):
    ...
    def __get_subject_queryset(self):
        return Subject.objects.exclude(periods__isnull=True).all()

    def __get_period_queryset(self, subject):
        return Period.objects.filter(parentnode_id=subject.id).filter(end_time__lt=self.end_time_older_than_datetime)
    ...
    def __delete_comment_files_on_period(self, period):
        ...
        group_comments = GroupComment.objects.filter(feedback_set__group__parentnode__parentnode_id=period.id)
    ...
    def get_extra_preview_data(self, period):
        ...
        comment_file_count = CommentFile.objects.filter(
            comment_id__in=group_comments.values_list("id", flat=True)
        ).count()
        return "\t\t- Assignments: {}\n\t\t- Assignment groups: {}\n\t\t- Comments: {}\n\t\t- Files: {}".format(
            assignment_count, assignment_group_count, group_comments.count(), comment_file_count
        )
