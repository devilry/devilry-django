...
def iter_archive_deliveries(archive, group_name, directory_prefix, deliveries):
    ...
    for delivery in deliveries:
        ...
        for f in metas:
            ...
            filename = "%s/%s/%s" % (directory_prefix, group_name, f.filename)
            if include_delivery_explanation:
                filename = "%s/%s/%d/%s" % (directory_prefix, group_name, delivery.number, f.filename)
            ...
            if f.size > settings.DEVILRY_MAX_ARCHIVE_CHUNK_SIZE:
                if not archive.can_write_chunks():
                    raise ArchiveException(
                        "The size of file %s is greater than the maximum allowed size. Download stream aborted."
                    )
    ...
    if include_delivery_explanation:
        archive.add_file(
            "%s/%s/%s" % (directory_prefix, group_name, "Deliveries.txt"), multiple_deliveries_content.encode("ascii")
        )
