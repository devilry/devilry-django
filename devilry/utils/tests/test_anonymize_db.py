...
class AnonymizeDbTestMixin:
    def create_useremail(self, user, email):
        return baker.make("devilry_account.UserEmail", email=email, user=user)

    def create_username(self, user, username):
        return baker.make("devilry_account.UserName", username=username, user=user)
...
class TestAnonymizeUserFast(AnonymizeDbTestMixin, test.TestCase):
    ...
    def test_anonymize_user_shortname(self):
        ...
        self.assertEqual(get_user_model().objects.get(id=user.id).shortname, "{}".format(user.id))
    ...
    def test_shortname_is_anonymized_with_email_if_cradmin_legacy_user_email_auth_backend_is_true(self):
        user = baker.make(settings.AUTH_USER_MODEL, shortname="testuser")
        anonymize_database.AnonymizeDatabase().anonymize_user()
        self.assertEqual(get_user_model().objects.get(id=user.id).shortname, "{}@example.com".format(user.id))
    ...
    def test_anonymize_user_email(self):
        ...
        anonymize_database.AnonymizeDatabase().anonymize_user()
        self.assertEqual(
            UserEmail.objects.get(id=useremail.id).email, "{}_{}@example.com".format(user.id, useremail.id)
        )
    ...
    def test_anonymize_user_name(self):
        ...
        anonymize_database.AnonymizeDatabase().anonymize_user()
        self.assertEqual(UserName.objects.get(id=username.id).username, "{}_{}".format(user.id, username.id))

    def test_anonymize_multiple_users(self):
        user1 = baker.make(settings.AUTH_USER_MODEL, shortname="testuser1")
        user2 = baker.make(settings.AUTH_USER_MODEL, shortname="testuser2")
        user3 = baker.make(settings.AUTH_USER_MODEL, shortname="testuser3")
        anonymize_database.AnonymizeDatabase().anonymize_user()
    ...
    def test_anonymize_multiple_useremails_for_same_user(self):
        user = baker.make(settings.AUTH_USER_MODEL, shortname="testuser1@example.com")
        ...
        anonymize_database.AnonymizeDatabase().anonymize_user()
        self.assertTrue(UserEmail.objects.filter(email="{}_{}@example.com".format(user.id, useremail1.id)))
        self.assertTrue(UserEmail.objects.filter(email="{}_{}@example.com".format(user.id, useremail2.id)))

    def test_anonymize_multiple_user_emails(self):
        user1 = baker.make(settings.AUTH_USER_MODEL, shortname="testuser1@example.com")
        user2 = baker.make(settings.AUTH_USER_MODEL, shortname="testuser2@example.com")
        user3 = baker.make(settings.AUTH_USER_MODEL, shortname="testuser3@example.com")
        ...
        anonymize_database.AnonymizeDatabase().anonymize_user()
        self.assertEqual(
            UserEmail.objects.get(id=useremail1.id).email, "{}_{}@example.com".format(user1.id, useremail1.id)
        )
        self.assertEqual(
            UserEmail.objects.get(id=useremail2.id).email, "{}_{}@example.com".format(user2.id, useremail2.id)
        )
        self.assertEqual(
            UserEmail.objects.get(id=useremail3.id).email, "{}_{}@example.com".format(user3.id, useremail3.id)
        )

    def test_anonymize_multiple_usernames_for_same_user(self):
        user = baker.make(settings.AUTH_USER_MODEL, shortname="testuser1")
        username1 = self.create_username(username="testuser1", user=user)
        username2 = self.create_username(username="testuser2", user=user)
        anonymize_database.AnonymizeDatabase().anonymize_user()
        self.assertEqual(UserName.objects.get(id=username1.id).username, "{}_{}".format(user.id, username1.id))
        self.assertEqual(UserName.objects.get(id=username2.id).username, "{}_{}".format(user.id, username2.id))
    ...
    def test_anonymize_multiple_user_names(self):
        ...
        anonymize_database.AnonymizeDatabase().anonymize_user()
        self.assertEqual(UserName.objects.get(id=username1.id).username, "{}_{}".format(user1.id, username1.id))
        self.assertEqual(UserName.objects.get(id=username2.id).username, "{}_{}".format(user2.id, username2.id))
        self.assertEqual(UserName.objects.get(id=username3.id).username, "{}_{}".format(user3.id, username3.id))
...
class TestAnonymizeUser(AnonymizeDbTestMixin, test.TestCase):
    ...
    def test_anonymize_user_shortname(self):
        ...
        baker.make(settings.AUTH_USER_MODEL, shortname=shortname_to_anonymize)
    ...
    def test_anonymize_user_fullname(self):
        ...
        baker.make(settings.AUTH_USER_MODEL, fullname=fullname_to_anonymize)
        anonymize_database.AnonymizeDatabase(fast=False).anonymize_user()
    ...
    def test_anonymize_user_lastname(self):
        ...
        baker.make(settings.AUTH_USER_MODEL, lastname=lastname_to_anonymize)
        anonymize_database.AnonymizeDatabase(fast=False).anonymize_user()
...
class TestAnonymizeCommentFast(test.TestCase):
    ...
    def test_anonymize_comment_text(self):
        ...
        testcomment = baker.make("devilry_comment.Comment", text=comment_text)
    ...
    def test_anonymize_comment_edit_history(self):
        testedithistory = baker.make("devilry_comment.CommentEditHistory", post_edit_text="Test", pre_edit_text="Tst")
    ...
    def test_anonymize_comment_file_name(self):
        testcommentfile = baker.make("devilry_comment.CommentFile", filename="Test.txt")
    ...
    def test_anonymize_group_comment_text(self):
        comment_text = "This is a comment."
        testcomment = baker.make("devilry_group.GroupComment", text=comment_text)
        self.assertEqual(testcomment.text, "This is a comment.")
        anonymize_database.AnonymizeDatabase().anonymize_comments()
        self.assertFalse(Comment.objects.filter(text=comment_text).exists())
    ...
    def test_anonymize_group_comment_edit_history(self):
        testedithistory = baker.make(
            "devilry_group.GroupCommentEditHistory", post_edit_text="Test", pre_edit_text="Tst"
        )
        self.assertEqual(testedithistory.post_edit_text, "Test")
        self.assertEqual(testedithistory.pre_edit_text, "Tst")
        anonymize_database.AnonymizeDatabase().anonymize_comments()
