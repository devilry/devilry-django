...
class TestIsoformatNoseconds(TestCase):
    ...
    def test_no_seconds(self):
        self.assertEqual(
            "2015-12-24 17:42",
            datetimeutils.isoformat_noseconds(datetimeutils.default_timezone_datetime(2015, 12, 24, 17, 42, 41)),
        )
    ...
    def test_target_weekday_smaller_than_source(self):
        self.assertEqual(
            ...
            datetimeutils.datetime_with_same_day_of_week_and_time(
                ...
                target_datetime=datetimeutils.default_timezone_datetime(2015, 9, 23, 0, 0),
            ),
        )  # Wed

    def test_target_weekday_larger_than_source_test1(self):
        self.assertEqual(
            datetimeutils.default_timezone_datetime(2015, 9, 28, 12, 30),  # Mon week after target
            datetimeutils.datetime_with_same_day_of_week_and_time(
                weekdayandtimesource_datetime=datetimeutils.default_timezone_datetime(2015, 9, 14, 12, 30),  # Mon
                target_datetime=datetimeutils.default_timezone_datetime(2015, 9, 23, 0, 0),
            ),
        )  # Wed
