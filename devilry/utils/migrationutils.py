...
def create_foreignkey_table_change_sql(table, field, old_constraint_name, new_referenced_table):
    """
    ...
    new_constraint_name = "{table}_{field}_id_fk_{new_referenced_table}_id".format(
        table=table, field=field, new_referenced_table=new_referenced_table
    )
    return """
        ALTER TABLE {table}
