...
def make_input_encoding_option():
    ...
    return make_option(
        ...
        help=(
            "Input encoding. Defaults to ``sys.stdin.encoding``, falling back "
            "to ``sys.getdefaultencoding()`` and back to utf-8 if both are undefined. "
            "It is currently is set to: {0}"
        ).format(get_input_encoding()),
    )
...
def make_output_encoding_option():
    ...
    return make_option(
        ...
        help=(
            "Output encoding. Defaults to ``sys.stdout.encoding``, falling back "
            "to ``sys.getdefaultencoding()`` and back to utf-8 if both are undefined. "
            "It is currently is set to: {0}"
        ).format(get_output_encoding()),
    )
