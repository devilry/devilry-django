...
class StreamableArchive(object):
    ...
    def add_file(self, filename, bytes):
        raise UnsupportedOperation("%s.%s() not supported" % (self.__class__.__name__, filename))
...
class StreamableTar(StreamableArchive):
    ...
    def add_file(self, filename, bytes):
        if self.archive.filestream_active:
            raise FileStreamException("Cannot add a new file with a file stream currently active.")
...
class FileStreamTar(tarfile.TarFile):
    ...
    def open_filestream(self, tarinfo):
        ...
        if self.filestream_active:
            raise FileStreamException(
                "Cannot start a new filestream with "
    ...
    def append_file_chunk(self, fileobj, chunk_size):
        ...
        if not self.filestream_active:
            raise FileStreamException(
                "Cannot append file chunk without an active filestream. Start a filestream first."
            )
        if fileobj is None:
    ...
    def close_filestream(self):
        ...
        """
        if not self.filestream_active:
            raise FileStreamException("Cannot close a file stream when no file stream is active.")
        if self.filestream_tarinfo.size != self.filestream_sum:
            raise FileStreamException(
                "Expected size of filestream %s has not been met. "
                "Expected %d, but was %d."
                % (self.filestream_tarinfo.name, self.filestream_tarinfo.size, self.filestream_sum)
            )
