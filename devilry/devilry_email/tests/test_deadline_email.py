...
class TestNewAttemptEmail(test.TestCase):
    ...
    def __setup_feedback_set(self, new_deadline):
        ...
        test_feedbackset = group_baker.feedbackset_first_attempt_published(
            group=testgroup, grading_points=1, deadline_datetime=new_deadline
        )
    ...
    def test_send_new_attempt_email_body_text_with_deadline(self):
        ...
        self.assertEqual(
            ...
            .one(".devilry_email_deadline_new_attempt_info_text")
            .alltext_normalized,
        ...
        self.assertEqual(
            htmls.S(mail.outbox[0].message().as_string())
            .one(".devilry_email_deadline_new_attempt_detail_text")
            .alltext_normalized,
    ...
    def test_send_new_attempt_email_link(self):
        ...
        self.assertEqual(
            htmls.S(mail.outbox[0].message().as_string())
            .one(".devilry_email_deadline_new_attempt_detail_url")
            .alltext_normalized,
            feedback_link,
        )
...
class TestDeadlineMovedEmail(test.TestCase):
    ...
    def test_deadline_moved_email_body_text_without_deadline(self):
        ...
        self.assertEqual(
            htmls.S(mail.outbox[0].message().as_string())
            .one(".devilry_email_deadline_moved_info_text")
            .alltext_normalized,
            "The deadline for {} has been moved.".format(test_feedbackset.group.parentnode.long_name),
        )
        self.assertEqual(
            htmls.S(mail.outbox[0].message().as_string())
            .one(".devilry_email_deadline_moved_to_info_text")
            .alltext_normalized,
            "New deadline is {}".format(defaultfilters.date(test_feedbackset.deadline_datetime, "DATETIME_FORMAT")),
        )
        self.assertEqual(
            htmls.S(mail.outbox[0].message().as_string()).one(".devilry_email_deadline_detail_text").alltext_normalized,
            "See the delivery feed for more details:",
        )
    ...
    def test_deadline_moved_email_link(self):
        ...
        self.assertEqual(
            htmls.S(mail.outbox[0].message().as_string()).one(".devilry_email_deadline_detail_url").alltext_normalized,
            feedback_link,
        )
