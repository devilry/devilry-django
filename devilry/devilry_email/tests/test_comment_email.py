...
from devilry.devilry_email.comment_email.comment_email import send_student_comment_email, send_examiner_comment_email
...
class TestCommentEmailForUsersMixin(object):
    def _make_examineruser_with_email(self, group, email):
        examiner = baker.make(
            "core.Examiner", assignmentgroup=group, relatedexaminer__period=group.parentnode.parentnode
        )
    ...
    def _make_studentuser_with_email(self, group, email):
        student = baker.make(
            "core.Candidate", assignment_group=group, relatedstudent__period=group.parentnode.parentnode
        )
    ...
    def _make_feedbackset(self):
        ...
        """
        testassignment = baker.make_recipe("devilry.apps.core.assignment_activeperiod_start", long_name="Assignment 1")
...
class TestStudentCommentEmail(TestCommentEmailForUsersMixin, test.TestCase):
    ...
    def test_send_student_comment_email_kwargs(self):
        ...
        send_student_comment_email(
            comment_id=test_groupcomment.id, domain_url_start="http://www.example.com/", from_student_poster=True
        )
    ...
    def test_send_student_comment_body_comment_with_files(self):
        ...
        commentfile1 = baker.make(
            "devilry_comment.CommentFile", comment=test_groupcomment, filename="testfile1.py", filesize=5600
        )
        commentfile2 = baker.make(
            "devilry_comment.CommentFile", comment=test_groupcomment, filename="testfile2.py", filesize=3600
        )
        baker.make("devilry_account.UserEmail", user=comment_user, email="testuser@example.com")
        send_student_comment_email(
            comment_id=test_groupcomment.id, domain_url_start="http://www.example.com/", from_student_poster=True
        ...
        for outbox in mail.outbox:
            ...
            self.assertEqual(
                htmls.S(outbox.message().as_string())
                .one(".devilry-email-comment-uploaded-files > p")
                .alltext_normalized,
                "Uploaded files:",
            )
            file_meta_list = [
                elem.alltext_normalized
                for elem in htmls.S(outbox.message().as_string()).list(".devilry-email-comment-uploaded-file-meta")
            ]
    ...
    def test_send_student_comment_after_deadline_email_everyone_except_comment_poster_subject(self):
        ...
        self.assertEqual(
            ...
            "[Devilry] A student added a new comment AFTER THE DEADLINE for {} {}".format(
                test_feedbackset.group.parentnode.long_name, test_feedbackset.group.subject.short_name
            ),
        )
    ...
    def test_examiner_comment_post_subject(self):
        ...
        send_student_comment_email(
            comment_id=test_groupcomment.id, domain_url_start="http://www.example.com/", from_student_poster=False
        )
...
class TestExaminerNotAssignedCommentEmail(TestCommentEmailForUsersMixin, test.TestCase):
    ...
    def _setup_period_admin(self, assignment, admin_user=None):
        permission_group = baker.make(
            "devilry_account.PermissionGroup", grouptype=PermissionGroup.GROUPTYPE_PERIODADMIN
        )
        baker.make("devilry_account.PeriodPermissionGroup", period=assignment.period, permissiongroup=permission_group)
        if not admin_user:
        ...
        baker.make("devilry_account.PermissionGroupUser", permissiongroup=permission_group, user=admin_user)
    ...
    def _setup_subject_admin(self, assignment):
        ...
        baker.make(
            "devilry_account.SubjectPermissionGroup", subject=assignment.subject, permissiongroup=permission_group
        )
        admin_user = baker.make(settings.AUTH_USER_MODEL)
    ...
    def _setup_department_admin(self, assignment):
        permission_group = baker.make(
            "devilry_account.PermissionGroup", grouptype=PermissionGroup.GROUPTYPE_DEPARTMENTADMIN
        )
        baker.make(
            "devilry_account.SubjectPermissionGroup", subject=assignment.subject, permissiongroup=permission_group
        )
    ...
    def test_send_examiner_comment_no_examiner_body(self):
        ...
        for outbox in mail.outbox:
            ...
            url = reverse(
                "devilry_group_redirect_to_feedback_as_admin_or_examiner", kwargs={"assignment_group_id": testgroup.id}
            )
            link_url = "http://www.example.com" + url
            self.assertEqual(
                link_url,
    ...
    def test_send_examiner_comment_body_comment_with_files(self):
        ...
        baker.make("devilry_account.UserEmail", user=comment_user, email="testuser@example.com")
        send_examiner_comment_email(comment_id=test_groupcomment.id, domain_url_start="http://www.example.com/")

        self.assertEqual(len(mail.outbox), 1)
        self.assertTrue(htmls.S(mail.outbox[0].message().as_string()).exists(".devilry-email-comment-uploaded-files"))
        self.assertEqual(
            htmls.S(mail.outbox[0].message().as_string())
            .one(".devilry-email-comment-uploaded-files > p")
            .alltext_normalized,
            "Uploaded files:",
        )
        file_meta_list = [
            elem.alltext_normalized
            for elem in htmls.S(mail.outbox[0].message().as_string()).list(".devilry-email-comment-uploaded-file-meta")
        ]
        self.assertIn("{} (5.5 KB)".format(commentfile1.filename, commentfile1.filesize), file_meta_list)
        self.assertIn("{} (3.5 KB)".format(commentfile2.filename, commentfile2.filesize), file_meta_list)
...
class TestExaminerCommentEmail(TestCommentEmailForUsersMixin, test.TestCase):
    ...
    def test_send_examiner_empty_comment_before_deadline(self):
        ...
        testassignment = baker.make_recipe(
            "devilry.apps.core.assignment_activeperiod_start", long_name="Assignment 1", first_deadline=first_deadline
        )
        testgroup = baker.make("core.AssignmentGroup", parentnode=testassignment)
        test_feedbackset = group_baker.feedbackset_first_attempt_unpublished(group=testgroup)
