...
@override_settings(DATETIME_FORMAT=datetimeutils.ISODATETIME_DJANGOFORMAT)
class TestExaminerDeadlineListView(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def test_heading(self):
        ...
        self.assertEqual("Select deadline", mockresponse.selector.one("h1").alltext_normalized)
    ...
    def test_deadline_item_value_group_single_candidate(self):
        ...
        self.assertEqual(
            "candidate", mockresponse.selector.one(".devilry-deadlinmanagement-item-value-groups").alltext_normalized
        )
    ...
    def test_deadline_item_value_multiple_groups(self):
        ...
        group_value_list = [
            ...
            for elem in mockresponse.selector.list(".devilry-deadlinmanagement-item-value-groups")
        ]
    ...
    def test_only_distinct_deadlines_listed_multiple(self):
        ...
        group_baker.feedbackset_new_attempt_published(
            group=testgroup_new_attempt1, deadline_datetime=new_attempt_deadline
        )
        group_baker.feedbackset_new_attempt_published(
            group=testgroup_new_attempt2, deadline_datetime=new_attempt_deadline
        )
        ...
        self.assertIn(defaultfilters.date(timezone.localtime(new_attempt_deadline), "DATETIME_FORMAT"), deadline_list)
        ...
        self.assertTrue(
            mockresponse.selector.exists(
                "#devilry_manage_deadline_{}_new_attempt_all_link".format(
                    datetimeutils.datetime_to_url_string(testassignment.first_deadline)
                )
            )
        )
        self.assertTrue(
