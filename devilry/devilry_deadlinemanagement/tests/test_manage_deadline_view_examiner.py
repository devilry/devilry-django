...
class ExaminerTestCaseMixin(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def _get_mock_app(self, user=None):
        ...
        mock_app.get_accessible_group_queryset.return_value = (
            core_models.AssignmentGroup.objects.filter_examiner_has_access(user=user)
        )
        return mock_app
...
class TestManageDeadlineNewAttemptAllGroupsView(ExaminerTestCaseMixin):
    ...
    def test_info_box_not_showing_when_zero_groups_were_excluded(self):
        ...
        baker.make("core.Examiner", relatedexaminer__user=examiner_user, assignmentgroup=testgroup1)
        baker.make("core.Examiner", relatedexaminer__user=examiner_user, assignmentgroup=testgroup2)
    ...
    def test_info_box_showing_when_one_group_was_excluded(self):
        ...
        self.assertIn(
            "1 group(s) excluded", mockresponse.selector.one(".devilry-deadline-management-info-box").alltext_normalized
        )
    ...
    def test_info_box_showing_when_multiple_groups_were_excluded(self):
        ...
        baker.make("core.Examiner", relatedexaminer__user=examiner_user, assignmentgroup=testgroup1)
        baker.make("core.Examiner", relatedexaminer__user=examiner_user, assignmentgroup=testgroup2)
        baker.make("core.Examiner", relatedexaminer__user=examiner_user, assignmentgroup=testgroup3)
        mockresponse = self.mock_http200_getrequest_htmls(
            cradmin_role=testassignment,
            cradmin_instance=self._get_mock_instance(testassignment),
            requestuser=examiner_user,
        ...
        self.assertTrue(mockresponse.selector.exists(".devilry-deadline-management-info-box"))
        self.assertIn(
            "2 group(s) excluded", mockresponse.selector.one(".devilry-deadline-management-info-box").alltext_normalized
        )
...
class TestManageDeadlineNewAttemptFromPreviousView(ExaminerTestCaseMixin):
    ...
    def test_post_from_previous_view_selected_groups_are_hidden(self):
        ...
        mockresponse = self.mock_http200_postrequest_htmls(
            ...
            requestkwargs={"data": {"post_type_received_data": "", "selected_items": [testgroup1.id, testgroup2.id]}},
        )
        self.assertIn('type="hidden"', mockresponse.selector.one("#id_selected_items_0").__str__())
        self.assertIn('type="hidden"', mockresponse.selector.one("#id_selected_items_1").__str__())
        self.assertIn('value="{}"'.format(testgroup1.id), mockresponse.selector.one("#id_selected_items_0").__str__())
...
class TestManageDeadlineNewAttemptSingleGroup(ExaminerTestCaseMixin):
    ...
    def test_get_earliest_possible_deadline_last_deadline_in_past(self):
        ...
        earliest_date = mockresponse.selector.list(".devilry-deadlinemanagement-suggested-deadline")[0].get(
            "cradmin-legacy-setfieldvalue"
        )
    ...
    def test_get_all_suggested_deadlines_deadline_in_future(self):
        ...
        testfeedbackset = group_baker.feedbackset_first_attempt_published(
            group=testgroup, deadline_datetime=last_deadline
        )
        testuser = baker.make(settings.AUTH_USER_MODEL)
        baker.make("core.Examiner", assignmentgroup=testgroup, relatedexaminer__user=testuser)
        mockresponse = self.mock_http200_getrequest_htmls(
        ...
        added_days = 7
        feedbackset_current_deadline = (
            arrow.get(testfeedbackset.deadline_datetime)
        ...
        for element in mockresponse.selector.list(".devilry-deadlinemanagement-suggested-deadline"):
            ...
            self.assertEqual(suggested_date, suggested_deadline_from_current_deadline)
            added_days += 7
    ...
    def test_get_earliest_possible_deadline_last_deadline_in_future(self):
        ...
        testfeedbackset = group_baker.feedbackset_first_attempt_published(
            group=testgroup, deadline_datetime=last_feedbackset_last_deadline
        )
        testuser = baker.make(settings.AUTH_USER_MODEL)
        baker.make("core.Examiner", assignmentgroup=testgroup, relatedexaminer__user=testuser)
        mockresponse = self.mock_http200_getrequest_htmls(
        ...
        now_with_same_time_as_deadline = (
            arrow.get(last_feedbackset_last_deadline)
            .to(settings.TIME_ZONE)
            .replace(hour=23, minute=59, second=59, microsecond=0)
            .shift(days=+7)
            .datetime
...
class TestManageDeadlineMoveDeadlineSingleGroup(ExaminerTestCaseMixin):
    ...
    def test_get_earliest_possible_deadline_last_deadline_in_future(self):
        ...
        testfeedbackset1 = group_baker.feedbackset_first_attempt_published(
            group=testgroup, deadline_datetime=first_feedbackset_last_deadline
        )
        ...
        converted_datetime = from_isoformat_noseconds(earliest_date).replace(second=59)
        now_with_same_time_as_deadline = (
            arrow.get(testfeedbackset2.deadline_datetime)
            .to(settings.TIME_ZONE)
            .replace(hour=23, minute=59, second=59, microsecond=0)
            .shift(days=+7)
            .datetime
