...
class AdminTestCaseMixin(test.TestCase, cradmin_testhelpers.TestCaseMixin):
    ...
    def _get_mock_app(self, user=None):
        ...
        mock_app.get_accessible_group_queryset.return_value = core_models.AssignmentGroup.objects.filter_user_is_admin(
            user=user
        )
        return mock_app
    ...
    def _create_department_admin(self, period):
        ...
        baker.make("devilry_account.PermissionGroupUser", user=testuser, permissiongroup=permissiongroup)
        return testuser
