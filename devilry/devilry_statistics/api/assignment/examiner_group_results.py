...
class ExaminerGroupResultApi(AssignmentApiViewPreMixin, APIView):
    ...
    def get_result(self, queryset):
        return queryset.aggregate(
            ...
            num_groups_passed=models.Count(
                models.Case(
                    models.When(
            ...
            num_groups_failed=models.Count(
                models.Case(
                    models.When(
                        cached_data__last_published_feedbackset__isnull=False,
                        cached_data__last_feedbackset=models.F("cached_data__last_published_feedbackset"),
            ...
            num_groups_not_corrected=models.Count(
                models.Case(
                    models.When(
                        models.Q(cached_data__last_feedbackset__grading_published_datetime__isnull=True), then=1
                    )
                )
            ),
        )
    ...
    def get_data(self, serializer):
        ...
        data = {
            ...
            "groups_not_corrected": "{0:.2f}".format(
                self.get_percentage(p=groups_not_corrected, total=total_group_count)
            ),
