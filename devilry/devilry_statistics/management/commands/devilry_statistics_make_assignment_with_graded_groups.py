...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--subject-name", default="statistics_subject", dest="subject_name")
        parser.add_argument("--period-name", default="statistics_period", dest="period_name")
        parser.add_argument("--assignment-name", default="statistics_assignment", dest="assignment_name")
        parser.add_argument("--num-students", default=10, type=int, dest="num_students")
    ...
    def random_generate(self, **options):
        ...
        with transaction.atomic():
            subject = coremodels.Subject(long_name=subject_name, short_name="stat_subject")
            subject.save()
            ...
            for num in range(10):
                examiner_user = get_user_model()(
                    fullname="Examiner#{}".format(num), shortname="examiner#{}@example.com".format(num)
                )
                examiner_user.save()
                print(("Create examineruser#{}".format(num)))
                relatedexaminer = coremodels.RelatedExaminer(user=examiner_user, period=period)
            ...
            for num in range(num_students):
                assignment_group = coremodels.AssignmentGroup(parentnode=assignment)
                assignment_group.save()
                print(("Create assignment group#{}".format(num)))

                student_user = get_user_model()(
                    fullname="Student#{}".format(num), shortname="student#{}@example.com".format(num)
                )
                student_user.save()
                print(("Create studentuser#{}".format(num)))
                relatedstudent = coremodels.RelatedStudent(user=student_user, period=period)
                relatedstudent.save()
                print(("Create relatedstudent#{}".format(num)))
                coremodels.Candidate(relatedstudent=relatedstudent, assignment_group=assignment_group).save()
            ...
            while assignmentgroup_ids:
                for relatedexaminer_id in relatedexaminers:
                    ...
                    examiners_to_create.append(
                        coremodels.Examiner(
                            relatedexaminer_id=relatedexaminer_id, assignmentgroup_id=assignmentgroup_ids.pop(0)
                        )
                    )
    ...
    def create_group_with_student_and_examiner(
        self, relatedexaminer, student_shortname, student_fullname, assignment, grading_points=None
    ):
        assignment_group = coremodels.AssignmentGroup(parentnode=assignment)
        assignment_group.save()

        # Create student
        student_user = get_user_model()(fullname=student_fullname, shortname=student_shortname)
        student_user.save()
        relatedstudent = coremodels.RelatedStudent(user=student_user, period=assignment.parentnode)
        relatedstudent.save()
        coremodels.Candidate(relatedstudent=relatedstudent, assignment_group=assignment_group).save()

        # Create examiner
        coremodels.Examiner(relatedexaminer=relatedexaminer, assignmentgroup=assignment_group).save()
    ...
    def handle(self, *args, **options):
        ...
        if random:
            ...
            # Examiner 1
            examiner_user = get_user_model()(fullname="Examiner1", shortname="examiner1@example.com")
            examiner_user.save()
            relatedexaminer = coremodels.RelatedExaminer(user=examiner_user, period=assignment.parentnode)
            relatedexaminer.save()
