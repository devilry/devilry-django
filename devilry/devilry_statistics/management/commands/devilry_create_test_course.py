...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.subject = Subject(short_name="stat-test", long_name="Statistics test")
        ...
        self.examiner_user1 = get_user_model().objects.create_user(
            email="examiner1@example.com", password="test", fullname="Examiner User1"
        )
        self.examiner_user2 = get_user_model().objects.create_user(
            email="examiner2@example.com", password="test", fullname="Examiner User2"
        )
        self.examiner_user3 = get_user_model().objects.create_user(
            email="examiner3@example.com", password="test", fullname="Examiner User3"
        )
        self.relatedexaminer1 = RelatedExaminer(period=self.period, user=self.examiner_user1)
        self.relatedexaminer1.full_clean()
        self.relatedexaminer1.save()
        self.relatedexaminer2 = RelatedExaminer(period=self.period, user=self.examiner_user2)
        self.relatedexaminer2.full_clean()
        self.relatedexaminer2.save()
        self.relatedexaminer3 = RelatedExaminer(period=self.period, user=self.examiner_user3)
        ...
        for num in range(student_num):
            studentuser = get_user_model().objects.create_user(
                email=f"student{num}@example.com", password="test", fullname=f"Student User{num}"
            )
            relatedstudent = RelatedStudent(user=studentuser, period=self.period)
        ...
        for relatedstudent in relatedstudent_list:
            assignmentgroup = AssignmentGroup(parentnode=self.assignment)
            assignmentgroup.full_clean()
            assignmentgroup.save()
            assignmentgroup.refresh_from_db()
            candidate = Candidate(relatedstudent=relatedstudent, assignment_group=assignmentgroup)
        ...
        for assignmentgroup in assignmentgroup_list:
            examiner = Examiner(assignmentgroup=assignmentgroup, relatedexaminer=self.relatedexaminer1)
            examiner.full_clean()
            examiner.save()
            examiner = Examiner(assignmentgroup=assignmentgroup, relatedexaminer=self.relatedexaminer2)
            examiner.full_clean()
            examiner.save()
            examiner = Examiner(assignmentgroup=assignmentgroup, relatedexaminer=self.relatedexaminer3)
            examiner.full_clean()
            examiner.save()

        # AssignmentGroup devilry and grading
        self.stdout.write(
            f'Adding student delivery and examiner grading for all groups on "{self.assignment.long_name}"...'
        )
        for assignmentgroup in assignmentgroup_list:
