...
class TestExaminerDetailsApi(test.TestCase, api_test_mixin.ApiTestMixin):
    ...
    def __make_published_group_for_relatedexaminer(
        self, assignment, relatedexaminer, grading_points, feedbackset_deadline_datetime=None
    ):
        ...
        group_baker.feedbackset_first_attempt_published(
            group=group, grading_points=grading_points, deadline_datetime=feedbackset_deadline_datetime
        )
        return group

    def __make_unpublished_group_for_relatedexaminer(
        self, assignment, relatedexaminer, feedbackset_deadline_datetime=None
    ):
        if feedbackset_deadline_datetime is None:
            feedbackset_deadline_datetime = timezone.now()
        group = baker.make("core.AssignmentGroup", parentnode=assignment)
