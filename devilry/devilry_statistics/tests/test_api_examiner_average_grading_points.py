...
class TestExaminerAverageGradingPointsApi(test.TestCase, api_test_mixin.ApiTestMixin):
    ...
    def test_validator_assignment_id_missing_raises_403(self):
        ...
        response = self.make_get_request(requestuser=self.make_superuser(), viewkwargs={"relatedexaminer_id": 1})
        self.assertEqual(response.status_code, 403)

    def test_validator_relatedexaminer_id_missing(self):
        assignment = baker.make("core.Assignment")
        response = self.make_get_request(requestuser=self.make_superuser(), viewkwargs={"assignment_id": assignment.id})
    ...
    def test_period_admin_on_different_period_does_not_have_access(self):
        ...
        requestuser = self.make_user()
        permissiongroup = baker.make("devilry_account.PeriodPermissionGroup", period=other_period)
        baker.make(
            "devilry_account.PermissionGroupUser", user=requestuser, permissiongroup=permissiongroup.permissiongroup
        )
        response = self.make_get_request(
            requestuser=requestuser, viewkwargs={"assignment_id": assignment.id, "relatedexaminer_id": 1}
        )
        self.assertEqual(response.status_code, 403)
    ...
    def test_period_admin_has_access(self):
        ...
        requestuser = self.make_user()
        permissiongroup = baker.make("devilry_account.PeriodPermissionGroup", period=period)
        baker.make(
            "devilry_account.PermissionGroupUser", user=requestuser, permissiongroup=permissiongroup.permissiongroup
        )
        response = self.make_get_request(
    ...
    def test_subject_admin_on_different_subject_does_not_have_access(self):
        ...
        relatedexaminer = baker.make("core.RelatedExaminer", period=period, user__fullname="Test User")
        requestuser = self.make_user()
        permissiongroup = baker.make("devilry_account.SubjectPermissionGroup", subject=other_subject)
        baker.make(
            "devilry_account.PermissionGroupUser", user=requestuser, permissiongroup=permissiongroup.permissiongroup
        )
        response = self.make_get_request(
    ...
    def test_department_admin_has_access(self):
        ...
        requestuser = self.make_user()
        permissiongroup = baker.make(
            "devilry_account.SubjectPermissionGroup", subject=subject, permissiongroup__grouptype="departmentadmin"
        )
        baker.make(
            "devilry_account.PermissionGroupUser", user=requestuser, permissiongroup=permissiongroup.permissiongroup
        )
    ...
    def test_sanity(self):
        ...
        self.assertEqual(response.data, {"average_grading_points_given": "1.00", "user_name": "Test User"})
    ...
    def test_assignment_grading_average_sanity(self):
        ...
        self.assertEqual(response.data, {"average_grading_points_given": "25.00", "user_name": "Test User"})
    ...
    def test_assignment_multiple_groups_grading_average_sanity(self):
        ...
        self.assertEqual(response.data, {"average_grading_points_given": "10.00", "user_name": "Test User"})
    ...
    def test_assignment_grading_average_only_for_relatedexaminer_id_passed(self):
        ...
        self.__make_group_for_relatedexaminer(assignment=assignment, relatedexaminer=relatedexaminer1, grading_points=5)
        ...
        self.assertEqual(response.data, {"average_grading_points_given": "5.00", "user_name": "Test User 1"})
