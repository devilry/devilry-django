...
class ShortNameField(models.SlugField):
    ...
    def validate(self, value, *args, **kwargs):
        ...
        if not self.patt.match(value):
            raise ValidationError(gettext_lazy("Can only contain numbers, lowercase letters, '_' and '-'. "))


class LongNameField(models.CharField):
    def __init__(self, *args, **kwargs):
        kw = dict(max_length=100, verbose_name=gettext_lazy("Name"), db_index=True)
        kw.update(kwargs)
