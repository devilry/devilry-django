...
class SubjectQuerySet(models.QuerySet):
    def filter_user_is_admin(self, user):
        ...
        if user.is_superuser:
            ...
            subjectids_where_is_admin_queryset = SubjectPermissionGroup.objects.filter(
    ...
    def __get_subjectids_where_user_is_periodadmin(self, user):
        subjectids_where_is_periodadmin_queryset = (
            PeriodPermissionGroup.objects.filter(models.Q(permissiongroup__users=user))
    ...
    def filter_user_is_admin_for_any_periods_within_subject(self, user):
        ...
        if user.is_superuser:
            ...
            queryset = self.filter_user_is_admin(user) | self.filter(
                id__in=self.__get_subjectids_where_user_is_periodadmin(user=user)
            )
    ...
    def last_active_period(self):
        ...
        if not hasattr(self, "active_period_objects"):
            raise AttributeError(
                "The last_active_period property requires SubjectQuerySet.prefetch_active_period_objects()"
            )
