...
class DevilryUserProfile(models.Model):
    ...
    """
    user = models.OneToOneField(settings.AUTH_USER_MODEL) # This field is required, and it must be named ``user`` (because the model is used as a AUTH_PROFILE_MODULE)
