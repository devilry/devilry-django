...
class BulkCreateFromEmailsResult(object):
    ...
    def __init__(
        self,
        modelclass,
        created_users_queryset,
        existing_user_emails_set,
...
class AbstractRelatedUserManager(models.Manager):
    ...
    def bulk_create_from_emails(self, period, emails):
        ...
        """
        existing_relateduser_emails_set = set(
            self.model.objects.filter(period=period, user__useremail__email__in=emails).values_list(
                "user__useremail__email", flat=True
            )
        )
        ...
        created_relatedusers_queryset = self.model.objects.filter(
            period=period, user__in=new_relateduser_users_queryset
        )
    ...
    def bulk_create_from_usernames(self, period, usernames):
        ...
        Returns:
            ...
            that was not created.
        """
        existing_relateduser_usernames_set = set(
            self.model.objects.filter(period=period, user__username__username__in=usernames).values_list(
                "user__username__username", flat=True
            )
        )
        all_relateduser_usernames_set = set(usernames)
        new_relateduser_usernames_set = all_relateduser_usernames_set.difference(existing_relateduser_usernames_set)

        created_users_queryset, existing_user_usernames_set = get_user_model().objects.bulk_create_from_usernames(
            new_relateduser_usernames_set
        )

        new_relateduser_objects = []
        new_relateduser_users_queryset = get_user_model().objects.filter_by_usernames(new_relateduser_usernames_set)
        for user in new_relateduser_users_queryset:
            new_relateduser = self.model(period=period, user=user)
            new_relateduser_objects.append(new_relateduser)
        self.model.objects.bulk_create(new_relateduser_objects)
...
class RelatedUserBase(models.Model, AbstractIsAdmin):
    ...
    period = models.ForeignKey(Period, verbose_name="Period", help_text="The period.", on_delete=models.CASCADE)
    ...
    automatic_anonymous_id = models.CharField(max_length=255, blank=True, null=False, default="", editable=False)
    ...
    def clean(self):
        if self.tags and not self.tags_patt.match(self.tags):
            raise ValidationError(
                "tags must be a comma-separated list of tags, each tag only containing a-z, 0-9, ``_`` and ``-``."
            )

    def __str__(self):
        return "{}#{} on {}".format(self.__class__.__name__, self.user.shortname, self.period.get_path())
    ...
    def annotate_with_number_of_groups_on_assignment(self, assignment):
        ...
        return self.annotate(
            number_of_groups_on_assignment=models.Count(
                models.Case(models.When(examiner__assignmentgroup__parentnode=assignment, then=1))
            )
        )
...
class RelatedStudentQuerySet(models.QuerySet):
    ...
    def prefetch_syncsystemtag_objects(self):
        ...
        return self.prefetch_related(
            models.Prefetch(
                "periodtag_set", queryset=period_tag.PeriodTag.objects.order_by("tag"), to_attr="syncsystemtag_objects"
            )
        )
...
class RelatedStudentKeyValue(AbstractApplicationKeyValue, AbstractIsAdmin):
    ...
    student_can_read = models.BooleanField(help_text="Specifies if a student can read the value or not.", default=False)
