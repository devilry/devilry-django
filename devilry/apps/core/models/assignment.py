...
class AssignmentQuerySet(models.QuerySet):
    ...
    def filter_user_is_examiner(self, user):
        ...
        """
        return self.filter(
            assignmentgroups__examiners__relatedexaminer__user=user,
    ...
    def filter_is_active(self):
        ...
        """
        now = timezone.now()
        return self.filter_is_published().filter(parentnode__start_time__lt=now, parentnode__end_time__gt=now)
    ...
    def filter_examiner_has_access(self, user):
        ...
        Args:
            user: A User object.
        """
        return self.filter_is_active().filter(
            assignmentgroups__examiners__relatedexaminer__user=user,
            assignmentgroups__examiners__relatedexaminer__active=True,
        )
    ...
    def annotate_with_waiting_for_feedback_count(self):
        ...
        whenquery = models.Q(
            ...
            models.Q(
                ~models.Q(
                    assignmentgroups__cached_data__last_feedbackset=models.F(
        ...
        return self.annotate(waiting_for_feedback_count=models.Count(models.Case(models.When(whenquery, then=1))))
...
class Assignment(models.Model, BaseNode, AbstractIsExaminer, AbstractIsCandidate):
    ...
    parentnode = models.ForeignKey(Period, related_name="assignments", verbose_name="Period", on_delete=models.CASCADE)
    ...
    ANONYMIZATIONMODE_CHOICES_SHORT_LABEL_DICT = {
        ANONYMIZATIONMODE_OFF: pgettext_lazy("assignment anonymizationmode", "off"),
        ANONYMIZATIONMODE_SEMI_ANONYMOUS: pgettext_lazy("assignment anonymizationmode", "semi anonymous"),
        ANONYMIZATIONMODE_FULLY_ANONYMOUS: pgettext_lazy("assignment anonymizationmode", "fully anonymized"),
    }
    ...
    students_can_see_points = models.BooleanField(
        default=False, verbose_name=pgettext_lazy("assignment", "Students can see points?")
    )
    ...
    delivery_types = models.PositiveIntegerField(
        ...
        help_text=gettext_lazy(
            "This option controls what types of deliveries this "
    ...
    scale_points_percent = models.PositiveIntegerField(
        default=100,
        help_text=gettext_lazy(
            "Percent to scale points on this assignment by for "
    ...
    max_points = models.PositiveIntegerField(null=True, blank=True, default=1)
    passing_grade_min_points = models.PositiveIntegerField(null=True, blank=True, default=1)
    ...
    POINTS_TO_GRADE_MAPPER_CHOICES = [
        (POINTS_TO_GRADE_MAPPER_PASSED_FAILED, pgettext_lazy("assignment points-to-grade mapper", "Passed or failed")),
        (POINTS_TO_GRADE_MAPPER_RAW_POINTS, pgettext_lazy("assignment points-to-grade mapper", "Points")),
        (
            POINTS_TO_GRADE_MAPPER_CUSTOM_TABLE,
            pgettext_lazy(
                "assignment points-to-grade mapper", "Lookup in a table defined by you (A-F, and other grading systems)"
            ),
        ),
    ]
    ...
    GRADING_SYSTEM_PLUGIN_ID_CHOICES = [
        (
            ...
            pgettext_lazy("assignment grading plugin", "PASSED/FAILED. The examiner selects passed or failed."),
        ),
        (
            GRADING_SYSTEM_PLUGIN_ID_POINTS,
            pgettext_lazy(
                "assignment grading plugin",
                "POINTS. The examiner types in the number of points to award the student(s) for this assignment.",
            ),
        ),
        # (
    ...
    students_can_create_groups = models.BooleanField(
        ...
        help_text=gettext_lazy(
            "Select this if students should be allowed to join/leave groups. "
    ...
    students_can_not_create_groups_after = models.DateTimeField(
        ...
        help_text=gettext_lazy(
            "Students can not create project groups after this time. "
    ...
    feedback_workflow = models.CharField(
        ...
        choices=(
            (
                "",
                gettext_lazy(
                    "Simple - Examiners write feedback, and publish it whenever "
            ...
            (
                ...
                gettext_lazy(
                    "Trusted cooperative feedback editing - Examiners can only save feedback drafts. "
    ...
    examiner_self_assign_limit = models.PositiveIntegerField(
        default=1,
        help_text=gettext_lazy(
            "The max number of examiners that are allowed to self-assign to each project group within the assignment."
        ),
    )
    ...
    def students_can_create_groups_now(self):
        ...
        return self.students_can_create_groups and (
            self.students_can_not_create_groups_after is None
            or self.students_can_not_create_groups_after > timezone.now()
        )
    ...
    def setup_grading(
        self, grading_system_plugin_id, points_to_grade_mapper, passing_grade_min_points=None, max_points=None
    ):
        """
    ...
    def get_point_to_grade_map(self):
        ...
        if not hasattr(self, "prefetched_point_to_grade_map"):
            raise AttributeError("get_point_to_grade_map() requires AssignmentQuerySet.prefetch_point_to_grade_map().")
    ...
    def get_points_to_grade_map_as_cached_dict(self):
        ...
        if not hasattr(self, "_points_to_grade_map_as_cached_dict"):
            self._points_to_grade_map_as_cached_dict = self.get_point_to_grade_map().as_flat_dict()
    ...
    def points_to_grade(self, points):
        ...
        if self.points_to_grade_mapper == self.POINTS_TO_GRADE_MAPPER_PASSED_FAILED:
            if self.points_is_passing_grade(points=points):
                return pgettext_lazy("assignment grade passed-or-failed", "passed")
            else:
                return pgettext_lazy("assignment grade passed-or-failed", "failed")
            ...
            raise ValueError(
                "Assignment with id=#{} has invalid value for points_to_grade_mapper: {}".format(
    ...
    def _clean_first_deadline(self, errors):
        ...
        if self.first_deadline > self.parentnode.end_time or self.first_deadline < self.parentnode.start_time:
            errors["first_deadline"] = gettext_lazy(
                "First deadline must be within %(periodname)s, which lasts from %(start_time)s to %(end_time)s."
            ) % {
    ...
    def clean(self):
        ...
        if self.publishing_time is not None and self.parentnode_id is not None:
            if self.publishing_time < self.parentnode.start_time or self.publishing_time > self.parentnode.end_time:
                errors["publishing_time"] = gettext_lazy(
                    "Publishing time must be within %(periodname)s, which lasts from %(start_time)s to %(end_time)s."
                ) % {
                    "periodname": self.parentnode.long_name,
                    "start_time": defaultfilters.date(self.parentnode.start_time, "DATETIME_FORMAT"),
                    "end_time": defaultfilters.date(self.parentnode.end_time, "DATETIME_FORMAT"),
        ...
        if self.passing_grade_min_points > self.max_points:
            errors["passing_grade_min_points"] = gettext_lazy(
                "The minumum number of points required to pass must be less than "
    ...
    def get_groups_with_passing_grade(self, sourceassignment):
        ...
        """
        return (
            sourceassignment.assignmentgroups.filter(parentnode_id=sourceassignment.id)
    ...
    def copy_groups_from_another_assignment(self, sourceassignment, passing_grade_only=False):
        ...
        if self.assignmentgroups.exists():
            raise AssignmentHasGroupsError(
                gettext_lazy("The assignment has students. You can not copy use this on assignments with students.")
            )
        ...
        if not passing_grade_only:
            ...
            assignment_group_queryset = self.get_groups_with_passing_grade(sourceassignment=sourceassignment)
        for othergroup in assignment_group_queryset:
            newgroup = AssignmentGroup(parentnode=self, name=othergroup.name, copied_from=othergroup)
        ...
        for group in self.assignmentgroups.prefetch_related(
            models.Prefetch(
                "copied_from",
                to_attr="copied_from_list",
                queryset=AssignmentGroup.objects.prefetch_related(
                    models.Prefetch("candidates", to_attr="candidatelist", queryset=Candidate.objects.all()),
                    models.Prefetch("examiners", to_attr="examinerlist", queryset=Examiner.objects.all()),
                ),
            )
        ):
            ...
            for otherexaminer in group.copied_from_list.examinerlist:
                newexaminer = Examiner(assignmentgroup=group, relatedexaminer_id=otherexaminer.relatedexaminer_id)
    ...
    def create_groups_from_relatedstudents_on_period(self):
        ...
        for group, relatedstudent in zip(self.assignmentgroups.all(), relatedstudents):
            candidate = Candidate(assignment_group=group, relatedstudent=relatedstudent)
    ...
    def __prefetch_relatedstudents_with_candidates(self):
        ...
        return RelatedStudent.objects.select_related("user").prefetch_related(
            models.Prefetch("candidate_set", queryset=Candidate.objects.select_related("assignment_group"))
        )
    ...
    def setup_examiners_by_relateduser_syncsystem_tags(self):
        ...
        groupid_to_examineruserid_map = dict(
            Examiner.objects.filter(assignmentgroup__parentnode=self).values_list(
        ...
        for periodtag in period_tag.PeriodTag.objects.filter(period=period):
            ...
            if relatedstudentids:
                candidate_queryset = (
                    Candidate.objects.filter(assignment_group__parentnode=self, relatedstudent_id__in=relatedstudentids)
                ...
                for relatedexaminer in periodtag.relatedexaminers.all().select_related("user"):
                    ...
                    examinerobjects.extend(
                        [Examiner(assignmentgroup_id=groupid, relatedexaminer=relatedexaminer) for groupid in groupids]
                    )
        if examinerobjects:
