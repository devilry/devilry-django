...
class BaseNodeAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        return super(BaseNodeAdmin, self).get_queryset(request).prefetch_related("admins")
...
class AssignmentGroupHistoryInline(admin.StackedInline):
    ...
    def get_merge_history_json_pretty(self, obj):
        return format_html("<pre>{}</pre>", json.dumps(obj.merge_history, indent=2, sort_keys=True))
...
class AssignmentGroupAdmin(admin.ModelAdmin):
    ...
    raw_id_fields = ["batchoperation", "copied_from"]
    inlines = [AssignmentGroupHistoryInline]
    ...
    def get_queryset(self, request):
        return (
            super(AssignmentGroupAdmin, self)
...
class GroupInviteAdmin(admin.ModelAdmin):
    raw_id_fields = ["group", "sent_by", "sent_to"]

    list_display = ["group", "sent_by", "sent_to", "accepted", "responded_datetime"]

    readonly_fields = ["group", "sent_by", "sent_to", "accepted", "responded_datetime"]
...
class CandidateAssignmentGroupHistoryAdmin(admin.ModelAdmin):
    raw_id_fields = ["assignment_group", "user"]

    list_display = ["assignment_group", "user", "is_add", "created_datetime"]

    readonly_fields = ["assignment_group", "user", "is_add", "created_datetime"]
