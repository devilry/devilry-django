...
class UserInfo(object):
        ...
        if not hasattr(self, "_candidate"):
            try:
                self._candidate = core_models.Candidate.objects.get(
                    assignment_group=self.groupuserlookup.group, relatedstudent__user=self.user
                )
        ...
        if not hasattr(self, "_relatedexaminer"):
            try:
                self._relatedexaminer = core_models.RelatedExaminer.objects.get(
                    period_id=self.groupuserlookup.assignment.parentnode_id, user=self.user
                )
    ...
    def get_unanonymized_long_name_from_user(self, user, html=False):
        ...
        if html:
            return self._render_template(
                "{% load devilry_account_tags %}{% devilry_user_verbose_inline user %}", user=user
            )
        else:
    ...
    def __get_anonymized_name_from_user(self, user, user_role):
        if user_role == "student":
            ...
            raise ValueError('Can only call __get_anonymized_name_from_user with user_role "examiner" or "student".')
    ...
    def get_anonymized_name_from_user(self, user, user_role, html=False):
        ...
        if html:
            if user_role == "student":
                return self._render_span(cssclass="devilry-core-candidate-anonymous-name", content=name)
            else:
                return self._render_span(cssclass="devilry-core-examiner-anonymous-name", content=name)
        return name
...
class GroupUserLookup(object):
    ...
    def get_long_name_from_user(self, user, user_role, html=False):
        ...
        if not self.is_requestuser(user=user):
            if user_role == "student" and self.assignment.students_must_be_anonymized_for_devilryrole(
                devilryrole=self.requestuser_devilryrole
            ):
                return userinfo.get_anonymized_name_from_user(user=user, user_role=user_role, html=html)
            elif user_role == "examiner" and self.assignment.examiners_must_be_anonymized_for_devilryrole(
                devilryrole=self.requestuser_devilryrole
            ):
                return userinfo.get_anonymized_name_from_user(user=user, user_role=user_role, html=html)
