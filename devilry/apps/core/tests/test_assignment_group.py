...
class TestAssignmentGroup(TestCase):
    ...
    def test_anonymous_displayname_empty(self):
        ...
        self.assertEqual("no students in group".format(testgroup.id), testgroup.get_anonymous_displayname())

    def test_anonymous_displayname(self):
        testgroup = baker.make("core.AssignmentGroup")
        baker.make("core.Candidate", assignment_group=testgroup, relatedstudent__candidate_id="test")
        self.assertEqual("test".format(testgroup.id), testgroup.get_anonymous_displayname())

    def test_short_displayname_empty(self):
        testgroup = baker.make("core.AssignmentGroup")
        self.assertEqual("group#{} - no students in group".format(testgroup.id), testgroup.short_displayname)
    ...
    def test_short_displayname_empty_anonymous(self):
        ...
        self.assertEqual("no students in group".format(testgroup.id), testgroup.short_displayname)

    def test_short_displayname_students(self):
        testgroup = baker.make("core.AssignmentGroup")
        baker.make("core.Candidate", assignment_group=testgroup, relatedstudent__user__shortname="studenta")
        baker.make("core.Candidate", assignment_group=testgroup, relatedstudent__user__shortname="studentb")
    ...
    def test_short_displayname_anonymous_candidates(self):
        ...
        baker.make("core.Candidate", assignment_group=testgroup, relatedstudent__candidate_id="aa")
        baker.make(
            "core.Candidate",
    ...
    def test_short_displayname_named(self):
        ...
        baker.make("core.Candidate", assignment_group=testgroup, relatedstudent__user__shortname="ignored")
    ...
    def test_long_displayname_empty(self):
        testgroup = baker.make("core.AssignmentGroup")
        self.assertEqual("group#{} - no students in group".format(testgroup.id), testgroup.long_displayname)

    def test_long_displayname_empty_anonymous(self):
        testgroup = baker.make(
            "core.AssignmentGroup", parentnode__anonymizationmode=Assignment.ANONYMIZATIONMODE_SEMI_ANONYMOUS
        )
        self.assertEqual("no students in group", testgroup.long_displayname)

    def test_long_displayname_candidates(self):
        testgroup = baker.make("core.AssignmentGroup")
        baker.make("core.Candidate", assignment_group=testgroup, relatedstudent__user__fullname="Student \u00e5")
        baker.make("core.Candidate", assignment_group=testgroup, relatedstudent__user__shortname="studentb")
    ...
    def test_long_displayname_named_empty(self):
        testgroup = baker.make("core.AssignmentGroup", name="My group")
        self.assertEqual("My group (group#{} - no students in group)".format(testgroup.id), testgroup.long_displayname)
    ...
    def test_last_feedbackset_is_published(self):
        ...
        baker.make("devilry_group.FeedbackSet", group=testgroup, deadline_datetime=timezone.now(), grading_points=1)
    ...
    def test_bulk_create_groups_creates_group(self):
        ...
        AssignmentGroup.objects.bulk_create_groups(
            created_by_user=testuser, assignment=testassignment, relatedstudents=[relatedstudent]
        )
    ...
    def test_bulk_create_groups_creates_feedbackset(self):
        ...
        relatedstudents = []
        for i in range(5):
            relatedstudents.append(baker.make("core.RelatedStudent", period=testperiod))
        testassignment = baker.make("core.Assignment", parentnode=testperiod)
        group_list = list(
            AssignmentGroup.objects.bulk_create_groups(
                created_by_user=testuser, assignment=testassignment, relatedstudents=relatedstudents
            )
        )
    ...
    def test_bulk_create_groups_multiple(self):
        ...
        relatedstudent3 = baker.make("core.RelatedStudent", period=testperiod)
        testassignment = baker.make("core.Assignment", parentnode=testperiod)
        AssignmentGroup.objects.bulk_create_groups(
        ...
        self.assertEqual(
            ...
            len(
                {
                    created_candidates[0].assignment_group,
        ...
        self.assertEqual(
            3, len({created_feedbacksets[0].group, created_feedbacksets[1].group, created_feedbacksets[2].group})
        )
    ...
    def test_filter_has_passing_grade_unpublished_ignored(self):
        ...
        self.assertEqual([], list(AssignmentGroup.objects.filter_has_passing_grade(assignment=testassignment)))
    ...
    def test_filter_has_passing_grade_unpublished_ignored_but_has_older_published(self):
        ...
        self.assertEqual([testgroup], list(AssignmentGroup.objects.filter_has_passing_grade(assignment=testassignment)))
    ...
    def test_filter_user_is_candidate(self):
        ...
        baker.make("core.Candidate", assignment_group=group, relatedstudent__user=testuser)
        self.assertEqual([group], list(AssignmentGroup.objects.filter_user_is_candidate(user=testuser)))

    def test_filter_user_is_examiner(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        baker.make("core.AssignmentGroup")
        group = baker.make("core.AssignmentGroup")
        baker.make("core.Examiner", assignmentgroup=group, relatedexaminer__user=testuser)
        self.assertEqual([group], list(AssignmentGroup.objects.filter_user_is_examiner(user=testuser)))
...
class TestAssignmentGroupMerge(TestCase):
    ...
    def test_merge_type_first_attempt(self):
        ...
        merged_feedbacksets = FeedbackSet.objects.filter(
            group=group1, feedbackset_type=FeedbackSet.FEEDBACKSET_TYPE_MERGE_FIRST_ATTEMPT
        ).count()
    ...
    def test_merge_type_new_attempt(self):
        ...
        AssignmentGroup.merge_groups([group1, group2, group3])
        merged_feedbacksets = FeedbackSet.objects.filter(
            group=group1, feedbackset_type=FeedbackSet.FEEDBACKSET_TYPE_MERGE_NEW_ATTEMPT
        ).count()
    ...
    def test_merge_type_re_edit(self):
        ...
        AssignmentGroup.merge_groups([group1, group2, group3])
        merged_feedbacksets = FeedbackSet.objects.filter(
            group=group1, feedbackset_type=FeedbackSet.FEEDBACKSET_TYPE_MERGE_RE_EDIT
        ).count()
        self.assertEqual(merged_feedbacksets, 2)
...
class TestAssignmentGroupManager(TestCase):
    def test_filter_is_published(self):
        periodbuilder = SubjectBuilder.quickadd_ducku_duck1010().add_period(
            "period1",
        ...
        group1 = (
            periodbuilder.add_assignment(
                "assignment1", publishing_time=arrow.get(timezone.now()).replace(days=-30).datetime
            )
            .add_group()
            .group
        )
        periodbuilder.add_assignment(
            "assignment2", publishing_time=arrow.get(timezone.now()).replace(days=+10).datetime
        ).add_group()
        periodbuilder.add_assignment(
            "assignment3", publishing_time=arrow.get(timezone.now()).replace(days=+50).datetime
        ).add_group()
    ...
    def test_filter_is_active(self):
        ...
        currentgroupbuilder = duck1010builder.add_6month_active_period().add_assignment("week1").add_group()
...
class TestAssignmentGroupQuerySetPeriodTagFiltering(TestCase):
    ...
    def test_filter_periodtag_for_students_no_tag_sanity(self):
        ...
        self.assertNotIn(testgroup, AssignmentGroup.objects.filter_periodtag_for_students(periodtag_id=testtag.id))
    ...
    def test_filter_periodtag_for_students_has_tag_sanity(self):
        ...
        testtag = baker.make("core.PeriodTag")
        testtag.relatedstudents.add(teststudent.relatedstudent)
        self.assertIn(testgroup, AssignmentGroup.objects.filter_periodtag_for_students(periodtag_id=testtag.id))
    ...
    def test_filter_periodtag_for_students_has_other_tag_sanity(self):
        ...
        self.assertNotIn(testgroup, AssignmentGroup.objects.filter_periodtag_for_students(periodtag_id=testtag1.id))
    ...
    def test_filter_periodtag_for_examiners_no_tag_sanity(self):
        ...
        testtag = baker.make("core.PeriodTag")
        self.assertNotIn(testgroup, AssignmentGroup.objects.filter_periodtag_for_examiners(periodtag_id=testtag.id))
    ...
    def test_filter_periodtag_for_examiners_has_tag_sanity(self):
        ...
        testtag = baker.make("core.PeriodTag")
        testtag.relatedexaminers.add(testexaminer.relatedexaminer)
        self.assertIn(testgroup, AssignmentGroup.objects.filter_periodtag_for_examiners(periodtag_id=testtag.id))
    ...
    def test_filter_periodtag_for_examiners_has_other_tag_sanity(self):
        ...
        self.assertNotIn(testgroup, AssignmentGroup.objects.filter_periodtag_for_examiners(periodtag_id=testtag1.id))
...
class TestAssignmentGroupQuerySetFilterExaminerHasAccess(TestCase):
    def test_not_examiner_for_any_groups(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        self.assertEqual(0, AssignmentGroup.objects.filter_examiner_has_access(user=testuser).count())
    ...
    def test_examiner_for_group_and_active(self):
        ...
        self.assertEqual(1, AssignmentGroup.objects.filter_examiner_has_access(user=testuser).count())
...
class TestAssignmentGroupQuerySetExtraOrdering(TestCase):
    def test_extra_annotate_with_fullname_of_first_candidate_shortnameonly(self):
        testgroup = baker.make("core.AssignmentGroup")
        baker.make("core.Candidate", relatedstudent__user__shortname="userb", assignment_group=testgroup)
        baker.make("core.Candidate", relatedstudent__user__shortname="usera", assignment_group=testgroup)
    ...
    def test_extra_annotate_with_fullname_of_first_candidate_multiple_groups(self):
        ...
        baker.make("core.Candidate", relatedstudent__user__shortname="userb", assignment_group=testgroup1)
        baker.make("core.Candidate", relatedstudent__user__shortname="usera", assignment_group=testgroup1)
        testgroup2 = baker.make("core.AssignmentGroup")
        baker.make("core.Candidate", relatedstudent__user__shortname="userx", assignment_group=testgroup2)
        baker.make("core.Candidate", relatedstudent__user__shortname="usery", assignment_group=testgroup2)
    ...
    def test_extra_order_by_fullname_of_first_candidate_ascending_fullnames(self):
        ...
        baker.make("core.Candidate", relatedstudent__user__shortname="buser2", assignment_group=testgroup1)
        testgroup2 = baker.make("core.AssignmentGroup")
        baker.make("core.Candidate", relatedstudent__user__shortname="user3", assignment_group=testgroup2)
        baker.make(
            "core.Candidate",
    ...
    def test_extra_annotate_with_relatedstudents_anonymous_id_of_first_candidate_anonymousid(self):
        testgroup = baker.make("core.AssignmentGroup")
        baker.make("core.Candidate", relatedstudent__automatic_anonymous_id="anonymousb", assignment_group=testgroup)
        baker.make("core.Candidate", relatedstudent__automatic_anonymous_id="anonymousa", assignment_group=testgroup)
        annotatedgroup = (
            AssignmentGroup.objects.extra_annotate_with_relatedstudents_anonymous_id_of_first_candidate().first()
        )
        self.assertEqual("anonymousa", annotatedgroup.relatedstudents_anonymous_id_of_first_candidate)
    ...
    def test_extra_annotate_with_relatedstudents_anonymous_id_of_first_candidate_candidateid(self):
        ...
        self.assertEqual("candidatexanonymousb", annotatedgroup.relatedstudents_anonymous_id_of_first_candidate)

    def test_extra_annotate_with_relatedstudents_anonymous_id_of_first_candidate_multiple_groups(self):
        testgroup1 = baker.make("core.AssignmentGroup")
        baker.make("core.Candidate", relatedstudent__automatic_anonymous_id="anonymousb", assignment_group=testgroup1)
        baker.make("core.Candidate", relatedstudent__automatic_anonymous_id="anonymousa", assignment_group=testgroup1)
        testgroup2 = baker.make("core.AssignmentGroup")
        baker.make("core.Candidate", relatedstudent__automatic_anonymous_id="anonymousx", assignment_group=testgroup2)
        baker.make("core.Candidate", relatedstudent__automatic_anonymous_id="anonymousy", assignment_group=testgroup2)
        queryset = AssignmentGroup.objects.extra_annotate_with_relatedstudents_anonymous_id_of_first_candidate()
        self.assertEqual("anonymousa", queryset.get(id=testgroup1.id).relatedstudents_anonymous_id_of_first_candidate)
        self.assertEqual("anonymousx", queryset.get(id=testgroup2.id).relatedstudents_anonymous_id_of_first_candidate)
    ...
    def test_extra_order_by_relatedstudents_anonymous_id_of_first_candidate_descending_anonymousids(self):
        ...
        baker.make("core.Candidate", relatedstudent__automatic_anonymous_id="anonymousy", assignment_group=testgroup2)
        groups = list(
            AssignmentGroup.objects.extra_order_by_relatedstudents_anonymous_id_of_first_candidate(descending=True)
        )
        self.assertEqual(testgroup2, groups[0])
        self.assertEqual(testgroup1, groups[1])
    ...
    def test_extra_annotate_with_candidates_candidate_id_of_first_candidate(self):
        testgroup = baker.make("core.AssignmentGroup")
        baker.make("core.Candidate", candidate_id="candidatex", assignment_group=testgroup)
        baker.make("core.Candidate", candidate_id="candidatey", assignment_group=testgroup)
        annotatedgroup = (
            AssignmentGroup.objects.extra_annotate_with_candidates_candidate_id_of_first_candidate().first()
        )
        self.assertEqual("candidatex", annotatedgroup.candidates_candidate_id_of_first_candidate)

    def test_extra_annotate_with_candidates_candidate_id_of_first_candidate_multiple_groups(self):
        testgroup1 = baker.make("core.AssignmentGroup")
        baker.make("core.Candidate", candidate_id="candidateb", assignment_group=testgroup1)
        baker.make("core.Candidate", candidate_id="candidatea", assignment_group=testgroup1)
        testgroup2 = baker.make("core.AssignmentGroup")
        baker.make("core.Candidate", candidate_id="candidatex", assignment_group=testgroup2)
        baker.make("core.Candidate", candidate_id="candidatey", assignment_group=testgroup2)
        queryset = AssignmentGroup.objects.extra_annotate_with_candidates_candidate_id_of_first_candidate()
        self.assertEqual("candidatea", queryset.get(id=testgroup1.id).candidates_candidate_id_of_first_candidate)
        self.assertEqual("candidatex", queryset.get(id=testgroup2.id).candidates_candidate_id_of_first_candidate)

    def test_extra_order_by_candidates_candidate_id_of_first_candidate_ascending(self):
        testgroup1 = baker.make("core.AssignmentGroup")
        baker.make("core.Candidate", candidate_id="a", assignment_group=testgroup1)
        baker.make("core.Candidate", candidate_id="b", assignment_group=testgroup1)
        testgroup2 = baker.make("core.AssignmentGroup")
        baker.make("core.Candidate", candidate_id="c", assignment_group=testgroup2)
        baker.make("core.Candidate", candidate_id="d", assignment_group=testgroup2)
    ...
    def test_extra_order_by_candidates_candidate_id_of_first_candidate_descending(self):
        ...
        baker.make("core.Candidate", candidate_id="d", assignment_group=testgroup2)
        groups = list(
            AssignmentGroup.objects.extra_order_by_candidates_candidate_id_of_first_candidate(descending=True)
        )
        self.assertEqual(testgroup2, groups[0])
        self.assertEqual(testgroup1, groups[1])
    ...
    def test_extra_annotate_with_lastname_of_first_candidate(self):
        testgroup = baker.make("core.AssignmentGroup")
        baker.make("core.Candidate", relatedstudent__user__lastname="userb", assignment_group=testgroup)
        baker.make("core.Candidate", relatedstudent__user__lastname="usera", assignment_group=testgroup)
    ...
    def test_extra_annotate_with_lastname_of_first_candidate_multiple_groups(self):
        testgroup1 = baker.make("core.AssignmentGroup")
        baker.make("core.Candidate", relatedstudent__user__lastname="userb", assignment_group=testgroup1)
        baker.make("core.Candidate", relatedstudent__user__lastname="usera", assignment_group=testgroup1)
        testgroup2 = baker.make("core.AssignmentGroup")
        baker.make("core.Candidate", relatedstudent__user__lastname="userx", assignment_group=testgroup2)
        baker.make("core.Candidate", relatedstudent__user__lastname="usery", assignment_group=testgroup2)
...
class TestAssignmentGroupIsWaitingForFeedback(TestCase):
    ...
    def test_false_feedback_published(self):
        ...
        devilry_group_baker_factories.feedbackset_first_attempt_published(
            group=testgroup, grading_published_datetime=ACTIVE_PERIOD_START
        )
    ...
    def test_false_deadline_not_expired_first_attempt(self):
        testgroup = baker.make("core.AssignmentGroup", parentnode__first_deadline=ACTIVE_PERIOD_END)
    ...
    def test_false_deadline_not_expired_new_attempt(self):
        ...
        devilry_group_baker_factories.feedbackset_new_attempt_unpublished(
            group=testgroup, deadline_datetime=ACTIVE_PERIOD_END
        )
        testgroup.refresh_from_db()
        self.assertFalse(testgroup.is_waiting_for_feedback)

    def test_true_deadline_expired_first_attempt(self):
        testgroup = baker.make("core.AssignmentGroup", parentnode__first_deadline=ACTIVE_PERIOD_START)
        testgroup.refresh_from_db()
    ...
    def test_true_deadline_expired_new_attempt(self):
        testgroup = baker.make("core.AssignmentGroup")
        devilry_group_baker_factories.feedbackset_new_attempt_unpublished(
            group=testgroup, deadline_datetime=ACTIVE_PERIOD_START
        )
        testgroup.refresh_from_db()
        self.assertTrue(testgroup.is_waiting_for_feedback)
    ...
    def test_true_multiple_feedbacksets(self):
        ...
        devilry_group_baker_factories.feedbackset_new_attempt_unpublished(
            group=testgroup, deadline_datetime=ACTIVE_PERIOD_START + timedelta(days=10)
        )
        testgroup.refresh_from_db()
        self.assertTrue(testgroup.is_waiting_for_feedback)
...
class TestAssignmentGroupQuerySetAnnotateWithIsWaitingForFeedback(TestCase):
    ...
    def test_multiple_groups(self):
        testgroup1 = baker.make("core.AssignmentGroup", parentnode__first_deadline=ACTIVE_PERIOD_START)
        testgroup2 = baker.make("core.AssignmentGroup", parentnode__first_deadline=ACTIVE_PERIOD_START)
        testgroup3 = baker.make("core.AssignmentGroup", parentnode__first_deadline=ACTIVE_PERIOD_START)
        testgroup4 = baker.make("core.AssignmentGroup", parentnode__first_deadline=ACTIVE_PERIOD_START)
        testgroup5 = baker.make("core.AssignmentGroup", parentnode__first_deadline=ACTIVE_PERIOD_END)
        ...
        devilry_group_baker_factories.feedbackset_new_attempt_unpublished(
            group=testgroup2, deadline_datetime=ACTIVE_PERIOD_START + timedelta(days=1)
        )

        # Should not be waiting for feedback
        devilry_group_baker_factories.feedbackset_first_attempt_published(group=testgroup3)
        devilry_group_baker_factories.feedbackset_first_attempt_published(group=testgroup4)
        devilry_group_baker_factories.feedbackset_new_attempt_published(group=testgroup4)
        devilry_group_baker_factories.feedbackset_first_attempt_unpublished(group=testgroup5)
...
class TestAssignmentGroupQuerysetAnnotateWithIsWaitingForDeliveries(TestCase):
    ...
    def test_multiple_groups(self):
        testgroup1 = baker.make("core.AssignmentGroup", parentnode__first_deadline=ACTIVE_PERIOD_END)
        testgroup2 = baker.make("core.AssignmentGroup", parentnode__first_deadline=ACTIVE_PERIOD_START)
        testgroup3 = baker.make("core.AssignmentGroup", parentnode__first_deadline=ACTIVE_PERIOD_START)
        testgroup4 = baker.make("core.AssignmentGroup", parentnode__first_deadline=ACTIVE_PERIOD_START)
        testgroup5 = baker.make("core.AssignmentGroup", parentnode__first_deadline=ACTIVE_PERIOD_START)

        # Should be waiting for deliveries
        devilry_group_baker_factories.feedbackset_first_attempt_unpublished(group=testgroup1)
        devilry_group_baker_factories.feedbackset_first_attempt_published(group=testgroup2)
        devilry_group_baker_factories.feedbackset_new_attempt_unpublished(
            group=testgroup2, deadline_datetime=ACTIVE_PERIOD_END
        )
...
class TestAssignmentGroupQuerySetAnnotateWithIsCorrected(TestCase):
    ...
    def test_multiple_groups(self):
        ...
        devilry_group_baker_factories.feedbackset_first_attempt_published(group=testgroup2)
        devilry_group_baker_factories.feedbackset_new_attempt_published(group=testgroup2)

        # Should not be corrected
        devilry_group_baker_factories.feedbackset_first_attempt_unpublished(group=testgroup3)
        devilry_group_baker_factories.feedbackset_first_attempt_published(group=testgroup4)
        devilry_group_baker_factories.feedbackset_new_attempt_unpublished(group=testgroup4)
...
class TestAssignmentGroupPublishedGradingPoints(TestCase):
    ...
    def test_published_grading_points_not_published_none(self):
        testgroup = baker.make("core.AssignmentGroup")
        devilry_group_baker_factories.feedbackset_first_attempt_unpublished(group=testgroup, grading_points=10)
        testgroup.refresh_from_db()
        self.assertEqual(None, testgroup.published_grading_points)

    def test_published_grading_points(self):
        testgroup = baker.make("core.AssignmentGroup")
        devilry_group_baker_factories.feedbackset_first_attempt_published(group=testgroup, grading_points=10)
        testgroup.refresh_from_db()
        self.assertEqual(10, testgroup.published_grading_points)

    def test_published_grading_points_zero_is_not_none(self):
        testgroup = baker.make("core.AssignmentGroup")
        devilry_group_baker_factories.feedbackset_first_attempt_published(group=testgroup, grading_points=0)
        testgroup.refresh_from_db()
    ...
    def test_published_grading_points_multiple_last_published(self):
        ...
        devilry_group_baker_factories.feedbackset_new_attempt_published(
            group=testgroup, grading_points=20, deadline_datetime=timezone.now() + timedelta(days=3)
        )
        testgroup.refresh_from_db()
    ...
    def test_published_grading_points_multiple_last_unpublished(self):
        testgroup = baker.make("core.AssignmentGroup")
        devilry_group_baker_factories.feedbackset_first_attempt_published(group=testgroup, grading_points=10)
        devilry_group_baker_factories.feedbackset_new_attempt_unpublished(group=testgroup, grading_points=20)
        testgroup.refresh_from_db()
        self.assertEqual(10, testgroup.published_grading_points)
...
class TestAssignmentGroupDraftedGradingPoints(TestCase):
    ...
    def test_drafted_grading_points_zero_is_not_none(self):
        testgroup = baker.make("core.AssignmentGroup")
        devilry_group_baker_factories.feedbackset_first_attempt_unpublished(group=testgroup, grading_points=0)
        testgroup.refresh_from_db()
...
class TestAssignmentGroupPublishedGradeIsPassingGrade(TestCase):
    ...
    def test_published_grade_is_passing_grade_false_unpublished(self):
        testgroup = baker.make("core.AssignmentGroup", parentnode__passing_grade_min_points=10)
        devilry_group_baker_factories.feedbackset_first_attempt_unpublished(group=testgroup, grading_points=10)
        testgroup.refresh_from_db()
    ...
    def test_published_grade_is_passing_grade_false_published(self):
        testgroup = baker.make("core.AssignmentGroup", parentnode__passing_grade_min_points=10)
        devilry_group_baker_factories.feedbackset_first_attempt_published(group=testgroup, grading_points=9)
        testgroup.refresh_from_db()
        self.assertFalse(testgroup.published_grade_is_passing_grade)
    ...
    def test_published_grade_is_passing_grade_multiple_last_published(self):
        testgroup = baker.make("core.AssignmentGroup", parentnode__passing_grade_min_points=10)
        devilry_group_baker_factories.feedbackset_first_attempt_published(group=testgroup, grading_points=5)
        devilry_group_baker_factories.feedbackset_new_attempt_published(
            group=testgroup, grading_points=15, deadline_datetime=timezone.now() + timedelta(days=3)
        )
        testgroup.refresh_from_db()
        self.assertTrue(testgroup.published_grade_is_passing_grade)
...
class TestAssignmentGroupQuerySetExtraAnnotateWithDatetimeOfLastAdminComment(TestCase):
    def test_extra_annotate_datetime_of_last_admin_comment(self):
        ...
        self.assertEqual(default_timezone_datetime(2010, 12, 24, 0, 0), queryset.first().datetime_of_last_admin_comment)
    ...
    def test_extra_annotate_datetime_of_last_admin_comment_ignore_private_comment(self):
        ...
        queryset = AssignmentGroup.objects.all().extra_annotate_datetime_of_last_admin_comment()
        self.assertEqual(None, queryset.first().datetime_of_last_admin_comment)
    ...
    def test_extra_annotate_datetime_of_last_admin_comment_ordering(self):
        ...
        queryset = AssignmentGroup.objects.all().extra_annotate_datetime_of_last_admin_comment()
        self.assertEqual(default_timezone_datetime(2011, 12, 24, 0, 0), queryset.first().datetime_of_last_admin_comment)
...
class TestAssignmentGroupQuerySetFilterUserIsAdmin(TestCase):
    ...
    def test_filter_user_is_admin_superuser(self):
        ...
        testgroup = baker.make("core.AssignmentGroup")
        self.assertEqual({testgroup}, set(AssignmentGroup.objects.filter_user_is_admin(user=testuser)))

    def test_filter_user_is_admin_ignore_assignments_where_not_in_group(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        testgroup = baker.make("core.AssignmentGroup")
        baker.make("core.AssignmentGroup")
        periodpermissiongroup = baker.make("devilry_account.PeriodPermissionGroup", period=testgroup.period)
        baker.make(
    ...
    def test_filter_user_is_admin_on_period(self):
        ...
        self.assertEqual({testgroup1, testgroup2}, set(AssignmentGroup.objects.filter_user_is_admin(user=testuser)))
    ...
    def test_filter_user_is_admin_on_period_ignore_semi_anonymous(self):
        ...
        self.assertEqual({testgroup1}, set(AssignmentGroup.objects.filter_user_is_admin(user=testuser)))
...
class TestAssignmentGroupQuerySetAnnotateWithNumberOfPublishedFeedbacksets(TestCase):
    def test_annotate_with_number_of_published_feedbacksets_zero(self):
        ...
        self.assertEqual(
            ...
            .first()
            .number_of_published_feedbacksets,
        )

    def test_annotate_with_number_of_published_feedbacksets_multiple_feedbacksets(self):
        testgroup = baker.make("core.AssignmentGroup")
        baker.make("devilry_group.FeedbackSet", group=testgroup, grading_published_datetime=timezone.now())
        baker.make("devilry_group.FeedbackSet", group=testgroup, grading_published_datetime=timezone.now())
        baker.make("devilry_group.FeedbackSet", group=testgroup, grading_published_datetime=None)
        baker.make("devilry_group.FeedbackSet", group=testgroup, grading_published_datetime=timezone.now())
        self.assertEqual(
            3,
            AssignmentGroup.objects.annotate_with_number_of_published_feedbacksets()
    ...
    def test_annotate_with_number_of_published_feedbacksets_multiple_groups(self):
        testgroup1 = baker.make("core.AssignmentGroup")
        testgroup2 = baker.make("core.AssignmentGroup")
        baker.make("devilry_group.FeedbackSet", group=testgroup1, grading_published_datetime=timezone.now())
        baker.make("devilry_group.FeedbackSet", group=testgroup1, grading_published_datetime=timezone.now())
        baker.make("devilry_group.FeedbackSet", group=testgroup2, grading_published_datetime=None)
        baker.make("devilry_group.FeedbackSet", group=testgroup2, grading_published_datetime=timezone.now())
        queryset = AssignmentGroup.objects.annotate_with_number_of_published_feedbacksets().order_by(
            "number_of_published_feedbacksets"
        )
...
class TestAssignmentGroupQuerySetFilterWithPublishedFeedbackOrComments(TestCase):
    ...
    def test_filter_with_published_feedback_or_comments(self):
        ...
        self.assertEqual(
            {testgroup_with_published_feedback, testgroup_with_groupcomment, testgroup_with_imageannotationcomment},
            set(queryset),
        )
...
class TestAssignmentGroupHasUnpublishedFeedbackset(TestCase):
    ...
    def test_true(self):
        testgroup = baker.make("core.AssignmentGroup")
        devilry_group_baker_factories.feedbackset_first_attempt_unpublished(group=testgroup, grading_points=1)
        testgroup.refresh_from_db()
    ...
    def test_multiple_feedbacksets(self):
        testgroup = baker.make("core.AssignmentGroup")
        devilry_group_baker_factories.feedbackset_first_attempt_published(group=testgroup)
        devilry_group_baker_factories.feedbackset_new_attempt_unpublished(group=testgroup, grading_points=1)
        testgroup.refresh_from_db()
        self.assertTrue(testgroup.has_unpublished_feedbackdraft)
...
class TestAssignmentGroupQuerySetPrefetchAssignmentWithPointsToGradeMap(TestCase):
    def test_no_pointtogrademap(self):
        testgroup = baker.make("core.AssignmentGroup")
        annotated_group = AssignmentGroup.objects.prefetch_assignment_with_points_to_grade_map().get(id=testgroup.id)
    ...
    def test_has_pointtogrademap(self):
        ...
        annotated_group = AssignmentGroup.objects.prefetch_assignment_with_points_to_grade_map().get(id=testgroup.id)
        self.assertEqual(point_to_grade_map, annotated_group.prefetched_assignment.prefetched_point_to_grade_map)
...
class TestAssignmentGroupQuerySetAnnotateWithNumberOfPrivateGroupcommentsFromUser(TestCase):
    ...
    def test_zero(self):
        baker.make("core.AssignmentGroup")
        testuser = baker.make(settings.AUTH_USER_MODEL)
        queryset = AssignmentGroup.objects.annotate_with_number_of_private_groupcomments_from_user(user=testuser)
    ...
    def test_only_private(self):
        ...
        annotated_group = AssignmentGroup.objects.annotate_with_number_of_private_groupcomments_from_user(
            user=testuser
        ).first()
    ...
    def test_only_from_user(self):
        ...
        baker.make("devilry_group.GroupComment", feedback_set=feedbackset, visibility=GroupComment.VISIBILITY_PRIVATE)
        baker.make(
            "devilry_group.GroupComment",
            feedback_set=feedbackset,
            user=testuser,
    ...
    def test_multiple_groups(self):
        testgroup1 = baker.make("core.AssignmentGroup")
        testuser = baker.make(settings.AUTH_USER_MODEL)
        feedbackset1 = devilry_group_baker_factories.feedbackset_first_attempt_published(group=testgroup1)
        baker.make(
            "devilry_group.GroupComment",
        ...
        testgroup2 = baker.make("core.AssignmentGroup")
        feedbackset2 = devilry_group_baker_factories.feedbackset_first_attempt_published(group=testgroup2)
        baker.make(
            "devilry_group.GroupComment",
        ...
        queryset = AssignmentGroup.objects.annotate_with_number_of_private_groupcomments_from_user(user=testuser)
        self.assertEqual(2, queryset.get(id=testgroup1.id).number_of_private_groupcomments_from_user)
        self.assertEqual(1, queryset.get(id=testgroup2.id).number_of_private_groupcomments_from_user)
...
class TestAssignmentGroupQuerySetAnnotateWithNumberOfPrivateImageannotationcommentsFromUser(TestCase):
    ...
    def test_only_private(self):
        ...
        annotated_group = AssignmentGroup.objects.annotate_with_number_of_private_imageannotationcomments_from_user(
            user=testuser
        ).first()
    ...
    def test_multiple_groups(self):
        ...
        self.assertEqual(2, queryset.get(id=testgroup1.id).number_of_private_imageannotationcomments_from_user)
        self.assertEqual(1, queryset.get(id=testgroup2.id).number_of_private_imageannotationcomments_from_user)
...
class TestAssignmentGroupQuerySetAnnotateWithIsPassingGrade(TestCase):
    ...
    def test_true(self):
        testgroup = baker.make("core.AssignmentGroup", parentnode__passing_grade_min_points=10)
        devilry_group_baker_factories.feedbackset_first_attempt_published(group=testgroup, grading_points=20)
        queryset = AssignmentGroup.objects.all().annotate_with_is_passing_grade_count()
    ...
    def test_multiple_groups(self):
        testgroup1 = baker.make("core.AssignmentGroup", parentnode__passing_grade_min_points=10)
        testgroup2 = baker.make("core.AssignmentGroup", parentnode__passing_grade_min_points=30)
        testgroup3 = baker.make("core.AssignmentGroup", parentnode__passing_grade_min_points=20)
        devilry_group_baker_factories.feedbackset_first_attempt_published(group=testgroup1, grading_points=10)
        devilry_group_baker_factories.feedbackset_first_attempt_published(group=testgroup2, grading_points=20)
        devilry_group_baker_factories.feedbackset_first_attempt_published(group=testgroup3, grading_points=30)
        queryset = AssignmentGroup.objects.all().annotate_with_is_passing_grade_count()
