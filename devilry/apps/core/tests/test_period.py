...
class TestPeriodQuerySetFilterActive(TestCase):
    def test_filter_active(self):
        ...
        self.assertEqual(set(Period.objects.filter_active()), {active_period})
...
class TestPeriodQuerySetFilterHasStarted(TestCase):
    def test_filter_active(self):
        ...
        active_period = baker.make_recipe("devilry.apps.core.period_active")
        baker.make_recipe("devilry.apps.core.period_future")
        self.assertEqual(set(Period.objects.filter_has_started()), {old_period, active_period})
...
class TestPeriodQuerySetExtraAnnotateWithAssignmentcountForStudentuser(TestCase):
    def test_no_assignments(self):
        ...
        annotated_period = Period.objects.extra_annotate_with_assignmentcount_for_studentuser(user=testuser).get(
            id=testperiod.id
        )
    ...
    def test_no__assignments_where_candidate(self):
        ...
        baker.make(
            "core.Candidate",  # Not testuser Candidate
    ...
    def test_multiple_assignments(self):
        ...
        baker.make("core.Candidate", assignment_group__parentnode=testassignment3, relatedstudent=relatedstudent)
        annotated_period = Period.objects.extra_annotate_with_assignmentcount_for_studentuser(user=testuser).get(
            id=testperiod.id
        )
    ...
    def test_multiple_periods(self):
        ...
        annotated_period1 = Period.objects.extra_annotate_with_assignmentcount_for_studentuser(user=testuser).get(
            id=testperiod1.id
        )
        annotated_period2 = Period.objects.extra_annotate_with_assignmentcount_for_studentuser(user=testuser).get(
            id=testperiod2.id
        )
...
class TestPeriodQuerySetExtraAnnotateWithUserQualifiesForFinalExam(TestCase):
    def test_not_set(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        testperiod = baker.make("core.Period")
        baker.make("devilry_qualifiesforexam.Status", period=testperiod, status=Status.READY)
        annotated_period = Period.objects.extra_annotate_with_user_qualifies_for_final_exam(user=testuser).get(
            id=testperiod.id
        )
    ...
    def test_not_ready_qualified(self):
        ...
        status = baker.make("devilry_qualifiesforexam.Status", period=testperiod, status=Status.NOTREADY)
        baker.make(
    ...
    def test_almostready_qualified(self):
        ...
        testperiod = baker.make("core.Period")
        relatedstudent = baker.make("core.RelatedStudent", period=testperiod, user=testuser)
        status = baker.make("devilry_qualifiesforexam.Status", period=testperiod, status=Status.ALMOSTREADY)
        baker.make(
            "devilry_qualifiesforexam.QualifiesForFinalExam",
            relatedstudent=relatedstudent,
            status=status,
    ...
    def test_multiple_statuses_use_last(self):
        ...
        baker.make(
            "devilry_qualifiesforexam.Status",
            period=testperiod,  # The last status
    ...
    def test_use_correct_period(self):
        ...
        status = baker.make("devilry_qualifiesforexam.Status", period=otherperiod, status=Status.READY)
        baker.make(
            "devilry_qualifiesforexam.QualifiesForFinalExam",
            relatedstudent=relatedstudent,
            status=status,
...
class TestPeriodQuerySetPermission(TestCase):
    ...
    def test_filter_user_is_admin_superuser(self):
        ...
        testperiod = baker.make("core.Period")
        self.assertEqual({testperiod}, set(Period.objects.filter_user_is_admin(user=testuser)))
