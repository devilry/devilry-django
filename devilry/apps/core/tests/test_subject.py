...
class TestSubjectQuerySetFilterUserIsAdmin(test.TestCase):
    ...
    def test_superuser(self):
        ...
        self.assertEqual({testsubject}, set(Subject.objects.filter_user_is_admin(user=testuser)))

    def test_ignore_subjects_where_not_in_group(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        testsubject = baker.make("core.Subject")
        baker.make("core.Subject")
        baker.make("devilry_account.SubjectPermissionGroup", subject=testsubject)
        self.assertFalse(Subject.objects.filter_user_is_admin(user=testuser).exists())
...
class TestSubjectQuerySetFilterUserIsAdminForAnyPeriodsWithinSubject(test.TestCase):
    def test_is_not_admin_on_anything(self):
        testuser = baker.make(settings.AUTH_USER_MODEL)
        baker.make("core.Subject")
        self.assertEqual([], list(Subject.objects.filter_user_is_admin_for_any_periods_within_subject(user=testuser)))
    ...
    def test_admin_on_subject(self):
        ...
        subjectpermissiongroup = baker.make("devilry_account.SubjectPermissionGroup", subject=testsubject)
        baker.make(
            "devilry_account.PermissionGroupUser", permissiongroup=subjectpermissiongroup.permissiongroup, user=testuser
        )
        self.assertEqual(
            [testsubject], list(Subject.objects.filter_user_is_admin_for_any_periods_within_subject(user=testuser))
        )
    ...
    def test_admin_on_other_subject(self):
        ...
        subjectpermissiongroup = baker.make("devilry_account.SubjectPermissionGroup", subject=othersubject)
        baker.make(
            "devilry_account.PermissionGroupUser", permissiongroup=subjectpermissiongroup.permissiongroup, user=testuser
        )
        self.assertEqual(
            [othersubject], list(Subject.objects.filter_user_is_admin_for_any_periods_within_subject(user=testuser))
        )
    ...
    def test_admin_on_period(self):
        ...
        baker.make(
            "devilry_account.PermissionGroupUser", permissiongroup=periodpermissiongroup.permissiongroup, user=testuser
        )
        self.assertEqual(
            [testsubject], list(Subject.objects.filter_user_is_admin_for_any_periods_within_subject(user=testuser))
        )
    ...
    def test_admin_on_other_period(self):
        ...
        periodpermissiongroup = baker.make("devilry_account.PeriodPermissionGroup", period=otherperiod)
        baker.make(
            "devilry_account.PermissionGroupUser", permissiongroup=periodpermissiongroup.permissiongroup, user=testuser
        )
        self.assertEqual(
    ...
    def test_admin_on_multiple_periods(self):
        ...
        baker.make(
            "devilry_account.PermissionGroupUser", permissiongroup=periodpermissiongroup1.permissiongroup, user=testuser
        )
        ...
        baker.make(
            "devilry_account.PermissionGroupUser", permissiongroup=periodpermissiongroup2.permissiongroup, user=testuser
        )
        self.assertEqual(
            [testsubject], list(Subject.objects.filter_user_is_admin_for_any_periods_within_subject(user=testuser))
        )
...
class TestSubjectQuerySetPrefetchActivePeriodobjects(test.TestCase):
    ...
    def test_has_active_period(self):
        ...
        annotated_subject = Subject.objects.prefetch_active_period_objects().first()
        self.assertEqual([testperiod], annotated_subject.active_period_objects)
    ...
    def test_has_multiple_active_periods_ordering(self):
        ...
        annotated_subject = Subject.objects.prefetch_active_period_objects().first()
        self.assertEqual([testperiod1, testperiod2, testperiod3], annotated_subject.active_period_objects)
    ...
    def test_last_active_period_not_using_prefetch_active_period_objects(self):
        ...
        with self.assertRaisesMessage(
            AttributeError, "The last_active_period property requires SubjectQuerySet.prefetch_active_period_objects()"
        ):
