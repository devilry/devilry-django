...
class TestPeriodTag(test.TestCase):
    ...
    def test_filter_tags_string_list_on_period(self):
        ...
        self.assertListEqual(["a", "b", "prefix:a"], PeriodTag.objects.tags_string_list_on_period(period=testperiod))
    ...
    def get_tags_for_related_student_user(self):
        ...
        baker.make("core.PeriodTag", period=testperiod1, tag="period1_tag1").relatedstudents.add(
            baker.make("core.RelatedStudent", user=testuser)
        )
        baker.make("core.PeriodTag", period=testperiod2, tag="period2_tag1").relatedstudents.add(
            baker.make("core.RelatedStudent", user=testuser)
        )
    ...
    def get_tags_for_related_student_user_on_period(self):
        ...
        testperiod2 = baker.make("core.Period")
        testuser = baker.make(settings.AUTH_USER_MODEL)
        baker.make("core.PeriodTag", period=testperiod1, tag="period1_tag").relatedstudents.add(
            baker.make("core.RelatedStudent", user=testuser)
        )
        baker.make("core.PeriodTag", period=testperiod2, tag="period2_tag").relatedstudents.add(
            baker.make("core.RelatedStudent", user=testuser)
        )
        period_tags = PeriodTag.objects.filter_tags_for_related_student_user_on_period(
            user=testuser, period=testperiod1
        )
    ...
    def get_tags_for_related_examiner_user(self):
        ...
        testperiod2 = baker.make("core.Period")
        testuser = baker.make(settings.AUTH_USER_MODEL)
        baker.make("core.PeriodTag", period=testperiod1, tag="period1_tag1").relatedexaminer.add(
            baker.make("core.RelatedExaminer", user=testuser)
        )
        baker.make("core.PeriodTag", period=testperiod2, tag="period2_tag1").relatedexaminers(
            baker.make("core.RelatedExaminer", user=testuser)
        )
        baker.make("core.PeriodTag", period=testperiod2, tag="period2_tag2")
    ...
    def get_tags_for_related_examiner_user_on_period(self):
        ...
        testperiod2 = baker.make("core.Period")
        testuser = baker.make(settings.AUTH_USER_MODEL)
        baker.make("core.PeriodTag", period=testperiod1, tag="period1_tag").relatedexaminers.add(
            baker.make("core.RelatedExaminer", user=testuser)
        )
        baker.make("core.PeriodTag", period=testperiod2, tag="period2_tag").relatedexaminers.add(
            baker.make("core.RelatedExaminer", user=testuser)
        )
        period_tags = PeriodTag.objects.filter_tags_for_related_examiner_user_on_period(
            user=testuser, period=testperiod1
        )
        self.assertEqual(1, period_tags.count())
        self.assertEqual("period1_tag", period_tags[0].tag)
