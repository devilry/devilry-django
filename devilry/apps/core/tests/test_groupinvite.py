...
class TestGroupInviteErrors(TestCase):
    ...
    def test_user_sending_is_not_part_of_the_group(self):
        ...
        with self.assertRaisesMessage(ValidationError, "The user sending an invite must be a Candiate on the group."):
            invite = baker.make("core.GroupInvite", group=testgroup, sent_by=sent_by, sent_to=sent_to)
            invite.full_clean()
    ...
    def test_student_already_member_of_the_group(self):
        ...
        with self.assertRaisesMessage(ValidationError, "The student is already a member of the group."):
            invite = baker.make("core.GroupInvite", group=testgroup, sent_by=sent_by, sent_to=sent_to)
            invite.full_clean()
    ...
    def test_student_already_invited_but_not_responded(self):
        ...
        with self.assertRaisesMessage(
            ValidationError, "The student is already invited to join the group, but they have not responded yet."
        ):
            invite = baker.make("core.GroupInvite", group=testgroup, sent_by=sent_by, sent_to=sent_to)
            invite.full_clean()
    ...
    def test_assignment_does_not_allow_students_to_form_groups(self):
        testgroup = baker.make("core.AssignmentGroup", parentnode__students_can_create_groups=False)
        testgroup1 = baker.make("core.AssignmentGroup", parentnode=testgroup.parentnode)
        sent_by = core_baker.candidate(testgroup).relatedstudent.user
        sent_to = core_baker.candidate(testgroup1).relatedstudent.user

        with self.assertRaisesMessage(
            ValidationError, "This assignment does not allow students to form project groups on their own."
        ):
            invite = baker.make("core.GroupInvite", group=testgroup, sent_by=sent_by, sent_to=sent_to)
            invite.full_clean()
    ...
    def test_student_sent_to_is_not_registerd_on_assignment(self):
        ...
        with self.assertRaisesMessage(ValidationError, "The invited student is not registered on this assignment."):
            invite = baker.make("core.GroupInvite", group=testgroup, sent_by=sent_by, sent_to=sent_to)
            invite.full_clean()
    ...
    def test_student_sent_to_is_already_in_a_group_with_more_than_one_student(self):
        ...
        with self.assertRaisesMessage(
            ValidationError, "The invited student is already in a project group with more than 1 students."
        ):
...
class TestGroupInviteQueryset(TestCase):
    ...
    def test_filter_accepted(self):
        ...
        self.assertEqual(set(invite.id for invite in GroupInvite.objects.filter_accepted()), {100, 101})
    ...
    def test_filter_no_response(self):
        ...
        self.assertEqual(set(invite.id for invite in GroupInvite.objects.filter_no_response()), {10, 11})
    ...
    def test_filter_rejected(self):
        ...
        self.assertEqual(set(invite.id for invite in GroupInvite.objects.filter_rejected()), {10, 11})
    ...
    def test_filter_unanswered_received_invites(self):
        ...
        self.assertEqual(
            set(invite.id for invite in GroupInvite.objects.filter_unanswered_received_invites(sent_to)), {11, 101}
        )
    ...
    def test_filter_unanswered_sent_invites(self):
        ...
        self.assertEqual(
            set(invite.id for invite in GroupInvite.objects.filter_unanswered_sent_invites(group)), {11, 101}
        )
    ...
    def test_filter_allowed_to_create_groups(self):
        ...
        self.assertEqual(set(invite.id for invite in GroupInvite.objects.filter_allowed_to_create_groups()), {11, 101})
...
class GroupInviteRespond(TestCase):
    ...
    def test_send_invite_mail(self):
        ...
        self.assertDictEqual(
            message.metadata,
            {"groupinvite_id": invite.id, "sent_by_user_id": invite.sent_by.id, "sent_to_user_id": invite.sent_to.id},
        )
        ...
        self.assertIn(
            testgroup.parentnode.parentnode.parentnode.long_name,
            message_receiver.message_content_plain.replace("\n", " "),
        )
    ...
    def test_send_reject_mail(self):
        ...
        self.assertEqual(
            message_receiver.subject, f"{invite.sent_to.get_full_name()} rejected your project group invite"
        )
        self.assertIn(
    ...
    def test_send_accept_mail(self):
        ...
        self.assertEqual(message_receiver.user, invite.sent_by)
        self.assertEqual(
            message_receiver.subject, f"{invite.sent_to.get_full_name()} accepted your project group invite"
        )
        self.assertIn(
    ...
    def test_send_invite_to_choices_queryset(self):
        ...
        self.assertEqual(set(candidate.id for candidate in candidates), {candidate4.id, candidate5.id})
    ...
    def test_send_invite_to_choices_queryset_pending_is_excluded(self):
        ...
        self.assertEqual(set(candidate.id for candidate in candidates), {candidate5.id})
