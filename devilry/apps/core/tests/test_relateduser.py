...
class TestRelatedStudentModel(TestCase):
    ...
    def test_get_anonymous_name_has_anonymous_id_but_not_candidate_id(self):
        relatedstudent = baker.make("core.RelatedStudent", automatic_anonymous_id="MyAutomaticID")
    ...
    def test_get_anonymous_name_has_anonymous_id_and_candidate_id(self):
        relatedstudent = baker.make(
            "core.RelatedStudent", automatic_anonymous_id="MyAutomaticID", candidate_id="MyCandidateID"
        )
    ...
    def test_get_anonymous_name_no_anonymous_id_but_has_candidate_id(self):
        relatedstudent = baker.make("core.RelatedStudent", candidate_id="MyCandidateID")
        self.assertEqual("MyCandidateID", relatedstudent.get_anonymous_name())
...
class TestRelatedExaminerModel(TestCase):
    ...
    def test_get_anonymous_name_has_anonymous_id(self):
        relatedexaminer = baker.make("core.RelatedExaminer", automatic_anonymous_id="MyAutomaticID")
...
class TestRelatedExaminerManager(TestCase):
    ...
    def test_bulk_create_from_emails_single_new(self):
        testperiod = baker.make("core.Period")
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=True):
            result = RelatedExaminer.objects.bulk_create_from_emails(testperiod, ["testuser@example.com"])

            self.assertEqual(1, result.created_relatedusers_queryset.count())
            self.assertEqual(1, RelatedExaminer.objects.count())
            self.assertEqual("testuser@example.com", RelatedExaminer.objects.first().user.shortname)
            self.assertEqual(set(), result.existing_relateduser_emails_set)
    ...
    def test_bulk_create_from_emails_exclude_existing(self):
        testperiod = baker.make("core.Period")
        baker.make(
            "core.RelatedExaminer", period=testperiod, user=UserBuilder2().add_emails("testuser1@example.com").user
        )
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=True):
            result = RelatedExaminer.objects.bulk_create_from_emails(
                period=testperiod, emails=["testuser1@example.com", "testuser2@example.com"]
            )

            self.assertEqual(2, RelatedExaminer.objects.count())
            self.assertEqual(1, result.created_relatedusers_queryset.count())
            self.assertEqual("testuser2@example.com", result.created_relatedusers_queryset.first().user.shortname)
            self.assertEqual({"testuser1@example.com"}, result.existing_relateduser_emails_set)
    ...
    def test_bulk_create_from_emails_exclude_existing_in_other_period(self):
        ...
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=True):
            ...
            self.assertEqual(
                ...
                {relatedexaminer.user.shortname for relatedexaminer in result.created_relatedusers_queryset.all()},
            )
            self.assertEqual(set(), result.existing_relateduser_emails_set)
    ...
    def test_bulk_create_from_usernames_single_new(self):
        testperiod = baker.make("core.Period")
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=False):
            result = RelatedExaminer.objects.bulk_create_from_usernames(testperiod, ["testuser"])

            self.assertEqual(1, result.created_relatedusers_queryset.count())
            self.assertEqual(1, RelatedExaminer.objects.count())
            self.assertEqual("testuser", RelatedExaminer.objects.first().user.shortname)
            self.assertEqual(set(), result.existing_relateduser_usernames_set)
    ...
    def test_bulk_create_from_usernames_exclude_existing(self):
        testperiod = baker.make("core.Period")
        baker.make("core.RelatedExaminer", period=testperiod, user=UserBuilder2().add_usernames("testuser1").user)
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=False):
            result = RelatedExaminer.objects.bulk_create_from_usernames(
            ...
            self.assertEqual(2, RelatedExaminer.objects.count())
            self.assertEqual(1, result.created_relatedusers_queryset.count())
            self.assertEqual("testuser2", result.created_relatedusers_queryset.first().user.shortname)
            self.assertEqual({"testuser1"}, result.existing_relateduser_usernames_set)
...
class TestRelatedStudentManager(TestCase):
    ...
    def test_bulk_create_from_emails_single_new(self):
        testperiod = baker.make("core.Period")
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=True):
            result = RelatedStudent.objects.bulk_create_from_emails(testperiod, ["testuser@example.com"])

            self.assertEqual(1, result.created_relatedusers_queryset.count())
            self.assertEqual(1, RelatedStudent.objects.count())
            self.assertEqual("testuser@example.com", RelatedStudent.objects.first().user.shortname)
            self.assertEqual(set(), result.existing_relateduser_emails_set)
    ...
    def test_bulk_create_from_emails_exclude_existing(self):
        testperiod = baker.make("core.Period")
        baker.make(
            "core.RelatedStudent", period=testperiod, user=UserBuilder2().add_emails("testuser1@example.com").user
        )
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=True):
            result = RelatedStudent.objects.bulk_create_from_emails(
                period=testperiod, emails=["testuser1@example.com", "testuser2@example.com"]
    ...
    def test_bulk_create_from_usernames_single_new(self):
        testperiod = baker.make("core.Period")
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=False):
            result = RelatedStudent.objects.bulk_create_from_usernames(testperiod, ["testuser"])

            self.assertEqual(1, result.created_relatedusers_queryset.count())
            self.assertEqual(1, RelatedStudent.objects.count())
            self.assertEqual("testuser", RelatedStudent.objects.first().user.shortname)
            self.assertEqual(set(), result.existing_relateduser_usernames_set)
    ...
    def test_bulk_create_from_usernames_exclude_existing(self):
        testperiod = baker.make("core.Period")
        baker.make("core.RelatedStudent", period=testperiod, user=UserBuilder2().add_usernames("testuser1").user)
        with self.settings(CRADMIN_LEGACY_USE_EMAIL_AUTH_BACKEND=False):
            result = RelatedStudent.objects.bulk_create_from_usernames(
                period=testperiod, usernames=["testuser1", "testuser2"]
            )
...
class TestRelatedStudentQuerySet(TestCase):
    ...
    def test_annotate_with_total_grading_points_assignments_filter_sanity_before_annotation(self):
        ...
        relatedstudent = baker.make("core.RelatedStudent")
        queryset = RelatedStudent.objects.filter(
            candidate__assignment_group__parentnode_id__in=[test_assignment1.id, test_assignment2.id]
    ...
    def test_annotate_with_total_grading_points_sanity(self):
        ...
        queryset = RelatedStudent.objects.annotate_with_total_grading_points(
            assignment_ids=[test_assignment1.id, test_assignment2.id]
        )
    ...
    def test_annotate_with_total_grading_points_multiple_relatedstudents(self):
        ...
        self.__make_published_feedbackset_for_relatedstudent(
            relatedstudent=relatedstudent2, assignment=test_assignment1, grading_points=10
        )
        self.__make_published_feedbackset_for_relatedstudent(
            relatedstudent=relatedstudent2, assignment=test_assignment2, grading_points=10
        )
        queryset = RelatedStudent.objects.annotate_with_total_grading_points(
            assignment_ids=[test_assignment1.id, test_assignment2.id]
        )
    ...
    def test_get_userid_to_candidateid_map_no_relatedstudents(self):
        testperiod = baker.make("core.Period")
        self.assertEqual(dict(), testperiod.relatedstudent_set.get_userid_to_candidateid_map())

    def test_get_userid_to_candidateid_map_ignore_candidate_id_none(self):
        testperiod = baker.make("core.Period")
        baker.make("core.RelatedStudent", period=testperiod, candidate_id=None)
        self.assertEqual(dict(), testperiod.relatedstudent_set.get_userid_to_candidateid_map())

    def test_get_userid_to_candidateid_map_ignore_candidate_id_emptystring(self):
        testperiod = baker.make("core.Period")
        baker.make("core.RelatedStudent", period=testperiod, candidate_id="")
        self.assertEqual(dict(), testperiod.relatedstudent_set.get_userid_to_candidateid_map())

    def test_get_userid_to_candidateid_map(self):
        testperiod = baker.make("core.Period")
        relatedstudent1 = baker.make("core.RelatedStudent", period=testperiod, candidate_id="a")
        relatedstudent2 = baker.make("core.RelatedStudent", period=testperiod, candidate_id="b")
        relatedstudent3 = baker.make("core.RelatedStudent", period=testperiod, candidate_id="c")
        self.assertEqual(
            {
...
class TestRelatedStudentQuerySetPrefetchSyncsystemtags(TestCase):
    def test_none(self):
        relatedstudent = baker.make("core.RelatedStudent")
        prefetched_relatedstudent = RelatedStudent.objects.prefetch_syncsystemtag_objects().get(id=relatedstudent.id)
    ...
    def test_ordering(self):
        ...
        prefetched_relatedstudent = RelatedStudent.objects.prefetch_syncsystemtag_objects().get(id=relatedstudent.id)
        self.assertEqual(
            [testperiodtag_a, testperiodtag_b, testperiodtag_c], prefetched_relatedstudent.syncsystemtag_objects
        )
    ...
    def test_syncsystemtag_stringlist_not_using_prefetch_syncsystemtag_objects(self):
        ...
        with self.assertRaisesMessage(
            AttributeError,
            "The syncsystemtag_stringlist property requires RelatedStudentQuerySet.prefetch_syncsystemtag_objects().",
        ):
...
class RelatedExaminerQuerySetAnnotateWithNumberOfGroupsOnAssignment(TestCase):
    def test_no_groups(self):
        ...
        queryset = RelatedExaminer.objects.annotate_with_number_of_groups_on_assignment(assignment=testassignment)
        annotated_relatedexaminer = queryset.get(id=relatedexaminer.id)
        self.assertEqual(0, annotated_relatedexaminer.number_of_groups_on_assignment)
    ...
    def test_multiple_groups(self):
        ...
        queryset = RelatedExaminer.objects.annotate_with_number_of_groups_on_assignment(assignment=testassignment)
        annotated_relatedexaminer = queryset.get(id=relatedexaminer.id)
        self.assertEqual(2, annotated_relatedexaminer.number_of_groups_on_assignment)
    ...
    def test_multiple_relatedexaminers(self):
        ...
        testassignment = baker.make("core.Assignment")
        baker.make("core.Examiner", assignmentgroup__parentnode=testassignment, relatedexaminer=relatedexaminer1)
        baker.make("core.Examiner", assignmentgroup__parentnode=testassignment, relatedexaminer=relatedexaminer1)
        baker.make("core.Examiner", assignmentgroup__parentnode=testassignment, relatedexaminer=relatedexaminer2)
        queryset = RelatedExaminer.objects.annotate_with_number_of_groups_on_assignment(assignment=testassignment)
        annotated_relatedexaminer1 = queryset.get(id=relatedexaminer1.id)
        self.assertEqual(2, annotated_relatedexaminer1.number_of_groups_on_assignment)
        annotated_relatedexaminer2 = queryset.get(id=relatedexaminer2.id)
        self.assertEqual(1, annotated_relatedexaminer2.number_of_groups_on_assignment)
...
class RelatedExaminerQuerySetExtraAnnotateWithNumberOfCandidatesOnAssignment(TestCase):
    ...
    def test_no_groups(self):
        relatedexaminer = baker.make("core.RelatedExaminer")
        testassignment = baker.make("core.Assignment")
        queryset = RelatedExaminer.objects.extra_annotate_with_number_of_candidates_on_assignment(
            assignment=testassignment
        )
        annotated_relatedexaminer = queryset.get(id=relatedexaminer.id)
        self.assertEqual(0, annotated_relatedexaminer.number_of_candidates_on_assignment)
    ...
    def test_only_within_assignment_sanity(self):
        ...
        annotated_relatedexaminer = queryset.get(id=relatedexaminer.id)
        self.assertEqual(1, annotated_relatedexaminer.number_of_candidates_on_assignment)
        ...
        annotated_relatedexaminer = queryset.get(id=relatedexaminer.id)
        self.assertEqual(2, annotated_relatedexaminer.number_of_candidates_on_assignment)
    ...
    def test_multiple_examiner_objects(self):
        ...
        annotated_relatedexaminer = queryset.get(id=relatedexaminer.id)
        self.assertEqual(3, annotated_relatedexaminer.number_of_candidates_on_assignment)
