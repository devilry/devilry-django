...
class TestCandidateQuerySet(test.TestCase):
    def test_filter_has_passing_grade(self):
        ...
        self.assertEqual([testcandidate], list(Candidate.objects.filter_has_passing_grade(assignment=testassignment)))

    def test_filter_has_passing_grade_unpublished_ignored(self):
        testassignment = baker.make("core.Assignment", passing_grade_min_points=1)
        unpublished_feedbackset = baker.make(
            "devilry_group.FeedbackSet",
        ...
        self.assertEqual([], list(Candidate.objects.filter_has_passing_grade(assignment=testassignment)))
    ...
    def test_filter_has_passing_grade_unpublished_ignored_but_has_older_published(self):
        ...
        baker.make("devilry_group.FeedbackSet", grading_published_datetime=None, group=testgroup, grading_points=0)
        self.assertEqual([testcandidate], list(Candidate.objects.filter_has_passing_grade(assignment=testassignment)))
    ...
    def test_filter_has_passing_grade_correct_feedbackset_ordering1(self):
        ...
        baker.make(
            "devilry_group.FeedbackSet", grading_published_datetime=timezone.now(), group=testgroup, grading_points=1
        )
        baker.make(
            "devilry_group.FeedbackSet",
    ...
    def test_filter_has_passing_grade_correct_feedbackset_ordering2(self):
        ...
        baker.make(
            "devilry_group.FeedbackSet", grading_published_datetime=timezone.now(), group=testgroup, grading_points=0
        )
        baker.make(
            "devilry_group.FeedbackSet",
            grading_published_datetime=timezone.now() - timedelta(days=3),
...
class TestCandidateModel(test.TestCase):
    def test_get_anonymous_name_uses_custom_candidate_ids_true_no_candidate_id(self):
        candidate = baker.make("core.Candidate", assignment_group__parentnode__uses_custom_candidate_ids=True)
    ...
    def test_get_anonymous_name_uses_custom_candidate_ids_true_with_candidate_id(self):
        candidate = baker.make(
            "core.Candidate", assignment_group__parentnode__uses_custom_candidate_ids=True, candidate_id="MyCandidateId"
        )
