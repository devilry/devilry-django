...
class TestExaminerSubjects(TestCase):
    ...
    def test_get(self):
        ...
        qryset = Subjects.getqry(examiner0).qryset
        self.assertEquals(len(qryset), len(subjects))
        self.assertEquals(qryset[0].short_name, subjects[0].short_name)

        # query
        qryset = Subjects.getqry(examiner0, query="duck1").qryset
        self.assertEquals(len(qryset), 2)
        qryset = Subjects.getqry(examiner0, query="duck").qryset
        self.assertEquals(len(qryset), len(subjects))
        qryset = Subjects.getqry(examiner0, query="1100").qryset
...
class TestExaminerPeriods(TestCase):
    ...
    def test_get(self):
        ...
        qryset = Periods.getqry(examiner0).qryset
        self.assertEquals(len(qryset), len(periods))
        self.assertEquals(qryset[0].short_name, periods[0].short_name)

        # query
        qryset = Periods.getqry(examiner0, query="h01").qryset
        self.assertEquals(len(qryset), 3)
        qryset = Periods.getqry(examiner0, query="duck1").qryset
        self.assertEquals(len(qryset), 2)
...
class TestExaminerAssignments(TestCase):
    ...
    def test_get(self):
        ...
        qryset = Assignments.getqry(examiner0).qryset
        self.assertEquals(len(qryset), len(all_assignments))
        self.assertEquals(qryset[0].short_name, all_assignments[0].short_name)

        # query
        qryset = Assignments.getqry(examiner0, query="ek").qryset
        self.assertEquals(len(qryset), 9)
        qryset = Assignments.getqry(examiner0, query="h0").qryset
        self.assertEquals(len(qryset), len(all_assignments))
        qryset = Assignments.getqry(examiner0, query="1100").qryset
...
class TestExaminerGroups(TestCase):
    ...
    def test_get(self):
        ...
        qryset = Groups.getqry(examiner0, assignment.id,
        ...
        g = Groups.getqry(examiner0, assignment.id).qryset[0]
        g.name = "thisisatest"
        g.save()
        qryset = Groups.getqry(examiner0, assignment.id,
                query="thisisatest").qryset
