...
class TestAllResultsDownload(test.TestCase, cradmin_testhelpers.TestCaseMixin):
                ...
                requestkwargs={
                    "data": {
                        "report_options": json.dumps(
                            {"generator_type": "semesterstudentresults", "generator_options": {}}
                        )
                    }
                }
            )
    ...
    def test_get_report_does_not_exist(self):
        with self.assertRaises(Http404):
            self.mock_http200_getrequest_htmls(requestkwargs={"data": {"report": 1}})
    ...
    def test_get_report_not_created_by_requestuser(self):
        ...
        with self.assertRaises(Http404):
            self.mock_http200_getrequest_htmls(requestkwargs={"data": {"report": report.id}})

    def test_get_download_generated_report_not_finished(self):
        reportuser = baker.make(settings.AUTH_USER_MODEL)
        report = baker.make("devilry_report.DevilryReport", generated_by_user=reportuser)
        mockresponse = self.mock_http200_getrequest_htmls(
            requestuser=reportuser, requestkwargs={"data": {"report": report.id}}
        )
        self.assertEqual(
    ...
    def test_get_download_generated_report(self):
        ...
        with mock.patch.object(DevilryReport, "generator", TestGenerator):
            report = baker.make(
                "devilry_report.DevilryReport", generated_by_user=reportuser, generator_type="test-generator"
            )
            report.generate()
            mockresponse = self.mock_getrequest(requestuser=reportuser, requestkwargs={"data": {"report": report.id}})
